...
// VISIOCYTE plugin versions for use by the automatic updater.

#include "../basic_c_fun/visiocyte_interface.h"
#include "../visiocyte/visiocyte_version_info.h"
#include <iostream>
#include <QDir>
#include <QString>
#include <QUrl>
...
private:
            ...
            const QString& relativeUrl)
    {
        ...
        if (qobject_cast<VISIOCYTEPluginInterface2_1 *>(plugin))
        {
            VISIOCYTEPluginInterface2_1 *iface =
                    qobject_cast<VISIOCYTEPluginInterface2_1 *>(plugin);
            version = iface->getPluginVersion();
        }
        else if (qobject_cast<VISIOCYTESingleImageInterface2_1 *>(plugin))
        {
            VISIOCYTESingleImageInterface2_1 *iface =
                    qobject_cast<VISIOCYTESingleImageInterface2_1 *>(plugin);
            version = iface->getPluginVersion();
        }
        else
        {
        ...
        { // i.e. not NaN
            ...
            cout << indent << "<visiocyte_plugin" << endl;
    ...
    // Adapted from method in visiocyte_plugin_loader.cpp
...
int main(int argc, const char* argv[])
{
    ...
    visiocyte::PluginVersionFinder finder;
