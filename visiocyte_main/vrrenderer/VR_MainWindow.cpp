...
int VR_MainWindow::StartVRScene(QList<NeuronTree>* ntlist, My4DImage *i4d, MainWindow *pmain, bool isLinkSuccess,QString ImageVolumeInfo,XYZ* zoomPOS,XYZ *CreatorPos) {
	...
	if(ntlist != NULL)
	{
		if((ntlist->size()==1)&&(ntlist->at(0).name.isEmpty()))
		{
			...
			// we rename it as visiocyte_traced_neuron
			...
			newS.n = 1;
			newS.on = true;
			newS.name = "visiocyte_traced_neuron";
			newS.file = "visiocyte_traced_neuron";
		...
		else
		{
			...
			{
				if((ntlist->at(i).name == "visiocyte_traced_neuron")&&(ntlist->at(i).file == "visiocyte_traced_neuron"))
				{
					// means there is a NT named "visiocyte_traced_neuron", we only need to edit this NT.
