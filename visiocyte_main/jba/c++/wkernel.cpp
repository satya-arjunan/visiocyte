...
WeightKernel3D::WeightKernel3D(VISIOCYTELONG xw, VISIOCYTELONG yw, VISIOCYTELONG zw)
{
	initData();
	createInternalData(xw, yw, zw, (xw-1)/2, (yw-1)/2, (zw-1)/2);
}

WeightKernel3D::WeightKernel3D(VISIOCYTELONG r)
{
	initData();
	VISIOCYTELONG xw, yw, zw;
	rwid = r;
	xw = yw = zw = 2*rwid+1;
	createInternalData(xw, yw, zw, (xw-1)/2, (yw-1)/2, (zw-1)/2);
}
...
void WeightKernel3D::generateKernel(KernelType t, int pp, int qq, int rr)
{
	...
	VISIOCYTELONG i,j,k;
	VISIOCYTELONG ie, ib, je, jb, ke, kb;
	double dz, dy, dx;
	VISIOCYTELONG r2 = rwid*rwid;
...
int WeightKernel3D::createInternalData(VISIOCYTELONG xw, VISIOCYTELONG yw, VISIOCYTELONG zw, VISIOCYTELONG xw0, VISIOCYTELONG yw0, VISIOCYTELONG zw0)
{
	if (data1d || data3d)
	{
	...
	{
		for (VISIOCYTELONG k=0;k<zw; k++)
		{
			data3d[k] = 0;
			data3d[k] = new double * [yw];
			if (!data3d[k])
			{
				for (VISIOCYTELONG j=0;j<k;j++)
				{
					if (data3d[j])
					{
						delete [] (data3d[j]);
...
void WeightKernel3D::cleanData() 
{
    if (data3d)
	{
		for (VISIOCYTELONG k=0; k<zwid;k++)
		{
			if (data3d[k])
			{
				delete [] (data3d[k]);
