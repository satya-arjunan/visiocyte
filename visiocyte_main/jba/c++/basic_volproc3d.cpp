...
template <class T> bool vol3d_log(T *** res, T *** sa, double VISIOCYTELONG d0, VISIOCYTELONG d1, VISIOCYTELONG d2)
{
	if (!res || !sa || d0<=0 || d1<=0 || d2<=0)
		return false;
...
template <class T> bool mean_and_std(T *data, VISIOCYTELONG n, T & ave, T & sdev)
{
    if (!data || n<=0)
	  return false;
