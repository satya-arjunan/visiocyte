...
bool hist_eq_uint8(unsigned char * data1d, VISIOCYTELONG len)
{
	...
	VISIOCYTELONG NBIN=256;
	VISIOCYTELONG *h = new VISIOCYTELONG [NBIN]; 
...
bool hist_eq_range_uint8(unsigned char * data1d, VISIOCYTELONG len, unsigned char lowerbound, unsigned char upperbound) //only eq the [lowerbound,upperbound]
{
	if (!data1d || len<=0)
	{
		printf("The input parameters are invalid in hist_eq_uint8().\n");
