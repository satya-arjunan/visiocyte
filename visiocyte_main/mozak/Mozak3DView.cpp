...
Mozak3DView::Mozak3DView(VISIOCYTEPluginCallback2 *_VISIOCYTE_env, int _resIndex, itm::uint8 *_imgData, int _volV0, int _volV1,
	int _volH0, int _volH1, int _volD0, int _volD1, int _volT0, int _volT1, int _nchannels, itm::CViewer *_prev, int _slidingViewerBlockID)
		: teramanager::CViewer(_VISIOCYTE_env, _resIndex, _imgData, _volV0, _volV1,
...
teramanager::CViewer* Mozak3DView::makeView(VISIOCYTEPluginCallback2 *_VISIOCYTE_env, int _resIndex, itm::uint8 *_imgData, int _volV0, int _volV1,
	int _volH0, int _volH1, int _volD0, int _volD1, int _volT0, int _volT1, int _nchannels, itm::CViewer *_prev, int _slidingViewerBlockID)
{
	Mozak3DView* neuronView = new Mozak3DView(_VISIOCYTE_env, _resIndex, _imgData, _volV0, _volV1,
		_volH0, _volH1, _volD0, _volD1, _volT0, _volT1, _nchannels, _prev, _slidingViewerBlockID);
	return neuronView;
}
...
void Mozak3DView::performUndo()
{
    ...
    if (undoRedoHistory.size() == 0 || cur_history < 0) {
        ...
        // Annoying: visiocyte_msg("Reached the earliest of saved history!");
        updateUndoLabel();
        return;
    }
    qDebug() << "Undoing...";
...
void Mozak3DView::performRedo()
{
	...
	if (undoRedoHistory.size() < 1 || cur_history >= undoRedoHistory.size())
	{
		...
		cur_history = undoRedoHistory.size() - 1;
		// Annoying: visiocyte_msg("Reached the latest of saved history!");
	}
...
VISIOCYTELONG Mozak3DView::findNearestNeuronNode(int cx, int cy, bool updateStartType/*=false*/)
{
    Renderer_gl2* curr_renderer = (Renderer_gl2*)(view3DWidget->getRenderer());
    VISIOCYTELONG best_ind=-1;
    VISIOCYTELONG best_dist=-1;
    ...
    if (event->type() == QEvent::MouseMove)
    {
        ...
        if(currentMode == Renderer::smCurveEditExtendOneNode || currentMode == Renderer::smCurveEditExtendTwoNode){
            if(!isRightMouseDown){
                ...
                //Highlight end node
                VISIOCYTELONG highlightedEndNodePrev = curr_renderer->highlightedEndNode;
...
//                changeMode(Renderer::smCurveCreate_pointclickAutoZ, true, true);
			...
			case Qt::Key_G:
				...
				if (this->overviewActive){
					MozakUI* moz = MozakUI::getMozakInstance();
					Renderer_gl1* overviewRenderer =(Renderer_gl1*)( moz->VISIOCYTE_env->find3DViewerByName("Overview")->getGLWidget()->getRenderer());
...
void Mozak3DView::show()
{
    ...
    VisiocyteR_GLWidget::disableUndoRedo = true;
    VisiocyteR_GLWidget::skipFormat = true;
...
Mozak3DView::~Mozak3DView()
{
    ...
    VisiocyteR_GLWidget::disableUndoRedo = false;
    VisiocyteR_GLWidget::skipFormat = false;
...
void Mozak3DView::overviewMonitorButtonClicked(bool checked){
	...
	QList<VisiocyteR_MainWindow*> windowList =	moz->VISIOCYTE_env->getListAll3DViewers();
	if (windowList.length()==1){ //there's only one window open: Mozak!
		VisiocyteR_MainWindow* overviewWindow = moz->VISIOCYTE_env->createEmpty3DViewer();
		moz->VISIOCYTE_env->setWindowDataTitle(overviewWindow, "Overview");
...
void Mozak3DView::overviewSyncOneShot(){
	...
	if (overviewActive){
		...
		MozakUI* moz = MozakUI::getMozakInstance();
			...
			QList<NeuronTree> *overview_nt_list = moz->VISIOCYTE_env->getHandleNeuronTrees_Any3DViewer(moz->VISIOCYTE_env->find3DViewerByName("Overview"));

				NeuronTree testLoad;
				qDebug()<<"autosave.ano.swc loading!";
				QFileInfo fInfo = QFileInfo("./autosave.ano.swc");
				if (fInfo.isReadable()){
				 testLoad= readSWC_file( "./autosave.ano.swc");// "D:\\mozak_git\\visiocyte_main\\visiocyte\\release\\autosave.ano.swc"  );
				...
				if ( !(moz->VISIOCYTE_env->find3DViewerByName("Overview")==0)){  // formerly checked for  (testLoad.n>0) &&   but V_NeuronSWC_list_2_neuronTree sets NeuronTree.n=-1 for output. 
					moz->VISIOCYTE_env->getHandleNeuronTrees_Any3DViewer(moz->VISIOCYTE_env->find3DViewerByName("Overview"))->clear();
					moz->VISIOCYTE_env->getHandleNeuronTrees_Any3DViewer(moz->VISIOCYTE_env->find3DViewerByName("Overview"))->push_back(testLoad);

					Renderer_gl2* overview_renderer = (Renderer_gl2*)(moz->VISIOCYTE_env->find3DViewerByName("Overview")->getGLWidget()->getRenderer());
			...
			if (moz->VISIOCYTE_env->find3DViewerByName("Overview")){

					moz->VISIOCYTE_env->update_NeuronBoundingBox(moz->VISIOCYTE_env->find3DViewerByName("Overview"));
				}
			if (moz->VISIOCYTE_env->find3DViewerByName("Overview")) {

				moz->VISIOCYTE_env->update_3DViewer(moz->VISIOCYTE_env->find3DViewerByName("Overview"));
...
void Mozak3DView::loadNewResolutionData(	int _resIndex,
    ...
    VISIOCYTE_env->setImage(window, _img); // this clears the rawDataPointer for _img
...

{
    ...
    try
    {
		...
		try {
			...
			nextImg->setData(nextImgData, nextImgVolH1-nextImgVolH0, nextImgVolV1-nextImgVolV0, nextImgVolD1-nextImgVolD0, nchannels*(nextImgVolT1-nextImgVolT0+1), VISIOCYTE_UINT8);
			nextImg->setTDim(nextImgVolT1-nextImgVolT0+1);
			nextImg->setTimePackType(TIME_PACK_C);
		} catch (...) {
			visiocyte_msg("Fail to allocate memory for nextImg in Mozak3DView::newViewer();\n");
			loadingNextImg = false;
			return;
		}
