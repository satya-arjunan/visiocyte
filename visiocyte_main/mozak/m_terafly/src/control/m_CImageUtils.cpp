...
void CImageUtils::applyVisiocyteColorMap(QImage& image, RGBA8 cmap[4][256])
{
...
throw (itm::RuntimeException)
{
    ...
    imout->setDatatype(VISIOCYTE_UINT8);
    VISIOCYTELONG data_size = imout->getXDim()*imout->getYDim()*imout->getZDim()*imout->getCDim();
    ...
    for(VISIOCYTELONG k=0; k<data_size; k++)
...
throw (itm::RuntimeException)
{
    ...
    VISIOCYTELONG data_size = imout->getTotalBytes();
    unsigned char* data = new unsigned char[data_size];
    imout->setRawDataPointer(data);

    // data corruption
    if(imout->getDatatype() == VISIOCYTE_UINT8)
    {
        unsigned char* data_in = im->getRawData();
        for(VISIOCYTELONG k=0; k<data_size; k++)
            data[k] = static_cast<itm::uint8>((1-w)*data_in[k] + w*(rand()%256) +0.5f);
    }
    else if(imout->getDatatype() == VISIOCYTE_UINT16)
    {
        unsigned short* data_in = (unsigned short*)(im->getRawData());
        for(VISIOCYTELONG k=0; k<data_size/2; k++)
