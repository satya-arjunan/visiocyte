...
bool PAnoToolBar::disableNonMozakButtons = false;
...
PAnoToolBar::PAnoToolBar(QWidget *parent) : QWidget(parent)
{
    ...
    if (!PAnoToolBar::disableNonMozakButtons)
    {
...
void PAnoToolBar::buttonMarkerCreateChecked(bool checked)
{
    ...
    if(checked)
    {
        ...
        // switch to Visiocyte's 1-right-click marker create mode
        if(expl)
        {
...
void PAnoToolBar::buttonMarkerCreate2Checked(bool checked)
{
    ...
    if(checked)
    {
        ...
        setCursor(QCursor(cur_img, 0, 0));
        CViewer::setCursor(QCursor(cur_img, 0, 0), true);
        // switch to Visiocyte's 2-right-clicks marker create mode
        if(expl)
        {
...
//        buttonMarkerRoiDelete->setChecked(false);
        ...
        setCursor(QCursor(cur_img, 0,0));
        CViewer::setCursor(QCursor(cur_img, 0, 0), true);
        // switch to Visiocyte's mode "Zoom-in HighRezImage: 1-right stroke ROI"
        if(expl)
        {
    ...
    else
    {
        ...
        setCursor(Qt::ArrowCursor);
        CViewer::setCursor(Qt::ArrowCursor, true);
        // end Visiocyte's mode "Zoom-in HighRezImage: 1-right stroke ROI"
        if(expl)
        {
