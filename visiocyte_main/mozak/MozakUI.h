...
class mozak::MozakUI : public teramanager::PMain
{
	public:
		MozakUI(){}
		~MozakUI();
		MozakUI(VISIOCYTEPluginCallback2 *callback, QWidget *parent);
		static void createInstance(VISIOCYTEPluginCallback2 *callback, QWidget *parent);
		static MozakUI* getMozakInstance();
	//public:
		friend class Mozak3DView;
		static void init(Visiocyte_PluginLoader *pl);
		virtual void reset(); // override
        virtual teramanager::CViewer* initViewer(VISIOCYTEPluginCallback2* _VISIOCYTE_env, int _resIndex, itm::uint8* _imgData, int _volV0, int _volV1,
