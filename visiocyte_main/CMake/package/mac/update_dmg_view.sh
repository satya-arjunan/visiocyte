...
# Run this when changing the look of the visiocyte dmg window.
# usage: update_dmg_view.sh InstallVisiocyte-2.539.dmg
...
visiocyte_version=`echo $1 | cut -f2 -d '-' | cut -f1,2 -d '.'`
echo ${visiocyte_version}
...
mkdir /Volumes/Visiocyte-${visiocyte_version}/.background
# background file must not be hidden when setting background
chflags nohidden /Volumes/Visiocyte-${visiocyte_version}/background.png

# Use applescript language to arrange view
echo "
   tell application \"Finder\"
     tell disk \"Visiocyte-${visiocyte_version}\"
           open
           ...
           set position of item \"Visiocyte.app\" of container window to {80, 110}
...
chflags hidden /Volumes/Visiocyte-${visiocyte_version}/background.png
sleep 2

cp "/Volumes/Visiocyte-${visiocyte_version}/.DS_Store" "./DS_Store_visiocyte_${visiocyte_version}"
hdiutil detach "/Volumes/Visiocyte-${visiocyte_version}"
