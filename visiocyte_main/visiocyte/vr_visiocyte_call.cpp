...
{
    visiocyte_msg(QString("Now try to do VR neuron search plugin functions [%1]").arg(option), 0);
    ...
    try
    {
        ...
        if (!loader)
        {
            qDebug("ERROR in Visiocyte_PluginLoader::searchPluginFiles the VR neuron search module(%s)", qPrintable(fullpath));
            return false;
        }

        visiocyte_msg(fullpath, 0);

        Visiocyte_PluginLoader mypluginloader(pmain);
    ...
    {
        visiocyte_msg("Catch a problem in visiocyte_imaging() wrapper function.", 0);
        return false;
    }
