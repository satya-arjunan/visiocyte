...
// TODO - Use visiocyte logging system for messages
...
VisiocyteUrlDialog::VisiocyteUrlDialog(QWidget *parent) 
    : QDialog(parent)
{
    setupUi(this);
...
void VisiocyteUrlDialog::on_lineEdit_textChanged() {
    QPushButton *openButton = buttonBox->button(QDialogButtonBox::Open);
    openButton->setEnabled(lineEdit->hasAcceptableInput());
}

QUrl VisiocyteUrlDialog::getUrl()
{
    QUrl url(lineEdit->text());
    return url;
}

bool VisiocyteUrlDialog::getKeepLocalCache() {
...
{
    QUrl url;
    VisiocyteUrlDialog *urlDialog = new VisiocyteUrlDialog(guiParent);
