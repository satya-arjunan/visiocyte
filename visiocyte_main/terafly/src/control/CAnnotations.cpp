...
//                neuron.y, neuron.x, neuron.z);
                ...
                neuron.z >= p_octant->D_start+D_dim_halved			&& neuron.z < p_octant->D_start+p_octant->D_dim)
        {
            ...
            throw RuntimeException(strprintf("in CAnnotations::Octree::insert(...): Out of bounds neuron [%.0f,%.0f,%.0f] (visiocyte n = %d).\n\n"
                                             ...
                                             neuron.x, neuron.y, neuron.z, neuron.visiocyte_n));
    }
    else
    {
...
void CAnnotations::removeDuplicatedNode(QList<NeuronSWC> &neuron,QList<NeuronSWC> &result)
{
    ...
    for(VISIOCYTELONG i=0;i<neuron.size();i++)
    {
        ids0.push_back(neuron.at(i).n);
    }
    for(VISIOCYTELONG i=0;i<neuron.size();i++)
    {
        VISIOCYTELONG parentid=neuron.at(i).parent;
        neuron[i].n=i;
...
//    cout<<"parents size is "<<parents0.size()<<endl;
    //QList<VISIOCYTELONG>  child_num0;//vector<bool> parentexist;
    for(VISIOCYTELONG i=0;i<neuron.size();i++)// 0 or 1? check!
    {
        VISIOCYTELONG parentid=neuron.at(i).parent;
...
//        for(VISIOCYTELONG r=0;r<rootnodes.size();r++)
//        {
//            for(VISIOCYTELONG r1=r+1;r1<rootnodes.size();r1++)
//            {
...
//                    for(VISIOCYTELONG p=0;p<parentschild.size();p++)
//                    {
...
//            }
//        }
        for(VISIOCYTELONG tip=0;tip<tipnodes.size();tip++)
        {
            for(VISIOCYTELONG r=0;r<rootnodes.size();r++)
            {
                //if same position
    ...
    for(VISIOCYTELONG i=0;i<neuron.size();i++)
    {
        ...
        for(VISIOCYTELONG d=0;d<duplicatednodes.size();d++)
        {
...
bool CAnnotations::Sort_SWC(QList<NeuronSWC> & neurons, QList<NeuronSWC> & result, VISIOCYTELONG newrootid)
{
    double thres=0;
...
//        QList<VISIOCYTELONG> test = LUT.keys();
...
bool CAnnotations::Sort_SWC_NewVersion(QList<NeuronSWC> & neurons, QList<NeuronSWC> & result,VISIOCYTELONG newrootid)
{
...
void CAnnotations::save(const char* filepath,bool removedupnode) throw (RuntimeException)
{
    ...
    //saving ano file
//    visiocyte_msg(QString(filepath));
    ...
    if(filename.endsWith(".ano")){
        ...
        visiocyte_msg("Input is not an ano file.");
        return;
    }
//    visiocyte_msg(QString("filename: %1").arg(filename));

    QString fileprefix(filepath);
    if(fileprefix.indexOf("/") != -1){
        fileprefix.remove(fileprefix.lastIndexOf("/")+1, fileprefix.size());
    }
//    visiocyte_msg(QString("fileprefix: %1").arg(fileprefix));
    ...
    {
        if(SOMA_FOUND>1){
            visiocyte_msg("Multiple node locations typed as soma in your input swc.\n"
        ...
        if(soma_name == -1){
            visiocyte_msg("No soma detected in the input swc.\n"
                    "If root of output swc does not match with the real soma,\n"
                    "please double check your input swc.\n"
                    );
        ...
        for(VISIOCYTELONG countNode=0;countNode<nt_sort.size();countNode++)
        {
...
//            visiocyte_msg(QString("Test_1.1. Input swc is: %1").arg(dir.absolutePath().append("/").append(tf::clcr(tokens[1]).c_str())));
            std::map<int, annotation*> annotationsMap;
            std::map<int, NeuronSWC*> swcMap;
...
//                if((!soma_found) && (i->pn == -1) && (i->type == 1)){
                if(i->type == 1){
//                    visiocyte_msg(QString("Soma found: %1 %2 %3 %4").arg(i->n).arg(i->x).arg(i->y).arg(i->z));
                    if(SOMA_FOUND == 0){
...
void CAnnotations::convertMaMuT2APO(std::string MaMuTPath, std::string apoPath) throw (tf::RuntimeException)
{
    ...
    while(frameElem)
    {
        ...
        if(frame_id != 0)
        {
            ...
            if(first_time)
            {
                visiocyte_msg("Spots within frame != 0 will be ignored");
                first_time = false;
            }
            continue;
        }
