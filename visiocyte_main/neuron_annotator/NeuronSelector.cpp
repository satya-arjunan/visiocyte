...
//
bool NeuronSelector::inNeuronMask(VISIOCYTELONG x, VISIOCYTELONG y, VISIOCYTELONG z)
{
...
QList<ImageMarker> NeuronSelector::highlightIndexNeuron()
{
    ...
    { // read lock stanza
        ...
        // mesh grids
        for(VISIOCYTELONG k=0; k<neuronProxy.sz; k+=STEP){
            for(VISIOCYTELONG j=0; j<neuronProxy.sy; j+=STEP){
                for(VISIOCYTELONG i=0; i<neuronProxy.sx; i+=STEP){
                    ...
                    if(inNeuronMask(i,j,k))
                    {
                        ...
                        //
                        VISIOCYTELONG xb = i-NB; if(xb<0) xb = 0;
                        VISIOCYTELONG xe = i+NB; if(xe>=sx) xe = sx-1;
                        VISIOCYTELONG yb = j-NB; if(yb<0) yb = 0;
                        VISIOCYTELONG ye = j+NB; if(ye>=sy) ye = sy-1;
                        VISIOCYTELONG zb = k-NB; if(zb<0) zb = 0;
                        VISIOCYTELONG ze = k+NB; if(ze>=sz) ze = sz-1;

                        for(VISIOCYTELONG kk=zb; kk<=ze; kk++)
                        {
                            VISIOCYTELONG offset_kk = kk*sx*sy;
                            for(VISIOCYTELONG jj=yb; jj<=ye; jj++)
                            {
                                VISIOCYTELONG offset_jj = offset_kk + jj*sx;
                                for(VISIOCYTELONG ii=xb; ii<=xe; ii++)
                                {
                                    VISIOCYTELONG idx = offset_jj + ii;
                        ...
                        {
                            ImageMarker p;
                            p.x = (VISIOCYTELONG)(sumx/(float)count+1.5); // 1-based
                            p.y = (VISIOCYTELONG)(sumy/(float)count+1.5);
                            p.z = (VISIOCYTELONG)(sumz/(float)count+1.5);
                            // RGBA8 c;
