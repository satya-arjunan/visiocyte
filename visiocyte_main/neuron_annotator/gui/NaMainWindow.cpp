    ...
    , cutPlanner(NULL)
{
    ...
    //connect(ui->compartmentMapWidget, SIGNAL(viscomp3dview(QList<LabelSurf>)), (Renderer_gl1*)(ui->visiocyter_glwidget->getRenderer()), SLOT(setListLabelSurf(QList<LabelSurf>))); // vis compartments in Na3Dviewer
    ...
    connect(ui->rotationResetButton, SIGNAL(clicked()),
            ui->visiocyter_glwidget, SLOT(resetRotation()));
    ...
    connect(&(ui->visiocyter_glwidget->cameraModel), SIGNAL(rotationChanged(const Rotation3D&)),
    ...
    connect(ui->rotZWidget, SIGNAL(angleChanged(int)),
            this, SLOT(update3DViewerXYZBodyRotation()));
    connect(ui->visiocyter_glwidget, SIGNAL(progressValueChanged(int)),
            this, SLOT(set3DProgress(int)));
    connect(ui->visiocyter_glwidget, SIGNAL(progressComplete()),
            this, SLOT(complete3DProgress()));
    connect(ui->visiocyter_glwidget, SIGNAL(progressMessageChanged(QString)),
            this, SLOT(set3DProgressMessage(QString)));
    connect(ui->visiocyter_glwidget, SIGNAL(progressAborted(QString)),
            this, SLOT(complete3DProgress()));

    // 3D volume cut
    connect(ui->visiocyter_glwidget, SIGNAL(changeXCut0(int)), ui->XcminSlider, SLOT(setValue(int))); // x-cut
    connect(ui->XcminSlider, SIGNAL(valueChanged(int)), ui->visiocyter_glwidget, SLOT(setXCut0(int)));
    connect(ui->visiocyter_glwidget, SIGNAL(changeXCut1(int)), ui->XcmaxSlider, SLOT(setValue(int)));
    connect(ui->XcmaxSlider, SIGNAL(valueChanged(int)), ui->visiocyter_glwidget, SLOT(setXCut1(int)));

    connect(ui->visiocyter_glwidget, SIGNAL(changeYCut0(int)), ui->YcminSlider, SLOT(setValue(int))); // y-cut
    connect(ui->YcminSlider, SIGNAL(valueChanged(int)), ui->visiocyter_glwidget, SLOT(setYCut0(int)));
    connect(ui->visiocyter_glwidget, SIGNAL(changeYCut1(int)), ui->YcmaxSlider, SLOT(setValue(int)));
    connect(ui->YcmaxSlider, SIGNAL(valueChanged(int)), ui->visiocyter_glwidget, SLOT(setYCut1(int)));

    connect(ui->visiocyter_glwidget, SIGNAL(changeZCut0(int)), ui->ZcminSlider, SLOT(setValue(int))); // z-cut
    connect(ui->ZcminSlider, SIGNAL(valueChanged(int)), ui->visiocyter_glwidget, SLOT(setZCut0(int)));
    connect(ui->visiocyter_glwidget, SIGNAL(changeZCut1(int)), ui->ZcmaxSlider, SLOT(setValue(int)));
    connect(ui->ZcmaxSlider, SIGNAL(valueChanged(int)), ui->visiocyter_glwidget, SLOT(setZCut1(int)));

    connect(ui->XCutCB, SIGNAL(toggled(bool)), ui->visiocyter_glwidget, SLOT(setXCutLock(bool)));
    connect(ui->YCutCB, SIGNAL(toggled(bool)), ui->visiocyter_glwidget, SLOT(setYCutLock(bool)));
    connect(ui->ZCutCB, SIGNAL(toggled(bool)), ui->visiocyter_glwidget, SLOT(setZCutLock(bool)));

    connect(ui->slabThicknessSlider, SIGNAL(valueChanged(int)),
            ui->visiocyter_glwidget, SLOT(setSlabThickness(int)));
    connect(ui->slabPositionSlider, SIGNAL(valueChanged(int)),
            ui->visiocyter_glwidget, SLOT(setSlabPosition(int)));
    connect(ui->visiocyter_glwidget, SIGNAL(slabThicknessChanged(int)),
            this, SLOT(onSlabThicknessChanged(int)));
            // ui->slabThicknessSlider, SLOT(setValue(int)));
    connect(ui->visiocyter_glwidget, SIGNAL(slabPositionChanged(int)),
            ui->slabPositionSlider, SLOT(setValue(int)));
    connect(ui->freezeFrontBackButton, SIGNAL(clicked()),
            ui->visiocyter_glwidget, SLOT(clipSlab()));

    // alpha blending
    connect(ui->action3D_alpha_blending, SIGNAL(toggled(bool)),
            ui->visiocyter_glwidget, SLOT(setAlphaBlending(bool)));
    connect(ui->visiocyter_glwidget, SIGNAL(alphaBlendingChanged(bool)),
            ui->action3D_alpha_blending, SLOT(setChecked(bool)));

    // show axes
    connect(ui->actionShow_Axes, SIGNAL(toggled(bool)),
            ui->visiocyter_glwidget, SLOT(setShowCornerAxes(bool)));
    // show scale bar
    connect(ui->actionShow_Scale_Bar, SIGNAL(toggled(bool)),
            ui->visiocyter_glwidget, SLOT(showScaleBar(bool)));
    ...
    connect(this, SIGNAL(crosshairVisibilityChanged(bool)),
            ui->visiocyter_glwidget, SLOT(showCrosshair(bool)));
    connect(this, SIGNAL(crosshairVisibilityChanged(bool)),
    ...
    // connect(ui->actionShow_Axes, SIGNAL(toggled(bool)),
    //         ui->visiocyter_glwidget, SLOT(enableShowAxes(bool)));
    ...
    ui->visiocyter_glwidget->setUndoStack(*undoStack);
    ...
    connect(ui->visiocyter_glwidget, SIGNAL(neuronSelected(double,double,double)),
    ...
    connect(ui->visiocyter_glwidget, SIGNAL(benchmarkTimerPrintRequested(QString)),
            this, SIGNAL(benchmarkTimerPrintRequested(QString)));
    connect(ui->visiocyter_glwidget, SIGNAL(benchmarkTimerResetRequested()),
...
QString NaMainWindow::getDataDirectoryPathWithDialog()
{
    ...
    QSettings settings(QSettings::UserScope, "HHMI", "Visiocyte");
...
QString NaMainWindow::getStackPathWithDialog()
{
    ...
    QString fileName = QFileDialog::getOpenFileName(this,
                                                    "Select volume file",
                                                    initialDialogPath,
                                                    tr("Stacks (*.lsm *.tif *.tiff *.mp4 *.visiocyteraw *.visiocytepbd)"));
...
void NaMainWindow::on_action1280x720_triggered() {
    int w = ui->visiocyter_glwidget->width();
    int h = ui->visiocyter_glwidget->height();
    int dw = 1280 - w;
    int dh = 720 - h;
...
void NaMainWindow::on_actionAdd_landmark_at_cursor_triggered() {
    // qDebug() << "add landmark";
    RendererNeuronAnnotator * rna = ui->visiocyter_glwidget->getRendererNa();
    if (rna != NULL) {
        float radius = 20.0;
...
void NaMainWindow::on_actionMeasure_Frame_Rate_triggered()
{
    ...
    for (int deg = 0; deg < numSteps; ++deg)
    {
        ...
        ui->visiocyter_glwidget->updateGL();
        QCoreApplication::processEvents();
    }
...
void NaMainWindow::on_actionSave_movie_frames_triggered() {
    ...
    while (currentMovie.hasMoreFrames()) {
        ...
        QImage grabbedFrame = ui->visiocyter_glwidget->grabFrameBuffer(true); // true->with alpha
        frameIndex += 1;
        QString fileName;
...
void NaMainWindow::on_zThicknessDoubleSpinBox_valueChanged(double val) {
    ui->visiocyter_glwidget->setThickness(val);
}
...
QUrl checkDragEvent(QDropEvent* event)
{
    ...
    if (fileExtension.startsWith("tif")) // tif or tiff
        return url;
    if (fileExtension.startsWith("visiocyte")) // visiocyteraw or visiocytepdb
        return url;
#ifdef USE_FFMPEG
    if (fileExtension.startsWith("mp4")) // visiocyteraw or visiocytepdb
        return url;
#endif
...
void NaMainWindow::moveEvent ( QMoveEvent * event )
{
    ...
    ui->visiocyter_glwidget->updateScreenPosition();
    QMainWindow::moveEvent(event);
}

void NaMainWindow::loadSingleStack(QUrl url)
{
    // Default to NeuronAnnotator, not Visiocyte classic, when URL is given
...
/* slot */
void NaMainWindow::loadSingleStack(QUrl url, bool useVisiocyteClassic)
{
    if (url.isEmpty())
        return;
    if (! url.isValid())
    ...
    if (useVisiocyteClassic) {
        // Open in Visiocyte classic mode
        ui->actionVISIOCYTEDefault->trigger(); // switch mode
        QString fileName = url.toLocalFile();
        if (! fileName.isEmpty())
            emit defaultVisiocyteFileLoadRequested(fileName);
        else
            emit defaultVisiocyteUrlLoadRequested(url);
    }
    else
    {
        setViewMode(VIEW_SINGLE_STACK);
...
void NaMainWindow::applyCustomCut()
{
    ...
    ui->visiocyter_glwidget->applyCustomCut();
...
void NaMainWindow::applyCustomKeepPlane()
{
    ...
    // assert(isInCustomCutMode);
    ui->visiocyter_glwidget->applyCustomKeepPlane();
    if (isInCustomCutMode)
        toggleCustomCutMode();
}
...
void NaMainWindow::setCustomCutMode(bool doCustom)
{
    if (doCustom)
    {
        ...
        ui->visiocyter_glwidget->setCustomCutMode();
    }
    else
    {
        ...
        ui->visiocyter_glwidget->cancelCustomCutMode();
    }
...
void NaMainWindow::initializeStereo3DOptions()
{
    ...
    connect(ui->actionMono_Off, SIGNAL(toggled(bool)),
            ui->visiocyter_glwidget, SLOT(setStereoOff(bool)));
    connect(ui->actionLeft_eye_view, SIGNAL(toggled(bool)),
            ui->visiocyter_glwidget, SLOT(setStereoLeftEye(bool)));
    connect(ui->actionRight_eye_view, SIGNAL(toggled(bool)),
            ui->visiocyter_glwidget, SLOT(setStereoRightEye(bool)));
    connect(ui->actionQuadro_120_Hz, SIGNAL(toggled(bool)),
            ui->visiocyter_glwidget, SLOT(setStereoQuadBuffered(bool)));
    connect(ui->actionAnaglyph_Red_Cyan, SIGNAL(toggled(bool)),
            ui->visiocyter_glwidget, SLOT(setStereoAnaglyphRedCyan(bool)));
    connect(ui->actionAnaglyph_Green_Magenta, SIGNAL(toggled(bool)),
            ui->visiocyter_glwidget, SLOT(setStereoAnaglyphGreenMagenta(bool)));
    connect(ui->actionRow_Interleaved_Zalman, SIGNAL(toggled(bool)),
            ui->visiocyter_glwidget, SLOT(setStereoRowInterleaved(bool)));
    connect(ui->actionChecker_Interleaved_3DTV, SIGNAL(toggled(bool)),
            ui->visiocyter_glwidget, SLOT(setStereoCheckerInterleaved(bool)));
    connect(ui->actionColumn_Interleaved, SIGNAL(toggled(bool)),
            ui->visiocyter_glwidget, SLOT(setStereoColumnInterleaved(bool)));

    connect(ui->visiocyter_glwidget, SIGNAL(quadStereoSupported(bool)),
...
void NaMainWindow::initializeContextMenus()
{
    ...
    ui->visiocyter_glwidget->setContextMenus(viewerContextMenu, neuronContextMenu);
}
...
void NaMainWindow::nutate(const Rotation3D& R) {
    // qDebug() << "nutate!";
    // std::cout << R << std::endl;
    CameraModel& cam = ui->visiocyter_glwidget->cameraModel;
    if (!ui->visiocyter_glwidget->mouseIsDragging()) {
...
void NaMainWindow::resetView()
{
    ...
    Vector3D newFocus = ui->visiocyter_glwidget->getDefaultFocus();
...
void NaMainWindow::unifyCameras(bool bDoUnify)
{
    ...
    if (bDoUnify) {
        ...
        ui->visiocyter_glwidget->synchronizeWithCameraModel(&sharedCameraModel);
        ...
        ui->visiocyter_glwidget->decoupleCameraModel(&sharedCameraModel);
...
void NaMainWindow::handleCoordinatedCloseEvent(QCloseEvent *e)
{
    if (isVisible())
    {
        ...
        // These settings affect both NaMainWindow and classic VISIOCYTE MainWindows.  So only use
...
{
    VisiocyteApplication::handleCloseEvent(e);
}
...
void NaMainWindow::on_actionVISIOCYTEDefault_triggered() {
...
void NaMainWindow::setVISIOCYTEDefaultModeCheck(bool checkState) {
    QAction* ui_actionVISIOCYTEDefault = this->findChild<QAction*>("actionVISIOCYTEDefault");
    ui_actionVISIOCYTEDefault->setChecked(checkState);
}
...
{
    if (cutPlanner == NULL) {
        cutPlanner = new CutPlanner(sharedCameraModel, *ui->visiocyter_glwidget, this);
...
bool NaMainWindow::openMulticolorImageStack(QUrl url)
{
    ...
    ui->visiocyter_glwidget->clearImage();

    createNewDataFlowModel();
    // reset front/back clip slab
    ui->visiocyter_glwidget->resetSlabThickness();
...
void NaMainWindow::on_action3D_Volume_triggered()
{
    ...
    QString fileTypes = "*.visiocytepbd *.visiocyteraw *.tif";
#ifdef USE_FFMPEG
    fileTypes += " *.mp4";
#endif
    ...
    if (fi.suffix().isEmpty())
    	    filename = filename + ".visiocytepbd";
    fooDebug() << filename;
...
void NaMainWindow::on_actionScreenShot_triggered() {
    ...
    QString filename = QFileDialog::getSaveFileName(
        ui->visiocyter_glwidget,
        QObject::tr("Save 3D View to an image file"),
        dirname,
        QObject::tr("Images (*.tif *.png *.jpg *.ppm *.xpm)"));
    if (filename.isEmpty())
        return; // User cancelled
    bool saved = ui->visiocyter_glwidget->screenShot(filename);
    if (saved) {
        QMessageBox::information(ui->visiocyter_glwidget,
...
void NaMainWindow::on_actionX_Rotation_Movie_triggered()
{
    ...
    dRot.setRotationFromAngleAboutUnitVector(
            ...
            UnitVector3D(1, 0, 0));
    Rotation3D currentRotation = sharedCameraModel.rotation();
    ui->visiocyter_glwidget->resize(1280, 720);
    for (int f = 0; f < frameCount; ++f)
    {
        ...
        currentRotation = dRot * currentRotation;
        sharedCameraModel.setRotation(currentRotation);
        ui->visiocyter_glwidget->repaint();
        QCoreApplication::processEvents();
        QImage frameImage = ui->visiocyter_glwidget->grabFrameBuffer();
...
void NaMainWindow::setDataFlowModel(DataFlowModel* dataFlowModelParam)
{
    ...
    ui->visiocyter_glwidget->setDataFlowModel(dataFlowModel);
...
bool NaMainWindow::createNewDataFlowModel()
{
    ...
    ui->visiocyter_glwidget->invalidate();
    ui->naZStackWidget->invalidate();
    ui->naLargeMIPWidget->invalidate();
    ui->visiocyter_glwidget->clearImage();
    ui->visiocyter_glwidget->initializeDefaultTextures(); // <- this is how to reset the label texture
    return true;
}


void NaMainWindow::setTitle(QString title) {
    setWindowTitle(QString("%1 - Visiocyte Neuron Annotator").arg(title));
}
...
void NaMainWindow::update3DViewerXYZBodyRotation()
{
    ...
    ui->visiocyter_glwidget->setXYZBodyRotationInt(rotX, rotY, rotZ);
}
...
void NaMainWindow::set3DProgress(int prog)
{
    ...
    if (use3DProgress) {
        ...
        // ui->visiocyter_glwidget->setResizeEnabled(false); // don't show ugly brief resize behavior
...
void NaMainWindow::complete3DProgress()
{
    if (use3DProgress) {
        ...
        ui->visiocyter_glwidget->resizeEvent(NULL);
        ui->visiocyter_glwidget->setResizeEnabled(true);
        //
        ui->visiocyter_glwidget->update();
    }
