    ...
    , keepPlane(0,0,0,0)
{
    ...
    tryTexNPT = true; // Don't use Visiocyte rescale of image dimensions (because we are taking care of that...)
...
void RendererNeuronAnnotator::loadShader()
{
    ...
    try {
            ...
            bool bUseClassicVisiocyteShader = false;
        ...
        for(chno=0; chno <= maxChan; chno++)
        {
           ...
           P1 = P1ori;
            ...
            if (data_ptr)
            {
                ...
                // this one segfault.
                VISIOCYTELONG maxOffsets = dim1*dim2*dim3 - 1;
                VISIOCYTELONG minOffsets = 0;
...
void RendererNeuronAnnotator::setupData(void* idep)
{
    ...
    // So factor out call to visiocyter_getImage4d
