...
Na3DWidget::Na3DWidget(QWidget* parent)
        : VisiocyteR_GLWidget(NULL, parent, "Title")
...
{
        // qDebug() << "Na3DWidget::VisiocyteR_GLWidget::preparingRenderer" << __FILE__ << __LINE__;
...
void Na3DWidget::moveEvent(QMoveEvent * event)
{
    ...
    VisiocyteR_GLWidget::moveEvent(event);
}
...
void Na3DWidget::initializeGL()
{
    ...
    VisiocyteR_GLWidget::initializeGL();
...
void Na3DWidget::keyPressEvent(QKeyEvent *e)
{
    ...
    // Only forward certain keys to Visiocyte classic effect
    switch(e->key()) {
    // case Qt::Key_Minus: // zoom out
    // case Qt::Key_Equal: // zoom in
    case Qt::Key_F: // pixel interpolation
        VisiocyteR_GLWidget::keyPressEvent(e);
        break;
    default:
...
{
    updateCursor();
    VisiocyteR_GLWidget::keyReleaseEvent(e);
}
...
void Na3DWidget::mouseMoveEvent(QMouseEvent * event)
{
    ...
    // so revert to default VISIOCYTE behavior.
    /*
    if (! (event->buttons() & Qt::LeftButton) )
    {
        bMouseIsDragging = false;
        VisiocyteR_GLWidget::mouseMoveEvent(event); // use classic VISIOCYTE behavior
        return;
    }
     */
    ...
    {
        // qDebug() << "rotate";
        bool bUseClassicVisiocyteRotation = false;
        if (bUseClassicVisiocyteRotation)
            VisiocyteR_GLWidget::mouseMoveEvent(event); // regular VISIOCYTE rotate behavior
        else {
            ...
            // I think we have to use "dx" instead of "-dx" to compensate for a y-flip deep in VISIOCYTE
...
void Na3DWidget::mousePressEvent(QMouseEvent * event)
{
	...
	// VisiocyteR_GLWidget::mousePressEvent(event);
...
void Na3DWidget::mouseReleaseEvent(QMouseEvent * event)
{
	...
	// VisiocyteR_GLWidget::mouseReleaseEvent(event);
...
int Na3DWidget::neuronAt(QPoint pos)
{
    ...
    std::vector<int> sum;
    {
        ...
        //
        VISIOCYTELONG xb = xlc-NB; if(xb<0) xb = 0;
        VISIOCYTELONG xe = xlc+NB; if(xe>sx) xe = sx-1;
        VISIOCYTELONG yb = ylc-NB; if(yb<0) yb = 0;
        VISIOCYTELONG ye = ylc+NB; if(ye>sy) ye = sy-1;
        VISIOCYTELONG zb = zlc-NB; if(zb<0) zb = 0;
        VISIOCYTELONG ze = zlc+NB; if(ze>sz) ze = sz-1;
    ...
    int neuronSum=0;
    for(VISIOCYTELONG i=0; i<numNeuron; i++)
    {
...
void Na3DWidget::mouseDoubleClickEvent(QMouseEvent * event)
{
    ...
    updateCursor();
    if (event->button() != Qt::LeftButton) {
        VisiocyteR_GLWidget::mouseDoubleClickEvent(event);
        return;
    }
...
{
    if (bResizeEnabled)
        VisiocyteR_GLWidget::resizeGL(w, h);
}
...
void Na3DWidget::updateFocus(const Vector3D& f)
{
    ...
    // see VisiocyteR_GLWidget::paintGL() method in visiocyter_glwidget.cpp
...
void Na3DWidget::paint_mono(bool clearColorFirst)
{
    ...
    if (bPaintCrosshair)
    {
        ...
        // Flip axes corresponding to visiocyter_glwidget flip_X, flip_Y, flip_Z
        focus0.x() *= flip_X;
        focus0.y() *= flip_Y;
        focus0.z() *= flip_Z;
        focus0.z() *= _thickness;
