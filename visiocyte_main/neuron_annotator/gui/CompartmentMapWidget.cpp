...
CompartmentMapWidget::CompartmentMapWidget(QWidget* parent): VisiocyteR_GLWidget(NULL, parent, "Compartment Map")
{
...
{
    //QString atlasfile(":/atlas/flybraincompartmentmap.visiocytes"); // .qrc
    QString flybrainatlas("flybraincompartmentmap.visiocytes");
    ...
    {
        ((Renderer_gl1 *)renderer)->loadVISIOCYTESFile(atlasfile);
...
void CompartmentMapWidget::resizeGL(int width, int height){
    VisiocyteR_GLWidget::resizeGL(width, height);
}
void CompartmentMapWidget::paintGL(){
    VisiocyteR_GLWidget::paintGL();
}
...
void CompartmentMapWidget::mouseMoveEvent(QMouseEvent *event){
    VisiocyteR_GLWidget::mouseMoveEvent(event);
}
