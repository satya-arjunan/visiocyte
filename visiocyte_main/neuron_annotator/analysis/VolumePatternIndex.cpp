...
bool VolumePatternIndex::openIndexAndWriteHeader() {
    ...
    fwrite(&x0, sizeof(VISIOCYTELONG), 1, fid);
    fwrite(&x1, sizeof(VISIOCYTELONG), 1, fid);

    fwrite(&y0, sizeof(VISIOCYTELONG), 1, fid);
    fwrite(&y1, sizeof(VISIOCYTELONG), 1, fid);

    fwrite(&z0, sizeof(VISIOCYTELONG), 1, fid);
    fwrite(&z1, sizeof(VISIOCYTELONG), 1, fid);
    ...
    fwrite(&iXmax, sizeof(VISIOCYTELONG), 1, fid);
    fwrite(&iYmax, sizeof(VISIOCYTELONG), 1, fid);
    fwrite(&iZmax, sizeof(VISIOCYTELONG), 1, fid);
...
bool VolumePatternIndex::doSearch()
{
    ...
    VISIOCYTELONG indexTotal=iXmax*iYmax*iZmax;
    ...
    // Binary check
    VISIOCYTELONG zeroCount=0L;
    VISIOCYTELONG oneCount=0L;
    VISIOCYTELONG otherCount=0L;
    unsigned char* qData = queryImage->getRawDataAtChannel(queryChannel);
    VISIOCYTELONG units=queryImage->getTotalUnitNumber();
    ...
    if (skipzeros) {
        ...
        VISIOCYTELONG nonskipCount=0L;
        VISIOCYTELONG skipCount=0L;
        VISIOCYTELONG cubifiedTotal=iXmax*iYmax*iZmax;
  ...
  while( (readSize=fread(&idLength, 1, sizeof(int), fid))==sizeof(int) ) {
    ...
    VISIOCYTELONG score=calculateIndexScore(queryIndex, indexData, indexTotal, queryIndexSkipPositions);
    qDebug() << "score=" << score;
  ...
  QList< QPair<VISIOCYTELONG, int> > pairList;
  ...
  QList< QPair<VISIOCYTELONG, int> > finalResultList;
  ...
  for (int i=0;i<finalResultSize;i++) {
    int position=i+1;
    QPair<VISIOCYTELONG, int> p=finalResultList[i];
    VISIOCYTELONG score=p.first;
    int index=p.second;
...
VISIOCYTELONG VolumePatternIndex::computeTotalBytesFromIndexTotal(VISIOCYTELONG indexTotal) {
    VISIOCYTELONG result=ceil((indexTotal*1.0)/4.0);
    return result;
}

VISIOCYTELONG VolumePatternIndex::calculateIndexScore(unsigned char* queryIndex, unsigned char* subjectIndex, VISIOCYTELONG indexTotal, unsigned char* skipPositions=0L)
{
    VISIOCYTELONG score=0L;
    VISIOCYTELONG byteTotalFloor=indexTotal/4;
    VISIOCYTELONG byteTotal=computeTotalBytesFromIndexTotal(indexTotal);
    ...
    VISIOCYTELONG* matrixBins = 0L;
    VISIOCYTELONG* matrixScores = 0L;

    if (DEBUG_FLAG) {

        matrixBins = new VISIOCYTELONG[16];
        matrixScores = new VISIOCYTELONG[16];
    ...
    if (skipPositions==0) {
        ...
        for (VISIOCYTELONG i=0;i<byteTotalFloor;i++) {
        ...
        for (VISIOCYTELONG i=byteTotalFloor;i<byteTotal;i++) {
            unsigned char s=subjectIndex[i];
            unsigned char q=queryIndex[i];
            for (int p=0;p<4;p++) {
...
void VolumePatternIndex::indexImage(My4DImage* image, int channel, bool skipzeros=false)
{
    ...
    VISIOCYTELONG count0=0L;
    VISIOCYTELONG count1=0L;
    VISIOCYTELONG count2=0L;
    VISIOCYTELONG count3=0L;

    My4DImage* cubifiedImage=0L;

    VISIOCYTELONG subregion[6];
    ...
    debugLoader.saveImage(cubifiedImage, "cubifiedDebugImage.visiocytepbd");
    ...
    VISIOCYTELONG tempIndexTotalBytes=computeTotalBytesFromIndexTotal(cubifiedTotal);
    ...
    VISIOCYTELONG indexPosition=0;
    int bytePosition=0;
    unsigned char cvalue=0;

    visiocyte_uint8* cubifiedData=cubifiedImage->getRawDataAtChannel(0);
    for (VISIOCYTELONG z=0;z<iZmax;z++) {
        VISIOCYTELONG zoffset=z*iYmax*iXmax;
        for (VISIOCYTELONG y=0;y<iYmax;y++) {
            VISIOCYTELONG yoffset=zoffset+y*iXmax;
            for (VISIOCYTELONG x=0;x<iXmax;x++) {
                VISIOCYTELONG offset=yoffset+x;
                visiocyte_uint8 value=cubifiedData[offset];
                if (bytePosition>3) {
                    bytePosition=0;
...
bool VolumePatternIndex::openIndexAndReadHeader()
{
    ...
    // 1. Image Size
    fread(&x0, sizeof(VISIOCYTELONG), 1, fid);
    fread(&x1, sizeof(VISIOCYTELONG), 1, fid);

    fread(&y0, sizeof(VISIOCYTELONG), 1, fid);
    fread(&y1, sizeof(VISIOCYTELONG), 1, fid);

    fread(&z0, sizeof(VISIOCYTELONG), 1, fid);
    fread(&z1, sizeof(VISIOCYTELONG), 1, fid);
    ...
    // 4. Cube Size
    fread(&iXmax, sizeof(VISIOCYTELONG), 1, fid);
    fread(&iYmax, sizeof(VISIOCYTELONG), 1, fid);
    fread(&iZmax, sizeof(VISIOCYTELONG), 1, fid);
