...
#include "../../visiocyte/histogramsimple.h"
...
bool ScreenPatternAnnotator::createMaskGuide() {
  ...
  maskPath.append("Mask.visiocytepbd");
  ...
  for (int maskIndex=0;maskIndex<maskNameList.length();maskIndex++) {
    ...
    ghostStackFilename.append(".visiocytepbd");
...
My4DImage * ScreenPatternAnnotator::createMaskIndexGhostImage(My4DImage * rgbMask, int index, QList<int> redList, QList<int> greenList, QList<int> blueList) {
  VISIOCYTELONG cdim=rgbMask->getCDim();
  VISIOCYTELONG xdim=rgbMask->getXDim();
  VISIOCYTELONG ydim=rgbMask->getYDim();
  VISIOCYTELONG zdim=rgbMask->getZDim();
  My4DImage * ghostImage = new My4DImage();
  ghostImage->loadImage(xdim, ydim, zdim, 3, VISIOCYTE_UINT8);

  visiocyte_uint8 * rChan=rgbMask->getRawDataAtChannel(0);
  visiocyte_uint8 * gChan=rgbMask->getRawDataAtChannel(1);
  visiocyte_uint8 * bChan=rgbMask->getRawDataAtChannel(2);

  visiocyte_uint8 * rG=ghostImage->getRawDataAtChannel(0);
  visiocyte_uint8 * gG=ghostImage->getRawDataAtChannel(1);
  visiocyte_uint8 * bG=ghostImage->getRawDataAtChannel(2);

  int maskIndexLength=redList.length();
  for (VISIOCYTELONG z=0;z<zdim;z++) {
    for (VISIOCYTELONG y=0;y<ydim;y++) {
      for (VISIOCYTELONG x=0;x<xdim;x++) {
	VISIOCYTELONG p=z*ydim*xdim+y*xdim+x;
	int r=rChan[p];
	int g=gChan[p];
	int b=bChan[p];
	if (r==0 && g==0 && b==0) {
	  ...
	  if (m==index-1) {
	    ...
	    VISIOCYTELONG grayValue=(m*7633997)%32;
...
bool ScreenPatternAnnotator::createCompartmentIndex() {
    ...
    for (int i=0;i<fileList.size();i++) {
        ...
        if (fileInfo.fileName().endsWith(".tif")) {
            ...
            if (compartmentIndexImage==0) {
                ...
                compartmentIndexImage->loadImage(maskImage->getXDim(), maskImage->getYDim(), maskImage->getZDim(), 1 /* index channel */, VISIOCYTE_UINT8);
                visiocyte_uint8 * ciData=compartmentIndexImage->getRawData();
                for (VISIOCYTELONG ci=0;ci<compartmentIndexImage->getTotalBytes();ci++) {
                    ciData[ci]=0;
                }
            } else {
            ...
            VISIOCYTELONG maskSizeCount=0;
            visiocyte_uint8 * maskData=maskImage->getRawDataAtChannel(0L);
            visiocyte_uint8 * indexData=compartmentIndexImage->getRawData();

            VISIOCYTELONG zmax=maskImage->getZDim();
            VISIOCYTELONG ymax=maskImage->getYDim();
            VISIOCYTELONG xmax=maskImage->getXDim();

            for (VISIOCYTELONG z=0;z<zmax;z++) {
                VISIOCYTELONG zoffset=z*xmax*ymax;
                for (VISIOCYTELONG y=0;y<ymax;y++) {
                    VISIOCYTELONG yoffset=y*xmax+zoffset;
                    VISIOCYTELONG invertedYOffset=(ymax-1-y)*xmax+zoffset;
                    for (VISIOCYTELONG x=0;x<xmax;x++) {
                        VISIOCYTELONG offset=x+yoffset;
                        VISIOCYTELONG invertedOffset=x+invertedYOffset;
    ...
    outputResourceDirPathCopy.append(QDir::separator()).append("compartmentIndex.visiocytepbd");
...
bool ScreenPatternAnnotator::loadCompartmentIndex()
{
    ...
    QString compartmentIndexImageFilepath=resourceDirectoryPathCopy.append(QDir::separator()).append("maskIndex.visiocytepbd");
...
bool ScreenPatternAnnotator::annotate() {
    ...
    if (inputImage->getDatatype()!=VISIOCYTE_UINT8) {
    ...
    VISIOCYTELONG xSize=inputImage->getXDim();
    VISIOCYTELONG ySize=inputImage->getYDim();
    VISIOCYTELONG zSize=inputImage->getZDim();
    VISIOCYTELONG channelSize=xSize*ySize*zSize;
    global256BinHistogram.compute(inputImage->getRawData() + channelSize*patternChannelIndex ,channelSize);
    VISIOCYTELONG* global256BinHistogramArray=global256BinHistogram.getHistogram();

    // Compute Zone Thresholds using Histogram
    zoneThresholds[0]=LOWER_ZONE_THRESHOLD; // hard-coded
    VISIOCYTELONG total=0;
    VISIOCYTELONG weight=0;
    for (int h=(LOWER_ZONE_THRESHOLD+1);h<256;h++) {
        total+=global256BinHistogramArray[h];
        weight+=h*global256BinHistogramArray[h];
    }
    VISIOCYTELONG A1 = LOWER_ZONE_THRESHOLD+1;
    VISIOCYTELONG As = A1+1;
    VISIOCYTELONG A2 = As+1;
    if (total>0) {
        As = weight/total;
    ...
    visiocyte_uint8 * pData=inputImage->getRawDataAtChannel(patternChannelIndex);
    VISIOCYTELONG totalVoxels=0;
    for (VISIOCYTELONG p=0;p<inputImage->getTotalUnitNumberPerChannel();p++) {
        visiocyte_uint8 v=pData[p];
        totalVoxels++;
    ...
    QString filepathToSave(returnFullPathWithOutputPrefix("heatmap16Color.visiocytepbd"));
    ...
    QString compositeFilepathToSave(returnFullPathWithOutputPrefix("compositeMask.visiocytepbd"));
...
My4DImage * ScreenPatternAnnotator::createCompositeMaskImage(My4DImage * sourceImage, My4DImage * indexImage) {
  ...
  compositeImage->loadImage(sourceImage->getXDim(), sourceImage->getYDim(), sourceImage->getZDim(), sourceImage->getCDim(), VISIOCYTE_UINT8);
  ...
  QList<visiocyte_uint8*> sdataList;
  QList<visiocyte_uint8*> cdataList;
  for (int c=0;c<cdim;c++) {
    visiocyte_uint8* sdata=sourceImage->getRawDataAtChannel(c);
    sdataList.append(sdata);
    visiocyte_uint8* cdata=compositeImage->getRawDataAtChannel(c);
    cdataList.append(cdata);
  }
  visiocyte_uint8* idata=indexImage->getRawDataAtChannel(0);
  for (VISIOCYTELONG z=0;z<zdim;z++) {
    for (VISIOCYTELONG y=0;y<ydim;y++) {
      for (VISIOCYTELONG x=0;x<xdim;x++) {
	VISIOCYTELONG position=z*y*x+y*x+x;
	for (int c=0;c<cdim;c++) {
	  visiocyte_uint8* sdata=sdataList[c];
	  visiocyte_uint8* cdata=cdataList[c];
	  if (idata[position]==0) {
	    cdata[position]=0;
	  } else {
...
void ScreenPatternAnnotator::createCompartmentAnnotation(int index, QString abbreviation) {
    ...
    filenameCompartmentHeatmapFullSize.append("_heatmap16Color.visiocytepbd");
    ...
    filenameNormalizedCompartmentHeatmapFullSize.append("_normalized_heatmap16Color.visiocytepbd");
    ...
    VISIOCYTELONG cubeXmax=inputImageCubified->getXDim();
    VISIOCYTELONG cubeYmax=inputImageCubified->getYDim();
    VISIOCYTELONG cubeZmax=inputImageCubified->getZDim();
...
double * ScreenPatternAnnotator::quantifyCompartmentZones(My4DImage * sourceImage, My4DImage * compartmentIndex, int index, BoundingBox3D bb) {
    ...
    visiocyte_uint8 * pData=sourceImage->getRawDataAtChannel(patternChannelIndex);
    visiocyte_uint8 * iData=compartmentIndex->getRawData();
    VISIOCYTELONG zmax=sourceImage->getZDim();
    VISIOCYTELONG ymax=sourceImage->getYDim();
    VISIOCYTELONG xmax=sourceImage->getXDim();
    VISIOCYTELONG compartmentVoxelCount=0;

    for (VISIOCYTELONG z=bb.z0;z<=bb.z1;z++) {
        VISIOCYTELONG zoffset=z*ymax*xmax;
        for (VISIOCYTELONG y=bb.y0;y<=bb.y1;y++) {
            VISIOCYTELONG yoffset=y*xmax+zoffset;
            for (VISIOCYTELONG x=bb.x0;x<=bb.x1;x++) {
                VISIOCYTELONG offset=x+yoffset;
                if (iData[offset]==index) {
                    compartmentVoxelCount++;
                    visiocyte_uint8 v=pData[offset];
                    if (v<=zoneThresholds[0]) {
                        cz[0]++;
                    } else if (v<=zoneThresholds[1]) {
                        cz[1]++;
...
My4DImage * ScreenPatternAnnotator::createViewableImage(My4DImage * sourceImage, int borderSize) {
    ...
    VISIOCYTELONG ymax=sourceImage->getYDim();
    VISIOCYTELONG zmax=sourceImage->getZDim();
    VISIOCYTELONG cmax=sourceImage->getCDim();

    My4DImage * targetImage = new My4DImage();
    double targetRatio=1.0;
    VISIOCYTELONG xViewCenterOffset=0;
    VISIOCYTELONG yViewCenterOffset=0;
    if (xmax>=ymax) {
    ...
    VISIOCYTELONG xv=VIEWABLE_DIMENSION;
    VISIOCYTELONG yv=VIEWABLE_DIMENSION;
    VISIOCYTELONG zv=zmax*targetRatio+2*borderSize;

    targetImage->loadImage(xv, yv, zv, sourceImage->getCDim(), VISIOCYTE_UINT8 /* datatype */);
    memset(targetImage->getRawData(), 0, targetImage->getTotalBytes());

    for (int c=0;c<sourceImage->getCDim();c++) {
        visiocyte_uint8 * sData=sourceImage->getRawDataAtChannel(c);
        visiocyte_uint8 * vData=targetImage->getRawDataAtChannel(c);
        for (VISIOCYTELONG z=borderSize;z<(zv-borderSize);z++) {
            for (VISIOCYTELONG y=borderSize;y<(yv-borderSize);y++) {
                for (VISIOCYTELONG x=borderSize;x<(xv-borderSize);x++) {
                    VISIOCYTELONG sx=((x-borderSize)*1.0+0.5)/targetRatio;
                    VISIOCYTELONG sy=((y-borderSize)*1.0+0.5)/targetRatio;
                    VISIOCYTELONG sz=((z-borderSize)*1.0+0.5)/targetRatio;
                    ...
                    VISIOCYTELONG sPosition=sz*xmax*ymax+sy*xmax+sx;
                    VISIOCYTELONG tPosition=z*xv*yv+(y+yViewCenterOffset)*xv+(x+xViewCenterOffset);
...
My4DImage * ScreenPatternAnnotator::createNormalizedImage(My4DImage * sourceImage, int maskIndex) {
    if (sourceImage->getDatatype()!=VISIOCYTE_UINT8) {
    ...
    normalizedImage->loadImage(sourceImage->getXDim(), sourceImage->getYDim(), sourceImage->getZDim(), sourceImage->getCDim(), VISIOCYTE_UINT8);
    for (int c=0;c<sourceImage->getCDim();c++) {
        ...
        visiocyte_uint8 * cdata=sourceImage->getRawDataAtChannel(c);
        visiocyte_uint8 * ndata=normalizedImage->getRawDataAtChannel(c);
        for (VISIOCYTELONG p=0;p<sourceImage->getTotalUnitNumberPerChannel();p++) {
...
My4DImage * ScreenPatternAnnotator::createSub3DImageFromMask(My4DImage * sourceImage, int index, BoundingBox3D bb) {
    ...
    VISIOCYTELONG xmax=compartmentIndexImage->getXDim();
    VISIOCYTELONG ymax=compartmentIndexImage->getYDim();
    VISIOCYTELONG zmax=compartmentIndexImage->getZDim();

    visiocyte_uint8 * iData=compartmentIndexImage->getRawData();
    visiocyte_uint8 * rData=sourceImage->getRawDataAtChannel(0);
    visiocyte_uint8 * gData=sourceImage->getRawDataAtChannel(1);
    visiocyte_uint8 * bData=sourceImage->getRawDataAtChannel(2);
    ...
    subImage->loadImage( s_xlen, s_ylen, s_zlen , 3 /* rgb */, VISIOCYTE_UINT8 /* datatype */);
    visiocyte_uint8 * sRaw=subImage->getRawData();
    for (VISIOCYTELONG i=0;i<subImage->getTotalBytes();i++) {
        sRaw[i]=0;
    }

    visiocyte_uint8 * s_rData=subImage->getRawDataAtChannel(0);
    visiocyte_uint8 * s_gData=subImage->getRawDataAtChannel(1);
    visiocyte_uint8 * s_bData=subImage->getRawDataAtChannel(2);
...
BoundingBox3D ScreenPatternAnnotator::findBoundingBox3DFromIndex(int index) {
    ...
    VISIOCYTELONG ymax=compartmentIndexImage->getYDim();
    VISIOCYTELONG zmax=compartmentIndexImage->getZDim();
    VISIOCYTELONG x0=xmax;
    VISIOCYTELONG x1=-1;
    VISIOCYTELONG y0=ymax;
    VISIOCYTELONG y1=-1;
    VISIOCYTELONG z0=zmax;
    VISIOCYTELONG z1=-1;
    VISIOCYTELONG offset=0;
    visiocyte_uint8 * data=compartmentIndexImage->getRawData();
    VISIOCYTELONG totalBytes=compartmentIndexImage->getTotalBytes();
    VISIOCYTELONG totalUnits=compartmentIndexImage->getTotalUnitNumber();
    VISIOCYTELONG totalCalcUnits=xmax*ymax*zmax;
    for (VISIOCYTELONG z=0;z<zmax;z++) {
        VISIOCYTELONG zoffset=z*ymax*xmax;
        for (VISIOCYTELONG y=0;y<ymax;y++) {
            VISIOCYTELONG yoffset = y*xmax + zoffset;
...
My4DImage * ScreenPatternAnnotator::create3DHeatmapFromChannel(My4DImage * sourceImage, VISIOCYTELONG sourceChannel, visiocyte_uint8 * lookupTable)
{
    if (sourceImage->getDatatype()!=VISIOCYTE_UINT8) {
    ...
    visiocyte_uint8 * sourceData = sourceImage->getRawDataAtChannel(sourceChannel);
    visiocyte_uint8 * targetDataR = targetImage->getRawDataAtChannel(0L);
    visiocyte_uint8 * targetDataG = targetImage->getRawDataAtChannel(1L);
    visiocyte_uint8 * targetDataB = targetImage->getRawDataAtChannel(2L);
    VISIOCYTELONG totalVoxels=sourceImage->getXDim()*sourceImage->getYDim()*sourceImage->getZDim();
    for (VISIOCYTELONG i=0;i<totalVoxels;i++) {
        visiocyte_uint8 value=sourceData[i];
...
{
    visiocyte_uint8* sData=stackImage->getRawDataAtChannel(stackChannel);
    VISIOCYTELONG xmax=stackImage->getXDim();
    VISIOCYTELONG ymax=stackImage->getYDim();
    VISIOCYTELONG zmax=stackImage->getZDim();
    ...
    VISIOCYTELONG sStart=zOffset*xmax*ymax;
    visiocyte_uint8* rData=image->getRawDataAtChannel(0);
    visiocyte_uint8* gData=image->getRawDataAtChannel(1);
    visiocyte_uint8* bData=image->getRawDataAtChannel(2);
    for (VISIOCYTELONG i=0;i<xmax*ymax;i++) {
        visiocyte_uint8 sv=sData[sStart+i];
        if (sv>0) {
            visiocyte_uint8 rv=rData[i];
            if (rv<GHOST_INTENSITY_LEVEL) {
                rData[i]=GHOST_INTENSITY_LEVEL;
            }
            visiocyte_uint8 gv=gData[i];
            if (gv<GHOST_INTENSITY_LEVEL) {
                gData[i]=GHOST_INTENSITY_LEVEL;
            }
            visiocyte_uint8 bv=bData[i];
...
bool ScreenPatternAnnotator::updateIndex() {
  ...
  VISIOCYTELONG xdim=binaryMaskImage->getXDim();
  VISIOCYTELONG ydim=binaryMaskImage->getYDim();
  VISIOCYTELONG zdim=binaryMaskImage->getZDim();
  ...
  compartmentMaskImage->loadImage(binaryMaskImage->getXDim(), binaryMaskImage->getYDim(), binaryMaskImage->getZDim(), 1, VISIOCYTE_UINT8);
  ...
  visiocyte_uint8 * maskData=binaryMaskImage->getRawDataAtChannel(0);

  visiocyte_uint8 * cmaskData=compartmentMaskImage->getRawDataAtChannel(0);
  visiocyte_uint8 * omaskData=outsideMaskImage->getRawDataAtChannel(0);

  visiocyte_uint8 * compartmentData=compartmentIndexImage->getRawDataAtChannel(0);

  QMap<int,int> compartmentToMaskMap;
  QMap<QString,int> outsideToMaskMap;

  double OUTSIDE_CUBE_PERC = 0.10;
  VISIOCYTELONG OUTSIDE_CUBE_LENGTH = OUTSIDE_CUBE_PERC * xdim;
  ...
  for (VISIOCYTELONG z=0;z<zdim;z++) {
    for (VISIOCYTELONG y=0;y<ydim;y++) {
      for (VISIOCYTELONG x=0;x<xdim;x++) {
	VISIOCYTELONG position=z*ydim*xdim+y*xdim+x;
  ...
  VISIOCYTELONG dataLength=xdim*ydim*zdim;
  int startingOutsideIndex=compartmentToMaskMap.size()+1;
  for (VISIOCYTELONG i=0;i<dataLength;i++) {
    if (omaskData[i]>0) {
  ...
  rgbImage->loadImage(xdim,ydim,zdim,3,VISIOCYTE_UINT8);
  visiocyte_uint8 * r0=rgbImage->getRawDataAtChannel(0);
  visiocyte_uint8 * r1=rgbImage->getRawDataAtChannel(1);
  visiocyte_uint8 * r2=rgbImage->getRawDataAtChannel(2);
  for (VISIOCYTELONG p=0;p<dataLength;p++) {
...
{
  const visiocyte_uint8 T1 = 10;
  const visiocyte_uint8 T2 = 20;

  VISIOCYTELONG xsize=targetStack->getXDim();
  VISIOCYTELONG ysize=targetStack->getYDim();
  VISIOCYTELONG zsize=targetStack->getZDim();
  VISIOCYTELONG csize=targetStack->getCDim();
  ...
  visiocyte_uint8** targetChannelSet = new visiocyte_uint8*[csize];
  visiocyte_uint8** subjectChannelSet = new visiocyte_uint8*[csize];

  for (VISIOCYTELONG c=0;c<csize;c++) {
    targetChannelSet[c]=targetStack->getRawDataAtChannel(c);
    subjectChannelSet[c]=subjectStack->getRawDataAtChannel(c);
  }

  VISIOCYTELONG productTotal=0;
  VISIOCYTELONG offset=0;
  VISIOCYTELONG imageSize=zsize*ysize*xsize;

  for (VISIOCYTELONG i=0;i<imageSize;i++) {

    visiocyte_uint8 tr=targetChannelSet[0][i];
    visiocyte_uint8 tg=targetChannelSet[1][i];
    visiocyte_uint8 tb=targetChannelSet[2][i];

    visiocyte_uint8 sr=subjectChannelSet[0][i];
    visiocyte_uint8 sg=subjectChannelSet[1][i];
    visiocyte_uint8 sb=subjectChannelSet[2][i];

    visiocyte_uint8 targetValue=AnalysisTools::getReverse16ColorLUT(lut16Color, tr, tg, tb);
    visiocyte_uint8 subjectValue=AnalysisTools::getReverse16ColorLUT(lut16Color, sr, sg, sb);
    ...
    VISIOCYTELONG product=((subjectValue-targetValue)*(subjectValue-targetValue))/(subjectValue+targetValue);
...
{
  visiocyte_uint8 * lutV1=create16Color8BitLUT_V3();
  visiocyte_uint8 * lutV2=AnalysisTools::create16Color8BitLUT_fiji();

  ImageLoader v1Loader;
  My4DImage * stackV1 = v1Loader.loadImage(heatmapV1Filepath);
  VISIOCYTELONG zsize=stackV1->getZDim();
  VISIOCYTELONG ysize=stackV1->getYDim();
  VISIOCYTELONG xsize=stackV1->getXDim();
  ...
  stackV2->loadImage(xsize, ysize, zsize, 3, VISIOCYTE_UINT8);

  visiocyte_uint8 * rH1 = stackV1->getRawDataAtChannel(0);
  visiocyte_uint8 * gH1 = stackV1->getRawDataAtChannel(1);
  visiocyte_uint8 * bH1 = stackV1->getRawDataAtChannel(2);

  visiocyte_uint8 * rH2 = stackV2->getRawDataAtChannel(0);
  visiocyte_uint8 * gH2 = stackV2->getRawDataAtChannel(1);
  visiocyte_uint8 * bH2 = stackV2->getRawDataAtChannel(2);

  // Check if stack1 is already V2
  bool v2Flag=false;
  for (VISIOCYTELONG z=0;z<zsize && !v2Flag;z++) {
    for (VISIOCYTELONG y=0;y<ysize && !v2Flag;y++) {
      for (VISIOCYTELONG x=0;x<xsize && !v2Flag;x++) {
	VISIOCYTELONG offset=ysize*xsize*z+y*xsize+x;

	visiocyte_uint8 v1_r=rH1[offset];
	visiocyte_uint8 v1_g=gH1[offset];
	visiocyte_uint8 v1_b=bH1[offset];

    visiocyte_uint8 v1Index = AnalysisTools::getReverse16ColorLUT(lutV1, v1_r, v1_g, v1_b);
    visiocyte_uint8 v2Index = AnalysisTools::getReverse16ColorLUT(lutV2, v1_r, v1_g, v1_b);
  ...
  visiocyte_uint8 v1_r, v1_g, v1_b;
  visiocyte_uint8 v2_r, v2_g, v2_b;

  for (VISIOCYTELONG z=0;z<zsize;z++) {
    for (VISIOCYTELONG y=0;y<ysize;y++) {
      for (VISIOCYTELONG x=0;x<xsize;x++) {
	VISIOCYTELONG offset=ysize*xsize*z+y*xsize+x;
...
visiocyte_uint8 * ScreenPatternAnnotator::create16Color8BitLUT_V2()
{
...
visiocyte_uint8 * ScreenPatternAnnotator::create16Color8BitLUT_V3()
{
    visiocyte_uint8 * lut16 = new visiocyte_uint8[256*3];
