...
bool NeuronFragmentEditor::createMaskFromStack()
{
  ...
  labelImage->loadImage(xdim, ydim, zdim, 1, VISIOCYTE_UINT8);

  visiocyte_uint8* label8 = (visiocyte_uint8*)(labelImage->getRawDataAtChannel(0));

  if (sourceImage->getDatatype()==VISIOCYTE_UINT8) {
    visiocyte_uint8* pre8 = (visiocyte_uint8*)(preImage->getRawDataAtChannel(channel));
    visiocyte_uint8* source8 = (visiocyte_uint8*)(sourceImage->getRawDataAtChannel(0));
    visiocyte_uint8 t = 255.0*threshold;
  ...
  } else if (sourceImage->getDatatype()==VISIOCYTE_UINT16) {
    visiocyte_uint16* pre16 = (visiocyte_uint16*)(preImage->getRawDataAtChannel(channel));
    visiocyte_uint16* source16 = (visiocyte_uint16*)(sourceImage->getRawDataAtChannel(0));
    visiocyte_uint16 t = 4095.0*threshold;
    long rawSize=xdim*ydim*zdim;
    for (long i=0;i<rawSize;i++) {
      source16[i]=pre16[i];
      if (pre16[i]>t) {
...
bool NeuronFragmentEditor::createImagesFromFragmentList(QList<int> fragmentList, QString stackFilename, QString mipFilename)
{
    ...
    compositeImage->loadImage(xdim, ydim, zdim, cdim, VISIOCYTE_UINT8);

    qDebug() << "Setting up channel pointers";

    visiocyte_uint16 * label=(visiocyte_uint16*)(labelImage->getRawDataAtChannel(0));

    visiocyte_uint8 * sourceR=sourceImage->getRawDataAtChannel(0);
    visiocyte_uint8 * sourceG=sourceImage->getRawDataAtChannel(1);
    visiocyte_uint8 * sourceB=sourceImage->getRawDataAtChannel(2);

    visiocyte_uint8 * compR=compositeImage->getRawDataAtChannel(0);
    visiocyte_uint8 * compG=compositeImage->getRawDataAtChannel(1);
    visiocyte_uint8 * compB=compositeImage->getRawDataAtChannel(2);
    ...
    for (VISIOCYTELONG z=0;z<zdim;z++) {
        for (VISIOCYTELONG y=0;y<ydim;y++) {
            for (VISIOCYTELONG x=0;x<xdim;x++) {
                ...
                if (include) {
                    visiocyte_uint8 r=sourceR[offset];
                    visiocyte_uint8 g=sourceG[offset];
                    visiocyte_uint8 b=sourceB[offset];
                    if (r>maxIntensity) {
                        maxIntensity=r;
                    }
                    if (g>maxIntensity) {
...
bool NeuronFragmentEditor::reverseLabel()
{
    ...
    if (labelImage->getDatatype()==VISIOCYTE_UINT8) {
        label8=(visiocyte_uint8*)(labelImage->getRawDataAtChannel(0));
    } else {
        label16=(visiocyte_uint16*)(labelImage->getRawDataAtChannel(0));
    }
...
bool NeuronFragmentEditor::createSurfaceVertex()
{
  ...
  visiocyte_uint8** cdata=new visiocyte_uint8*[cdim];
