...
bool AlignerUtils::referenceRank() {
  ...
  } else if(inputImage->getDatatype()==VISIOCYTE_UINT16) {
    sourceImage=new My4DImage();
    sourceImage->loadImage(xSize, ySize, zSize, inputImage->getCDim(), VISIOCYTE_UINT8);
    VISIOCYTELONG datalength=xSize*ySize*zSize*inputImage->getCDim();
    visiocyte_uint16* p2=(visiocyte_uint16*)inputImage->getRawData();
    visiocyte_uint8* p1=sourceImage->getRawData();
  ...
  VISIOCYTELONG** histogramChannelSet=new VISIOCYTELONG*[sourceImage->getCDim()];
  double highestVH=0.0;
  int highestVHIndex=0;
  int* highScore=new int[sourceImage->getCDim()];
  for (int i=0;i<sourceImage->getCDim();i++) {
    histogramChannelSet[i]=new VISIOCYTELONG[256];
    histogram.compute(sourceImage->getRawData() + channelSize*i, channelSize);
    VISIOCYTELONG* tmpHistogram=histogram.getHistogram();
    VISIOCYTELONG voxelTotal=0L;
    VISIOCYTELONG voxelHigh=0L;
    VISIOCYTELONG voxelLow=0L;
    VISIOCYTELONG voxelVisible=0L;
    for (int j=0;j<256;j++) {
