...
int* VolumeIndex::subsampleAndThresholdToBinaryMask(My4DImage* sourceImage, char* targetMask, int unit, int threshold)
{
  ...
  visiocyte_uint8** rArr=new visiocyte_uint8*[sourceImage->getCDim()];

  for (int i=0;i<sourceImage->getCDim();i++) {
    rArr[i]=sourceImage->getRawDataAtChannel(i);
  }

  const visiocyte_uint8 cThreshold=threshold;
...
bool VolumeIndex::createSecondStageEntry(My4DImage* image, long fragmentId, long sampleId, bool updateFirstStage)
{
  ...
  visiocyte_uint8** rArr=new visiocyte_uint8*[c_dim];
	...
	for (int z2=z1*UNIT;z2<z2_limit;z2++) {
	  ...
	  for (int y2=y1*UNIT;y2<y2_limit;y2++) {
		...
		UNIT_BUFFER[bytePosition]=0; // clear as we move forward
	      ...
	      int rOffset=zyxOffset + yxOffset + x2;
	      char v=-1;
	      visiocyte_uint8 maxV=0;
	      for (int c=0;c<c_dim;c++) {
		visiocyte_uint8 rV=rArr[c][rOffset];
		if (rV > maxV) {
		  maxV=rV;
		}
	      }
...
bool VolumeIndex::loadQueryImage()
{
  ...
  queryImage->loadImage(x_dim, y_dim, z_dim, 1 /* channel */, VISIOCYTE_UINT8);
  ...
  visiocyte_uint8* qArr=queryImage->getRawDataAtChannel(0);
...
void VolumeIndex::dilateQueryMask()
{
  ...
  queryWorkspace->loadImage(X_SIZE, Y_SIZE, Z_SIZE, 1 /* channel */, VISIOCYTE_UINT8);
  DilationErosion de;
...
{
  visiocyte_uint8** rArr=new visiocyte_uint8*[image->getCDim()];
  ...
  for (int z2=z1*UNIT;z2<z2_limit;z2++) {
	...
	for (int c=0;c<c_dim;c++) {
	  visiocyte_uint8 v=rArr[c][rOffset];
	  if (v > maxV) {
	    maxV=v;
	  }
	}
