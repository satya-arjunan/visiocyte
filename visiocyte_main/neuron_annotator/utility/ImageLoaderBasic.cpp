...
static const VISIOCYTELONG   PBD_3_REPEAT_MAX = 4096;
static const double    PBD_3_LITERAL_MAXEFF = 2.66;
static const double    PBD_3_DIFF_MAXEFF = 5.2;
static const VISIOCYTELONG   PBD_3_DIFF_REPEAT_THRESHOLD = 14;
static const VISIOCYTELONG   PBD_3_MIN_DIFF_LENGTH = 6;
static const VISIOCYTELONG   PBD_3_MAX_DIFF_LENGTH = 210;
static const int       PBD_3_REPEAT_STARTING_POSITION = 0;
static const int       PBD_3_REPEAT_COUNT = 1;
static const VISIOCYTELONG   PBD_3_MAX_LITERAL = 24;
...
bool ImageLoaderBasic::loadImage(Image4DSimple * stackp, const char* filepath)
{
    ...
    if (extension == "lsm") {
        // Visiocyte is not const-correct...
        stackp->loadImage(const_cast<char*>(filepath), true);
        bSucceeded = true;
    } else if ((extension == "tif") ||
            (extension == "visiocyteraw") ||
            (extension == "raw")) {
...
int ImageLoaderBasic::saveStack2RawPBD(const char * filename, ImagePixelType datatype, unsigned char* data, const VISIOCYTELONG * sz)
{
    int berror=0;

        /* This function save a data stack to raw file */
                printf("size of [VISIOCYTELONG]=[%ld], [VISIOCYTELONG]=[%ld] [int]=[%ld], [short int]=[%ld], [double]=[%ld], [float]=[%ld]\n",
                       sizeof(VISIOCYTELONG), sizeof(VISIOCYTELONG), sizeof(int), sizeof(short int), sizeof(double), sizeof(float));
        VISIOCYTELONG i;
        ...
        char formatkey[] = "visiocyte_volume_pkbitdf_encod";
        int lenkey = strlen(formatkey);

        VISIOCYTELONG nwrite = fwrite(formatkey, 1, lenkey, fid);
        if (nwrite!=lenkey)
        {
	...
	channelLength = pbd_sz[0] * pbd_sz[1] * pbd_sz[2];
        ...
        VISIOCYTELONG maxSize = totalUnit*unitSize*2;                             // NOTE:
        ...
        VISIOCYTELONG compressionSize = 0;
	  ...
	  compressionSize=compressPBD3(&compressionBuffer[0], data, totalUnit*unitSize, maxSize);
        ...
        VISIOCYTELONG originalSize=totalUnit*unitSize;
...
VISIOCYTELONG ImageLoaderBasic::compressPBD8(unsigned char * compressionBuffer, unsigned char * sourceBuffer, VISIOCYTELONG sourceBufferLength, VISIOCYTELONG spaceLeft) {
    // bool debug=false;
    VISIOCYTELONG p=0;
    ...
    int dbuffer[95];
    VISIOCYTELONG activeLiteralIndex=-1; // if -1 this means there is no literal mode started
    for (VISIOCYTELONG i=0;i<sourceBufferLength;i++) {
        ...
        VISIOCYTELONG currentPosition=i+1;
        ...
        if (reEfficiency>=4.0) { // 4.0 is the max efficiency from the difference encoding
            ...
            if (i>0) { // I.e., if not first since we can't start with a difference encoding
                ...
                VISIOCYTELONG unitsToCheck=sourceBufferLength-i;
                if (unitsToCheck>95) {
...
VISIOCYTELONG ImageLoaderBasic::compressPBD16(unsigned char * compressionBuffer, unsigned char * sourceBuffer, VISIOCYTELONG sourceBufferLength, VISIOCYTELONG spaceLeft) {
    bool debug=false;
    VISIOCYTELONG p=0;
    ...
    visiocyte_uint16 currentValue=0;
    visiocyte_uint16 * source16Buffer = (visiocyte_uint16*)sourceBuffer;
    VISIOCYTELONG source16BufferLength = sourceBufferLength/2;
    int* dbuffer = 0;
    int d3buffer[256];
    int d4buffer[256];
    int d5buffer[256];
    ...
    for (VISIOCYTELONG i=0;i<source16BufferLength;i++) {
        ...
        if (repeatEfficiency>=THREE_BIT_MAX_EFF) { // we can't do better than this with difference encoding
            ...
            visiocyte_uint16 * repeatValuePointer = (visiocyte_uint16*)(compressionBuffer + p);
            *repeatValuePointer=currentValue;
            ...
            if (i>0) { // We can only use a difference encoding if we are initialized to have a prior value.
                visiocyte_uint16 priorValue=source16Buffer[i-1];
...
//                }
            ...
            if (repeatEfficiency>dfEfficiency && repeatEfficiency>1.0) {
                // Then use RE
                compressionBuffer[p++]=repeatTest+222; // The code for number of repeats
                visiocyte_uint16 * cbp = (visiocyte_uint16*)(compressionBuffer + p);
                *cbp=currentValue;
                p=p+2;
                i+=(repeatTest-1); // because will increment one more time at top of loop
                activeLiteralIndex=-1;
                ...
                if (activeLiteralIndex<0 || compressionBuffer[activeLiteralIndex]>=31) {
                    ...
                    compressionBuffer[p++]=0;
                    activeLiteralIndex=p-1; // Our new literal index
                    visiocyte_uint16 * cbp = (visiocyte_uint16*)(compressionBuffer + p); // Add the current value onto current sequence of literals
                    *cbp=currentValue;
                    //visiocyte_uint16 testValue=*((visiocyte_uint16*)(compressionBuffer + p));
                    if (debug)
...
VISIOCYTELONG ImageLoaderBasic::decompressPBD8(unsigned char * sourceData, unsigned char * targetData, VISIOCYTELONG sourceLength) {
    ...
    while(cp<sourceLength) {
        ...
        if (value<33) {
            ...
            for (VISIOCYTELONG j=cp+1;j<cp+1+count;j++) {
...
VISIOCYTELONG ImageLoaderBasic::decompressPBD16(unsigned char * sourceData, unsigned char * targetData, VISIOCYTELONG sourceLength)
{
    // bool debug=false;
    ...
    visiocyte_uint16* target16Data=(visiocyte_uint16*)targetData;
    ...
    while(cp<sourceLength) {
        ...
        if (code<32) {
            ...
            visiocyte_uint16 * initialOffset = (visiocyte_uint16*)(sourceData + cp + 1);
        ...
        // Repeat 223-255
            ...
            cp++;
            visiocyte_uint16 repeatValue=*((visiocyte_uint16*)(sourceData + cp));
...
void ImageLoaderBasic::updateCompressionBuffer8(unsigned char * updatedCompressionBuffer) {
    ...
    VISIOCYTELONG compressionLength=lookAhead-compressionPosition;
    ...
    VISIOCYTELONG dlength=decompressPBD8(compressionPosition, decompressionPosition, compressionLength);
...
//            qDebug() << "a=" << a << " b=" << b << " c=" << c << " d=" << d << " e=" << e << " f=" << f;
    ...
    //qDebug() << "updateCompressionBuffer calling decompressPBD compressionPosition=" << compressionPosition << " decompressionPosition=" << decompressionPosition
    //        << " size=" << compressionLength << " previousTotalDecompSize=" << getDecompressionSize() << " maxDecompSize=" << maxDecompressionSize;
    VISIOCYTELONG dlength=decompressPBD16(compressionPosition, decompressionPosition, compressionLength);
    compressionPosition=lookAhead;
    decompressionPosition+=dlength;
    //printf("d2\n");
}
...
/* virtual */
int ImageLoaderBasic::loadRaw2StackPBD(DataStream& fileStream, VISIOCYTELONG fileSize, Image4DSimple * image, bool useThreading)
{
    if (useThreading) {
    ...
    // VISIOCYTELONG nread = fread(&keyread[0], 1, lenkey, fid);
    VISIOCYTELONG nread = fileStream.read(&keyread[0], lenkey);
    if (nread!=lenkey)
    {
    ...
    if (datatype==1 || datatype==PBD_3_BIT_DTYPE) {
        image->setDatatype(VISIOCYTE_UINT8);
    } else if (datatype==2) {
        image->setDatatype(VISIOCYTE_UINT16);
    } else {
    ...
    std::vector<VISIOCYTELONG> sz(4, 0); // avoid memory leak
    // VISIOCYTELONG * sz = new VISIOCYTELONG [4]; // reallocate the memory if the input parameter is non-null. Note that this requests the input is also an NULL point, the same to img.
    ...
    VISIOCYTELONG headerSize=4*4+2+1+lenkey;
    VISIOCYTELONG compressedBytes=fileSize-headerSize;
    ...
    VISIOCYTELONG remainingBytes = compressedBytes;
    //VISIOCYTELONG nBytes2G = VISIOCYTELONG(1024)*VISIOCYTELONG(1024)*VISIOCYTELONG(1024)*VISIOCYTELONG(2);
    VISIOCYTELONG readStepSizeBytes = VISIOCYTELONG(1024)*20000; // PREVIOUS
    totalReadBytes = 0;
    ...
    while (remainingBytes>0)
    {
        ...
        VISIOCYTELONG curReadBytes = (remainingBytes<readStepSizeBytes) ? remainingBytes : readStepSizeBytes;
	pbd3_current_channel=totalReadBytes/channelLength;
	cerr << "pbd3_current_channel " << pbd3_current_channel << "\n";
	VISIOCYTELONG bytesToChannelBoundary=(pbd3_current_channel+1)*channelLength - totalReadBytes;
...
VISIOCYTELONG ImageLoaderBasic::compressPBD3(unsigned char * compressionBuffer, unsigned char * sourceBuffer, VISIOCYTELONG sourceBufferLength, VISIOCYTELONG spaceLeft) {
    ...
    for (int c=0;c<channelCount;c++) {
      sourceMin=255;
      sourceMax=0;
      VISIOCYTELONG cStart=c*channelLength;
      VISIOCYTELONG cEnd=cStart+channelLength;
      // Find the min and max values so that we know our 3-bit range within the 8-bit envelope
      for (VISIOCYTELONG cp=cStart;cp<cEnd;cp++) {
    ...
    VISIOCYTELONG* firstRepeatBuffer=new VISIOCYTELONG[2];
	...
	VISIOCYTELONG currentRepeatTest=pbd3FindRepeatCountFromCurrentPosition(sourceBuffer, i, sourceBufferLength);
	...
	if (currentRepeatTest>5) {
	  ...
	  VISIOCYTELONG diffTest = pbd3FindDiffCountFromCurrentPosition(sourceBuffer, i, sourceBufferLength);
...
VISIOCYTELONG ImageLoaderBasic::pbd3FindRepeatCountFromCurrentPosition(unsigned char* sourceBuffer, VISIOCYTELONG position, VISIOCYTELONG maxPosition)
{
  VISIOCYTELONG p=position;
  VISIOCYTELONG repeatLength=0;
...
void ImageLoaderBasic::pbd3EncodeRepeat(unsigned char sourceValue, VISIOCYTELONG count, unsigned char* keyByte, unsigned char* valueByte)
{
  if (sourceValue > 7) {
...
VISIOCYTELONG ImageLoaderBasic::pbd3FindDiffCountFromCurrentPosition(unsigned char* sourceBuffer, VISIOCYTELONG position, VISIOCYTELONG maxPosition)
{
...
// Returns [0] the index of the first repeat relative to the starting position, [1] the length of the first repeat. A length of 0 means there are no repeated values, a length of 1 means two values are the same, etc.
 void ImageLoaderBasic::pbd3FindFirstRepeatOfMinLength(unsigned char* sourceBuffer, VISIOCYTELONG position, VISIOCYTELONG searchLength, VISIOCYTELONG minLength, VISIOCYTELONG* returnBuffer)
{
  VISIOCYTELONG repeatCount=0;
  VISIOCYTELONG maxPosition=position+searchLength;
...
unsigned char* ImageLoaderBasic::pbd3EncodeDiff(unsigned char* sourceBuffer, VISIOCYTELONG position, VISIOCYTELONG length, int* byteCount)
{
  ...
  VISIOCYTELONG doubletCount=length/2;
  ...
  int d1=0;
  int d2=0;
  VISIOCYTELONG maxPosition=position+length;
  ...
  VISIOCYTELONG maxCurrentByte=0L;
  int currentByte=0;
  while (p < maxPosition) {
    int pVal=priorValue;
...
void ImageLoaderBasic::pbd3FlushLiteral(unsigned char* compressionBuffer, unsigned char* sourceBuffer, VISIOCYTELONG* activeLiteralIndex, VISIOCYTELONG* pp, VISIOCYTELONG i) {
  //  cerr << "flush literal, i=" << i << "\n";
  if (*activeLiteralIndex>-1) {
    VISIOCYTELONG a=*activeLiteralIndex;
    VISIOCYTELONG aLength=i-a;
    if (aLength<1) {
...
void ImageLoaderBasic::updateCompressionBuffer3(unsigned char * updatedCompressionBuffer) {
  ...
  DEBUG_SUMMARY=false;
  VISIOCYTELONG uP=(VISIOCYTELONG)updatedCompressionBuffer;
  //  cerr << "updateCompressionBuffer3 - updatedCompressionBuffer=" << uP << "\n";
    ...
    if (DEBUG_SUMMARY) {
      VISIOCYTELONG cStart=(VISIOCYTELONG)&compressionBuffer[0];
      VISIOCYTELONG cCurrent=(VISIOCYTELONG)compressionPosition;
      VISIOCYTELONG cOffset=cCurrent-cStart;
      cerr << "Compression Buffer Offset=" << cOffset << " " << std::hex << cOffset << std::dec << "\n";
    }

    VISIOCYTELONG dlength=decompressPBD3(compressionPosition, decompressionPosition, compressionLength);
    compressionPosition=lookAhead;
    decompressionPosition+=dlength;
...
VISIOCYTELONG ImageLoaderBasic::decompressPBD3(unsigned char * sourceData, unsigned char * targetData, VISIOCYTELONG sourceLength) {
  ...
  VISIOCYTELONG dStart=(VISIOCYTELONG)decompressionBuffer;
  VISIOCYTELONG tStart=(VISIOCYTELONG)targetData;

  VISIOCYTELONG i=0L;

  VISIOCYTELONG sdStart=(VISIOCYTELONG)sourceData;
  VISIOCYTELONG sdEnd=sdStart + sourceLength;
  ...
  while(cp<sourceLength) {
	...
	int ic=0;
	  ...
	  VISIOCYTELONG iCheck=dp+(tStart-dStart);
