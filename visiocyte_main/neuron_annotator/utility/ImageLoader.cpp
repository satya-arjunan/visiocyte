...
bool ImageLoader::mapChannels()
{
    ...
    else
    {
        ...
        {
            targetImage->loadImage(sourceProxy.sx, sourceProxy.sy, sourceProxy.sz, (maxTargetChannel+1), VISIOCYTE_UINT8);
        }
        else if ( sourceImage->getDatatype() == 2 )
        {
            targetImage->loadImage(sourceProxy.sx, sourceProxy.sy, sourceProxy.sz, (maxTargetChannel+1), VISIOCYTE_UINT16);
        }
        else
        {
...
unsigned char* ImageLoader::convertType2Type1( My4DImage* image )
{
    ...
    VISIOCYTELONG totalSize=sz[0]*sz[1]*sz[2]*sz[3];
    ...
    for ( VISIOCYTELONG s3 = 0; s3 < sz[3]; s3++ )
    {
        for ( VISIOCYTELONG s2 = 0; s2 < sz[2]; s2++ )
        {
            for ( VISIOCYTELONG s1 = 0; s1 < sz[1]; s1++ )
            {
                for ( VISIOCYTELONG s0 = 0; s0 < sz[0]; s0++ )
                {
...
bool ImageLoader::saveImageByMode(My4DImage * stackp, const char* filepath, int saveMode)
{
  qDebug() << "Saving to file " << filepath << " under mode=" << saveMode;
    ...
    if ( hasPbdExtension( filepath ) )
    {
        ...
        {
	  // using VISIOCYTE_UNKNOWN as a signal to do 3-bit PBD compression, since we are also assuming the stack has been converted to 8-bit in this case
	  qDebug() << "ImageLoader::saveImageByMode: calling saveStack2RawPBD with datatype = VISIOCYTE_UNKNOWN";
	  saveStack2RawPBD(filepath, VISIOCYTE_UNKNOWN, data, sz);
        }
        else
        {
...
int ImageLoader::loadRaw2StackPBDFromUrl(QUrl url, Image4DSimple * image, bool useThreading)
{
    ...
    VISIOCYTELONG fileSize = reply->header(QNetworkRequest::ContentLengthHeader).toLongLong();
    int result = false;
    // if (true) {
    if ( fileSize == 0 )
    {
...
int ImageLoader::loadRaw2StackPBD(const char * filename, Image4DSimple * image, bool useThreading)
{
    ...
    VISIOCYTELONG fileSize = fileStream.size();
    return loadRaw2StackPBDFromStream(fileStream, fileSize, image, useThreading);
}

int ImageLoader::loadRaw2StackPBDFromStream(QIODevice& fileStream, VISIOCYTELONG fileSize, Image4DSimple * image, bool useThreading)
{
    ...
    VISIOCYTELONG readStepSizeBytes = VISIOCYTELONG(1024)*20000;
    totalReadBytes = 0;
