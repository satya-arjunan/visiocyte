 ...
 loadNextVolume() <------------|
   ...
   <volumeLoadSequenceCompleted>
                  ...
                  |------------|
                               ...
                               v
                   loadNextVolume()
                     (loads visiocyteraw files for 3D viewer)
                     <volumeLoadSequenceCompleted>--|
                                                    |
                                                    v
                                    loadSecondaryVolumeDataFromFiles()
                                      (loads visiocyteraw files for all viewers)
*/
...
// Only mp4 files in this block
    ...
    if (useFastLoad)
    {
        ...
        // Feb 2013, BUT loading lossless visiocytepbd into VolumeTexture is not
        ...
        while (mvoxels0 > 25)
        {
            ...
            QString mv = QString("%1").arg(mvoxels0);
            // First the full color signal with poor color separation
            *candidate << new ProgressiveSingleFileCompanion("ConsolidatedSignal2_"+mv+".visiocytepbd");
            // Second the fourth reference channel to round out the channels
            *candidate << new ProgressiveSingleFileCompanion("Reference2_"+mv+".visiocytepbd", CHANNEL_ALPHA); // companion file
            // (lossless version needs no individual color channels)
            *candidate << new ProgressiveSingleFileCompanion("ConsolidatedLabel2_"+mv+".visiocytepbd", CHANNEL_LABEL); // companion file
            mvoxels0 /= 2;
