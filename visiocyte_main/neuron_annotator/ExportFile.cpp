...
Toutput* getCurrentStack(Tinput *input1d, Tmask *mask1d, Tref *ref1d, VISIOCYTELONG *szStack, QList<bool> maskStatusList, QList<bool> overlayStatusList, int datatype){
    ...
    //
    VISIOCYTELONG sx = szStack[0];
    VISIOCYTELONG sy = szStack[1];
    VISIOCYTELONG sz = szStack[2];
    VISIOCYTELONG sc = szStack[3]; // stack_sc + ref (0/1)

    VISIOCYTELONG pagesz = sx*sy*sz;
    VISIOCYTELONG totalplx = pagesz*sc;
    ...
    try{
            ...
            for(VISIOCYTELONG k=0; k<sz; k++)
            {
                ...
                for(VISIOCYTELONG j=0; j<sy; j++)
                {
                    ...
                    for(VISIOCYTELONG i=0; i<sx; i++)
                    {
                        ...
                        //VISIOCYTELONG idxref = idx - offset_c;
        ...
        //
        for(VISIOCYTELONG c=0; c<numcolor; c++)
        {
            ...
            for(VISIOCYTELONG k=0; k<sz; k++)
            {
                ...
                for(VISIOCYTELONG j=0; j<sy; j++)
                {
                    ...
                    {
                        VISIOCYTELONG idx = offset_j + i;
                        VISIOCYTELONG idxmask = idx - offset_c;
...
void copyChannelType(My4DImage& dest, int destChannel,
		...
		bool bDoMask)
{
	const VISIOCYTELONG sx = src.sx;
	const VISIOCYTELONG sy = src.sy;
	const VISIOCYTELONG sz = src.sz;
	const VISIOCYTELONG su = dest.getUnitBytes();
...
void ExportFile::run()
{
    ...
    { // acquire read locks in this block
		...
		VISIOCYTELONG sx = originalStack.sx;
		VISIOCYTELONG sy = originalStack.sy;
		VISIOCYTELONG sz = originalStack.sz;
		...
		ImagePixelType pixelType = VISIOCYTE_UINT8;
        ...
        if (is2D) {
            ...
            rotatedString.append("_rotate.visiocytepbd");
...
My4DImage* ExportFile::padAndRotateImage(My4DImage* image, const Rotation3D& rotation, bool flipY)
{
    ...
    visiocyte_uint8** data8=0L;
    visiocyte_uint16** data16=0L;

    visiocyte_uint8** target8=0L;
    visiocyte_uint16** target16=0L;

    qDebug() << "Image datatype=" << image->getDatatype();

    if (image->getDatatype()==VISIOCYTE_UINT8) {
        qDebug() << "Using 8-bit data";
        data8=new visiocyte_uint8*[cDim];
        target8=new visiocyte_uint8*[cDim];
        ...
        data16=new visiocyte_uint16*[cDim];
        target16=new visiocyte_uint16*[cDim];
        for (int c=0;c<cDim;c++) {
            data16[c]=(visiocyte_uint16*)image->getRawDataAtChannel(c);
            target16[c]=(visiocyte_uint16*)rotatedImage->getRawDataAtChannel(c);
        }
    }
    ...
    for (long z=0;z<zDim;z++) {
        ...
        for (long y=0;y<yDim;y++) {
            ...
            for (long x=0;x<xDim;x++) {
                ...
                for (int c=0;c<cDim;c++) {
                    if (data8 != 0L) {
                        visiocyte_uint8* d8=image->getRawDataAtChannel(c);
                        visiocyte_uint8* t8=rotatedImage->getRawDataAtChannel(c);
