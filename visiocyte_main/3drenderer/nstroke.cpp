...
void Renderer_gl1::solveCurveExtendGlobal(){
    ...
    last_seg_id = minIndex;
     ...
     if(true) //ZS:11052015 always join in this gamified mode for intuition.
     {
          ...
          V_NeuronSWC_unit nu;
          VISIOCYTELONG last_n =curImg->tracedNeuron.maxnoden();
          ...
          VISIOCYTELONG nums=curImg->tracedNeuron.seg.size();

          MainWindow* VISIOCYTEmainwindow = 0;
          VISIOCYTEmainwindow = visiocyter_getVISIOCYTEmainwindow(_idep);
          bool bConnectCurve = (VISIOCYTEmainwindow && VISIOCYTEmainwindow->global_setting.b_3dcurve_autoconnecttips);
          if(bConnectCurve && nums>1)
          {
               VISIOCYTELONG cursegid = last_seg_id;
	...
	if (NC<1 || N <3)  return; //data is not enough
     ...
     if(vdot>=0)
     {
     	...
     	smooth_curve(loc_vecsub, 5);
          ...
          VISIOCYTELONG nsegs = curImg->tracedNeuron.nsegs();
          for(VISIOCYTELONG j=0; j<nsegs; j++)
          {
               if(j!=last_seg_id)
               {
...
void Renderer_gl1::getPerpendPointDist(XYZ &P, XYZ &P0, XYZ &P1, XYZ &Pb, double &dist)
{
    ...
    if(c2==0.0)
    {
         ...
         visiocyte_msg(" getPerpendPointDist: c2 = %d, cannot processing further!!!!", c2);
         return;
    }
...
void Renderer_gl1::getRgnPropertyAt(XYZ &pos, LocationSimple &pt)
{
	...
	VISIOCYTELONG cc = (VISIOCYTELONG) checkCurChannel();
     if (cc<0) cc=0;
	...
	if (curImg->compute_rgn_stat(pt, cc)!=true)
	{
          ...
          // mass value
          // double mass = pt.mass;
          visiocyte_msg("Failed to get region properties.\n");
          return ;
	}
...
void Renderer_gl1::solveCurveCenterV2(vector <XYZ> & loc_vec_input, vector <XYZ> &loc_vec, int index)
{
	...
	if (b_use_seriespointclick==false)
     ...
     if (b_addthiscurve && selectMode==smCurveRefineInit)
     {
          ...
          VISIOCYTELONG N = curImg->tracedNeuron.seg.size();
...
*/
void Renderer_gl1::reorderNeuronIndexNumber(VISIOCYTELONG curSeg_id, VISIOCYTELONG NI, bool newInLower)
{
     // get the control points of the primary curve
     VisiocyteR_GLWidget* w = (VisiocyteR_GLWidget*)widget;
     My4DImage* curImg = 0;
     ...
     VISIOCYTELONG nseg=curImg->tracedNeuron.nsegs();

     V_NeuronSWC& curSeg = curImg->tracedNeuron.seg[curSeg_id];
     // curSeg's info
     VISIOCYTELONG cnsu = curSeg.nrows(); // this already includes NI
     VISIOCYTELONG cn0, cnn; // min max n of curSeg before inserting NI points
     if(newInLower==true)
     {
     ...
     for(VISIOCYTELONG i=0; i<curSeg_id; i++)
     {
          V_NeuronSWC& segi = curImg->tracedNeuron.seg.at(i);
          VISIOCYTELONG num=segi.nrows();
          for(VISIOCYTELONG ii=0;ii<num; ii++)
          {
               VISIOCYTELONG p_old=segi.row.at(ii).parent;
     ...
     for(VISIOCYTELONG j=0; j<cnsu; j++)
     {
     ...
     // after the curSeg
     for(VISIOCYTELONG i=curSeg_id+1; i<nseg; i++)
     {
          V_NeuronSWC& segi = curImg->tracedNeuron.seg[i];
          VISIOCYTELONG num=segi.nrows();
          for(VISIOCYTELONG ii=0;ii<num; ii++)
          {
               VISIOCYTELONG pn = segi.row.at(ii).n; // old n
               VISIOCYTELONG p_old = segi.row.at(ii).parent; // old p
...
void Renderer_gl1::callCreateMarkerNearestNode(int x, int y)
{
    ...
    if (w && listNeuronTree.size()>0)
    {
        ...
        if(listNeuronTree.at(0).editable==true || listNeuronTree.at(listNeuronTree.size()-1).editable==true)
        {
            NeuronTree *p_tree = (NeuronTree *)(&(listNeuronTree.at(curEditingNeuron-1)));
            double best_dist;
            VISIOCYTELONG n_id = findNearestNeuronNode_WinXY(x, y ,p_tree, best_dist);
            NeuronSWC cur_node;
            if (n_id>=0)
            {
...
void Renderer_gl1::_updateDragPoints(int x, int y)
{
     ...
     if(!bInitDragPoints) // process at the first click
     {
          ...
          VISIOCYTELONG start_id = center_id - halfw; // start point for draging
          VISIOCYTELONG end_id = center_id + halfw;   // end point for draging
          ...
          for(VISIOCYTELONG j=start_id; j<= end_id; j++)
          {
               V_NeuronSWC_unit v;
               v = segi.row.at(j);
     ...
     }else
     {
          ...
          {
               VISIOCYTELONG ind = DraggedNeurons.at(i).n;
...
void Renderer_gl1::setDragWinSize(int csize)
{
     ...
     for (int i=0; i<N; i++)
     {
          VISIOCYTELONG ind = segi.row.at(i).n;
          if(ind == draggedCenterIndex)
          {
               sindex =i;
               break;
     ...
     VISIOCYTELONG start_ind = sindex - halfw; // start point for draging
     VISIOCYTELONG end_ind = sindex + halfw;   // end point for draging
     ...
     DraggedNeurons.clear();
     for(VISIOCYTELONG j=start_ind; j<= end_ind; j++)
     {
          V_NeuronSWC_unit v;
          v=segi.row.at(j);
          DraggedNeurons.append(v);
 ...
 *  NeuronTree* ptree).
 */
VISIOCYTELONG Renderer_gl1::findNearestNeuronNode_WinXYV2(int cx, int cy)
{
     VisiocyteR_GLWidget* w = (VisiocyteR_GLWidget*)widget;
     My4DImage* curImg = 0;
     if(w) curImg = visiocyter_getImage4d(_idep);
 ...
 */
void Renderer_gl1::canCurveConnect(XYZ &e, VISIOCYTELONG &closest_seg, VISIOCYTELONG &closest_node,
     bool &bConnect)
{
     ...
     {
          V_NeuronSWC& segi = curImg->tracedNeuron.seg.at(i);
          VISIOCYTELONG nr = segi.row.size();
          for(VISIOCYTELONG j=0;j<nr;j++)
          {
               XYZ pt;
 ...
 */
void Renderer_gl1::connectCurve(VISIOCYTELONG &segid)
{
     // for curve connection
     VisiocyteR_GLWidget* w = (VisiocyteR_GLWidget*)widget;
     My4DImage* curImg = 0;
     ...
     VISIOCYTELONG closest_seg, closest_node;
     bool bConnect;
     // justify p0
