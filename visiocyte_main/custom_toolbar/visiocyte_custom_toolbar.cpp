#include <iostream>
#include <fstream>
#include "visiocyte_custom_toolbar.h"

using namespace std;

static QString pluginRootPath = QObject::tr("/Users/xiaoh10/Applications/visiocyte/plugins");
static QString settingFilePath = QObject::tr("/Users/xiaoh10/.visiocyte_toolbox_layout");
...
{
	//settingFilePath = qApp->applicationDirPath() + QObject::tr("/.visiocyte_toolbar");
	settingFilePath = QDir::homePath() + QObject::tr("/.visiocyte_toolbar");
...
#ifdef __visiocyte_custom_toolbar_plugin__
static QString visiocyte_getInterfaceName(QObject *plugin)
{
	QString name;
...
static QStringList visiocyte_getInterfaceFuncList(QObject *plugin)
{
	QStringList qslist;
...
CustomToolbarSelectWidget::CustomToolbarSelectWidget(CustomToolbarSetting* _cts, VISIOCYTEPluginCallback2 *callback, QWidget * parent): QWidget(0/*parent*/)
{
	if(!_cts) return;
...
#ifndef __visiocyte_custom_toolbar_plugin__
	...
	try
	{
		if(bt == 0) // Main Window
		{
			...
			if(mw)
			{
				...
				else if(buttonName.indexOf(tr("Plug-In::")) != -1) (mw->pluginLoader->*(VisiocytePluginLoaderFunc)slot_func)();
		...
		if(bt == 1) // Triview
		{
			visiocytehandleList winlist = callback->getImageWindowList();
			if(winlist.empty()) {QMessageBox::information(0,"","No image stack!"); return true;}
			visiocytehandle handle = callback->currentImageWindow();
			if(handle)
			{
...
QList<pair<QString, VoidFunc> > getMainWindowButtonStringAndFuncList()
{
	...
	return QList<StringAndFunc>()
		...
		<<SAF (QObject::tr("Advanced::3D tracing (VISIOCYTE-Neuron tracing v2.0)::trace from one landmark to all others"), \
				(VoidFunc)(&MainWindow::func_procTracing_one2others))
		<<SAF (QObject::tr("Advanced::3D tracing (VISIOCYTE-Neuron tracing v2.0)::trace a path between two landmarks"), \
				(VoidFunc)(&MainWindow::func_procTracing_trace_a_curve))
		<<SAF (QObject::tr("Advanced::3D tracing (VISIOCYTE-Neuron tracing v2.0)::undo the last tracing step"), \
				(VoidFunc)(&MainWindow::func_procTracing_undo_laststep))
		<<SAF (QObject::tr("Advanced::3D tracing (VISIOCYTE-Neuron tracing v2.0)::redo the last tracing step"), \
				(VoidFunc)(&MainWindow::func_procTracing_redo_laststep))
		<<SAF (QObject::tr("Advanced::3D tracing (VISIOCYTE-Neuron tracing v2.0)::clear the traced neuron"), \
				(VoidFunc)(&MainWindow::func_procTracing_clear))
		<<SAF (QObject::tr("Advanced::3D tracing (VISIOCYTE-Neuron tracing v2.0)::update 3D view(s) of traced neuron"), \
				(VoidFunc)(&MainWindow::func_procTracing_update3Dview))
		<<SAF (QObject::tr("Advanced::3D tracing (VISIOCYTE-Neuron tracing v2.0)::save the traced neuron to a file"), \
		...
		<<SAF (QObject::tr("Plug-In::Plug-in manager"), \
				(VoidFunc)(&Visiocyte_PluginLoader::aboutPlugins))
		<<SAF (QObject::tr("Plug-In::Re-scan all plugins"), \
				(VoidFunc)(&Visiocyte_PluginLoader::rescanPlugins))
...
//				(VoidFunc)(&QMdiArea::arrangeIcons))
		...
		//<<SAF (QObject::tr("Work-Mode::VISIOCYTE Default"), \
...
QList<pair<QString, VoidFunc> > getTriViewButtonStringAndFuncList()
{
	return QList<SAF >()
		<<SAF (QObject::tr("See in 3D"), \
				(VoidFunc)(&VISIOCYTEPluginCallback2::open3DWindow))
		;
}

QList<pair<QString, VoidFunc> > getView3dButtonStringAndFuncList()
{
	// refer to 3drenderer/visiocyter_control_signal.cpp
	return QList<SAF >()
...
CustomToolbar::CustomToolbar(QString title, VISIOCYTEPluginCallback2 * callback, QWidget * parent) : QToolBar(0/*parent*/)
{
...
CustomToolbar::CustomToolbar(CustomToolbarSetting* _cts, VISIOCYTEPluginCallback2 * callback, QWidget * parent) : QToolBar(0/*parent*/)
{
	cts = _cts;
