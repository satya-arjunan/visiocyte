...
bool read_nrrd(const char imgSrcFile[], unsigned char *& data1d, VISIOCYTELONG * &sz, int & datatype)
{
    float pixelsz[4];
    float spaceorigin[3];
...
bool read_nrrd_with_pxinfo(const char imgSrcFile[], unsigned char *& data1d, VISIOCYTELONG * &sz, int & datatype,
                           float pixelsz[4], float spaceorigin[3])
{
    ...
    {
        visiocyte_msg(QString("nrrd [%1] reading direct fail").arg(imgSrcFile));
        return false;
    }
    else
    {
        if ( nrrd->dim > 4 )
        {
            visiocyte_msg("ERROR: for now, nrrd input can only handle data with dimension 4 or less.");
            return false;
        } else if ( nrrd->spaceDim > 3 ){
            visiocyte_msg("ERROR: for now, nrrd input can only handle data with space dimension 3 or less.");
            return false;
        }
        ...
        default:
            visiocyte_msg("nrrd error: unsupported data type");
            dt = 0;
            break;
        }
        ...
        if(rangeAxisNum>1) {
            visiocyte_msg("nrrds can only have 1 colour axis!");
            return false;
        }
        int colourAxis = rangeAxisNum>=0 ? rangeAxisIdx[0] : -1;
        
        if(domainAxisNum>4) {
            visiocyte_msg("nrrds can only have 4 domain (spatio-temporal) axes!");
            return false;
        } else if (domainAxisNum<2) {
            visiocyte_msg("nrrds must have at least 2 domain (spatio-temporal) axes!");
            return false;
        }
            ...
            for (unsigned int i=0; i<domainAxisNum; i++) {
                ...
                switch ( nrrd->axis[ax].kind )
                {
                    ...
                    case nrrdKindTime: 
                        ...
                        if(timeAxis>0)
                        {
                            visiocyte_msg("nrrds cannot have multiple time axes!");
                            return false;
                        }
                        timeAxis=ax; break;
        ...
        if(unallocatedDomainNum>0) {
            ...
            if (spaceAxisNum == 0) {
                if(unallocatedDomainNum<2) {
                    visiocyte_msg("nrrds must have at least two spatial dimensions!");
                    return false;
                }
            ...
            if(still_left) {
                ...
                if((still_left + rangeAxisNum)>2) {
                    visiocyte_msg("nrrds should only have 2 non-spatial dimensions!");
                    return false;
                }
                if(rangeAxisNum>0) {
                    ...
                    if(timeAxis>0)
                    {
                        visiocyte_msg("nrrd has unidentified non-space non-time non-colour axis!");
                        return false;
                    }
                    ...
                    // 2 axes left
                    if (timeAxis>0) {
                        visiocyte_msg("nrrds can only have 2 non-spatial axes!");
                        return false;
                    }
        ...
        if(spaceAxisNum==2) {
            ...
            visiocyte_msg("nrrds must have 2 or 3 spatial axes!");
            return false;
        }
        
        // FIXME
        if(timeAxis>=0) {
            visiocyte_msg("No support yet for nrrds with time information!");
            return false;
        }
...
bool write_nrrd(const char imgSrcFile[], unsigned char * data1d, VISIOCYTELONG sz[4], int datatype)
{
    float pixelsz[4];
...
bool write_nrrd_with_pxinfo(const char imgSrcFile[], unsigned char * data1d, VISIOCYTELONG sz[4], int datatype,
                            float pixelsz[4], float spaceorigin[3])
{
    int nrrdType = nrrdTypeUnknown;
    switch ( datatype )
    {
        case VISIOCYTE_UINT8: nrrdType = nrrdTypeUChar; break;
        case VISIOCYTE_UINT16: nrrdType = nrrdTypeUShort; break;
        case VISIOCYTE_FLOAT32: nrrdType = nrrdTypeFloat; break;
        default: 
            visiocyte_msg("Error: unsupported type");
            return false;
            break;
    }
    ...
    try
    {
        ...
        // Visiocyte Image4DSimple assumes xyzct ordering
    ...
    {
        visiocyte_msg(QString("ERROR: write_nrrd returned error '[%1]'\n").arg(err));
        free( err );
        return false;
    }
