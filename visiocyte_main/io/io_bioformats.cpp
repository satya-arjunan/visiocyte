...
{
    QSettings setting("Visiocyte_tools", "bioformats");
    ...
    if (!(tmpfilePath.endsWith("visiocyteraw")))
        tmpfilePath = ""; //reset the file name
    visiocyte_msg(QString("The last saved name of the tmp file or the reset name is [%1]\n").arg(tmpfilePath),0);
...
//    {
//        tmpfilePath = QDir::tempPath().append("/").append("test1.visiocyteraw");
//    }
    ...
    {
        visiocyte_msg(QString("The temporary folder [%1] for holding the intermediate file is not useable. "
        ...
        tmpfilePath = tmpfilePathFolder.append("/").append("test1.visiocyteraw");
//        tmpfilePath = tmpfilePathFolder.append("/").append("test1.tif");
        visiocyte_msg(QString("The name of the newly selected tmp file is [%1]\n").arg(tmpfilePath), 0);
        QFileInfo qtf1(tmpfilePath);
        QFile tf1(tmpfilePath);
        if (!(tf1.open(QIODevice::ReadWrite))) //the test fail
        {
            visiocyte_msg(QString("The temporary folder [%1] is still not valid. Try to rerun and use a different folder.\n").arg(qtf1.dir().absolutePath()));
            return false;
        }
    }
    ...
    visiocyte_msg(QString("The name of the currently used tmp file is [%1]\n").arg(outfilename), 0);
    ...
    visiocyte_msg(QString("The system call to delete the existing temp file returns a value [%1]").arg(res_syscall), 0);
    if (res_syscall<0)
    {
        visiocyte_msg(QString("Fail to delete the tmp file [%1].").arg(outfilename));
        return false;
    }
    ...
    if (!QFile(lociLibPath).exists())
    {
        visiocyte_msg(QString("Bioformats Java library is not detected specified correctly at [%1], so please specify the Bioformats Java library location.\n").arg(lociLibPath));
    ...
    visiocyte_msg(QString("The system call to the Bioformats returns a value [%1]").arg(res_syscall), 0);
    if (res_syscall<0)
        return false;

    if (!QFile(outfilename).exists())
    {
        visiocyte_msg("The temprary converted file does not exist. The conversion of format using Bioformats has failed. Please check you have Java properly installed and/or use another way to convert and load using Visiocyte.\n");
        outfilename = "";
        return false;
    }
