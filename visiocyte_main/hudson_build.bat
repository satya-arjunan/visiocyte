...
if NOT "%BUILD_VISIOCYTE_WINDOWS%"=="1" (
...
echo call DEVENV Visiocyte.sln /Build Release /Project ALL_BUILD.vcxproj
call DEVENV Visiocyte.sln /Build Release /Project ALL_BUILD.vcxproj

echo.
echo.
echo Attempting PACKAGE project, to produce an NSIS Installer
echo call DEVENV Visiocyte.sln /Build Release /Project PACKAGE.vcxproj
call DEVENV Visiocyte.sln /Build Release /Project PACKAGE.vcxproj

cd %OLD_CD%
cd ../
 
set OUTPUT_BASE=%MAKEDIR%\visiocyte\Windows_MSVC10_64
set EXETGT=%OUTPUT_BASE%\visiocyte.exe
...
copy %MAKEDIR%\InstallVisiocyte-*-Windows_MSVC10*.exe %GATHER_LOC%

:: Notify caller of failure, if the required outputs were not created.
if NOT EXIST %GATHER_LOC%\bin\visiocyte.exe (
  echo ERROR: No Visiocyte Executable found
  exit 1
) 

if NOT EXIST %GATHER_LOC%\bin\plugins\ (
  echo ERROR: No Visiocyte plugins produced
  exit 2
)

if NOT EXIST %GATHER_LOC%\InstallVisiocyte*.exe (
...
copy %GATHER_LOC%\bin\visiocyte.exe %WIN_STAGING_LOC%
if NOT EXIST %WIN_STAGING_LOC%\visiocyte.exe (
  echo Failed to copy %GATHER_LOC%\bin\visiocyte.exe to %WIN_STAGING_LOC%
  exit 8
)
