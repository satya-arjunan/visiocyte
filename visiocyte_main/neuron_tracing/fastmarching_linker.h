...
//{
//    for (VISIOCYTELONG i=outswc.size()-1;i>=0;i--)
//    {
     ...
     T * inimg1d, vector<MyMarker *> &outswc, int sz0, int sz1, int sz2, int cnn_type = 2) //float time,
{
        ...
        map<VISIOCYTELONG, MyMarker> sub_map, tar_map;
        for(i = 0; i < tar_markers.size(); i++)
        {
                VISIOCYTELONG x = tar_markers[i].x + 0.5;
                VISIOCYTELONG y = tar_markers[i].y + 0.5;
                VISIOCYTELONG z = tar_markers[i].z + 0.5;
                VISIOCYTELONG ind = z*sz01 + y*sz0 + x;
        ...
        {
                VISIOCYTELONG x = sub_markers[i].x + 0.5;
                VISIOCYTELONG y = sub_markers[i].y + 0.5;
                VISIOCYTELONG z = sub_markers[i].z + 0.5;
                VISIOCYTELONG ind = z*sz01 + y*sz0 + x;
        ...
        vector<VISIOCYTELONG> submarker_inds;
        for(VISIOCYTELONG s = 0; s < sub_markers.size(); s++) {
                VISIOCYTELONG ii = (VISIOCYTELONG)(sub_markers[s].x + 0.5);
                VISIOCYTELONG jj = (VISIOCYTELONG)(sub_markers[s].y + 0.5);
                VISIOCYTELONG kk = (VISIOCYTELONG)(sub_markers[s].z + 0.5);
                VISIOCYTELONG ind = kk*sz01 + jj*sz0 + ii;
                submarker_inds.push_back(ind);
                state[ind] = ALIVE;
                phi[ind] = 0.0;
        }
        cout << "totalsize=" << tol_sz <<endl;
        VISIOCYTELONG * parent = new VISIOCYTELONG[tol_sz]; for(VISIOCYTELONG ind = 0; ind < tol_sz; ind++) parent[ind] = ind;

        BasicHeap<HeapElemX> heap;
        map<VISIOCYTELONG, HeapElemX*> elems;

        // init heap
        for(VISIOCYTELONG s = 0; s < submarker_inds.size(); s++)
        {
                VISIOCYTELONG index = submarker_inds[s];
        ...
        while(!heap.empty())
        {
                ...
                state[min_ind] = ALIVE;
                VISIOCYTELONG i = min_ind % sz0;
                VISIOCYTELONG j = (min_ind/sz0) % sz1;
                VISIOCYTELONG k = (min_ind/sz01) % sz2;
                VISIOCYTELONG w, h, d;

                for(VISIOCYTELONG kk = -1; kk <= 1; kk++)
                {
                        d = k+kk;
                        if(d < 0 || d >= sz2) continue;
                        for(VISIOCYTELONG jj = -1; jj <= 1; jj++)
                        {
                                h = j+jj;
                                if(h < 0 || h >= sz1) continue;
                                for(VISIOCYTELONG ii = -1; ii <= 1; ii++)
                                {
                                        w = i+ii;
                                        if(w < 0 || w >= sz0) continue;
                                        VISIOCYTELONG offset = ABS(ii) + ABS(jj) + ABS(kk);
                                        if(offset == 0 || offset > cnn_type) continue;
                                        double factor = (offset == 1) ? 1.0 : ((offset == 2) ? 1.414214 : ((offset == 3) ? 1.732051 : 0.0));
                                        VISIOCYTELONG index = d*sz01 + h*sz0 + w;

                                        if(state[index] != ALIVE)
                                        {
                                                double new_dist = phi[min_ind] + (GI(index) + GI(min_ind))*factor*0.5;
                                                VISIOCYTELONG prev_ind = min_ind;
        ...
        // connect markers according to disjoint set
        {
                ...
                {
                        VISIOCYTELONG i = ind % sz0;
                        VISIOCYTELONG j = ind/sz0 % sz1;
                        VISIOCYTELONG k = ind/sz01 % sz2;
        ...
        if(!elems.empty()) for(map<VISIOCYTELONG, HeapElemX*>::iterator it = elems.begin(); it != elems.end(); it++) delete it->second;
...
template<class T> bool fastmarching_drawing_serialbboxes(vector<MyMarker> & near_markers, vector<MyMarker> &far_markers, T * inimg1d, vector<MyMarker *> &outswc, int sz0, int sz1, int sz2, int cnn_type = 2, int margin = 5,bool intensityThresholdmode=false)
{
	...
	nm2 = near_markers[0];
	fm2 = far_markers[0];
    VISIOCYTELONG sz01 = (VISIOCYTELONG)sz0 * sz1;

    VISIOCYTELONG mx = MAX_INT, my = MAX_INT, mz = MAX_INT;
    VISIOCYTELONG Mx = 0, My = 0, Mz = 0;

    for(VISIOCYTELONG m = 1; m < near_markers.size(); m++)
	{
		nm1 = nm2; fm1 = fm2;
        ...
        VISIOCYTELONG bsz0 = dist(rect[0], rect[3]) + 1 + 2 * margin + 0.5;
        VISIOCYTELONG bsz1 = dist(rect[0], rect[1]) + 1 + 2 * margin + 0.5;
        VISIOCYTELONG bsz2 = 1 + 2 * margin + 0.5;
        VISIOCYTELONG bsz01 = bsz0 * bsz1;
        for(VISIOCYTELONG k = 0; k < bsz2; k++)
		{
            for(VISIOCYTELONG j = 0; j < bsz1; j++)
			{
                for(VISIOCYTELONG i = 0; i < bsz0; i++)
				{
                    VISIOCYTELONG ii = o.x + i * a[0] + j * b[0] + k * c[0] + 0.5;
                    VISIOCYTELONG jj = o.y + i * a[1] + j * b[1] + k * c[1] + 0.5;
                    VISIOCYTELONG kk = o.z + i * a[2] + j * b[2] + k * c[2] + 0.5;
    ...
    VISIOCYTELONG msz0 = Mx - mx + 1;
    VISIOCYTELONG msz1 = My - my + 1;
    VISIOCYTELONG msz2 = Mz - mz + 1;
    VISIOCYTELONG msz01 = msz0 * msz1;
    VISIOCYTELONG mtol_sz = msz2 * msz01;
    ...
    if(intensityThresholdmode)
    {
        ...
        for (VISIOCYTELONG z = 0; z < msz2; z++)
        {
            for (VISIOCYTELONG y = 0; y < msz1; y++)
            {
                for (VISIOCYTELONG x = 0; x < msz0; x++)
                {
