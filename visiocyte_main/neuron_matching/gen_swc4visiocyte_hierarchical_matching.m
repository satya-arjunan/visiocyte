function gen_swc4visiocyte_hierarchical_matching(a, b, matchingList_allterms, filedir)
% function gen_swc4visiocyte_hierarchical_matching(a, b, matchingList_allterms, filedir)

% generate swc file for visiocyte to display hierarchical matching result
% tree1, tree2, and lines indicating the matching branching points and
% leaves are all combined into one tree called visiocytetree for visiocyte to display
%
...
for level = 0:maxlevel-1
    ...
    visiocytetree = []; %the swc tree that combines tree1, tree2, and lines between matching branching points and leaves
    lenold = size(visiocytetree,1);
    ...
    rootidx = []; % list the index of roots of sub-branches in visiocytetree
    rootmap1 = []; % map the index of roots of sub-branches of tree1 in visiocytetree and their original nodeid
    rootmap2 = []; % map the index of roots of sub-branches of tree2 in visiocytetree and their original nodeid
...
%         c(:,6) = 0.5;
        ...
        % revise parentid of branch root in c (tree 2) so that branches are connected in visiocyte
        idx1 = find(c(:,7)==-1);
        idx2 = find(c(idx1,1)==b(:,1));
        c(idx1,7) = b(idx2,7);
        listidx1 = [listidx1, idx1];
        
        % revise parentid of branch root in d (tree 1) so that branches are connected in visiocyte
        idx3 = find(d(:,7)==-1);
        ...
        visiocytetree = [visiocytetree; c; d]; 
        % order of visiocytetree is: from level 0 to the maximum level: tree2 matched sub-branch, tree1 matched sub-branch, tree1 unmatched sub-branches, tree2 unmatched sub-branches
        % finally the leave level
        
        len = size(visiocytetree,1);       
        visiocytetree(lenold+1:len,2) = j+1; % avoid using white color, since it will be used to highlight matched branching points
        nodeBranchIdx(lenold+1:len) = i; % recode for each node in visiocytetree, which branch (index of branch) it is in. This is useful in determining branch color when two branching points are direct parent-child
    ...
    for i=1:length(unmatchedidx)
       ...
       % revise parentid of branch root in d so that branches are connected in
        ...
        visiocytetree = [visiocytetree; d];
        len = size(visiocytetree,1);       
        visiocytetree(lenold+1:len,2) = 1; % use black color to indicate mismatch, white color to indicate match
        lenold = len;    
...
%         c(:,6) = 0.5;
    ...
    oringinalcolor = visiocytetree(:,2);
...
%         if (visiocytetree(rootidx(i),7)~=-1)
%             nn = find(visiocytetree(:,1) == visiocytetree(rootidx(i),7)); % note length(nn) can be bigger than 1, due to replicate of nodes in visiocytetree
% 
...
% %                 end;
% %                     
% %                 visiocytetree(rootidx(i),2) = oringinalcolor(nn(j));
% 
%                 visiocytetree(rootidx(i),2) = nodeBranchIdx(rootidx(i)); % the above is incorrect, fix it on 20090911, incorrect
% 
%             else
%                 visiocytetree(rootidx(i),2) = visiocytetree(nn(1),2);
%             end;
% 
%         end;
%     end;
    ...
    for i=1:size(branchmatchlist,1)
        ...
        while (j<=lenn)
            if(subTreeSeedNodes1(ii,j)>0)
                ...
                visiocytetree(idx,2) = cc+1; % set the color of the subtree root to the color of the parent branch
                j = j+1;
            else
               break;
            end;
    ...
    % add lines indicating matching between nodes
     ...
     for i=1:size(matchingList_allterms,1)
        ...
        if (matchingList_allterms(i,end)<=level)
            ...
            idx1 = find(visiocytetree(:,1) == (matchingList_allterms(i,1)+labelshift)); % there can be multiple idx1
            idx2 = find(visiocytetree(:,1) == matchingList_allterms(i,2)); % there can be multiple idx2

            nodeid1 = max(visiocytetree(:,1))+1;
            nodeid2 = max(visiocytetree(:,1))+2;
            
            len = len+1;
            visiocytetree(len,:) = visiocytetree(idx1(1), :);
            
            visiocytetree(len,1) = nodeid1; % add a new node
            visiocytetree(len, 2) = 0; % line color
%             visiocytetree(len, 2) = visiocytetree(len, 2)+0.001; % chnage x coordinate a little bit, to make it a different node
            visiocytetree(len, 6) = 0.25; % line thickness
            visiocytetree(len,7) = nodeid2;
            
            len = len +1;
            visiocytetree(len,:) = visiocytetree(idx2(1), :);
            visiocytetree(len,1) = nodeid2; % add a new node
%             visiocytetree(len, 2) = 0; % line color
            visiocytetree(len, 2) = 1; % line color, for recomb paper

            visiocytetree(len, 6) = 0.25; % line thickness
           
            visiocytetree(len,7) = nodeid1;
           
            
        end
    end;

    saveSwcVariable2File(visiocytetree, [filedir, 'visiocytetree', num2str(level), '.swc']);
    length(unique(visiocytetree(:,1)))    
...
% visiocytetree(:,6)=visiocytetree(:,6)*3;

saveSwcVariable2File(visiocytetree, [filedir, 'visiocytetree', num2str(maxlevel), '.swc']);
