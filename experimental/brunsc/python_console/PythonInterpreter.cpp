...
// situation we have here with VISIOCYTE.
#ifdef __linux__
#include <dlfcn.h> // dlopen()
void *visiocyte_ref_to_python_library = NULL;
void apply_embedded_dynamic_python_hack()
{
	...
	visiocyte_ref_to_python_library = dlopen(pylibname.c_str(), RTLD_LAZY | RTLD_GLOBAL);
	if (! visiocyte_ref_to_python_library) {
...
void clean_up_embedded_dynamic_python_hack() {
	if (visiocyte_ref_to_python_library)
		dlclose(visiocyte_ref_to_python_library);
}
#else
void apply_embedded_dynamic_python_hack() {}
void clean_up_embedded_dynamic_python_hack() {}
