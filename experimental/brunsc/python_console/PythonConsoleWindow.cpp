		  ...
		  bPythonReadline(false)
{
	...
	plainTextEdit->appendPlainText("Welcome to the VISIOCYTE python console!");
	...
	if (bRunPythonInSeparateThread) {
		visiocyte::qtGuiThread = QThread::currentThread();
	...
	if (parent)
	    visiocyte::qtParentWidget = parent;
	else
	    visiocyte::qtParentWidget = this;
