...
class VISIOCYTEWrapper:
    def __init__(self):
        "Container for pyplusplus module builder for wrapping VISIOCYTE"
        ...
        for path in ['.',
                       os.path.join(homedir, 'svn/visiocyte_cmake/visiocyte_main/basic_c_fun'),
                       os.path.join(homedir, 'svn/visiocyte/visiocyte_main/basic_c_fun'),
                       os.path.join(homedir, 'Documents/svn/visiocyte_cmake/visiocyte_main/basic_c_fun'),
                       os.path.abspath('../../../visiocyte_main/basic_c_fun'),
                       '/usr/include/Qt',
                       '/usr/include/QtCore',
                       '/usr/include/QtGui',
                       '/usr/include/qt4',
    ...
    def wrap(self):
        ...
        boost::python::scope().attr("__doc__") = 
            "Python module for interacting with the VISIOCYTE visualization program."
            "\\n\\n"
            "The visiocyte python module enables control of the "
            "VISIOCYTE biomedical volume visualization program "
            "from the python programming language.  The visiocyte "
            "module exposes the VISIOCYTE plug-in API; thus most "
            "things that can be done from a VISIOCYTE plug-in can also be "
            "done interactively from the VISIOCYTE python console."
            ;
        """)
        self.wrap_QList()
        self.wrap_QPolygon()
        ...
        self.mb.class_('VISIOCYTE_GlobalSetting').include()
        ...
        self.wrap_VISIOCYTEPluginArgItem()
        self.wrap_callPluginFunc()
        self.wrap_c_array_struct()
        self.wrap_visiocyte_qt_environment()
        self.wrap_QString()
        self.wrap_Vec3()
        self.wrap_Rotation()
    ...
    def wrap_VISIOCYTEPluginArgItem(self):
        cls = self.mb.class_('VISIOCYTEPluginArgItem')
        cls.include()
    ...
    def wrap_visiocyte_qt_environment(self):
        # Too bad.  this does not seem to work with pyside.
        fn = self.mb.namespace('visiocyte').free_function('get_qt_gui_parent')
        fn.include()
    ...
    def write_out(self, outputDir):
        ...
        self.mb.build_code_creator(module_name='visiocyte', doc_extractor=extractor)
        self.mb.split_module(outputDir)
        # Record success for makefile dependencies: "touch" in python is long...
        open(os.path.join(os.path.abspath('.'), 'generated_code', 'generate_visiocyte.stamp'), "w").close()


if __name__ == "__main__":
    visiocyteWrapper = VISIOCYTEWrapper()
    visiocyteWrapper.wrap()
    visiocyteWrapper.write_out('generated_code')
