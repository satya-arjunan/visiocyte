...
class CameraPosition:
    ...
    def populate_quaterion(self):
        ...
        R = visiocyte.Rotation()
        R.setRotationFromThreeAnglesThreeAxes(
                visiocyte.BodyRotationSequence, 
                xRot, visiocyte.XAxis, 
                yRot, visiocyte.YAxis, 
                zRot, visiocyte.ZAxis)
        self.quaternion = visiocyte.Quaternion(R)
        

class VisiocyteMovieFrame:
    """
    Any single frame of a Visiocyte movie.
...
class VisiocyteKeyFrame(VisiocyteMovieFrame):
    def __init__(self, camera_position, interval = 0):
        VisiocyteMovieFrame.__init__(self, camera_position)
...
class VisiocyteMovie:
    def _find_image_window(self):
        try:
            self.image_window = visiocyte.ImageWindow.current()
        except:
    ...
    def generate_write_frames(self, directory, file_root=None):        
        ...
        Parameters:
          ...
          file_root -- base name for frame files.  For example, a file_root
            ...
            the file name is based on the name of the Visiocyte image.
            
        Unlike VisiocyteMovie.play(), VisiocyteMovie.write_frames() might play back
        ...
        for key_frame in self.key_frames:
            ...
            if frame_index > 0:
                while frame_time < key_frame.interval:
                    ...
                    # yield VisiocyteMovieFrame() # TODO interpolate
    ...
    def set_current_visiocyte_camera(self, camera_position):
        if not self.view_control:
            self._find_view_control()
        if not self.view_control:
            raise ValueError("No Visiocyte window is attached")
        ...
        for getter_name in self.view_control_param_names:
            ...
            fn(val) # set parameter in Visiocyte view_control
    ...
    def get_current_visiocyte_camera(self):
        if not self.view_control:
            self._find_view_control()
        if not self.view_control:
            raise ValueError("No Visiocyte window is attached")
    ...
    def append_current_view(self, interval=2.0):
        camera = self.get_current_visiocyte_camera()
        if len(self.key_frames) == 0:
            interval = 0.0
        self.key_frames.append(VisiocyteKeyFrame(camera_position = camera, 
                                           interval = interval))
    ...
    def save_parameter_file(self, file_object):
        ...
        """
        paramDocument = getDOMImplementation().createDocument(None, "visiocyte_movie", None)
