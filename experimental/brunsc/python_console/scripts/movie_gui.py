...
try:
    ...
    try:
        ...
        print """
To use Visiocyte Movie GUI, you must install either PySide or PyQt4.
    http://www.pyside.org/
      OR
...
class SingleKeyFrameLabel(QtGui.QLabel):
    ...
    def mouseDoubleClickEvent(self, event):
        "After double click, set Visiocyte 3D viewer to this key frame"
...
class MovieGui(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        self.movie = movie_maker.VisiocyteMovie()
        iconPath = os.path.join(os.path.dirname(movie_maker.__file__), 'icons')
    ...
    def help(self):
        QtGui.QMessageBox.information(self, 
                "About Visiocyte movie maker", 
                """
First adjust the Visiocyte 3D window to the view you want to begin your movie with.  Next press "Add current view".  
...
Press "Save images..." to save the movie frames to disk.
    ...
    def show_frame(self, frame):
        self.movie.set_current_visiocyte_camera(frame.camera_position)
    ...
    def replace_frame_cartoon(self, frame_cartoon):
        # print "replaced"
        frame_cartoon.frame.camera_position = self.movie.get_current_visiocyte_camera()
        
    def append_view(self):
        "Add a new key frame based on the current view in the Visiocyte 3D viewer"
        ...
        try:
            ...
            try:
                ...
                QtGui.QMessageBox.information(self,
                        "Movie maker cannot find a Visiocyte 3D window.",
                        "Movie maker cannot find a Visiocyte 3D window.  Perhaps you need to open one")
                return
    ...
    def load_parameters(self):
        result = QtGui.QFileDialog.getOpenFileName(
                        self,
                        "Open Visiocyte movie parameters file",
                        self.previous_save_dir,
                        "Visiocyte movie files (*.vmv)")
        self.show() # why does window get hidden in this method?
