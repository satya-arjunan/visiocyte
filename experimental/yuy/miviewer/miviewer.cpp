...
//func
int iViewer(VISIOCYTEPluginCallback &callback, QWidget *parent);
...
void MIViewerPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback &callback, QWidget *parent)
{
...
int iViewer(VISIOCYTEPluginCallback &callback, QWidget *parent)
{
	...
	while (1) 
	{
		...
		if(cur_status != pre_status && inw->flag_syn)
		{
			...
			inw->update_visiocyteviews(callback, inw->roi_start_x*scaleFactor, inw->roi_start_y*scaleFactor, inw->roi_start_z*scaleFactor, inw->roi_end_x*scaleFactor, inw->roi_end_y*scaleFactor, inw->roi_end_z*scaleFactor);
