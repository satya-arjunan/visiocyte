...
CParaDialog_rigidaffine::CParaDialog_rigidaffine(VISIOCYTEPluginCallback &callback,QWidget *parent):QDialog(parent)
{
	setupUi(this);
	IniDialog();
}
...
{
	QSettings settings("VISIOCYTE plugin","image_registration_rigidaffine");
...
void CParaDialog_rigidaffine::_slots_openDlg_tar()
{
	...
	d.setNameFilter("Image file (*.tif *.raw *.lsm *.visiocyteraw *.visiocyteraw)");
	if(d.exec())
	{
