...
void convering(Tpre *pre1d, Tpost *pPost, VISIOCYTELONG imsz)
{
...
template <class T> QPixmap getXYPlane(const T * pdata, VISIOCYTELONG sx, VISIOCYTELONG sy, VISIOCYTELONG sz, VISIOCYTELONG sc, VISIOCYTELONG curz, float *p_vmax, float *p_vmin)
{
...
public:
	HDRFilerDialog(VISIOCYTEPluginCallback2 &cb, QWidget *parent)
	{
		//create a dialog
...
public slots:
	...
	void do_HDRfilter()
	{
		...
		VISIOCYTELONG tmpsum = start_x + end_x;
		...
		VISIOCYTELONG offset_z = cur_z * sx * sy; // current slice
		
		for(VISIOCYTELONG j=start_y; j<end_y; j++)
		{
			VISIOCYTELONG offset_j = offset_c + offset_z + j*sx;
		...
		for(VISIOCYTELONG ic=0; ic<sc; ic++)
		{
			if(ic==c) continue;
	...
	void copydata2disp()
	{
		...
		for(VISIOCYTELONG ic = 0; ic<sc; ic++)
		{
			...
			VISIOCYTELONG offset_c = ic*pagesz;
...
public:	
	VISIOCYTELONG roi_top, roi_left, roi_bottom, roi_right; // ROI boundary of the search box
	...
	VISIOCYTELONG cx, cy, cz, cc, cr;
	VISIOCYTELONG cur_x, cur_y, cur_z, cur_c;
	...
	VISIOCYTELONG start_x, end_x, start_y, end_y;
...
public:
	HDRwidget(VISIOCYTEPluginCallback2 *cb, QWidget *parent, VISIOCYTELONG *imgsz, unsigned char *data1d, int datatype)
	{	
		// update current triview in VISIOCYTE
		callback = (void *)cb;		
		
		b_showinvisiocyte = false;
		//cur_c = 1; // init here
		...
		//
		if(pHDRview->pDispData1d && b_showinvisiocyte)
		{
			// syncronizing in triview
			Image4DSimple p4DImage;
			p4DImage.setData((unsigned char*)(pHDRview->pDispData1d), sx, sy, sz, sc, VISIOCYTE_FLOAT32); //
			
			visiocytehandle newwin = ((VISIOCYTEPluginCallback2 *)callback)->newImageWindow();
			((VISIOCYTEPluginCallback2 *)callback)->setImage(newwin, &p4DImage);
			((VISIOCYTEPluginCallback2 *)callback)->setImageName(newwin, "HDR Filtered Image");
			((VISIOCYTEPluginCallback2 *)callback)->updateImageWindow(newwin);
		}
...
public:
	void initHDRViwer(VISIOCYTELONG *imgsz, unsigned char *data1d, int datatype)
	{
		...
		VISIOCYTELONG pagesz = sx*sy*sz;
		VISIOCYTELONG imagesz = pagesz*sc;
...
public slots:
	...
	void updateCurWin()
	{
		if(b_showinvisiocyte)
		{
			visiocytehandle curwin = ((VISIOCYTEPluginCallback2 *)callback)->currentImageWindow();
			((VISIOCYTEPluginCallback2 *)callback)->updateImageWindow(curwin);
			
			((VISIOCYTEPluginCallback2 *)callback)->pushImageIn3DWindow(curwin);
		}
