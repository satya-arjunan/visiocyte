...
bool loadImageWSC(char imgSrcFile[], unsigned char *& data1d, VISIOCYTELONG * &sz, int & datatype)
{
	if (data1d)
	{
	...
	else if ( strcasecmp(curFileSuffix, "raw5")==0 ) //read lsm stacks
	{
		...
		{
			printf("Error happens in VISIOCYTE .raw5 (5D) file reading. Stop. \n");
			return false;
		}
		b_5d = true;
	}
...
int hdr_filtering(VISIOCYTEPluginCallback2 &callback, QWidget *parent);

// func convering kernel
template <class Tpre, class Tpost>
int convering(void *subject1d, ImagePixelType visiocyte_dt, ImagePixelType dt);
...
void HDRFilterPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
int hdr_filtering(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	VISIOCYTELONG *imgsz=NULL, sx, sy, sz, sc;
	int datatype;
	...
	// HDR filtering
	HDRwidget *pHDRwidget = new HDRwidget(&callback, parent, imgsz, data1d, datatype);
	pHDRwidget->show();
