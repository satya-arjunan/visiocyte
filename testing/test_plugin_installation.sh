...
VISIOCYTE="../bin/visiocyte"
plugin_path="../bin/plugins"

TESTING_PACKAGE=1

if [[ "$OSTYPE" == "darwin"* ]]; then
  VISIOCYTE=../bin/visiocyte64.app/Contents/MacOS/visiocyte64
  plugin_path=../bin/plugins
  if [[ $TESTING_PACKAGE>0 ]]; then
    VISIOCYTE=../Visiocyte.app/Contents/MacOS/visiocyte
    plugin_path=../plugins
  fi
fi

if [[ "$OSTYPE" == "cygwin" ]]; then
  VISIOCYTE=./bin/visiocyte.exe
  plugin_path=./bin/plugins
  if [[ TESTING_PACKAGE>0 ]]; then
    VISIOCYTE=../bin/visiocyte.exe
    plugin_path=../bin/plugins
...
if ! [ -e $VISIOCYTE ]; then
    echo "$VISIOCYTE does not exsit"
    echo "please input visiocyte executable path:"
    read input
    VISIOCYTE=$input
fi
...
do
  ...
  if [[ "$OSTYPE" == "cygwin" ]]; then
    command="$VISIOCYTE /h /x $plugin /f help"
    echo $command
  else
    command="$VISIOCYTE -h -x $plugin -f help"
    echo $command
