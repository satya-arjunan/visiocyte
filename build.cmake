...
boost_prefix=$BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib
CMAKE_PLATFORM_ARGS="-DBOOST_ROOT:PATH=$boost_prefix "
if [ $PLATFORM = "windows-x86_64" ]; then
    CMAKE_PLATFORM_ARGS+="-DTIFF_INCLUDE_DIR:PATH=$BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib/include "
    CMAKE_PLATFORM_ARGS+="-DTIFF_LIBRARY:PATH=$BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib/winlib64/libtiff.lib "
    CMAKE_PLATFORM_ARGS+="-DZLIB:FILEPATH=$BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib/winlib64/libzlib.lib "
    CMAKE_PLATFORM_ARGS+="-DFFTW_INCLUDE_DIR:PATH=$BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib/fftw-3.3.4-dll64 "
    CMAKE_PLATFORM_ARGS+="-DFFTW_LIBRARY:PATH=$BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib/fftw-3.3.4-dll64/libfftw3f-3.lib"
...
case $OPERATION in
    install)
        ...
        if [[ ! -e visiocyte_main/terafly/src/terarepo/src ]]; then
            echo "Missing the terafly repository. Did you do a git submodule command?"
            exit 1
        fi

        if [[ ! -e $BUILD_DIR/build_$PLATFORM ]]; then
            mkdir -p $BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib
            mkdir -p $BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib/build
            mkdir -p $BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib/include
            cp -r $ROOT_DIR/visiocyte_main/common_lib/winlib* $BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib
        fi
        ...
        if [[ ! -e $boost_include_prefix ]]; then
            ...
            if [ $PLATFORM = "windows-x86_64" ]; then
                if [[ ! -e boost_$BOOST_VERSION ]]; then
                    /c/Program\ Files/7-Zip/7z x -y $ROOT_DIR/visiocyte_main/common_lib/src_packages/boost_$BOOST_VERSION.tar.gz
                ...
                tar xzf $ROOT_DIR/visiocyte_main/common_lib/src_packages/boost_$BOOST_VERSION.tar.gz
                cd boost_$BOOST_VERSION
        ...
        if [ $PLATFORM = "windows-x86_64" ]; then
          if [[ ! -e $BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib/include/tiff.h ]]; then
              echo "Configuring TIFF headers"
              cd $BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib/build
              tar xzf $ROOT_DIR/visiocyte_main/common_lib//src_packages/tiff-4.0.2.tar.gz
              cd tiff-4.0.2
              nmake Makefile.vc
            ...
            cd $BUILD_DIR/build_$PLATFORM/visiocyte_main/common_lib
            if [[ ! -e fftw-3.3.4-dll64 ]]; then
                tar xzf $ROOT_DIR/visiocyte_main/common_lib/fftw-3.3.4-dll64.tgz
            fi
            if [[ ! -e ffmpeg-2.5.2-win64 ]]; then
                tar xzf $ROOT_DIR/visiocyte_main/common_lib/ffmpeg-2.5.2-win64.tgz
            fi
            cd ../../
        fi
        ...
        if [ $PLATFORM = "windows-x86_64" ]; then
            if [ $BUILD_HDF5 = 1 ]; then
                echo "Building HDF5"
                devenv Visiocyte.sln -project HDF5 -build $CMAKE_BUILD -out hdf5.txt
            fi
            echo "Building Visiocyte"
            devenv Visiocyte.sln -build $CMAKE_BUILD -out all_build.txt
            echo "Installing"
            devenv Visiocyte.sln -project INSTALL -build $CMAKE_BUILD -out install.txt
            echo "Done."
        else
