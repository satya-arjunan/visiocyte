...
void sort_menu(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
void printHelp(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output)
{
	...
	cout<<"Demo1:\t ./visiocyte -x sort_neuron_swc -f sort_swc -i test.swc -o test_sorted.swc -p 0  1 (no new links) \n";
	cout<<"Demo2:\t ./visiocyte -x sort_neuron_swc -f sort_swc -i test.swc -o test_sorted.swc <-p Inifinity> (Default threshold Infinity:all points will be connected automatically, use the default first root as the root id) \n";
