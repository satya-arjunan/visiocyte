...
void SWC_TO_MASKIMAGElugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
		visiocyte_msg("(version 0.11) Convert a SWC file to a mask image, where the area of the swc tubes will have non-zero values, and other area will have 0s; Use mask imge to filter tif image, where the area of the swc non-zero will tif image values not change, and other area will have 0");
		return;
	}
}
...
)
{
	...
	//subtraction
//	visiocyte_msg(QString("xmin %1 ymin %2 zmin %3 bool %4").arg(xmin).arg(ymin).arg(zmin).arg(b_subtractMinFromAllNonnegatives));
	for (ii=0; ii<neuron.listNeuron.size(); ii++)
	{
		p_cur = (NeuronSWC *)(&(neuron.listNeuron.at(ii)));
...
//		pImMask[VISIOCYTELONG(zs)*sx*sy + VISIOCYTELONG(ys)*sx + VISIOCYTELONG(xs)] = random()%250 + 1;
//		continue;
...
//			zn = ( zn > sz )? sz : zn;
//
//			VISIOCYTELONG idex=(zn)*sx*sy + (yn)*sx + xn;
//			if (method_code == 1)
//			{
//				pImMask[idex] = 255;
//			}
...
//		}
		...
		VISIOCYTELONG idex1=lroundf(z)*sx*sy + lroundf(y)*sx + lroundf(x);
//
		if (method_code == 1)
		{
			pImMask[idex1] = 255;
		...
		for (int i = 0; i <= steps; i++)
		{
			...
			VISIOCYTELONG idex=lroundf(z)*sx*sy + lroundf(y)*sx + lroundf(x);
		...
		double x_down = (xs < xe) ? xs : xe; x_down -= rbox; x_down = VISIOCYTELONG(x_down); if (x_down<0) x_down=0; if (x_down>=sx-1) x_down = sx-1;
		double x_top  = (xs > xe) ? xs : xe; x_top  += rbox; x_top  = VISIOCYTELONG(x_top ); if (x_top<0)  x_top=0;  if (x_top>=sx-1)  x_top  = sx-1;
		double y_down = (ys < ye) ? ys : ye; y_down -= rbox; y_down = VISIOCYTELONG(y_down); if (y_down<0) y_down=0; if (y_down>=sy-1) y_down = sy-1;
		double y_top  = (ys > ye) ? ys : ye; y_top  += rbox; y_top  = VISIOCYTELONG(y_top ); if (y_top<0)  y_top=0;  if (y_top>=sy-1)  y_top = sy-1;
		double z_down = (zs < ze) ? zs : ze; z_down -= rbox; z_down = VISIOCYTELONG(z_down); if (z_down<0) z_down=0; if (z_down>=sz-1) z_down = sz-1;
		double z_top  = (zs > ze) ? zs : ze; z_top  += rbox; z_top  = VISIOCYTELONG(z_top ); if (z_top<0)  z_top=0;  if (z_top>=sz-1)  z_top = sz-1;
...
//////////////////////////////////////////////////////
	...
	if(QMessageBox::Yes == QMessageBox::question (0, "", QString("Do you want set the image size?"), QMessageBox::Yes, QMessageBox::No))
	{
		...
		for (VISIOCYTELONG ii=0; ii<nx*ny*nz; ii++)
			pData[ii] = 0;
...
void mrskimage_originalimage(VISIOCYTEPluginCallback2 &callback, QWidget *parent, int method_code)
{
	...
	VISIOCYTELONG min,min1,max,max1,filter;
	...
	VISIOCYTELONG sx1 = image1->getXDim();
    VISIOCYTELONG sy1 = image1->getYDim();
    VISIOCYTELONG sz1 = image1->getZDim();
	VISIOCYTELONG sc1= image1->getCDim();
	VISIOCYTELONG pagesz_sub1 = sx1*sy1*sz1;
	printf("sx=%d sy=%d sz=%d sc1=%d\n",sx1,sy1,sz1,sc1);

	Image4DProxy<Image4DSimple> pSub2(image2);
	unsigned char * pData2  = pSub2.begin();
	VISIOCYTELONG sx2 = image2->getXDim();
    VISIOCYTELONG sy2 = image2->getYDim();
    VISIOCYTELONG sz2 = image2->getZDim();
	VISIOCYTELONG sc2 = image2->getCDim();
	VISIOCYTELONG pagesz_sub2 = sx2*sy2*sz2;

	unsigned char * pData = new unsigned char[sx1*sy1*sz1];
	for (VISIOCYTELONG ii=0; ii<sx1*sy1*sz1; ii++)
		pData[ii] = 0;
	//	printf("sx2=%d sy2=%d sz2=%d sc2=%d\n",sx2,sy2,sz2,sc2);

	if (sx1!=sx2 || sy1!=sy2 || sz1!=sz2 )
	{
		visiocyte_msg(" the image size differ ");
		return;
	}
	for (VISIOCYTELONG k = 0; k < sz1; k++)
	{
		for(VISIOCYTELONG j = 0; j < sy1; j++)
		{
			for(VISIOCYTELONG i = 0; i < sx1; i++)
			{
	...
	Image4DSimple tmp;
	tmp.setData(pData, sx1, sy1, sz1, 1, VISIOCYTE_UINT8);
...
void printHelp()
{
	...
	printf("Usage visiocyte -x swc_to_maskimage_sphere_unit -f swc_to_maskimage -i <intput.swc> [-p <sz0> <sz1> <sz2>] [-o <output_image.raw>]\n");
}
*/
