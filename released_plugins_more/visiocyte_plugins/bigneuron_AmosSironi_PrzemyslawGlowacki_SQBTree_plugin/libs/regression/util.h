...
typename ITKImageType::Pointer visiocyte2ItkImage(Image4DSimple *inimg,const long int *in_sz, unsigned int c=1);

template<typename ImageType>
Image4DSimple itk2visiocyteImage(typename ImageType::Pointer itk_image);
...
typename ITKImageType::Pointer downsample_image_visiocyte(Image4DSimple *inimg,const double * dfactor, VISIOCYTELONG * szout);

template<typename ITKImageType,typename T>
typename ITKImageType::Pointer upsample_image_visiocyte(Image4DSimple *inimg,const double * scale_factor, VISIOCYTELONG * out_sz);

template<typename ITKImageType,typename T>
typename ITKImageType::Pointer upsample_image_visiocyte_size(Image4DSimple *inimg,VISIOCYTELONG * szout);

template<typename ITKImageType,typename T>
typename ITKImageType::Pointer resize_image_visiocyte(Image4DSimple * pred_img_visiocyte,const double * scale_factor, VISIOCYTELONG * out_sz);
...
//  std::cout << "Converting to VISIOCYTE" << std::endl << std::flush;
        ...
        const long int n_pixels = in_sz[0]*in_sz[1]*in_sz[2];
            ...
            Image4DSimple outimg_visiocyte;
        ...
        outimg_visiocyte.setData((unsigned char *)(out_data_copy), in_sz[0], in_sz[1], in_sz[2], 1, VISIOCYTE_FLOAT32);



        return outimg_visiocyte;
}
  ...
  for(std::vector< SwcFileContent::SwcRow >::iterator rowi = swcFileContent.rows.begin(); rowi != swcFileContent.rows.end(); rowi++) {
    ...
    if(rowi->parentInd != -1) {
      ...
      for(double i = 0.0; i < 1.0; i += 1.0 / segmentLength) {
        ...
        if(transpose_y){
            pixelIndex[1] = in_sz[1] -1 -round(rowi->y + vecY * i);//visiocyte transpose y axis
        }else{
...
typename ITKImageType::Pointer downsample_image_visiocyte(Image4DSimple *inimg,const double * dfactor, VISIOCYTELONG * szout){
   ...
   // typename ITKImageType::Pointer output = ITKImageType::New();
        ...
        VISIOCYTELONG *szin = new VISIOCYTELONG[3];
...
template<typename ITKImageType,typename T>
typename ITKImageType::Pointer upsample_image_visiocyte(Image4DSimple *inimg,const double * dfactor, VISIOCYTELONG * szout){
    ...
    szout[0] = (VISIOCYTELONG)(ceil(dfactor[0]*((double)szin[0])));
    szout[1] = (VISIOCYTELONG)(ceil(dfactor[1]*((double)szin[1])));
    szout[2] = (VISIOCYTELONG)(ceil(dfactor[2]*((double)szin[2])));
...
//     VISIOCYTELONG *szout_1 = new VISIOCYTELONG[3];

//    typename ITKImageType::Pointer outputItkImage = upsample_image_visiocyte<ITKImageType,T>(inimg,dfactor,szout_1);
...
//template<typename ITKImageType>
//typename ITKImageType::Pointer downsample_image_visiocyte(Image4DSimple *inimg,const double * scale_factor, VISIOCYTELONG * out_sz){
...
//    if(downsampled){
//        outImg->setData((unsigned char *)(outdata), out_sz[0], out_sz[1], out_sz[2], 1, VISIOCYTE_FLOAT32);
//    }else{
//        std::cout << "could not downsample image !" << std::endl;

//    }

//    return visiocyte2ItkImage<ITKImageType>(outImg,out_sz);
