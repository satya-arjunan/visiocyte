...
VISIOCYTELONG gpiDims_InputImage[3];
...
void autotrace_PSF(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
bool autotrace_PSF(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback);


void PreProcessDataImage(double *, int , VISIOCYTELONG *, int , double *,
...
void PSF_zhi::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
        visiocyte_msg(tr("PSF tracing plugin. "
            "Imported by Zhi Zhou, 2013-03-07"));
	}
}

bool PSF_zhi::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    ...
    {
        cout<<"Usage : visiocyte -x dllname -f PSF_trace -i <inimg_file> -p <ch> "<<endl;
        cout<<endl;
...
void autotrace_PSF(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    VISIOCYTELONG iVolWidth, iVolHeight;
    VISIOCYTELONG iNumber_Of_Elements_in_ProjectedPoints;
    VISIOCYTELONG iIndex;
    double sigma = 3;
    ...
    hmirror = 7;
    VISIOCYTELONG N_new = N+2*hmirror;
    VISIOCYTELONG M_new = M+2*hmirror;
    VISIOCYTELONG P_new = P+2*hmirror;
    VISIOCYTELONG pagesz_new = N_new*M_new*P_new;
    ...
    for(VISIOCYTELONG iz = hmirror; iz < P_new - hmirror; iz++)
    {
        VISIOCYTELONG offsetk = iz*M_new*N_new;
        for(VISIOCYTELONG iy = hmirror; iy < M_new - hmirror; iy++)
        {
            VISIOCYTELONG offsetj = iy*N_new;
            for(VISIOCYTELONG ix = hmirror; ix < N_new-hmirror; ix++)
            {
    ...
    for(VISIOCYTELONG iz = 0; iz < hmirror; iz++)
    {
        VISIOCYTELONG offsetk = iz*M_new*N_new;
        VISIOCYTELONG offsetk2 = (2*hmirror - iz)*M_new*N_new;
        for(VISIOCYTELONG iy = hmirror; iy < M_new - hmirror; iy++)
        {
            VISIOCYTELONG offsetj = iy*N_new;
            for(VISIOCYTELONG ix = hmirror; ix < N_new-hmirror; ix++)
    ...
    for(VISIOCYTELONG iz = P_new-hmirror; iz < P_new; iz++)
    {
        VISIOCYTELONG offsetk = iz*M_new*N_new;
        VISIOCYTELONG offsetk2 = (2*P_new-2*hmirror-2-iz)*M_new*N_new;
        for(VISIOCYTELONG iy = hmirror; iy < M_new - hmirror; iy++)
        {
            VISIOCYTELONG offsetj = iy*N_new;
            for(VISIOCYTELONG ix = hmirror; ix < N_new-hmirror; ix++)
    ...
    {
        VISIOCYTELONG offsetk = iz*M_new*N_new;
        for(VISIOCYTELONG iy = 0; iy < hmirror; iy++)
        {
            VISIOCYTELONG offsetj = iy*N_new;
            VISIOCYTELONG offsetj2 = (2*hmirror - iy)*N_new;
            for(VISIOCYTELONG ix = hmirror; ix < N_new-hmirror; ix++)
            {
    ...
    {
        VISIOCYTELONG offsetk = iz*M_new*N_new;
        for(VISIOCYTELONG iy = M_new-hmirror; iy < M_new; iy++)
        {
            VISIOCYTELONG offsetj = iy*N_new;
            VISIOCYTELONG offsetj2 = (2*M_new-2*hmirror-2-iy)*N_new;
            for(VISIOCYTELONG ix = hmirror; ix < N_new-hmirror; ix++)
            {
                gpdInputImage[offsetk + offsetj + ix] = gpdInputImage[offsetk + offsetj2 + ix];
                i++;
    ...
    for(VISIOCYTELONG iz = hmirror; iz < P_new - hmirror; iz++)
    {
        ...
        {
            VISIOCYTELONG offsetj = iy*N_new;
            for(VISIOCYTELONG ix = 0; ix < hmirror; ix++)
            {
    ...
    for(VISIOCYTELONG iz = hmirror; iz < P_new - hmirror; iz++)
    {
        ...
        {
            VISIOCYTELONG offsetj = iy*N_new;
            for(VISIOCYTELONG ix = N_new - hmirror; ix < N_new; ix++)
            {
   ...
   /* int iColIter,iRowIter;
    ...
    for(VISIOCYTELONG i = 0; i<number_points*k1*N1; i++)
    {
    ...
    double sum_w;
    for(VISIOCYTELONG i = 0; i < number_points*k1; i++)
    {
        sum_w = 0;
        for(int j = 0; j < N1; j++)
        {
    ...
    for(VISIOCYTELONG i = 0; i <number_points*number_points; i++ )
        DScore[i] = INFINITY;
    ...
    double *DScore_wRoot = new double[(number_points+1)*(number_points+1)];
     for(VISIOCYTELONG i = 0; i < (number_points+1)*(number_points+1); i++)
     {
         DScore_wRoot[i] = INFINITY;
     }

    for(VISIOCYTELONG i = 0; i < number_points; i++)
    {
        for(VISIOCYTELONG j = 0; j < number_points; j++)
        {
    ...
    for(VISIOCYTELONG i = 0; i < number_points+1; i++)
    {
        for(VISIOCYTELONG j = i; j < number_points+1; j++)
        {
    ...
    visiocyte_msg(QString("Now you can drag and drop the generated swc fle [%1] into Visiocyte.").arg(outswc_file));
    return;
...
bool autotrace_PSF(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback)
{
    ...
    visiocyte_msg(QString("Now you can drag and drop the generated swc fle [%1] into Visiocyte.").arg(outswc_file),0);
...
void preIntegral(double *gpdProjected_Points_updated,double *tangentialSpace_updated,int N,int k,int number_points,int giNum_of_Dims_of_Input_Image, double *T, double *Y, double *W, int *idx)
{
    ...
    for(int ix = 0; ix < number_points; ix++)
    {
        VISIOCYTELONG offsetx = ix*N*k;
        for(int iy = 0; iy < k; iy ++ )
        {
            VISIOCYTELONG offsety = iy*N;
