...
float MOSTImage::getRayburstRadius(  V_NeuronSWC_unit &center, const bool &force, const int &rayburstThreshold )
{
    ...
    VISIOCYTELONG dim0 = this->getXDim();
    VISIOCYTELONG dim1 = this->getYDim();
    VISIOCYTELONG dim2 = this->getZDim();
    float x = center.x;
    float y = center.y;
    float z = center.z;
...
V_NeuronSWC_unit MOSTImage::createNode_br(VoxelCluster &conCom)
{
    ...
    //newNode = this->recenter(newNode, conCom.threshold);
    newNode.r = this->getRayburstRadius(newNode, true, conCom.threshold);
    vector<VISIOCYTELONG> node_ID =swc.getIndexofParent_nodeid(conCom.parent_node_ID);
...
void MOSTImage::auto_detect_seedz(LandmarkList &seedList,VISIOCYTELONG slice,const int InitThreshold,const int seed_size_all)
{
   {
    VISIOCYTELONG zz=slice;
    VISIOCYTELONG xx=this->getXDim();
    VISIOCYTELONG yy=this->getYDim();
  ...
  /* {
    VISIOCYTELONG zz=this->getZDim()-1;
    VISIOCYTELONG xx=this->getXDim();
    VISIOCYTELONG yy=this->getYDim();
    VoxelCluster all_seed_region;
    QList < VoxelCluster > conCltList;
...
void MOSTImage::auto_detect_seedx(LandmarkList &seedList,VISIOCYTELONG slice,const int InitThreshold,const int seed_size_all)
{
   {
    VISIOCYTELONG zz=this->getZDim();
    VISIOCYTELONG xx=slice;
    VISIOCYTELONG yy=this->getYDim();
    VoxelCluster all_seed_region;
    QList < VoxelCluster > conCltList;
    LocationSimple seed;
...
void MOSTImage::auto_detect_seedy(LandmarkList &seedList,VISIOCYTELONG slice,const int InitThreshold,const int seed_size_all)
{
   {
    VISIOCYTELONG zz=this->getZDim();
    VISIOCYTELONG xx=this->getXDim();
    VISIOCYTELONG yy=slice;
    VoxelCluster all_seed_region;
    QList < VoxelCluster > conCltList;
    LocationSimple seed;
    for(int i =0;i<xx;i++)
