...
void printHelp()
{
    cout<<"\nUsage: visiocyte -x neuron_connector -f connect_neuron_SWC -i <input.swc> -o <output.swc> "
    ...
    for(VISIOCYTELONG i=0; i<childNum.size(); i++){
        if(connNum.at(i)<1){
            ...
            if(childNum[id]==1){ //single child root
                VISIOCYTELONG sid = pList.indexOf(id);
                if(sid>=0){
...
//                    connMap.insert(pair<double, QVector<VISIOCYTELONG> >(mvdis,tmp));
//                }
//            }else{
                if(mvid>=0){
                    QVector<VISIOCYTELONG> tmp;
                    tmp.append(tidx); tmp.append(mvid);
                    connMap.insert(pair<double, QVector<VISIOCYTELONG> >(mvdis+disThr,tmp));
                }
                if(mtid>=0){
                    QVector<VISIOCYTELONG> tmp;
...
bool export_list2file(const QList<NeuronSWC>& lN, QString fileSaveName)
{
    ...
    myfile<<"# generated by Visiocyte Plugin neuron_connector"<<endl;
    if(eswc_flag)
