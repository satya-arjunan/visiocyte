...
is_analysis_fun::is_analysis_fun(VISIOCYTEPluginCallback2 *cb, QStringList name_list,QVector<int> para_list)
{
    callback=cb;
    p_img1D=0;
    mask=0;
...
//    VISIOCYTELONG count=0;
    ...
    for(VISIOCYTELONG nid=0; nid<nt.listNeuron.size(); nid++){
        ...
        for(VISIOCYTELONG x=MAX(nx-disthr,0); x<=MIN(nx+disthr,sz_img[0]-1); x++){
            for(VISIOCYTELONG y=MAX(ny-disthr,0); y<=MIN(ny+disthr,sz_img[1]-1); y++){
                for(VISIOCYTELONG z=MAX(nz-disthr,0); z<=MIN(nz+disthr,sz_img[2]-1); z++){
                    VISIOCYTELONG pos=xyz2pos(x,y,z,sz_img[0],sz_img[0]*sz_img[1]);
...
//    QString filename="mask1.visiocyteraw";
//    simple_saveimage_wrapper(*callback,filename.toStdString().c_str(),mask,mask_sz,VISIOCYTE_UINT8);
    if (p_img1D!=0)
    {
        delete[] p_img1D;
        p_img1D=0;
...
void is_analysis_fun::connected_components()
{
    ...
    for(map<VISIOCYTELONG, bool>::iterator iter_map=voxels_map.begin(); iter_map!=voxels_map.end(); iter_map++)
    {
...
void is_analysis_fun::stat_generate()
{
    ...
    vector<VISIOCYTELONG> coord(3,0);
    VISIOCYTELONG z_offset=sz_img[0]*sz_img[1];
...
void is_analysis_fun::visualize_image()
{
    ...
    image4d.setData(visual_copy,mask_sz[0],mask_sz[1],mask_sz[2],3,VISIOCYTE_UINT8);
...
//    QString filename="test1.visiocyteraw";
//    simple_saveimage_wrapper(*callback,filename.toStdString().c_str(),mask,mask_sz,VISIOCYTE_UINT8);
