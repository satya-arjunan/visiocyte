...
combiner::combiner(VISIOCYTEPluginCallback2 *cb)
{
...
bool combiner::get_spine_csv_name()
{
    ...
    QSettings MySettings("VISIOCYTE plugin","combiner");
    ...
    for (int i=0;i<spine_csv_name_list.size();i++)
    {
        ...
        else
        {
            ...
            if (!tmp_list.empty())
            {
                visiocyte_msg("Repeated! This file is in the list");
...
void combiner::spine_dialog_ok_clicked()
{
    ...
    {
        visiocyte_msg("No files selected. Please select");
    }
}
...
bool combiner::spine_files_check()
{
    ...
    for (int i=0;i<load_spine_combo->count();i++)
    {
        if (!load_spine_csv(load_spine_combo->item(i)->text())){
            visiocyte_msg("Loading SPINE csv error.\n" + load_spine_combo->item(i)->text()+"\nPlease check.");
            return false;
        }
    }
    return true;
...
bool combiner::is_files_check()
{
    ...
    for (int i=0;i<load_is_combo->count();i++)
    {
        if (!load_is_csv(load_is_combo->item(i)->text())){
            visiocyte_msg("Loading IS csv error.\n" + load_is_combo->item(i)->text() + "\nPlease check.");
            return false;
        }
    }
    return true;
...
{
    if(!check_button()){
        visiocyte_msg("You have not provided valid input");
        return;
    }
    this->accept();
...
//            qDebug()<<"node id 1, key:"<<iter_map.key()<<"spine:"<<spine_vec.size()<<" is:"<<is_vec.size();
        ...
        {
            visiocyte_msg("SWC and tables do not match. Please check inputs.\n");
            return false;
        }
