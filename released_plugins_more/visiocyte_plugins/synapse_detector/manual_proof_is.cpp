...
QDataStream &operator>>(QDataStream &in, GOI &tmp)
{
    ...
    vector<VISIOCYTELONG> oldstylevoxels;
    bool on_dendrite;

    in >>voxels>> nearest_id >> label_id >>on_dendrite;

    for (int i=0;i<voxels.size();i++)
        oldstylevoxels.push_back((VISIOCYTELONG)voxels.at(i));
...
manual_proof_is::manual_proof_is(VISIOCYTEPluginCallback2 *cb, bool menu_flag)
{
    callback=cb;
    seg_edit_flag=false;
    edit_flag=false;
...
void manual_proof_is::initDlg()
{
    if(debugF)
        qDebug()<<"in initDlg";
    QSettings settings("VISIOCYTE plugin","IS_quantifier");
...
void manual_proof_is::run_interface_with_auto(int ret,NeuronTree auto_neuron,bool eswc_tmp_flag,
    LandmarkList LList,vector<GOI> voxel_group_in,unsigned char *image1Dc,VISIOCYTELONG sz_img_in[4],
...
vector<VISIOCYTELONG> manual_proof_is::image_seg_plan(vector<int> seg, NeuronTree *neuron_tmp)
{
...
//    qDebug()<<"xyz max:"<<x_start<<":"<<y_start<<":"<<z_start;
    ...
    for (int j=0;j<LList_in.size();j++)
    {
        ...
        if (LList_in.at(j).category!=seg_id)  //need to check if the IS is complete
        {
            ...
            {
                vector<VISIOCYTELONG> coord = pos2xyz(tmp_is.voxel[k],sz_img[0],sz_img[0]*sz_img[1]);
        ...
        for (int k=0;k<tmp_is.voxel.size();k++)
        {
            vector<VISIOCYTELONG> old_coord = pos2xyz(tmp_is.voxel[k],sz_img[0],sz_img[0]*sz_img[1]);
            VISIOCYTELONG x=old_coord[0]-x_start;
            VISIOCYTELONG y=old_coord[1]-y_start;
            VISIOCYTELONG z=old_coord[2]-z_start;
            VISIOCYTELONG pos=xyz2pos(x,y,z,sz[0],sz[0]*sz[1]);
...
void manual_proof_is::erode()
{
    ...
    vector<VISIOCYTELONG> erodeList, remainList;
    QHash<VISIOCYTELONG, bool> hash;
    ...
    for (int i=0;i<tmp_group.voxel.size();i++)
    {
        ...
        VISIOCYTELONG pos= tmp_group.voxel[i];
        bg_count=0;
    ...
    {
        vector<VISIOCYTELONG> coord= pos2xyz(erodeList.at(i),sz_img[0],sz_img[0]*sz_img[1]);
        //image_is[erodeList.at(i)+2*size_page]=0; //do we need to maintain image_is?
        VISIOCYTELONG x= coord[0]-x_start;
        VISIOCYTELONG y= coord[1]-y_start;
        VISIOCYTELONG z= coord[2]-z_start;
        VISIOCYTELONG trun_pos = xyz2pos(x,y,z,sz[0],sz[0]*sz[1]);
...
void manual_proof_is::dilate()
{
    ...
    vector<VISIOCYTELONG> dilateList,totalList;
    QHash<VISIOCYTELONG, bool> hash;
    for (int i=0;i<tmp_group.voxel.size();i++)
        hash.insert(tmp_group.voxel.at(i),true);
    ...
    for (int i=0;i<tmp_group.voxel.size();i++)
    {
        ...
        VISIOCYTELONG x=coord[0];
        VISIOCYTELONG y=coord[1];
        VISIOCYTELONG z=coord[2];
    ...
    {
        vector<VISIOCYTELONG> coord= pos2xyz(totalList.at(i),sz_img[0],sz_img[0]*sz_img[1]);
...
void manual_proof_is::reset_edit(bool visual_flag,int mid)
{
    ...
    for (int i=0;i<tmp_group.voxel.size();i++)
    {
        vector<VISIOCYTELONG> coord= pos2xyz(tmp_group.voxel.at(i),sz_img[0],sz_img[0]*sz_img[1]);
        VISIOCYTELONG x= coord[0]-x_start;
        VISIOCYTELONG y= coord[1]-y_start;
        VISIOCYTELONG z= coord[2]-z_start;
        VISIOCYTELONG trun_pos = xyz2pos(x,y,z,sz[0],sz[0]*sz[1]);
...
//          <<l.y<<":" <<l.z<<" rgb:"<<l.color.r<<":"<<l.color.g<<":"<<l.color.b;
    ...
    {
        visiocyte_msg("Could not open file");
        return;
    }
...
bool manual_proof_is::loadProjectData()
{
    ...
    //LandmarkList result;
    //QFile file("C:\\Users\\Jade\\Documents\\Visiocyte\\ISquantifer.txt");
    ...
    {
//        if (LList_in.size()<=0)
//            visiocyte_msg("LList in size <0",0);
        if (voxel_group.size()<=0)
            visiocyte_msg("Voxel group<=0",0);
        return false;
    }
    QSettings settings("VISIOCYTE plugin","IS_Quantifer");
    settings.setValue("fname_proj",edit_load->text());
...
int manual_proof_is::csv_generate()
{
    ...
    QString fileDefaultName=baseName.remove(".visiocyteraw")+"_ISprofile.csv";
    QString completName=QDir(baseDir).filePath(fileDefaultName);
...
//    callback->pushObjectIn3DWindow(main_win);
    ...
    image4d.setData(input_image,sz_img[0],sz_img[1],sz_img[2],3,VISIOCYTE_UINT8);
    QString final_name=QString(MAINWINNAME) +"_result";
    //visiocytehandle test_win=callback->newImageWindow("newwindowforTest");
