...
spine_fun::spine_fun(const char * inname_img, const char* inname_skel, const char* inname_out, VISIOCYTEPluginCallback * cb)
{
    //init names
    callback=cb;
...
spine_fun::spine_fun(VISIOCYTEPluginCallback * cb,parameters set_para,int channel=0)
{
    p_img1D = 0;
    ppp_img3D = 0;
    callback=cb;
...
spine_fun::spine_fun(VISIOCYTEPluginCallback * cb,int channel,int bg_thr,int max_dis,unsigned short *group_label)  //only for initiation in manual proofreading
{
    p_img1D = 0;
    ppp_img3D = 0;
    callback=cb;
...
bool spine_fun::init()
{
    ...
    VISIOCYTELONG sz_page=sz_img[0]*sz_img[1]*sz_img[2];
    map<VISIOCYTELONG, VOI *> voxels_map;
    ...
    try{
        ...
        label=new VISIOCYTELONG[sz_page];
    }
    catch(...){
    ...
    for(VISIOCYTELONG i=0; i<sz_page; i++){
        dst[i]=INF;
    }

    //map<VISIOCYTELONG,int> nearest_node_map;

    for(VISIOCYTELONG nid=0; nid<nt.listNeuron.size(); nid++){
        VISIOCYTELONG nx = nt.listNeuron.at(nid).x;
        VISIOCYTELONG ny = nt.listNeuron.at(nid).y;
        VISIOCYTELONG nz = nt.listNeuron.at(nid).z;
        ...
        for(VISIOCYTELONG x=MAX(nx-nt.listNeuron.at(nid).radius,0); x<=MIN(nx+nt.listNeuron.at(nid).radius,sz_img[0]-1); x++){
            for(VISIOCYTELONG y=MAX(ny-nt.listNeuron.at(nid).radius,0); y<=MIN(ny+nt.listNeuron.at(nid).radius,sz_img[1]-1); y++){
                for(VISIOCYTELONG z=MAX(nz-nt.listNeuron.at(nid).radius,0); z<=MIN(nz+nt.listNeuron.at(nid).radius,sz_img[2]-1); z++){
...
//                             sz_img,VISIOCYTE_FLOAT32);
    ...
    {
        visiocyte_msg("no voxels were found. Error!");
        return false;
    }
    qDebug()<<"voxels map size:"<<voxels_map.size();
    qDebug()<<"~~~~~Spine Dectector: init neighbors";
    for(map<VISIOCYTELONG, VOI *>::iterator iter_map=voxels_map.begin(); iter_map!=voxels_map.end(); iter_map++){
        if(iter_map->second->dst>=0) //only consider selected skel points
        {
            VOI * tmp_voxel =iter_map->second;
            voxels.push_back(tmp_voxel);
            //construct neighbors
            for(VISIOCYTELONG x=MAX(tmp_voxel->x-1,0); x<=MIN(tmp_voxel->x+1,sz_img[0]-1); x++)
                for(VISIOCYTELONG y=MAX(tmp_voxel->y-1,0); y<=MIN(tmp_voxel->y+1,sz_img[1]-1); y++)
                    for(VISIOCYTELONG z=MAX(tmp_voxel->z-1,0); z<=MIN(tmp_voxel->z+1,sz_img[2]-1); z++){
                        if(x==tmp_voxel->x && y==tmp_voxel->y && z==tmp_voxel->z) //self
                            continue;
                        VISIOCYTELONG pos = sub2ind(x,y,z);
...
bool spine_fun::init_for_manual_proof(vector<GOV> &label_record)
{
    qDebug()<<"~~~~~Spine Detector: init voxels of interest along neuron skeleton";
...
//                             sz_img,VISIOCYTE_FLOAT32);
    ...
    qDebug()<<"~~~~~Spine Dectector: init voxels";
    //need to know the largest label,how many groups
    int max_label=0;
    for(VISIOCYTELONG i=0; i<sz_page; i++){
        if(dst[i]>param.max_dis*param.max_dis){ //too far away
            continue;
        ...
        if (known_label[i]>0)
        {
            ...
            if (known_label[i]>max_label)
                max_label=known_label[i];
        }
        else
            tmp_voxel->intensity_label=-1;
        voxels_map[i]=tmp_voxel;
    ...
    //organize them in vector<GOV>
    label_record.clear();
    label_record.resize(max_label);
    qDebug()<<"final groups size:"<<max_label;
    for (int k=0;k<voxels.size();k++)
    {
        //qDebug()<<"k:"<<k;
        if (voxels[k]->intensity_label>0)
            label_record[voxels[k]->intensity_label-1].push_back(voxels[k]);
    }
    //testing
//    for (int k=0;k<final_groups.size();k++)
//        qDebug()<<"k:"<<k<<"size:"<<final_groups[k].size();
    return true;
}
...
bool spine_fun::init_old()
{
    ...
    voxels.clear();
    map<VISIOCYTELONG, VOI *> voxels_map;
    vector<VISIOCYTELONG> cur_layer, pre_layer;
    VISIOCYTELONG sz_page=sz_img[0]*sz_img[1]*sz_img[2];
...
//            cout<<"\r"<<nid<<":"<<voxels_map.size();
    ...
    for(int idst=0; idst<param.max_dis; idst++){
        pre_layer=cur_layer;
        cur_layer.clear();
        for(VISIOCYTELONG sid=0; sid<pre_layer.size(); sid++){
            VISIOCYTELONG nx, ny, nz;
            ind2sub(nx,ny,nz,pre_layer[sid]);
            //search its neighbor to find not visited node
            for(VISIOCYTELONG x=MAX(nx-1,0); x<=MIN(nx+1,sz_img[0]-1); x++){
                for(VISIOCYTELONG y=MAX(ny-1,0); y<=MIN(ny+1,sz_img[1]-1); y++){
                    for(VISIOCYTELONG z=MAX(nz-1,0); z<=MIN(nz+1,sz_img[2]-1); z++){
                        VISIOCYTELONG pos=sub2ind(x,y,z);
                        if(mask[pos]==0){//not visited yet
                            set<VISIOCYTELONG> nids;
                            //search its neighbors to find the candidate neuron node
                            for(VISIOCYTELONG cx=MAX(x-1,0); cx<=MIN(x+1,sz_img[0]-1); cx++){
                                for(VISIOCYTELONG cy=MAX(y-1,0); cy<=MIN(y+1,sz_img[1]-1); cy++){
                                    for(VISIOCYTELONG cz=MAX(z-1,0); cz<=MIN(z+1,sz_img[2]-1); cz++){
                                        VISIOCYTELONG cpos=sub2ind(cx,cy,cz);
                                        if(mask[cpos]!=0){
                            ...
                            VISIOCYTELONG nid=*(nids.begin());
                            for(set<VISIOCYTELONG>::iterator iter=nids.begin(); iter!=nids.end(); iter++){
...
bool spine_fun::run_dstGroup()
{
    ...
    while(dst_floor>0){
        ...
        for(VISIOCYTELONG vid=vid_begin; vid<vid_end; vid++){ //check the rest nodes in this layer, grow the rest if there is any left
...
//        for(VISIOCYTELONG vid=0; vid<cur_layer.size(); vid++){
...
bool spine_fun::reverse_dst_grow()
{
    ...
    seeds.clear();
    for(VISIOCYTELONG vid=vid_begin; vid<vid_end; vid++)
    {
        if (voxels[vid]->dst_label<0)
        {
            seeds.push_back(voxels[vid]);
...
//            for (VISIOCYTELONG i=0;i<voxels.size();i++)
//            {
...
//                }
//            }
//            QString local_fn=QString::number(dst_floor)+".visiocyteraw";
...
//        }
    ...
    for (VISIOCYTELONG i=vid_end;i<voxels.size();i++)
    {
    ...
    QString local_fn ="final.visiocyteraw";
    simple_saveimage_wrapper(*callback, local_fn.toStdString().c_str(), (unsigned char *)tmp_label, label_sz, 1);
    return true;
...
//        VISIOCYTELONG sz_tmp[4]; sz_tmp[0]=sz_img[0]; sz_tmp[1]=sz_img[1]; sz_tmp[2]=sz_img[2]; sz_tmp[3]=3;
...
//            tmp_out[voxels.at(i)->pos+sz_page]=255;
//        }
        QString fname_output = fname_out + "_tmp.visiocyteraw";
...
void spine_fun::ind2sub(VISIOCYTELONG &x, VISIOCYTELONG &y, VISIOCYTELONG &z, VISIOCYTELONG ind)
{
...
long spine_fun::sub2ind(VISIOCYTELONG x, VISIOCYTELONG y, VISIOCYTELONG z)
{
...
LandmarkList spine_fun::get_center_landmarks()
{
    ...
    {
        GOV tmp=final_groups[i];
        VISIOCYTELONG sum_x,sum_y,sum_z;
        sum_x=sum_y=sum_z=0;
