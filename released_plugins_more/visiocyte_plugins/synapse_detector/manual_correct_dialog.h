...
class manual_correct_dialog:public QDialog
{
    Q_OBJECT
public:
    explicit manual_correct_dialog(VISIOCYTEPluginCallback2 *cb,int proofread_code);
    //code=0 view by spine //code=1 view by segment
    manual_correct_dialog(VISIOCYTEPluginCallback2 *cb);
...
private:
    ...
    vector<VISIOCYTELONG> image_seg_plan(vector<int> seg,NeuronTree neuron_tmp);
    void create_big_image();
    NeuronTree prep_seg_neurontree(vector<VISIOCYTELONG> coord);
    bool auto_spine_detect_seg_image(unsigned char *data1d, VISIOCYTELONG *sz, NeuronTree nt_seg,
                                     int image_id, VISIOCYTELONG x_start, VISIOCYTELONG y_start, VISIOCYTELONG z_start, QSet<VISIOCYTELONG> &spine_map);
...
private:
    ...
    int view_code;
    visiocytehandle curwin,main_win;
    ...
    VISIOCYTELONG sz_img[4],sz[4],sz_seg[4];
