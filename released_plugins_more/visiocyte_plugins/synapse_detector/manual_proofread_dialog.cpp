...
manual_proofread_dialog::manual_proofread_dialog(VISIOCYTEPluginCallback2 *cb, bool menu_flag)
{
    callback=cb;
    edit_flag=false;
    seg_edit_flag=false;
...
void manual_proofread_dialog::run_saved_proj()
{
    ...
    if (!loadImage())
    {
        visiocyte_msg("Invalid image.Error");
        return;
    }
    if (!loadSWC())
    {
        visiocyte_msg("Invalid swc. Error");
        return;
    }
    if (!loadLabel())
    {
        visiocyte_msg("Invalid label.Error");
        return;
    }
    if (!loadMarker(LList_in))
    {
        visiocyte_msg("Invalid marker.Error");
        return;
    }
    if (!prep_spine_voi())//get ready label_group and image1Dc_spine
    {
        visiocyte_msg("Initiation error");
        return;
    }
    qDebug()<<"LList set";
...
void manual_proofread_dialog::run_interface_with_auto(int ret,NeuronTree auto_neuron,bool eswc_tmp_flag,
    LandmarkList LList,vector<GOV> label_group_in,unsigned char *image1Dc,VISIOCYTELONG sz_img_in[4],
...
bool manual_proofread_dialog::loadLabel()
{
    ...
    {
        label_sz_img[3]=1;
        visiocyte_msg("More than 1 channel loaded."
...
//    //write_svm_file("training.txt",keep_spine);
    ...
    image_label.setData(label_color,sz_img[0],sz_img[1],sz_img[2],3,VISIOCYTE_UINT8);
...
bool manual_proofread_dialog::save_current_project()
{
    ...
    QString output_label_name="auto_manual_label.visiocyteraw";
...
void manual_proofread_dialog::set_trunc_image_marker(vector<int> one_seg,int seg_id)
{
    ...
    for (int j=0;j<LList_in.size();j++)
    {
        ...
        for (int k=0;k<tmp_spine.size();k++)
        {
            VISIOCYTELONG x=tmp_spine[k]->x-x_start;
            VISIOCYTELONG y=tmp_spine[k]->y-y_start;
            VISIOCYTELONG z=tmp_spine[k]->z-z_start;
...
vector<VISIOCYTELONG> manual_proofread_dialog::image_seg_plan(vector<int> seg, NeuronTree *neuron_tmp)
{
...
void manual_proofread_dialog::dilate_for_seg_view()
{
    ...
    for (int i=0;i<tmp_group.size();i++)
    {
        VISIOCYTELONG pos_trun=xyz2pos(tmp_group[i]->x-x_start,tmp_group[i]->y-y_start,tmp_group[i]->z-z_start
                                 ,sz[0],sz[0]*sz[1]);
...
void manual_proofread_dialog::erode_for_seg_view()
{
    ...
    for (int i=vid_begin;i<vid_end;i++)
    {
        VISIOCYTELONG trun_pos= xyz2pos(tmp_group[i]->x-x_start ,tmp_group[i]->y-y_start,
    ...
    {
        VISIOCYTELONG trun_pos= xyz2pos(update_group[i]->x-x_start ,update_group[i]->y-y_start,
