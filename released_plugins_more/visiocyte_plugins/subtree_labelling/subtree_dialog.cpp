...
subtree_dialog::subtree_dialog(VISIOCYTEPluginCallback2 *cb)
{
    callback=cb;
    qDebug()<<"in subtree labelling";
}
...
bool subtree_dialog::get_swc_marker()
{
    ...
    QList <VisiocyteR_MainWindow *> used_list_3dviwer;
    QStringList items;
    ...
    if (items.size()==0){
        visiocyte_msg("No window with single swc is found");
        return false;
    }
    else if (items.size()==1)
    {
        ...
        {
            visiocyte_msg("Error loading swc file, please check swc file");
            return false;
        }
        for (VISIOCYTELONG ii=0; ii<nt->listNeuron.size(); ii++)
        {
            ...
            else
            {
                ...
                QList<float> tmp_float;
                tmp_float=nt->listNeuron[ii].fea_val;
                fea_floats.append(tmp_float);
...
//        if (LList_in.size()==0)
//        {
//            visiocyte_msg("Please load markers");
//            return false;
//        }
...
//    for (VISIOCYTELONG i=0;i<fea_floats.size();i++)
//    {
//        qDebug()<<"fea_floats: first:"<<fea_floats[i].at(0)<<" last:"<<fea_floats[i].back();
//    }
    return true;
}
...
int subtree_dialog::calc_nearest_node_around_marker()
{
    ...
    qDebug()<<"nearest node is: "<<close_node;
    return close_node;
}
...
void subtree_dialog::sort_type_def(int type, float dendrite_id, int marker_id)
{
    ...
    VISIOCYTELONG seg_id=1;
    VISIOCYTELONG level=1;
...
void subtree_dialog::refresh_marker()
{
    ...
    if (swc_win!=0)
    {
        ...
        if (LList_new.size()==0)
        {
            visiocyte_msg("No markers were found");
            return;
        }
    }
    else
    {
        visiocyte_msg("swc window is closed.Please relaunch the plug-in");
        return;
    }
...
void subtree_dialog::define_sort_id_for_trees()
{
    ...
    for (int i=0;i<all_trees.size();i++)
    {
        ...
        {
            QString tmp;
            tmp=QString::fromStdString(LList_in[j].comments);
            int tmp_type=tmp.toInt();
            if(tmp_type<=0) //if the type is not set, not consider this node
                continue;
...
//    connected_components(); //build all_trees nodes/tree_id
    ...
    for (VISIOCYTELONG ii=0; ii<nt->listNeuron.size(); ii++)
    {
        nt->listNeuron[ii].fea_val.append(fea_floats.at(ii));
        float new_seg=nt->listNeuron[ii].fea_val.at(0)*10000+nt->listNeuron[ii].level*100+
                nt->listNeuron[ii].seg_id;
        nt->listNeuron[ii].seg_id=new_seg;
    }
}
    ...
    for (int i=0;i<all_trees.size();i++)
    {
        ...
        for (int j=0;j<this_tree.belong_markers.size();j++)
        {
            ...
            else
            {
                ...
                label_single_tree(seg_type,j+1,node_id);
            }
        }
    }
...
void subtree_dialog::label_single_tree(int type, int tree_id, int marker_id)
{
    ...
    vector<int> seeds;
    VISIOCYTELONG seg_id=1;
    VISIOCYTELONG parent_tmp=nt->listNeuron[marker_id].parent;
    VISIOCYTELONG level=nt->listNeuron[parent_tmp].level;
    if (level<1)
        level=1;
    ...
    seeds.push_back(marker_id);
    //qDebug()<<"first seeds set:"<<seeds.size();
    //we also need to maintain a hashtable for how many segments have appeared for each level.
    //Otherwise, 20201 will be assigned to half of the branches of the level 2.
    map<VISIOCYTELONG,int> segmentMap; //maintains the seg id for each level
    segmentMap[level]=seg_id;
    ...
    while (sid<seeds.size())
    {
        ...
        //qDebug()<<"sid:"<<sid;
        parent=seeds[sid];
        VISIOCYTELONG p_x=nt->listNeuron[parent].x;
        VISIOCYTELONG p_y=nt->listNeuron[parent].y;
        VISIOCYTELONG p_z=nt->listNeuron[parent].z;
        ...
        else if(parent_LUT[parent].size()==1)
        {
            ...
            nt->listNeuron[child_node].seg_id=nt->listNeuron.at(parent).seg_id;
            nt->listNeuron[child_node].level=nt->listNeuron.at(parent).level;
            VISIOCYTELONG c_x=nt->listNeuron[child_node].x;
            VISIOCYTELONG c_y=nt->listNeuron[child_node].y;
            VISIOCYTELONG c_z=nt->listNeuron[child_node].z;
            float distance=(c_x-p_x)*(c_x-p_x)+(c_y-p_y)*(c_y-p_y)+(c_z-p_z)*(c_z-p_z);
            nt->listNeuron[child_node].fea_val[1]=(sqrt(distance)+p_dis);
            seeds.push_back(child_node);
                //qDebug()<<"one child__parent:"<<parent<<" type:"<<nt->listNeuron[child_node].type<<endl;
...
//                       <<nt->listNeuron[child_node].seg_id<<" level:"<<nt->listNeuron[child_node].level;
        ...
        else if (parent_LUT[parent].size()>1 && nt->listNeuron[parent].parent!=-1)
        {
            for (int i=0;i<parent_LUT[parent].size();i++)
            {
                ...
                if(segmentMap.find(nt->listNeuron[child_node].level) == segmentMap.end()) //if level info is not in
                {
                    segmentMap[nt->listNeuron[child_node].level]=i+1;
                    nt->listNeuron[child_node].seg_id=i+1;
                }
                else   //we already used some seg_id in this level
                {
                    int used_id=segmentMap[nt->listNeuron[child_node].level];
                    segmentMap[nt->listNeuron[child_node].level]++;
                    nt->listNeuron[child_node].seg_id = used_id+1;
                }
                VISIOCYTELONG c_x=nt->listNeuron[child_node].x;
                VISIOCYTELONG c_y=nt->listNeuron[child_node].y;
                VISIOCYTELONG c_z=nt->listNeuron[child_node].z;
                float distance=(c_x-p_x)*(c_x-p_x)+(c_y-p_y)*(c_y-p_y)+(c_z-p_z)*(c_z-p_z);
                nt->listNeuron[child_node].fea_val[1]=(sqrt(distance)+p_dis);
                    //qDebug()<<"2+child__parent:"<<parent<<"  type:"<<nt->listNeuron[parent_LUT[parent][i]].type<<endl;
...
//                           <<nt->listNeuron[parent_LUT[parent][i]].seg_id<<" level:"<<nt->listNeuron[parent_LUT[parent][i]].level;
        ...
        else if (parent_LUT[parent].size()>1 && nt->listNeuron[parent].parent==-1)
        {
            for (int i=0;i<parent_LUT[parent].size();i++)
            {
                ...
                float distance=(c_x-p_x)*(c_x-p_x)+(c_y-p_y)*(c_y-p_y)+(c_z-p_z)*(c_z-p_z);
                nt->listNeuron[child_node].fea_val[1]=(sqrt(distance)+p_dis);
                //qDebug()<<"2+child__parent:"<<parent<<"fea value:"<< nt->listNeuron[parent_LUT[parent][i]].type;
...
void subtree_dialog::calc_distance_to_subtree_root()
{
    ...
    for (int i=0;i<LList_in.size();i++)
    {
        ...
        VISIOCYTELONG root_id=tmp.toInt();
        map<int,bool> mark_used;
        ...
        while (sid<seeds.size())
        {
            ...
            else if (seeds_next.size()>=1)
            {
                for (int i=0;i<seeds_next.size();i++)
                {
                    VISIOCYTELONG child_node=seeds_next[i];
                    VISIOCYTELONG c_x=nt->listNeuron[child_node].x;
                    VISIOCYTELONG c_y=nt->listNeuron[child_node].y;
                    VISIOCYTELONG c_z=nt->listNeuron[child_node].z;
                    float distance=(c_x-p_x)*(c_x-p_x)+(c_y-p_y)*(c_y-p_y)+(c_z-p_z)*(c_z-p_z);
