...
unsigned char * downsample(VISIOCYTELONG* in_sz, VISIOCYTELONG c, unsigned char* data1d, VISIOCYTELONG * downsz);
unsigned char * crop(const VISIOCYTELONG in_sz[4], unsigned char *data1d, VISIOCYTELONG sz_img_crop[4], vectype * boxlowsize, double threshold);
...
void NeuroStalker::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    {
        visiocyte_msg(tr("A tracing algorithm. "
                          ...
                          QWidget * parent)
{
    ...
    else if (func_name == tr("help"))
    {
        ...
        printf("visiocyte -x NeuroStalker -f tracing_func -i <inimg_file> -p <channel> <preprocessing> <run unit-tests>\n");
...
void reconstruction_func(VISIOCYTEPluginCallback2 &callback, 
                         QWidget *parent, 
                         input_PARA &PARA, 
                         bool bmenu)
{
    ...
    if (PARA.preprocessing & 1)
    {
        ...
        //cout<<"Saving cropped image to downsample.visiocyteraw"<<endl;
        //saveImage("test/cropoutside.visiocyteraw", p_img8u_crop, sz_img_crop, VISIOCYTE_UINT8);
        if(!bmenu)
        {
    ...
    if (PARA.preprocessing & 2)
    {
        ...
        //cout<<"Saving downsampled image to test/downsample.visiocyteraw"<<endl;
        //saveImage("test/downsample.visiocyteraw", downdata1d, downsz, VISIOCYTE_UINT8);
        cout<<"=============== Image Downsampled..."<<endl;
    }



    // Using the Image Operation found in visiocyte_tools/hackathon/zhi/snake_tracing/TracingCore/ in for some simple Image Processing
    ...
    //----------------------------vesselness begin
/*    VISIOCYTELONG l_npixels_vessel;
...
/*    unsigned char *p_vessel = new(std::nothrow) unsigned char[l_npixels_vessel]();
    ...
    saveImage("test/Vesselness.visiocyteraw", p_vessel, in_sz, VISIOCYTE_UINT8);*/
    //---------------------------vesselness end
    ...
    //saveImage("test/binaryimage.visiocyteraw", binaryimg2uchar, in_sz, VISIOCYTE_UINT8);
...
unsigned char * downsample(VISIOCYTELONG *in_sz,
                           VISIOCYTELONG c, 
                           unsigned char* data1d, 
                           VISIOCYTELONG * downsz)
{
    VISIOCYTELONG N, M, P;
    N = in_sz[0];
    M = in_sz[1];
    ...
    catch(...)  {visiocyte_msg("cannot allocate memory for data1d_1ch."); return NULL;}
...
{    
    ...
    VISIOCYTELONG VISIOCYTELONG l_boundbox_min[3], l_boundbox_max[3];//xyz
    VISIOCYTELONG l_npixels_crop;
    ...
    for(VISIOCYTELONG Z = 0;Z < sz_img_crop[2];Z++)
        for(VISIOCYTELONG Y = 0;Y < sz_img_crop[1];Y++)
            for(VISIOCYTELONG X = 0;X < sz_img_crop[0];X++)
            {
    ...
    //saveImage("test/cropinside.visiocyteraw", p_img8u_crop, sz_img_crop, VISIOCYTE_UINT8);
