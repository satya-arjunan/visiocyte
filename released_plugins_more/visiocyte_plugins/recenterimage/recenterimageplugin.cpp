...
bool ReCenterImagePlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & visiocyte, QWidget * parent)
{
     if(func_name == tr("help"))
	{
		cout<<"Usage: visiocyte -x recenterimage -f iRecenter -i <input_image> -o <output_image> -p \"#s <save_blending_result zero(false)/nonzero(true)> #x <dimx> #y <dimy> #z <dimz>\" "<<endl;
		cout<<endl;
          cout<<"e.g. visiocyte -x recenterimage -f iRecenter -i input.raw -o output.raw -p 0 64 64 64"<<endl;
		cout<<endl;
		return true;
	}
    ...
    {
        //print Help info
        printf("\nUsage: visiocyte -x recenterimage -f iRecenter -i <input_image> -o <output_image> -p \"#s <save_blending_result zero(false)/nonzero(true)> #x <dimx> #y <dimy> #z <dimz> \"\n");
    ...
    VISIOCYTELONG int inx=1, iny=1, inz=1;   // user input new dimensions
    if(output.size()>0)
    {  //output
    ...
    VISIOCYTELONG ndimx=1, ndimy=1, ndimz=1;
 //   printf("\n ndimx %d %d %d \n", ndimx, ndimy, ndimz);
 //   printf("\n input stuff %d", input.size());

    VISIOCYTELONG paras[4];
...
//-------BRL cut a bunch of stuff here and inserted simpler parsing of -p inputs following ZZ's code
    ...
    if(!outfile)
        outputImageName = QString(infile).append("_recentered.visiocyteraw");
    else
        outputImageName = QString(outfile);

    if(QFileInfo(outputImageName).suffix().toUpper() != "VISIOCYTERAW")
    {
        outputImageName.append(".visiocyteraw"); // force to save as .visiocyteraw file
    }

    //
    int datatype=0;
    VISIOCYTELONG sz_input[4];
    unsigned char* input1d = 0;

    if (simple_loadimage_wrapper(visiocyte, const_cast<char *>(infile), input1d, sz_input, datatype)!=true)
    {
        printf("Error happens in reading the subject file [%s]. Exit. \n", infile);
        return false;
    }
    VISIOCYTELONG sx = sz_input[0], sy = sz_input[1], sz = sz_input[2], sc = sz_input[3];
...
//    printf("double check datatype %d \n", datatype);
    ...
    if(datatype == 1)
    {
        ...
        recentering<VISIOCYTELONG, unsigned char>( pRecenteredImage, (unsigned char*)input1d, ndimx, ndimy, ndimz, sx, sy, sz, sc);
        //output
        if(b_saveimage)
        {
//            printf("brl test %s \n",outputImageName.toStdString().c_str());
//            printf("size output  %d %d %d %d \n", sz_output[0], sz_output[1], sz_output[2], sz_output[3]);
           if (simple_saveimage_wrapper(visiocyte, outputImageName.toStdString().c_str(), ( unsigned char *)pRecenteredImage, sz_output, 1)!=true)
            {
    ...
    else if(datatype == 2)
    {
        ...
        recentering<VISIOCYTELONG, unsigned short>( pRecenteredImage, (unsigned short*)input1d, ndimx, ndimy, ndimz, sx, sy, sz, sc);

        // output
        if(b_saveimage)
        {
            //save
            if (simple_saveimage_wrapper(visiocyte, outputImageName.toStdString().c_str(), ( unsigned char *)pRecenteredImage, sz_output, 2)!=true)
            {
                printf("Error happens in file writing. Exit. \n");
                return false;
            }
    ...
    else if(datatype == 4)
    {
        ...
        recentering<VISIOCYTELONG, float>( pRecenteredImage, (float*)input1d, ndimx, ndimy, ndimz, sx, sy, sz, sc);

        // output
        if(b_saveimage)
        {
            //save
            if (simple_saveimage_wrapper(visiocyte, outputImageName.toStdString().c_str(), ( unsigned char *)pRecenteredImage, sz_output, 4)!=true)
            {
                printf("Error happens in file writing. Exit. \n");
                return false;
            }
...
void ReCenterImagePlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    // interactively call ReCenterImage plugin for current focused image opened by VISIOCYTE
    if (menu_name == tr("ReCenterImage"))
    {
        ...
        bool ok;
        VISIOCYTELONG ndimx = QInputDialog::getInteger(parent, tr("Dimension x"),
             tr("Enter dimx:"),
             N, 0, 10000, 1, &ok);

        VISIOCYTELONG ndimy = QInputDialog::getInteger(parent, tr("Dimension y"),
             tr("Enter dimy:"),
             M, 0, 10000, 1, &ok);

        VISIOCYTELONG ndimz = QInputDialog::getInteger(parent, tr("Dimension z"),
             tr("Enter dimz:"),
             P, 0, 10000, 1, &ok);

        qDebug("dimx %ld dimy %ld dimz %ld \n", ndimx, ndimy, ndimz);

        VISIOCYTELONG ntotalpxls = sc*ndimx*ndimy*ndimz;
        ...
        if (ok)
        {
			...
			if(datatype == 1)
			{
				...
				recentering<VISIOCYTELONG, unsigned char>( pImage, (unsigned char*)p4DProxy.begin(), ndimx, ndimy, ndimz, N, M, P, sc);
                pRecenteredImage=(void *)pImage;
			}
			else if(datatype == 2)
			{
				unsigned short *pImage = NULL;
				recentering<VISIOCYTELONG, unsigned short>( pImage, (unsigned short*)p4DProxy.begin(), ndimx, ndimy, ndimz, N, M, P, sc);
                pRecenteredImage=(void *)pImage;
			}
			else if(datatype == 3)
			{
				float *pImage = NULL;
				recentering<VISIOCYTELONG, float>( pImage, (float*)p4DProxy.begin(), ndimx, ndimy, ndimz, N, M, P, sc);
                pRecenteredImage=(void *)pImage;
			}
			else
			{
