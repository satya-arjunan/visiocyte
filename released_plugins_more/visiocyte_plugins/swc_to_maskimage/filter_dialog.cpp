...
filter_dialog::filter_dialog(VISIOCYTEPluginCallback2 *cb): QDialog()
{
    callback=cb;
    image_data=0;
...
void filter_dialog::loadImage()
{
    ...
    fileName = QFileDialog::getOpenFileName(0, QObject::tr("Choose the input image "),
             QDir::currentPath(),QObject::tr("Images (*.raw *.tif *.lsm *.visiocytepbd *.visiocyteraw);;All(*)"));
...
void filter_dialog::swc_filter_image()
{
    ...
    VISIOCYTELONG nx,ny,nz; //the image size
    ...
    tmp.setData(image_data, nx, ny, nz, sz_img[3], VISIOCYTE_UINT8);
    tmp_out.setData(image_filter,nx,ny,nz,sz_img[3],VISIOCYTE_UINT8);
    visiocytehandle newwin = callback->newImageWindow();
    visiocytehandle newwin_out=callback->newImageWindow();
...
void ComputemaskImage(NeuronTree neurons,unsigned char* pImMask,VISIOCYTELONG sx,VISIOCYTELONG sy,VISIOCYTELONG sz, double margin)
{
    NeuronSWC *p_cur = 0;
    //create a LUT
    ...
    for (VISIOCYTELONG ii=0; ii<neurons.listNeuron.size(); ii++)
    {
        ...
        {
            visiocyte_msg(QString("Detect overlapping coordinates of node\n"), 0);
            continue;
        }
