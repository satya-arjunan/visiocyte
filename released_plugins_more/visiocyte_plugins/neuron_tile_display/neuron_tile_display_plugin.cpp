...
void MethodForBigScreenDisplay(VISIOCYTEPluginCallback2 &callback, QWidget *parent, int displayNum);
void MethodForUpdateSWCDispaly(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
...
int ZmovieMaker(VISIOCYTEPluginCallback2 &callback, QWidget *parent/*,QStringList namelist*/);
void ZmovieMaker(VISIOCYTEPluginCallback2 &callback, QWidget *parent,const VISIOCYTEPluginArgList & input);
void ZmovieMaker(VISIOCYTEPluginCallback2 &callback, QWidget *parent,int samplingRate);
void LoadAnchorFile();
//void LoadAnchorFile(QString fileOpenName);
void LoadAnchorFile(const VISIOCYTEPluginArgList & input);
...
QStringList importFileList_addnumbersort(const QString & curFilePath, int method_code)
{
    ...
    if (method_code ==1)
        ...
        imgSuffix<<"*.raw"<<"*.visiocyteraw"<<"*.visiocytepbd"<<"*.tif"<<"*.RAW"<<"*.VISIOCYTERAW"<<"*.TIF"<<"*.VISIOCYTEPBD";
...
void neuron_tile_display::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    else if (menu_name == tr("tile_multi_windows (swc and image files selected)"))
    {
        ...
        fileList = QFileDialog::getOpenFileNames(0, QObject::tr("Open File"),
                "",
                QObject::tr("Supported file (*.swc *.eswc *.raw *.visiocyteraw *.visiocytepbd *.tif)"
                    ));
        if(fileList.isEmpty())
            return;
        ...
        int offsety = -1;
        for(VISIOCYTELONG i = 0; i < fileList.size(); i++)
        {           
            ...
            for (VISIOCYTELONG i = 0; i < cur_list_3dviewer.size(); i++)
            {
                if( (i%col)*xRez ==0)
                    offsety++;
    ...
    else if (menu_name == tr("tile_multi_windows (swc and image files under one folder)"))
    {
        ...
        for(VISIOCYTELONG i=0; i < imagelist.size(); i++)
        {
            QString curPathIMAGE = imagelist.at(i);
           // visiocyte_msg(curPathIMAGE);
           // char * inimg_file  = &curPathIMAGE.toStdString()[0];
          //  visiocyte_msg(QString("%1").arg(inimg_file));
        ...
        VisiocyteR_MainWindow * new3DWindow = NULL;
        int offsety = -1;
        for(VISIOCYTELONG i = 0; i < swcList.size(); i++)
        {
    ...
    else if (menu_name == tr("set configuration (tile_multi_windows options only)"))
    {
             ...
             visiocyte_msg("Configuration Done!");
    }
...
//    }
	...
	{
		visiocyte_msg(tr("This Plugin will tile neuron to display. "
			"Developed by Hanbo Chen, 2014-10-28"));
	}
}

void MethodFunForUpdateSWCDispaly(VISIOCYTEPluginCallback2 &callback, QWidget *parent, const VISIOCYTEPluginArgList & input)
{
...
//    int displayNum = atof(paralist->at(0));
    ...
    VisiocyteR_MainWindow * surface_combine_win=cur_list_3dviewer.at(cur_list_3dviewer.size()-1);
...
void MethodForCombineSWCDisplay(VISIOCYTEPluginCallback2 &callback, QWidget *parent,QStringList nameList)
{
    VisiocyteR_MainWindow * surface_win = callback.createEmpty3DViewer();
    if (!surface_win)
    {
...
void MethodForBigScreenDisplay(VISIOCYTEPluginCallback2 &callback, QWidget *parent,int updateInterval=30)
{
...
void MethodFunForBigScreenDisplay(VISIOCYTEPluginCallback2 &callback, QWidget *parent/*,int displayNum=9*/, const VISIOCYTEPluginArgList & input)
{
...
void printHelpForBigScreenUsage()
{
    printf("\nVisiocyte plugin: BigScreenDisplay, including: \n");
    ...
    printf("Usage (linux): visiocyte -x tile_display_multiple_neurons -f BigScreenDisplay -i /home/data/SEUAllenJointDataCenter/finished_annotations/17545_finished_neurons/ /home/penglab/Data/jsd/anchorPointFile.txt -p 30 500\" \n");
    printf("Usage (windows): visiocyte_msvc.exe /x tile_display_multiple_neurons /f BigScreenDisplay /i d:/home/data/SEUAllenJointDataCenter/finished_annotations/17545_finished_neurons/ d:/home/penglab/Data/jsd/anchorPointFile.txt /p 30 500\" \n");
    printf("\n                                                           \n");
...
bool neuron_tile_display::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
void neuron_tile_display::dotile(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    if(selectWindowList.size()<1){
        visiocyte_msg("Cannot find 3D view with SWC file. Please load the SWC files you want to tile in the same 3D view");
        return;
    ...
    myDialog = new NeuronTileDisplayDialog(&callback, visiocytewin);
    myDialog->show();
}


void neuron_tile_display::dotile_groups(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
void neuron_tile_display::doxytile(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    //select the window to operate
    QList <VisiocyteR_MainWindow *> allWindowList = callback.getListAll3DViewers();
    QList <VisiocyteR_MainWindow *> selectWindowList;
    ...
    myDialog = new NeuronXYTileDialog(&callback, visiocytewin);
    myDialog->show();
}
void angles_to_quaternions(MYFLOAT q[], MYFLOAT xRot, MYFLOAT yRot,MYFLOAT zRot)
{
