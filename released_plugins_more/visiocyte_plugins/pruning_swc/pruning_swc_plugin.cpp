...
bool export_list2file(QList<NeuronSWC> & lN, QString fileSaveName, QString fileOpenName)
{
    ...
    if (! qf.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        ...
        visiocyte_msg(QString("open file [%1] failed!").arg(fileOpenName));
#endif
        return false;
    }
    QString info;
...
void subtree(int ID, QVector<QVector<VISIOCYTELONG> > childs,VISIOCYTELONG *flag);
...
void pruning_swc::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    if (menu_name == tr("pruning"))
	{
                ...
                VISIOCYTELONG *parent_id = new VISIOCYTELONG[neuronNum];
	...
	{
        visiocyte_msg(tr("This is a plugin to prune the swc file. "
...
void controlPanel::_slot_prune()
{
    ...
    QList<NeuronTree> *nt_list = m_visiocyte.getHandleNeuronTrees_Any3DViewer(surface_win);
    if(nt_list->size()>0)
    {
        NeuronTree nt = nt_list->at(0);
        LandmarkList *markers = m_visiocyte.getHandleLandmarkList_Any3DViewer(surface_win);
        if(markers->size() == 0)
        {
            visiocyte_msg("Please define markers for small branches pruning.");
            return;
        }
        int ID1 = 0;
        int ID2 = 0;
        ...
        for(VISIOCYTELONG i=0; i<nt.listNeuron.size();i++)
        {
            ...
            {
                visiocyte_msg("Please sort your annotation first!");
                return;
            }
        }
...
bool pruning_swc::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
