...
void neuronConverter::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    if (menu_name == tr("asc_to_swc"))
	{
        ...
        QSettings settings("VISIOCYTE plugin","neuronTypeConverter");
...
//            fname_swc = settings.value("fname_swc").toString();
	...
	{
		visiocyte_msg(tr("The plugin will convert neuron file to swc format. It is based on the code of L-Measure, Sridevi Polavaram. Imported by Hanbo Chen.. "
			"Developed by Hanbo Chen, 2015-4-27"));
	}
}

bool neuronConverter::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
void printHelp()
{
    qDebug()<<"visiocyte -x libname -f asc_to_swc -p input.asc -o output.swc";
}
