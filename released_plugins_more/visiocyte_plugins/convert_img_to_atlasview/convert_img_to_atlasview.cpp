...
void convertImg2Atlas(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
bool convertImg2Atlas(VISIOCYTEPluginCallback2 &callback, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output);
...
void ConvertImg2AtlasPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    {
        visiocyte_msg(QString("This plugin extracts an image's channels and save each of them as a separate image file, "
                        "and then generates a linker file (.atlas) to link all of them, so that one can use VISIOCYTE's "
...
bool ConvertImg2AtlasPlugin::dofunc(const QString &func_name, const VISIOCYTEPluginArgList &input, VISIOCYTEPluginArgList &output, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    {
        cout<<"Usage : visiocyte -x dll_name -f toatlas -i <inimg_file> -o <outimg_file_folder>"<<endl;
        cout<<endl;
        cout<<"<outimg_file_folder>    output folder name in the current input data folder"<<endl;
        cout<<"e.g. visiocyte -x dll_name -f toatlas -i input.raw -o output"<<endl;
        cout<<endl;
        return true;
    }
}
...
bool convertImg2Atlas(VISIOCYTEPluginCallback2 &callback, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output)
{
    ...
    if (!fp)
    {
        visiocyte_msg("Unable to open the output file location for writing. Check your folder path.", 0);
        return false;
    }
    ...
    {
        visiocyte_msg("This plugin fails to load input image file.", 0);
        if(fp) fclose(fp);
        return false;
    }
    ...
    for (VISIOCYTELONG i=0;i<indata->getCDim();i++)
    {
        QString tag = QString("ch") + QString("").setNum(i+1);
        QString curfile = folderName + "/" + tag + ".visiocyteraw";

        int j = i%13+1;
        fprintf(fp, "%d, %s, %d, %d, %d, %s\n", i, qPrintable(tag), simple_cmap[j*3+0], simple_cmap[j*3+1], simple_cmap[j*3+2],  qPrintable(tag+".visiocyteraw"));
    ...
    //display a message
    visiocyte_msg(QString("The data have been converted and saved to the atlas file [%1]. You can drag and drop it into VISIOCYTE to use AtlasViewer to blend different channels.").arg(fileName), 0);
...
void convertImg2Atlas(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    QString folderName = QFileDialog::getSaveFileName(0, QString("Select a folder to save the visiocyte-atlas files"),
    ...
    for (VISIOCYTELONG i=0;i<subject->getCDim();i++)
    {
        QString tag = QString("ch") + QString("").setNum(i+1);
        QString curfile = folderName + "/" + tag + ".visiocyteraw";
    ...
    //display a message
    visiocyte_msg(QString("The data have been converted and saved to the atlas file [%1]. You can drag and drop it into VISIOCYTE to use AtlasViewer to blend different channels.").arg(fileName));
}
