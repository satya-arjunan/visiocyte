...
assemble_neuron_live_dialog::assemble_neuron_live_dialog(VISIOCYTEPluginCallback2 * cb, QList<NeuronTree> &ntList, Image4DSimple * p_img4d_in,
    ...
    QDialog(parent)
{
    ...
    if(ntList.size()>0){
        ...
        visiocyte_msg("Empty neuron file. Nothing to assemble.");
        return;
    }
...
void assemble_neuron_live_dialog::initNeuron(QList<NeuronTree> &ntList)
{
    ...
    noffset=0;
    QMultiHash<NOI*,VISIOCYTELONG> parents;
    prev_root=-1;
    //init nodes
    nodes.clear();
    for(int i=0; i<ntList.size(); i++){
        for(int j=0; j<ntList.at(i).listNeuron.size(); j++){
            VISIOCYTELONG n=ntList.at(i).listNeuron.at(j).n+noffset;
    ...
    //init connections
    for(QMultiHash<NOI*,VISIOCYTELONG>::Iterator iter=parents.begin(); iter!=parents.end(); iter++){
        VISIOCYTELONG pn=iter.value();
    ...
    long dupcount=0;
    for(QHash<VISIOCYTELONG, NOI*>::Iterator iter = nodes.begin(); iter!=nodes.end(); ){
        NOI * node=iter.value();
        bool mask_rm = false;
    ...
    {
        VISIOCYTELONG cid=0;
        for(QHash<VISIOCYTELONG, NOI*>::Iterator iter = nodes.begin(); iter!=nodes.end(); iter++){
            NOI * node=iter.value();
            if(node->cid>=0)
                continue;
            QQueue<NOI*> seeds;
    ...
    nt.hashNeuron.clear();
    {
        QHashIterator<VISIOCYTELONG, NOI*> iter(nodes);
        while(iter.hasNext()){
            iter.next();
            NOI * node=iter.value();
            for(QSet<NOI *>::Iterator iter_c=node->conn.begin(); iter_c!=node->conn.end(); iter_c++){
...
void assemble_neuron_live_dialog::searchLoop()
{
    ...
    if(loop.size()<=0){
        visiocyte_msg("No circle identified. Your reconstruction is good.");
        return;
    }

    //find 3d window
    VisiocyteR_MainWindow * _3dwin = check3DWindow();
    if(_3dwin==0){
        visiocyte_msg("Error: failed to open 3D viewer.");
        return;
    }
    //update the neuron stored
    for(VISIOCYTELONG idx=0; idx<nt.listNeuron.size(); idx++){
    ...
    visiocyte_msg("A circle identified and highlighted in magenta. Please break the loop and search again.");
...
void assemble_neuron_live_dialog::searchPair()
{
    ...
    QSet<QPair<VISIOCYTELONG, VISIOCYTELONG> > linknew = searchConnection(cpDlg.spin_xscale->value(), cpDlg.spin_yscale->value(), cpDlg.spin_zscale->value(),
    ...
    if(linknew.size()==0){
        visiocyte_msg("No qualified connection found, please change parameter.");
        return;
    }

    //find exist links
    QSet<QPair<VISIOCYTELONG,VISIOCYTELONG> > links;
    QList<int> info;
    ...
    for(int mid=0; mid<mList->size(); mid++){
        ...
        while(iter!=info.end()){
            if(info.at(0)!=*iter && markerTable.contains(*iter)){
                QPair<VISIOCYTELONG,VISIOCYTELONG> tmp(MIN(info.at(0), *iter),MAX(info.at(0), *iter));
                links.insert(tmp);
                iter++;
            }else{
    ...
    for(QSet<QPair<VISIOCYTELONG,VISIOCYTELONG> >::Iterator iter = linknew.begin(); iter!=linknew.end(); iter++){
        //check marker
        int mid1, mid2;
        VISIOCYTELONG pid1 = (*iter).first;
        VISIOCYTELONG pid2 = (*iter).second;
        QList<int> info;
    ...
    multimap<int, QPair<VISIOCYTELONG, VISIOCYTELONG> > mlinks;
    for(QSet<QPair<VISIOCYTELONG, VISIOCYTELONG> >::iterator iter = links.begin(); iter!=links.end(); iter++){
        VISIOCYTELONG pid1, pid2;
        pid1=iter->first;
        pid2=iter->second;
        int mid1 = markerTable[pid1]+1;
        int mid2 = markerTable[pid2]+1;
        if(mid1<mid2){
            mlinks.insert(pair<int, QPair<VISIOCYTELONG, VISIOCYTELONG> >(mid1*markerTable.size()+mid2, QPair<VISIOCYTELONG,VISIOCYTELONG>(pid1, pid2)));
        }else{
            mlinks.insert(pair<int, QPair<VISIOCYTELONG, VISIOCYTELONG> >(mid2*markerTable.size()+mid1, QPair<VISIOCYTELONG,VISIOCYTELONG>(pid2, pid1)));
        }
    }
    //update list
    for(multimap<int, QPair<VISIOCYTELONG, VISIOCYTELONG> >::iterator iter = mlinks.begin(); iter!=mlinks.end(); iter++){
        VISIOCYTELONG pid1, pid2;
...
QSet<QPair<VISIOCYTELONG, VISIOCYTELONG> > assemble_neuron_live_dialog::searchConnection
(double xscale, double yscale, double zscale, double angThr, double disThr, int matchType, bool b_minusradius)
{
    ...
    for(VISIOCYTELONG tid=0; tid<cand.size(); tid++){
        ...
        for(VISIOCYTELONG cid=0; cid<coffset; cid++){
    ...
    for(multimap<double, QPair<NOI *, NOI * > >::iterator iter=connMap.begin(); iter!=connMap.end(); iter++){
        ...
        links.insert(QPair<VISIOCYTELONG, VISIOCYTELONG>(MIN(node1->n, node2->n),MAX(node1->n, node2->n)));
...
bool assemble_neuron_live_dialog::breakNode(VISIOCYTELONG pid1, VISIOCYTELONG pid2)
{
    bool modified = false;
    //find the node
    NOI * node1, * node2;
...
bool assemble_neuron_live_dialog::connectNode(VISIOCYTELONG pid1, VISIOCYTELONG pid2)
{
    bool modified = false;
    //find the node
    NOI * node1, * node2;
...
void assemble_neuron_live_dialog::breakEdge()
{
    ...
    if(idx<0){
        visiocyte_msg("Please select the edge to break. You can manually define markers on both side of the edge to break and then click load_edge button.");
        return;
    }
    ...
    if(!check || !nodes.contains(pid1)){
        visiocyte_msg("Encounter unexpected error 1. Plase contact developer.");
        return;
    }
    ...
    if(!nodes.value(pid1)->conn.contains(nodes.value(pid2))){
        visiocyte_msg("Edge does not exist in record. This usually should not happen. Please contact developer.");
        return;
    }
    //connect
...
void assemble_neuron_live_dialog::connectPair()
{
    ...
    if(idx<0){
        visiocyte_msg("Please select the node to connect. You can: a) search connection; or b) manually define markers on reconstruction and link them.");
        return;
    }
...
void assemble_neuron_live_dialog::connectAll()
{
    ...
    for(int idx=0; idx<list_tolink.size(); idx++){
        ...
        pid1=items.at(5).toInt(&check);
        if(!check || !nodes.contains(pid1)){
            visiocyte_msg("Encounter unexpected error 2. Plase contact developer.");
            return;
        }
        pid2=items.at(7).toInt(&check);
        if(!check || !nodes.contains(pid2)){
    ...
    //message
    visiocyte_msg(QString::number(list_tolink.size()-list_conflict.size())+" pairs of markers were connected successfully;\n"+
...
void assemble_neuron_live_dialog::sortsaveSWC()
{
    ...
    if(!nodes.contains(n_root)){
        visiocyte_msg("Invalide root node selected.");
        return;
    }
    ...
    NeuronTree nt_sort = SortSWC_pipeline(NeuronList,VOID, 0);     //added one more sorting step by Zhi Zhou
    ...
    export_list2file(nt_sort.listNeuron, fname_output);
}
...
void assemble_neuron_live_dialog::syncMarker()
{
    ...
    list_edge->clear();
    for(VISIOCYTELONG i=0; i<mList->size(); i++){
        QList<int> info_i;
        get_marker_info(mList->at(i), info_i);
        for(VISIOCYTELONG j=i+1; j<mList->size(); j++){
            QList<int> info_j;
    ...
    if(list_edge->count()==0){
        visiocyte_msg("Not marked edge identified. Please define the edge to break in the 3D viewer by defining markers on both side of the edge that you want to break.");
    }else{
...
void assemble_neuron_live_dialog::syncMarkerOnly()
{
    ...
    if(!dataTerafly)
        return;
    LandmarkList * mList = getMarkerList();
    VisiocyteR_MainWindow * mainwin_roi = 0;
    QList <VisiocyteR_MainWindow *> tmpwinlist = callback->getListAll3DViewers();
    ...
    if(list_marker->count()==0){
        visiocyte_msg("Not marker in the 3D viewer.");
    }else{
    ...
    if(mainwin_roi)
    {
        ...
        if(items.size()>8){
            ...
            pid1=items.at(5).toInt(&check);
            if(!check || !nodes.contains(pid1)){
                visiocyte_msg("Encounter unexpected error 4. Plase contact developer.");
                return;
            }
            pid2=items.at(7).toInt(&check);
            if(!check || !nodes.contains(pid2)){
        ...
        if(pid1<0 || pid2<0){
            visiocyte_msg("All markers are assigned radius information.");
            return;
        }
...
void assemble_neuron_live_dialog::syncTips()
{
    ...
    if(localwin != 0)
    {
        ...
        for(VISIOCYTELONG i = 0; i < local_landmark.size(); i++)
        {
...
void assemble_neuron_live_dialog::pairMarker()
{
    ...
    if(mList->size()<=0){
        visiocyte_msg("Not marker identified. Please define some in the 3D viewer.");
    ...
    if(pairDlg.exec()){ //accepted
        ...
        if(info1.at(0)!=info2.at(0)){
            QPair<VISIOCYTELONG,VISIOCYTELONG> tmp(MIN(info1.at(0), info2.at(0)),MAX(info1.at(0), info2.at(0)));
...
LandmarkList * assemble_neuron_live_dialog::getMarkerList()
{
    ...
    double dis;
    QVector<VISIOCYTELONG> pidList, tidList, midList;
    ...
    for(int i=0; i<mList->size(); i++){ //find new markers
        ...
        if(nodes.contains(info.at(0)))
            continue;
        VISIOCYTELONG pid=(*nodes.begin())->n;
...
visiocytehandle assemble_neuron_live_dialog::updateImageWindow()
{
    if(!p_img4d)
        return 0;
    visiocytehandle winhandle = callback->currentImageWindow();
    if(!winhandle)
    {
...
visiocytehandle assemble_neuron_live_dialog::checkImageWindow()
{
    if(!p_img4d)
        return 0;
    visiocytehandle winhandle = callback->currentImageWindow();
    if(!winhandle)
    {
        visiocytehandle winhandle = getImageWindow();
        if(winhandle == 0){
            winhandle = callback->newImageWindow(WINNAME_ASSEM);
            callback->setImage(winhandle, p_img4d);
            callback->updateImageWindow(winhandle);
...
visiocytehandle assemble_neuron_live_dialog::getImageWindow()
{
    if(!p_img4d)
        return 0;
    visiocytehandleList allWindowList = callback->getImageWindowList();
    visiocytehandle winhandle = 0;
    for (VISIOCYTELONG i=0;i<allWindowList.size();i++)
    {
        if(callback->getImageName(allWindowList.at(i))==winname_main){
...
VisiocyteR_MainWindow * assemble_neuron_live_dialog::update3DWindow()
{
    VisiocyteR_MainWindow * _3dwin = get3DWindow();
    if(_3dwin==0){
...
VisiocyteR_MainWindow * assemble_neuron_live_dialog::check3DWindow()
{
    VisiocyteR_MainWindow * _3dwin = get3DWindow();
    if(_3dwin==0){
...
VisiocyteR_MainWindow * assemble_neuron_live_dialog::get3DWindow()
{
    //search 3d window
    VisiocyteR_MainWindow * _3dwin = 0;
    QList <VisiocyteR_MainWindow *> list_3dwin = callback->getListAll3DViewers();
...
void assemble_neuron_live_dialog::zoomin()
{
    ...
    if(tab->currentIndex()==0){
        int idx=list_link->currentRow();
        if(idx<0){
            visiocyte_msg("Please select the pair of node to zoomin. You can: a) search connection; or b) manually define markers on reconstruction and link them.");
            return;
        }
        ...
        int idx=list_edge->currentRow();
        if(idx<0){
            visiocyte_msg("Please select the edge to zoomin. You can manually define markers on both side of the edge and then click load_edge button.");
            return;
        }
        ...
        if(idx<0){
            visiocyte_msg("Please select the marker to zoomin.");
            return;
        }
        tmp=list_marker->currentItem()->text();
    ...
    if(pid1<0 || pid2<0){
        visiocyte_msg("Please select the pair of node to zoomin.");
        return;
    }
...
void assemble_neuron_live_dialog::updateROIWindow(const QList<VISIOCYTELONG>& pids)
{
    if(pids.size()<=0)
        return;
    //find the ROI window
    ...
    if(p_img4d!=0 && tab->currentIndex()<=1){
        ...
        if(!mainwin_roi){
            visiocyte_msg("Error: failed open ROI window");
            return;
        }

        //qDebug()<<"===========ROI============= update ROI window with triview";
        View3DControl * visiocytelocalcontrol = callback->getLocalView3DControl(winhandle);
        ...
        if(localwin == 0){
            ...
            VisiocyteR_MainWindow * local3dwin = callback->find3DViewerByName(winname_roi);
            if(local3dwin)
...
//~~~~~~~~~~sort param dialog~~~~~~~~~~~
sort_neuron_dialog::sort_neuron_dialog(LandmarkList * mList, VISIOCYTELONG prev_root, VISIOCYTELONG type1_root, QWidget *parent)
{
    QGridLayout * layout = new QGridLayout(this);
...
connect_param_dialog::~connect_param_dialog()
{
    QSettings settings("VISIOCYTE plugin","neuronLiveAssemble_ConnParam");
                ...
                for(QSet<NOI*>::Iterator niter=cur_node->conn.begin(); niter!=cur_node->conn.end(); niter++){
                    ...
                    if(hash_child_parent.contains(*niter)){ //found the loop!!
                        ...
                        if(p_node==0){//should not happen
                            visiocyte_msg("Encounter unexpected error 3. Plase contact developer.");
                            return loop;
                        }else{
                            loop.push_back(p_node);
...
QList<NeuronSWC> generate_swc_typesort(QHash<VISIOCYTELONG, NOI*>& nodes, VISIOCYTELONG n_root)
{
    ...
    QHash<VISIOCYTELONG, VISIOCYTELONG> hash_old_new;
    ...
    QList<QQueue<QPair<NOI*,VISIOCYTELONG> > > seeds_next;
    for(int i=0; i<c_roots.size(); i++){
        QQueue<QPair<NOI*,VISIOCYTELONG> > seeds_tmp;
        seeds_tmp.append(QPair<NOI*,VISIOCYTELONG>(nodes[c_roots.at(i)],-1));
    ...
    for(int type=1; type<20; type++){ //sort with type priority
        ...
        for(int cidx=0; cidx<seeds_next.size(); cidx++){
            QQueue<QPair<NOI*,VISIOCYTELONG> > seeds_pre=seeds_next[cidx];
            ...
            while(!seeds_pre.isEmpty()){
                QPair<NOI*,VISIOCYTELONG> pair_cur = seeds_pre.dequeue();
            ...
            while(!seeds_cur.isEmpty()){
                ...
                for(QSet<NOI*>::Iterator niter=node_cur->conn.begin(); niter!=node_cur->conn.end(); niter++){
                    ...
                    if((*niter)->type<=type || type>=19){
                        ...
                        QPair<NOI*,VISIOCYTELONG> pair_cur(*niter, hash_old_new[node_cur->n]);
                        seeds_next[cidx].append(pair_cur);
                    }
                }
            }
...
bool export_list2file(const QList<NeuronSWC>& lN, QString fileSaveName)
{
    ...
    myfile<<"# generated by Visiocyte Plugin neuron_assembler_live"<<endl;
    if(eswc_flag)
