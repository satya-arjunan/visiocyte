...
OpenSWCDialog::OpenSWCDialog(QWidget * parent, VISIOCYTEPluginCallback2 * _callback)
: QDialog(parent)
{
	callback = _callback;
    p_img4d = 0;
	...
	//page open from file
    QSettings settings("VISIOCYTE plugin","neuron_live_assembler");
    QString prevFile = "";
...
bool OpenSWCDialog::chooseNeuron()
{
    ...
    if(row<0){
        visiocyte_msg("please select neuron");
        return false;
    }
    //get neuron
    ...
    for(int i=0; i<winlist.size(); i++){
        if(callback->getImageName(winlist.at(i))==winname){
            ...
            VISIOCYTELONG datasize = p_tmp->getXDim()*p_tmp->getYDim()*p_tmp->getZDim()*p_tmp->getCDim()*((int)p_tmp->getDatatype());
...
bool OpenSWCDialog::chooseWindow()
{
    ...
    if(row<0){
        visiocyte_msg("please select window");
        return false;
    }
    //get neuron
...
void OpenSWCDialog::getAllWindowsAndNeurons(VISIOCYTEPluginCallback2 * callback)
{
	QList <VisiocyteR_MainWindow *> windowList = callback->getListAll3DViewers();
	for (VISIOCYTELONG i=0;i<windowList.size();i++)
    {
