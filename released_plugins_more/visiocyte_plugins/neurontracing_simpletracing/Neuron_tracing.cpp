...
void NeuronPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
		visiocyte_msg(title + " A Visiocyte plugin for tracing neurons using several local search methods. "
...
bool NeuronPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback, QWidget * parent)
{
	...
	else if (func_name==tr("help"))
	{
	    ...
	    cout<<"demo:visiocyte -x <path_to_this_dll or the unique sub-string> -f <one of the above options> -i <input_image_file> -o <out_swc_file>"<<endl;
		return true;
	}
	return false;
}
...
template <class T1, class T2> bool assign_val(T1 *dst, T2 *src, VISIOCYTELONG total_num)
{
	if (!dst || !src || total_num<=0) return false;
	for (VISIOCYTELONG i=0; i<total_num; i++) 
		dst[i] = src[i];
	return true;
}

template <class T1, class T2> bool assign_val_condition(T1 *dst, T2 *src, VISIOCYTELONG total_num, T2 condition_val, T1 condition_yes_val, T1 condition_no_val)
{
	if (!dst || !src || total_num<=0) return false;
	for (VISIOCYTELONG i=0; i<total_num; i++) 
...
template <class T1, class T2> bool do_seg(T1 * inputData,
                               T2 * outputData,
                               VISIOCYTELONG sx, VISIOCYTELONG sy, VISIOCYTELONG sz, 
                               int & iVesCnt, bool b_binarization=true)
{
	...
	VISIOCYTELONG channel_sz = sx*sy*sz;
	...
	{
		visiocyte_msg("Fail to allocate memory in do_seg().");
        res = false;
    ...
    {
        for (VISIOCYTELONG i=0;i<channel_sz;i++)
...
bool do_seg(short *pData, VISIOCYTELONG sx, VISIOCYTELONG sy, VISIOCYTELONG sz, int & iVesCnt, bool b_binarization=true)
{
...
void Neuron_segment_entry_func(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback &callback,bool b_binarization)
{
    ...
    unsigned char * inimg1d = 0; VISIOCYTELONG in_sz[4]; int datatype;
    simple_loadimage_wrapper(callback,infile, inimg1d, in_sz, datatype);
    
	VISIOCYTELONG channel_sz = in_sz[0]*in_sz[1]*in_sz[2];
    VISIOCYTELONG outsz[4];
	...
	{
		visiocyte_msg("Fail to allocate memory in Neuron_segment_entry_func().");
...
void Neuron_segment_entry_func(VISIOCYTEPluginCallback &callback, QWidget *parent)///segment
{
	...
	p4DImage.setData((unsigned char*)m_OutImgData, in_sz[0], in_sz[1], in_sz[2], 1, VISIOCYTE_UINT8);
...
void NeuronPlugin::NeuronTracing_Ray_cast(VISIOCYTEPluginCallback &callback, QWidget *parent)
{
	visiocytehandle curwin = callback.currentImageWindow();
	...
	VISIOCYTELONG x,y,z,index;
	
	VISIOCYTELONG imagesize = sy*sx;
    VISIOCYTELONG *apsInput=0;
	try
	{
        apsInput = new VISIOCYTELONG[sx*sy*sz];
        memset(apsInput, 0, sx*sy*sz * sizeof(VISIOCYTELONG));
	}
	catch (...)
	{
	...
	{
		for (VISIOCYTELONG j=0; j<sy; j++)
		{
			for (VISIOCYTELONG i=0; i<sx; i++)
			{
...
//    VISIOCYTELONG m_x=0, m_y=0, m_z=0;
//    if (Landmark.size()>0)
//    {
//        m_x= Landmark.at(0).x;
//        m_y= Landmark.at(0).y;
...
//	orgPoint.m_z = m_z;
	...
	for(VISIOCYTELONG jj = 0; jj< centerpath.size(); jj++)
	{
		if(jj==0)
		{
			v.n	= jj+1;
	...
	for(VISIOCYTELONG jj = 0; jj < Branchpoint.size(); jj++)
	{
	...
	/////////
    /*Image4DSimple p4DImage;
	p4DImage.setData(m_OutImgData, m_iImgWidth, m_iImgHeight, m_iImgCount-2, 1, VISIOCYTE_UINT8);
	visiocytehandle newwin;
	if(QMessageBox::Yes == QMessageBox::question (0, "", QString("Do you want to use the existing window?"), QMessageBox::Yes, QMessageBox::No))
		newwin = callback.currentImageWindow();
	else
...
void NeuronPlugin::NeuronTracing_Ray_D(VISIOCYTEPluginCallback &callback, QWidget *parent)
{
	visiocytehandle curwin = callback.currentImageWindow();
 ...
 /*   LandmarkList  Landmark;
    ...
    if (Landmark.size()<1)
    {
        visiocyte_msg("Landmark has not been set in the image. Quit.");
        return;
    }

    VISIOCYTELONG m_x= Landmark.at(0).x;
    VISIOCYTELONG m_y= Landmark.at(0).y;
    VISIOCYTELONG m_z= Landmark.at(0).z;*/
	...
	try
	{
		m_piMskDFS = new VISIOCYTELONG[m_ulVolumeSize]; 
		memset(m_piMskDFS, 0, m_ulVolumeSize * sizeof(VISIOCYTELONG));
	}
	catch (...)
	{
		visiocyte_msg("Fail to allocate memory in Distance Transform.");
...
//	for(VISIOCYTELONG j = 0; j < centerpath.size(); j++)
//	{
...
//	}
//	QString curImageName = "/Users/yangj13/work/visiocyte_internal/visiocyte_2.0/yang_jinzhu/ray cast and distance/tracing1.swc";
//	
//	SaveSwcfile(centerpath,curImageName);
  ...
  //  QString filename = "neu_tracing_D.swc";
	...
	/////////////////////////////////////
//    centerpathall.push_back(centerpath);
//    for(VISIOCYTELONG jj = 0; jj< centerpathall.size(); jj++)
//    {
//        //		file.sprintf("%d",jj);
//        //		QString filepath = "/Users/yangj13/work/visiocyte_internal/visiocyte_2.0/yang_jinzhu/ray cast and distance/";
//        //		QString curImageName1 = filepath + file + ".swc";
//        //		SaveSwcfile(centerpathall.at(jj),curImageName1);
//        for(VISIOCYTELONG ii = 0; ii<centerpathall.at(jj).size(); ii++)
//        {
...
//    hashNeuron.clear();
//    VISIOCYTELONG tt =0;
//    for(VISIOCYTELONG i = 0; i < centerpathall.size(); i++)
//    {
		
//        int size = centerpathall.at(i).size();
//        //printf("siz=%ld\n",size);
//        for(VISIOCYTELONG j = 0; j < size; j++)
//        {
//            tt++;
...
//    }
	...
	//	SS.listNeuron = listNeuron;
	//	SS.name = "neu_tracing";
	//	filename = "/Users/yangj13/work/visiocyte_internal/visiocyte_2.0/yang_jinzhu/ray cast and distance/neu_tracing1.swc";
	//writeSWC_file(filename,SS);	
	...
	//		filename = "/Users/yangj13/work/visiocyte_internal/visiocyte_2.0/yang_jinzhu/ray cast and distance/tracing.swc";
	//		writeSWC_file(filename,NS);
	//	}
	
	
	//	for (VISIOCYTELONG k=0; k<m_iImgCount; k++)
	//	{
	//		for (VISIOCYTELONG j=0; j<m_iImgHeight; j++)
	//		{
	//			for (VISIOCYTELONG i=0; i<m_iImgWidth; i++)
	//			{
...
//    /////////
//    Image4DSimple p4DImage;
//    p4DImage.setData(m_ppsMaskData, m_iImgWidth, m_iImgHeight, m_iImgCount-2, 1, VISIOCYTE_UINT8);
//    visiocytehandle newwin;
//    if(QMessageBox::Yes == QMessageBox::question (0, "", QString("Do you want to use the existing window?"), QMessageBox::Yes, QMessageBox::No))
//        newwin = callback.currentImageWindow();
//    else
...
void NeuronPlugin::Dilation3D(unsigned char *apsInput, unsigned char *aspOutput, VISIOCYTELONG sz, VISIOCYTELONG sy, VISIOCYTELONG sx)
{
	static int nDx[] = {-1,0,1,-1,0,1,-1,0,1};
	static int nDy[] = {-1,-1,-1,0,0,0,1,1,1};
	static int nDz[] = {-1,0,1};
	
	VISIOCYTELONG i,j,k,x,y,z,index;
	VISIOCYTELONG imagesize = sy*sx;
	bool end = true;
	printf("x=%ld y=%ld z=%ld\n",sx,sy,sz);
...
bool NeuronPlugin::Tracing_Ray_SWC(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback &callback)
{
...
bool NeuronPlugin::Tracing_Ball_SWC(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback &callback)
{
...
bool NeuronPlugin::Neuron_Seg(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback &callback)
{
	cout<<"Welcome to Neurontracing_jinzhu1"<<endl;
	//if(input.size() != 2 || output.size() != 1) return false;
	char * paras = 0;
...
void NeuronPlugin::SetImageInfo1D(VISIOCYTELONG* data, VISIOCYTELONG count, VISIOCYTELONG width, VISIOCYTELONG height)
{
	m_ppsOriData1D = data;
...
bool NeuronPlugin::Tracing_DistanceField_entry_func(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback &callback)
{
	...
	VISIOCYTELONG ch_tracing=0; 	//here need to add the channel info later, noted by PHC, 2012-01-25
...
void NeuronPlugin::Tracing_DistanceField_entry_func(VISIOCYTEPluginCallback &callback, QWidget *parent) 
{
	visiocytehandle curwin = callback.currentImageWindow();
	Image4DSimple* subject = callback.getImage(curwin);
	if (!subject)
                                                                       ...
                                                                       VISIOCYTELONG ch_tracing, VISIOCYTELONG mx ,VISIOCYTELONG my, VISIOCYTELONG mz)
{
	...
	try
	{
		...
		aspOutput = new VISIOCYTELONG [channel_sz];
		memset(aspOutput, 0, channel_sz * sizeof(VISIOCYTELONG));
	...
	{
		visiocyte_msg("Fail to allocate memory in NeuronTracing_Distance_SWC().");
		if (pData) {delete []pData; pData=0;}
		if (m_OutImgData) {delete []m_OutImgData; m_OutImgData=0;}
	...
	assign_val_condition(aspOutput, m_OutImgData, channel_sz, (unsigned char)(255), (VISIOCYTELONG)(255), (VISIOCYTELONG)(BACKGROUND));
	
	//Dilation3D(m_OutImgData,aspOutput,sz, sy, sx);
	
	//visiocyte_msg("segmatation end.");
	...
	for (i=0; i<iVesCnt; i++)
	{
		...
		if(blog==1)
		{
			for(VISIOCYTELONG ii = 0; ii < m_vvsptCenterpaths.size(); ii++)
			{
				...
				for(VISIOCYTELONG jj = 0; jj < size; jj++)
				{
					tt++;
					num++;
					x = branch.at(jj).m_x;
...
void NeuronPlugin::GetOrgPoint(short *apsInput, VISIOCYTELONG iImgLayer, VISIOCYTELONG iImgHei, VISIOCYTELONG iImgWid, VISIOCYTELONG cnt, SpacePoint_t &pt)
{
	VISIOCYTELONG i, j, k, n;
...
NeuronTree NeuronPlugin::NeuronTracing_Ray_SWC(unsigned char *inimg1d, VISIOCYTELONG sx, VISIOCYTELONG sy, VISIOCYTELONG sz)
{
...
NeuronTree NeuronPlugin::NeuronTracing_Rollerball_SWC(unsigned char *inimg1d, VISIOCYTELONG sx, VISIOCYTELONG sy, VISIOCYTELONG sz)
{
	...
	for(VISIOCYTELONG jj = 0; jj <  Branchpoint.size(); jj++)
	{
	    VISIOCYTELONG x = Branchpoint.at(jj).m_x;
        VISIOCYTELONG y = Branchpoint.at(jj).m_y;
        VISIOCYTELONG z = Branchpoint.at(jj).m_z;
		m_Markerpoint.push_back(Branchpoint.at(jj));
		NueronTree_Ray_D(Branchpoint.at(jj),(Branchpoint.at(jj).node_id));
	}
	...
	//QString filename = "/Users/yangj13/work/visiocyte_internal/visiocyte_2.0/yang_jinzhu/ray cast and distance/neu_tracing.swc";
	//writeSWC_file(filename,SS);	
    return SS;
}
...
vector<SpacePoint_t> NeuronPlugin::Rivise_centerpath(vector<SpacePoint_t> centerpath)
{
	...
	for(VISIOCYTELONG jj = 0; jj < centerpath.size(); jj++)
	{
		...
		//printf("x=%ld y=%ld z=%ld\n",x,y,z);
		int maxr = 0;
		for (VISIOCYTELONG cz= z-r; cz <= z+r; cz++)
		{
			for (VISIOCYTELONG cy= y-r; cy <= y+r; cy++)
			{
				for (VISIOCYTELONG cx= x-r; cx <= x+r; cx++)
				{
...
SpacePoint_t NeuronPlugin::Center_mass(SpacePoint_t point)
{
	...
	for (VISIOCYTELONG cz= z-ir; cz <= z+ir; cz++)
	{
		for (VISIOCYTELONG cy= y-ir; cy <= y+ir; cy++)
		{
			for (VISIOCYTELONG cx= x-ir; cx <= x+ir; cx++)
			{
				if (cz > m_iImgCount-2 ){continue;}
				if (cy > m_iImgHeight){continue;}
				if (cx > m_iImgWidth) {continue;}
...
vector<SpacePoint_t> NeuronPlugin::BranchDetect2(vector<SpacePoint_t> centerpath)
{
	...
	//for(VISIOCYTELONG jj = centerpath.size()-1 ; jj>=0; jj--)
	for(VISIOCYTELONG jj = 0; jj < centerpath.size(); jj++)
	{
...
QHash<VISIOCYTELONG, VISIOCYTELONG> NeuronPlugin::ChildParent(QList<NeuronSWC> &neurons, const QList<VISIOCYTELONG> & idlist, const QHash<VISIOCYTELONG,VISIOCYTELONG> & LUT) 
{
...
QHash<VISIOCYTELONG, VISIOCYTELONG> NeuronPlugin::getUniqueLUT(QList<NeuronSWC> &neurons)
{
...
void NeuronPlugin::DFS(bool** matrix, VISIOCYTELONG* neworder, VISIOCYTELONG node, VISIOCYTELONG* id, VISIOCYTELONG siz, int* numbered, int *group)
{
	numbered[node] = *group;
	neworder[*id] = node;
	(*id)++;
...
bool NeuronPlugin::SortSWC(QList<NeuronSWC> & neurons, QList<NeuronSWC> & result, VISIOCYTELONG newrootid, double thres)
{
...
{	
	...
	m_ppsImgData = new VISIOCYTELONG*[m_iImgCount + 2];
	
	m_psTemp = new VISIOCYTELONG[m_iImgSize];
	memset(m_psTemp, BACKGROUND, m_iImgSize * sizeof(VISIOCYTELONG));
	m_ppsImgData[0] = m_psTemp;
	
	for(VISIOCYTELONG c = 0; c < m_iImgCount; ++c)
	{
...
void NeuronPlugin::Set_local_DFB()
{
	...
	try
	{
		m_piDFB = new VISIOCYTELONG[m_ulVolumeSize]; 
		memset(m_piDFB, BACKGROUND, m_ulVolumeSize * sizeof(VISIOCYTELONG));
	}
	catch (...)
	{
		visiocyte_msg("Fail to allocate memory in Distance Transform.");
...
void NeuronPlugin::Set_local_DFS()
{
	...
	VISIOCYTELONG temp_l = 0;
	VISIOCYTELONG index_nei = 0;
	...
	try
	{
		m_piDFS = new VISIOCYTELONG[m_ulVolumeSize]; 
		memset(m_piDFS, BACKGROUND, m_ulVolumeSize * sizeof(VISIOCYTELONG));
	}
	catch (...)
	{
		visiocyte_msg("Fail to allocate memory in Distance Transform.");
...
void NeuronPlugin::SaveSwcTree_cast(vector<SpacePoint_t> centerpath,VISIOCYTELONG pn_id,VISIOCYTELONG node_count)
{
	QString file;
	QHash <int, int>  hashNeuron;
	//listNeuron.clear();
	...
	//	else 
	{
		for(VISIOCYTELONG jj = 1; jj< centerpath.size(); jj++)
		{
			//			if(jj==0)
			//			{
			//				v.n	= pn_num;
	...
	//	SS.name = "neu_tracing";
	//	QString filename = "/Users/yangj13/work/visiocyte_internal/visiocyte_2.0/yang_jinzhu/neu_tracing.swc";
	//	writeSWC_file(filename,SS);
}
...
void NeuronPlugin::NueronTree2(SpacePoint_t point,int n)
{
	...
	{   
		...
		{
			for(VISIOCYTELONG ii = 0; ii<point.seg_id-1; ii++)
			{
		...
		//			QString file;
		//			file.sprintf("%d",n);
		//			QString filepath = "/Users/yangj13/work/visiocyte_internal/visiocyte_2.0/yang_jinzhu/ray cast/";
		//			QString curImageName1 = filepath + file + ".swc";
		...
		{
			for(VISIOCYTELONG ii = 0 ; ii< Branchpoint.size(); ii++)
			{
...
void NeuronPlugin::NueronTree_Ray_D(SpacePoint_t point,int n)
{
	...
	//	for(VISIOCYTELONG ii = 0; ii<Rcenterpath.size();ii++)
	//	{
	...
	//	}
	//	for(VISIOCYTELONG ii = 0; ii<centerpath.size();ii++)
	//	{
	...
	{  
		...
		QString file;
		file.sprintf("%d",n);
		QString filepath = "/Users/yangj13/work/visiocyte_internal/visiocyte_2.0/yang_jinzhu/";
		QString curImageName1 = filepath + file + ".swc";
		//SaveSwcfile(centerpath,curImageName1);
		printf("CCCCCCCCCCCCCCCCCCCCCCCCCCCC");
		...
		{
			for(VISIOCYTELONG ii = 0 ; ii< Branchpoint.size(); ii++)
				//for(VISIOCYTELONG ii = Branchpoint.size()-1 ; ii>=0; ii--)
			{
...
SpacePoint_t NeuronPlugin::Coumpere_ball_center(SpacePoint_t points, int direct, double r)
{
	...
	static int nDy[] = {-1,-1,-1,0,0,0,1,1,1};
	static int nDz[] = {-1,0,1};
	VISIOCYTELONG x = points.m_x;
	VISIOCYTELONG y = points.m_y;
	VISIOCYTELONG z = points.m_z;
	VISIOCYTELONG x1=0; 
	VISIOCYTELONG y1=0; 
	VISIOCYTELONG z1=0;
	SpacePoint_t center;
...
SpacePoint_t NeuronPlugin::Coumpere_Next_Point(SpacePoint_t points, int direct, double r)
{
	...
	//	VISIOCYTELONG tx,ty,tz;
	//	VISIOCYTELONG b = m_piDFB[center.m_z * m_iImgSize + m_iImgWidth * center.m_y + center.m_x];
...
bool NeuronPlugin::PathMask(vector<SpacePoint_t> centerpath)
{
	...
	//	for (VISIOCYTELONG cz= 0; cz <m_iImgCount; cz++)
	//	{
	//		for (VISIOCYTELONG cy= 0; cy <m_iImgHeight; cy++)
	//		{
	//			for (VISIOCYTELONG cx= 0; cx < m_iImgWidth; cx++)
	//			{
	//	
	//				if(m_ppsMaskData[cz*m_iImgWidth*m_iImgHeight + cy*m_iImgWidth + cx]==255)
	//				{
...
double NeuronPlugin::fitRadiusPercent_cast(VISIOCYTELONG **img3d, VISIOCYTELONG dim0, VISIOCYTELONG dim1, VISIOCYTELONG dim2, double imgTH, double bound_r,
										   float x, float y, float z, float zthickness, bool b_est_in_xyplaneonly)
{
		...
		for (dz= zlower; dz <= zupper; ++dz)
			for (dy= -ir; dy <= +ir; ++dy)
				for (dx= -ir; dx <= +ir; ++dx)
				{
					...
					if (r>ir-1 && r<=ir)
					{
						VISIOCYTELONG i = x+dx;	if (i<0 || i>=dim0) goto end;
						VISIOCYTELONG j = y+dy;	if (j<0 || j>=dim1) goto end;
						VISIOCYTELONG k = z+dz;	if (k<0 || k>=dim2) goto end;
	...
	//m_ppsMaskData[(VISIOCYTELONG)z*m_iImgWidth*m_iImgHeight + (VISIOCYTELONG)y*m_iImgWidth + (VISIOCYTELONG)x]=255;
	for (VISIOCYTELONG cz= z-ir; cz <= z+ir; cz++)
	{
		for (VISIOCYTELONG cy= y-ir; cy <= y+ir; cy++)
		{
			for (VISIOCYTELONG cx= x-ir; cx <= x+ir; cx++)
			{
				...
				if (cy <= 0 ){continue;}
				if (cx <= 0) {continue;}
				if(m_ppsMaskData[(VISIOCYTELONG)cz*m_iImgWidth*m_iImgHeight + (VISIOCYTELONG)cy*m_iImgWidth + (VISIOCYTELONG)cx]!=255 && m_ppsImgData[cz][m_iImgWidth * cy + cx] != BACKGROUND )
					//printf("cx=%ld cy = %ld cz=%ld \n",cx,cy,cz);
					m_ppsMaskData[(VISIOCYTELONG)cz*m_iImgWidth*m_iImgHeight + (VISIOCYTELONG)cy*m_iImgWidth + (VISIOCYTELONG)cx]=255;
...
double NeuronPlugin::fitRadiusPercent(VISIOCYTELONG **img3d, VISIOCYTELONG dim0, VISIOCYTELONG dim1, VISIOCYTELONG dim2, double imgTH, double bound_r,
									  float x, float y, float z, float zthickness, bool b_est_in_xyplaneonly)
{
	if (zthickness<=0) { zthickness=1.0; printf("Your zthickness value in fitRadiusPercent() is invalid. disable it (i.e. reset it to 1) in computation.\n"); }//if it an invalid value then reset
