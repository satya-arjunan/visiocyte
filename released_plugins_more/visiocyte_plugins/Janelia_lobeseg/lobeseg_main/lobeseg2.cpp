...
#include "../../../../visiocyte_main/neurolabi/c/tz_stack_utils.h"
#include "../../../../visiocyte_main/neurolabi/c/tz_stack_graph.h"
#include "../../../../visiocyte_main/neurolabi/c/tz_stack_lib.h"
#include "../../../../visiocyte_main/neurolabi/c/tz_image_lib.h"

#include "../../../../visiocyte_main/basic_c_fun/stackutil.h"
#include "../../../../visiocyte_main/basic_c_fun/volimg_proc.h"

#include "lobeseg2.h"

#include "../../../../visiocyte_main/neurolabi/c/tz_darray.h"
#include "../../../../visiocyte_main/neurolabi/c/tz_u8array.h"

#include "../../../../visiocyte_main/worm_straighten_c/bdb_minus.h"

bool do_lobeseg_bdbminus(unsigned char *inimg1d, const VISIOCYTELONG sz[4], int in_channel_no, const BDB_Minus_ConfigParameter & mypara, vector<vector<Coord2D> > &vl, vector<vector<Coord2D> > &vr)
//note: assume the inimg1d and outimg1d have the same size, and normally out_channel_no should always be 2 (i.e. the third channel)
{
	...
	//do the computation
	{
		...
		try{
			tmp1d = new unsigned char [(VISIOCYTELONG)sz[0]*sz[1]];
		}
...
bool gen_lobeseg_mask(const VISIOCYTELONG sz[4], unsigned char *outimg1d, int out_channel_no, vector<vector<Coord2D> > &vl, vector<vector<Coord2D> > &vr, int *left_bound1d, int *right_bound1d)
{	
	int **left_bound=0;
	int **right_bound=0;
	Coord2D pt;
...
bool do_shortestpath(unsigned char *inimg1d, const VISIOCYTELONG sz[4], int in_channel_no, int out_channel_no)
{
	Stack stack;
...
bool do_shortestpath_cm(unsigned char *inimg1d, const VISIOCYTELONG sz[4], int in_channel_no, int out_channel_no)
{
	Stack stack;
	...
	//do the computation
	{
		...
		try{
			tmp1d = new unsigned char [(VISIOCYTELONG)sz[0]*sz[1]];
			c_tmp1d = new unsigned char [(VISIOCYTELONG)sz[0]*sz[1]];
			cx_tmp1d = new double [(VISIOCYTELONG)sz[0]*sz[1]];
			cy_tmp1d = new double [(VISIOCYTELONG)sz[0]*sz[1]];
			dist_cm1d = new double [(VISIOCYTELONG)sz[0]*sz[1]];
		}
		catch(...){cerr << "Fail to allocate memory for temp image plane.\n" << endl;}
		...
		{	
			...
			for (int jj=0; jj<sz[1]; jj++)
			{
				for (int ii=0; ii<sz[0]; ii++)
				{
					VISIOCYTELONG x0 = round(ii - radius_x);	x0 = (x0<0)?0:x0;
					VISIOCYTELONG x1 = round(ii + radius_x);	x1 = (x1>=(sz[0]-1))?(sz[0]-1):x1;
					VISIOCYTELONG y0 = round(jj - radius_y); y0 = (y0<0)?0:y0;
					VISIOCYTELONG y1 = round(jj + radius_y);	y1 = (y1>=(sz[1]-1))?(sz[1]-1):y1;
...
bool do_shortestpath_xz(unsigned char *inimg1d, const VISIOCYTELONG sz[4], unsigned char *outimg1d, int in_channel_no, int out_channel_no, vector<int> &pp_left, vector<int> &pp_right, vector<int> &left_zz, vector<int> &right_zz)
{	
	Stack stack;
	...
	//do the computation
	{
		...
		try{
			tmp1d_xz = new unsigned char [(VISIOCYTELONG)sz[0]*sz[2]];
		}
		catch(...){cerr << "Fail to allocate memory for temp image plane.\n" << endl;}
...
bool do_lobeseg_bdbminus2(unsigned char *inimg1d, const VISIOCYTELONG sz[4], int in_channel_no, const BDB_Minus_ConfigParameter & mypara, vector<vector<Coord2D> > &vl_high,
...
bool do_lobeseg_bdbminus3(unsigned char *inimg1d, const VISIOCYTELONG sz[4], int in_channel_no, const BDB_Minus_ConfigParameter & mypara, vector<vector<Coord2D> > &vl_low, 
...
//now generate the separating surface
bool gen_lobeseg_mask2(const VISIOCYTELONG sz[4], unsigned char *outimg1d, int out_channel_no, vector<vector<Coord2D> > &vl_high, vector<vector<Coord2D> > &vr_high, 
