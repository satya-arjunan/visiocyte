...
int lobeseg_two_sides(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	if(image->getCDim() <= c) {visiocyte_msg(QObject::tr("The channel isn't existed.")); return -1;}
	...
	for(VISIOCYTELONG i = 0 ; i < sz[0] * sz[1] * sz[2] * sz[3]; i++) outimg1d[i] = inimg1d[i];
	for(VISIOCYTELONG i =  sz[0] * sz[1] * sz[2] * sz[3] ; i < sz[0] * sz[1] * sz[2] * (sz[3] + 1); i++) outimg1d[i] = 0;
	...
	{
		visiocyte_msg("lobeseg two sides error!");
		return -1;
	}
...
void printHelp()
{
    printf("\nUsage: visiocyte -p <lobeseg_plugin_name> -n lobeseg -i <input_image_file> -o <output_image_file> -par \"#c <channalNo_reference> #A <alpha: image force> #B <beta: length force> #G <gamma: smoothness force> #n <nloop> #w <local win radius> [#s #p <position> #k <direction> #N <num_of_controls>]\"\n");
    ...
    printf("\n");
    printf("Demo :\t visiocyte -x lobeseg -f lobeseg -i input.tif -o output.tif -p \"#s #N 20 #p 30x0+100x30 #k left #S\"\n");
    printf("Version: 0.91 (Copyright: Hanchuan Peng)\n");
    return;
}

bool lobeseg(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback)
{
