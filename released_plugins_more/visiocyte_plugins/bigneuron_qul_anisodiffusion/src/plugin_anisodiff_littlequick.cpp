...
void printHelp();
bool anisodiff_func(VISIOCYTEPluginCallback2 &callback, QWidget *parent, input_PARA &PARA, bool bmenu);
...
void AnisoDiffPlugin_littlequick::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    if (menu_name == tr("anisodiff_littlequick_menu"))
	{
    	...
    	{
    		visiocyte_msg(tr("ERROR: anisodiff_func() return false!"));
    		return;
    	}
	}
	else
	{
		visiocyte_msg(tr("3D anisotropic diffusion for neuron reconstruction.\n "
...
bool AnisoDiffPlugin_littlequick::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
bool anisodiff_func(VISIOCYTEPluginCallback2 &callback, QWidget *parent, input_PARA &PARA, bool bmenu)
{
	...
	//saveImage("d:/SVN/Visiocyte_source_code/visiocyte_external/released_plugins/visiocyte_plugins/anisodiffusion_littlequick/crop.raw",p_img8u_crop,sz_img_crop,1);
	...
	if(bmenu)
	{
		printf("5. Display the processed image in Visiocyte. \n");
		//push result image back to visiocyte
		visiocytehandle newwin=callback.newImageWindow("output");
		Image4DSimple img4D_output;
		img4D_output.setData(p_img8u_output,sz_img_input[0],sz_img_input[1],sz_img_input[2],1,VISIOCYTE_UINT8);
...
void printHelp()
{
    ...
    printf("\nUsage: visiocyte -x <anisodiff> -f anisodiff_littlequick_func -i <inimg_file> \n");
