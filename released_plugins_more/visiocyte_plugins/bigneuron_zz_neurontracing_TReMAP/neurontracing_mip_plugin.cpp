...
void autotrace_largeScale_mip(VISIOCYTEPluginCallback2 &callback, QWidget *parent,APP2_LS_PARA &p,bool bmenu);
void autotrace_largeScale_raw(VISIOCYTEPluginCallback2 &callback, QWidget *parent,APP2_LS_PARA &p,bool bmenu);
...
void neurontracing_mip::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
bool neurontracing_mip::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
	...
	else if (func_name == tr("help"))
	{
        ...
        printf("visiocyte -x plugin_name -f trace_mip -i <inimg_file> -p <mip_plane> <channel> <bkg_thresh> <b_256cube> <is_gsdt> <is_gap> <length_thresh>\n");
...
void autotrace_largeScale_mip(VISIOCYTEPluginCallback2 &callback, QWidget *parent,APP2_LS_PARA &Para,bool bmenu)
{
    unsigned char* data1d = 0;
    VISIOCYTELONG N,M,P,C;
    ...
    catch(...)  {visiocyte_msg("cannot allocate memory for image_binary."); return;}
    for(VISIOCYTELONG i = 0; i < pagesz_mip; i++)
    {
        if(image_mip[i] > th)
            image_binary[i] = 255;
        else
    ...
    catch(...)  {visiocyte_msg("cannot allocate memory for image_binary_median."); return;}
    ...
    for(VISIOCYTELONG iz = 0; iz < mip_sz[2]; iz++)
    {
        VISIOCYTELONG offsetk = iz*mip_sz[1]*mip_sz[0];
        for(VISIOCYTELONG iy = 0; iy < mip_sz[1]; iy++)
        {
            VISIOCYTELONG offsetj = iy*mip_sz[0];
            for(VISIOCYTELONG ix = 0; ix < mip_sz[0]; ix++)
            {

                VISIOCYTELONG xb = ix-1; if(xb<0) xb = 0;
                VISIOCYTELONG xe = ix+1; if(xe>=mip_sz[0]-1) xe = mip_sz[0]-1;
                VISIOCYTELONG yb = iy-1; if(yb<0) yb = 0;
                VISIOCYTELONG ye = iy+1; if(ye>=mip_sz[1]-1) ye = mip_sz[1]-1;
                VISIOCYTELONG zb = iz-1; if(zb<0) zb = 0;
                VISIOCYTELONG ze = iz+1; if(ze>=mip_sz[2]-1) ze = mip_sz[2]-1;
                ii = 0;

                for(VISIOCYTELONG k=zb; k<=ze; k++)
                {
                    VISIOCYTELONG offsetkl = k*mip_sz[1]*mip_sz[0];
                    for(VISIOCYTELONG j=yb; j<=ye; j++)
                    {
                        VISIOCYTELONG offsetjl = j*mip_sz[0];
    ...
    simple_saveimage_wrapper(callback, input_image_name.toStdString().c_str(),  (unsigned char *)image_binary_median, in_sz, VISIOCYTE_UINT8);
    ...
    QString output_image_name = tmpfolder +"/region.raw";
        ...
        QString cmd_region = QString("%1/visiocyte -x regiongrow -f rg -i %2 -o %3 -p 1 0 1 400").arg(getAppPath().toStdString().c_str()).arg(input_image_name.toStdString().c_str()).arg(output_image_name.toStdString().c_str());
        system(qPrintable(cmd_region));
    #elif defined(Q_OS_MAC)
        QString cmd_region = QString("%1/visiocyte64.app/Contents/MacOS/visiocyte64 -x regiongrow -f rg -i %2 -o %3 -p 1 0 1 400").arg(getAppPath().toStdString().c_str()).arg(input_image_name.toStdString().c_str()).arg(output_image_name.toStdString().c_str());
        system(qPrintable(cmd_region));
    #else
   ...
   int tmp_index;
   for(VISIOCYTELONG i = 0; i < groupNum; i++)
   {
       if (i > 0)
       {
           VISIOCYTELONG j = i;
   ...
   for(int dd = 0; dd < groupmax; dd++)
   {
       ...
       VISIOCYTELONG group_type = groupIndex[dd];


       for(VISIOCYTELONG i = 0; i < pagesz_mip*datatype; i++)
       {
       ...
       visiocyte_msg(QString("x is %1 and y is %2").arg(tmpx).arg(tmpy));
       return;
       ...
       simple_saveimage_wrapper(callback, APP2_image_name.toStdString().c_str(),  (unsigned char *)image_region_one, in_sz, VISIOCYTE_UINT8);
       ...
       QString APP2_swc =  APP2_image_name + QString("_group_%1.swc").arg(group_type);
        ...
        #if  defined(Q_OS_LINUX)
            QString cmd_APP2 = QString("%1/visiocyte -x Visiocyte_Neuron2 -f app2 -i %2 -o %3 -p NULL %4 %5 %6 %7 %8 %9 %10").arg(getAppPath().toStdString().c_str()).arg(APP2_image_name.toStdString().c_str()).arg(APP2_swc.toStdString().c_str())
            ...
            QString cmd_resample = QString("%1/visiocyte -x resample_swc -f resample_swc -i %2 -o %3 -p 2").arg(getAppPath().toStdString().c_str()).arg(APP2_swc.toStdString().c_str()).arg(APP2_swc.toStdString().c_str());
            system(qPrintable(cmd_resample));
        #elif defined(Q_OS_MAC)
            QString cmd_APP2 = QString("%1/visiocyte64.app/Contents/MacOS/visiocyte64 -x Visiocyte_Neuron2 -f app2 -i %2 -o %3 -p NULL %4 %5 %6 %7 %8 %9 %10").arg(getAppPath().toStdString().c_str()).arg(APP2_image_name.toStdString().c_str()).arg(APP2_swc.toStdString().c_str())
                    .arg(Para.channel-1).arg(Para.bkg_thresh - 5).arg(Para.b_256cube).arg(Para.b_RadiusFrom2D).arg(Para.is_gsdt).arg(Para.is_break_accept).arg(Para.length_thresh);
            system(qPrintable(cmd_APP2));
            QString cmd_resample = QString("%1//visiocyte64.app/Contents/MacOS/visiocyte64 -x resample_swc -f resample_swc -i %2 -o %3 -p 2").arg(getAppPath().toStdString().c_str()).arg(APP2_swc.toStdString().c_str()).arg(APP2_swc.toStdString().c_str());
            system(qPrintable(cmd_resample));
        #else
                 visiocyte_msg("The OS is not Linux or Mac. Do nothing.");
                 return;
       ...
       for (VISIOCYTELONG i=0;i<seg_list.size();i++)
       {
           ...
           if(seg_list[i]->size() > 2)
           {
             ...
             //  smooth_curve(outswc,5);
               ...
               for(VISIOCYTELONG d = 0; d <outswc.size(); d++)
               {
                   outswc[d]->radius = 2;
                   outswc[d]->type = dd + 2;
...
//       outswc_final.clear();
   ...
   VISIOCYTEPluginArgList input_resample;
   VISIOCYTEPluginArgList input_sort;
   ...
   visiocyte_msg(QString("Now you can drag and drop the generated swc fle [%1] into Visiocyte.").arg(final_swc.toStdString().c_str()),bmenu);

   return;
}

void autotrace_largeScale_raw(VISIOCYTEPluginCallback2 &callback, QWidget *parent,APP2_LS_PARA &Para,bool bmenu)
{
    QString image_name = Para.inimg_file;
    QString image_mip_name = Para.mip_image_file;

    unsigned char *image_mip=0;
    VISIOCYTELONG *im_sz = 0;
    int datatype = 0;
    ...
    #if  defined(Q_OS_LINUX)
    QString cmd_region = QString("%1/visiocyte -x regiongrow -f rg -i %2 -o %3 -p 1 2 1 200 %4").arg(getAppPath().toStdString().c_str()).arg(image_mip_name.toStdString().c_str()).arg(output_image_name.toStdString().c_str()).arg(Para.bkg_thresh);
        system(qPrintable(cmd_region));
    #elif defined(Q_OS_MAC)
        QString cmd_region = QString("%1/visiocyte64.app/Contents/MacOS/visiocyte64 -x regiongrow -f rg -i %2 -o %3 -p 1 2 1 200 %4").arg(getAppPath().toStdString().c_str()).arg(image_mip_name.toStdString().c_str()).arg(output_image_name.toStdString().c_str()).arg(Para.bkg_thresh);
        system(qPrintable(cmd_region));
    #else
        visiocyte_msg("The OS is not Linux or Mac. Do nothing.");
        return;
    ...
    VISIOCYTELONG pagesz_mip = in_zz[0]*in_zz[1]*in_zz[2];
    int groupNum = 0;
    for(VISIOCYTELONG i = 0; i < pagesz_mip; i++)
    {
        if(image_region[i] > groupNum)
    ...
    for(int dd = 0; dd < groupmax; dd++)
    {
        ...
        QString APP2_image_name = tmpfolder + "/group_one.raw";
        simple_saveimage_wrapper(callback, APP2_image_name.toStdString().c_str(),  (unsigned char *)image_region_one, im_sz, VISIOCYTE_UINT8);
        if(image_region_one) {delete []image_region_one; image_region_one = 0;}
     ...
     //	printf("tree constructed.\n");
        ...
        unsigned char * data1d = 0;
        VISIOCYTELONG *im3D_zz = 0;
        VISIOCYTELONG *im3D_sz = 0;
        ...
        if(data1d) {delete []data1d; data1d = 0;}
        VISIOCYTELONG N = im3D_zz[0];
        VISIOCYTELONG M = im3D_zz[1];
        VISIOCYTELONG P = im3D_zz[2];


        for (VISIOCYTELONG i=0;i<seg_list.size();i++)
        {
            VISIOCYTELONG xb = N-1;
            VISIOCYTELONG xe = 0;
            VISIOCYTELONG yb = M-1;
            VISIOCYTELONG ye = 0;
...
//            simple_saveimage_wrapper(callback, APP2_image_name.toStdString().c_str(),  (unsigned char *)data1d, im3D_sz, VISIOCYTE_UINT8);
