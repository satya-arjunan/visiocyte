...
{
	return QStringList() 
        <<tr("Import using ImageJ and save as .visiocyteraw")
        <<tr("About");
}
...
void call_open_using_imagej(bool ismenu, QString inputfile, QString savefile,VISIOCYTEPluginCallback2 &callback)
{
    ...
    visiocyte_msg(QString("The default path of imagej is [%1]").arg(imagej_binary_file), 0);
...
//        visiocyte_msg("Cannot find the specified default ImageJ location. Adding path");
#if defined(Q_OS_MAC)
        ...
        visiocyte_msg(QString("Now set the new imagej path to [%1]").arg(imagej_binary_file), 0);
        f_imagej_binary_file.setFileName(imagej_binary_file);
    }

    if (!f_imagej_binary_file.exists())
    {
        visiocyte_msg("Cannot locate the executable of ImageJ/Fiji program. Please specify Fiji location.");
//        visiocyte_msg("Cannot locate the executable of ImageJ/Fiji program. Now you can specify where it is.",0);
    ...
    QString visiocyteAppPath = getAppPath();

    QString cmd_Fiji = QString("%1  --headless -batch  %2/brl_FijiConvert.js %3:%4").arg(imagej_binary_file.toStdString().c_str()).arg(visiocyteAppPath.toStdString().c_str()).arg(inputfile.toStdString().c_str()).arg(savefile.toStdString().c_str());
    visiocyte_msg(cmd_Fiji, 0);

    //the folowing direct call work nicely: //20141004, PHC
    //  /Users/pengh/work/visiocyte_external/bin/Fiji.app/Contents/MacOS/ImageJ-macosx  --headless -batch  /Users/pengh/work/visiocyte_external/bin/brl_FijiConvert.js /workdata/visiocyte_demo_data/single_neuron/Twin-Spot_GH146_lAL_39-2.lsm:/Users/pengh/Downloads/44/11.avi
    //
    //Thus it seems the problem is "Unrecognized command: "raw writer""

    system(qPrintable(cmd_Fiji));

    if (!QFile(savefile).exists()) visiocyte_msg("File conversion failed.\n"); //need change later
...
void open_fiji::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    printf(menu_name.toStdString().c_str());
    if (menu_name == tr("Import using ImageJ and save as .visiocyteraw"))
    {
	...
	{
        visiocyte_msg(tr("Uses Fiji to save .visiocyteraw file and open in visiocyte. "
			"Developed by Brian Long, 2013-12-12"));
	}
}

bool open_fiji::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    if (func_name == tr("openfiji"))
    {
    vector<char*> infiles, outfiles;
    if(input.size()==0) {
        cout<<"for usage type: visiocyte -x open_fiji -f help"<<endl;
        return false;
    }
	...
	{
        cout<<"Usage : visiocyte -x open_fiji -f openfiji -i <input_file_name> -o <output_file_name>  "<<endl;
    }
	else return false;
