...
void NeuroGPSTreePlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
bool NeuroGPSTreePlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    //visiocyte_msg(tr("dofunc"));
    ...
    else if (func_name == tr("help"))
    {
        ...
        printf("visiocyte -x NeuroGPSTree -f tracing_func -i <inimg_file> -p <XRESOLUTION> <YRESOLUTION> <ZRESOLUTION> <BINARYTHRESHOLD> <TRACEVALUE> <ENHANCE> \
               <SWCFILE> <ACCELERATE>\n");
...
#ifdef _WIN32
bool NeuroGPSTreePlugin::ConvertVisiocyteImg2NGImg(UChar *data1d,
                                               int X, int Y, int Z,
...
#endif
{
    //initialize
    if(!data1d){
        printf("error ocured in ConvertVisiocyteImg2NGImg.\n");
        return false;
    }
    //
    xres_ = xR;
...
bool NeuroGPSTreePlugin::SaveSeperateTree(const QString &filename, const QStringList &infostring)
{
    ...
    if (curFile.trimmed().isEmpty()) //then open a file dialog to choose file
    {
        ...
        visiocyte_msg(QString("save file: %1").arg(curFile), false);
#endif
...
#ifndef DISABLE_VISIOCYTE_MSG
    visiocyte_msg(QString("done with saving file: ")+filename, false);
#endif
    return true;
}

void NeuroGPSTreePlugin::reconstruction_func(VISIOCYTEPluginCallback2 &callback, QWidget *parent, input_PARA &PARA, bool bmenu)
{
    ...
    //convert visiocyte image to neurogps image
    printf("prepare to convert visiocyte img to neurogps image.\n");
    if(!ConvertVisiocyteImg2NGImg(data1d, N, M, P, PARA.xRes_, PARA.yRes_,
                              PARA.zRes_, OrigImage)){
        printf("cannot convert visiocyte img to neurogps image.\n");
        if(!bmenu)
        {
    ...
    printf("convert visiocyte img to neurogps image.\n");
