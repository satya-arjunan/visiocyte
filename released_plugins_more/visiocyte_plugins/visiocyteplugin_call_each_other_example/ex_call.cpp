...
QString plugin_name = "plugins/Visiocyte_PluginInterface_Demos/call_each_other/libex_matrix_debug.dylib";
//#else
...
QString plugin_name = "plugins/Visiocyte_PluginInterface_Demos/call_each_other/ex_matrix.dll";  //for Windows

#else

QString plugin_name = "plugins/Visiocyte_PluginInterface_Demos/call_each_other/libex_matrix.so";  //for Linux
...
void ExCallPlugin::domenu(const QString & menu_name, VISIOCYTEPluginCallback2 & visiocyte, QWidget * parent)
{
    if (menu_name == tr("Call another plugin (ex_matrix) to do matrix computation"))
    {
    	matrixPanel panel(visiocyte,parent);
    	panel.exec();
    }
	else
	{
...
matrixPanel::matrixPanel(VISIOCYTEPluginCallback2 &_visiocyte, QWidget *parent)
	: visiocyte(_visiocyte), QDialog(parent)
{
	for (int i=0; i<3; i++)
	for (int j=0; j<3; j++)
	{
...
void matrixPanel::add()
{
	...
	if (! visiocyte.callPluginFunc(full_plugin_name, func_name, input, output) )
		return;
