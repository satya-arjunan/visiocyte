...
void autotrace(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
...
void snake_tracing::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
        visiocyte_msg(tr("Snake tracing plugin "
            "Imported by Zhi Zhou, 2014-04-01"));
	}
}

bool snake_tracing::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    ...
    {
        cout<<"Usage : visiocyte -x dllname -f snake_trace -i <inimg_file> -p <ch> "<<endl;
        cout<<endl;
...
void autotrace(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    visiocyte_msg(QString("Now you can drag and drop the generated swc fle [%1] into Visiocyte.").arg(fileName));
    return;
...
bool autotrace(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback)
{
    ...
    visiocyte_msg(QString("Now you can drag and drop the generated swc fle [%1] into Visiocyte.").arg(fileName),0);
    if (subject) {delete subject; subject=0;}
    return true;
