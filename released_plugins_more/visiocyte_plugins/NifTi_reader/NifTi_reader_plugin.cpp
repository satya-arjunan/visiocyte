...
void NifTi_reader::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	if (menu_name == tr("Import ANALYZE image files (.nii, .nii.gz, .hdr+.img..."))
	{
		...
		//visiocyte_msg(string_mainName);
		//visiocyte_msg(string_extName);
		...
		if (strcmp(string_extName, ".hdr")==0)
		{
			...
			{
				visiocyte_msg("Cannot find corresponding .img file!");
				return;
			}
			else
			{
		...
		{
			visiocyte_msg("Cannot find header info!");
			return;
		}
		else
		{
			//visiocyte_msg("File opened correctly!");
		...
		*/
		...
		if (int_dataType == DT_INT16)
		{
			...
			Image4D_Main.setData((unsigned char*)vct_data, nX, nY, nZ, 1, VISIOCYTE_UINT16);
		}
		else if (int_dataType == DT_DOUBLE)
		{
			double* vct_data = (double *)calloc(1,ntot);
			vct_data = (double *)nimage_Input->data;
			Image4D_Main.setData((unsigned char*)vct_data, nX, nY, nZ, 1, VISIOCYTE_FLOAT32);
		}
		else
		{
	...
	{
		visiocyte_msg(tr("Reader for NifTi format file. "
			"Developed by Xiang Li, 2014-09-29"));
	}
}

bool NifTi_reader::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
