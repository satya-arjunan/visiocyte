...
void NeutubeConfig::MainWindowConfig::loadXmlNode(const ZXmlNode *node)
{
  ...
  if (!childNode.empty()) {
    ...
    expandNode = childNode.queryNode("VISIOCYTE_APO");
    if (!expandNode.empty()) {
      m_isExpandVisiocyteApoOn = (expandNode.getAttribute("status") != "off");
    }

    expandNode = childNode.queryNode("VISIOCYTE_Marker");
    if (!expandNode.empty()) {
      m_isExpandVisiocyteMarkerOn = (expandNode.getAttribute("status") != "off");
    }
  }
