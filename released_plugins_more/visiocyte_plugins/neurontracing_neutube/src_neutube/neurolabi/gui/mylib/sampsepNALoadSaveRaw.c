...
/////////// From VISIOCYTE for RAW file utilities
...
{ 
  ...
  sprintf(NameBuf,"%s/ConsolidatedLabel.visiocyteraw", RezFolder);
  writeRawImage8bit(NameBuf, mask, 1);
...
{ Tiff *tif;
  ...
  } else if (strcmp(getSuffix(name), "raw")==0 || strcmp(getSuffix(name), "visiocyteraw")==0) {
...
{ FILE *output;
  ...
  { Histogram *hist;
	...
	int channelsInCurrentFile=NumChans-curchan;
...

	  {
	    ...
	    if (channelsInCurrentFile>2) { // should work with both lsm pair with channels=3, or raw file with channels=4
	      ...
	      sprintf(NameBuf, "%s/Reference.visiocyteraw", RezFolder, CoreName, i);
	      writeRawImage16bit(NameBuf, Images[wch], 1);
	    }
	...
	for (channelIndex=0;channelIndex<Images[m]->size;channelIndex++) {
      ...
      sprintf(NameBuf, "%s/ConsolidatedSignal.visiocyteraw", RezFolder);
      writeRawImage8bit(NameBuf, signalStack, 3);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// This is adapted from the VISIOCYTE function for reading Hanchuan's RAW format in stackutil.cpp
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
int writeRawImage8bit(const char * filename, Array * stack, int chans) {
  long sz[4];
  sz[0]=stack->dims[0];
  sz[1]=stack->dims[1];
  sz[2]=stack->dims[2];
  ...
  uint8* v;
  v = AUINT8(stack);
  int result = saveStack2Raw(filename, v, sz, 1);
  return result;
}

int writeRawImage16bit(const char * filename, Array * stack, int chans) {
  long sz[4];
  sz[0]=stack->dims[0];
  sz[1]=stack->dims[1];
  sz[2]=stack->dims[2];
  ...
  uint16* v;
  v = AUINT16(stack);
  int result = saveStack2Raw(filename, v, sz, 2);
  return result;
}
...
int saveStack2Raw(const char * filename, const unsigned char * img, const long * sz, int datatype) {
  ...
  printf("size of [long]=[%ld], [long]=[%ld] [int]=[%ld], [short int]=[%ld], [double]=[%ld], [float]=[%ld]\n", sizeof(long), sizeof(long), sizeof(int), sizeof(short int), sizeof(double), sizeof(float));
	...
	//short int mysz[4];
        unsigned int mysz[4];//060806
					   //if (b_swap)  {
					   //for (i=0;i<4;i++) mysz[i] = (short int) sz[i];
		for (i=0;i<4;i++) mysz[i] = (unsigned int) sz[i];
		//swap2bytes((void *)(mysz+i));
		//}
