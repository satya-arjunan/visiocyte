...
void findMinMax8bit(double* min, double* max, unsigned char* data, long units) {
  *min=255.0;
  *max=0.0;
  long i=0;
  for (;i<units;i++) {
...
void findMinMax16bit(double* min, double* max, unsigned char* data, long units) {
  if (sizeof(unsigned short)!=2) {
    fprintf(stderr, "findMinMax16bit makes false assumption that unsigned short is size 2\n");
    exit(1);
  }
  *min=4095.0;
  *max=0.0;
  long i=0;
  for (;i<units;i++) {
    unsigned short* v = (unsigned short*)(data + 2*i);
    if (*v<*min) {
      *min=*v;
    } 
    if (*v>*max) {
...
void findMinMaxFloat(double* min, double* max, unsigned char* data, long units) {
  if (sizeof(float)!=4) {
    fprintf(stderr, "findMinMaxFloat makes false assumption that float is size 4\n");
    exit(1);
  }
  *min=1000000.0;
  *max=0.0;
...
static uint16     **COL_cval;
static uint16     *COL_mval;
...
{ Array     *mark;
  ...
  mark = Make_Array(PLAIN_KIND,UINT16_TYPE,3,Images[0]->dims);
  mval = AUINT16(mark);

  cval = (uint16 **) Guarded_Malloc(sizeof(uint16 *)*NumChans,Program_Name());
