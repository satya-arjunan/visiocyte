...
bool ZFileParser::writeVisiocyteMakerFile(std::string filePath,
                                      vector<ZVisiocyteMarker> markerArray)
{
  ...
  for (vector<ZVisiocyteMarker>::iterator iter = markerArray.begin();
...
bool ZFileParser::writeVisiocyteApoFile(std::string filePath,
                                    vector<ZVisiocyteApo> markerArray)
{
  ofstream stream(filePath.c_str());
  if (!stream.is_open()) {
    RECORD_WARNING(true, "Cannot open file " + filePath);
  ...
  for (vector<ZVisiocyteApo>::iterator iter = markerArray.begin();
       iter != markerArray.end(); ++iter) {
    stream << iter->toString() << endl;
  }
...
std::vector<ZVisiocyteMarker> ZFileParser::readVisiocyteMarkerFile(
