...
QList<ZPunctum*> ZPunctumIO::load(const QString &file)
{
  ...
  case ZFileType::VISIOCYTE_APO_FILE:
    readVISIOCYTEApoFile(file, punctaList);
    break;
  case ZFileType::VISIOCYTE_MARKER_FILE:
    readVISIOCYTEMarkerFile(file, punctaList);
    break;
...
void ZPunctumIO::readVISIOCYTEApoFile(const QString &file, QList<ZPunctum *> &punctaList)
{
  QFile qFile(file);
...
void ZPunctumIO::readVISIOCYTEMarkerFile(const QString &file, QList<ZPunctum *> &punctaList)
{
  std::vector<ZVisiocyteMarker> markerArray =
      FlyEm::ZFileParser::readVisiocyteMarkerFile(file.toStdString());
