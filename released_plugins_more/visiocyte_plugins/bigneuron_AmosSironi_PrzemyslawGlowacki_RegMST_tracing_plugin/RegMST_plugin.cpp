...
void RegMST::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	if (menu_name == tr("tracing_menu"))
	{
        ...
        visiocyte_msg("To be implemented. Please call this plugin from command line.\n "
                ...
                "visiocyte -x RegMST -f tracing_func -i <inimg_file_1> ... <inimg_file_N> -p <channel> <n_AC> <reg_path_1.cfg> ... <reg_path_n_AC.cfg> <window size> <threshold>\n"
	...
	{
        visiocyte_msg(tr("plugin for applying the regression tubularity approach [1]\n "
                   "in combiantion with a MST algorithm [2] to reconstruct tubular structures.\n\n"
...
bool RegMST::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    ...
    else if (func_name == tr("help"))
    {
        ...
        printf("visiocyte -x RegMST -f tracing_func -i <inimg_file> -p <channel> <n_AC> <reg_path_1.cfg> ... <reg_path_n_AC.cfg> <window size> <threshold>\n");
...
void reconstruction_func(VISIOCYTEPluginCallback2 &callback, QWidget *parent, input_PARA &PARA, bool bmenu)
{
 ...
 VISIOCYTEPluginArgItem name_input_visiocyte;
...
buf_input_vec->push_back(buf_input);
 name_input_visiocyte.p =buf_input_vec;
 input.append(name_input_visiocyte);


VISIOCYTEPluginArgList  output;
VISIOCYTEPluginArgItem name_output_visiocyte;
...
if(PARA.n_ac_iters>0){
   ...
   // QDir reg_path =  QDir(PARA.regressor_paths.at(PARA.n_ac_iters-1));
    ...
    if(list_folders_reg.size()>1){
        ...
        out_tubularity_filename = QString("%1_tubularity_%2.visiocyteraw")
                             .arg(PARA.inimg_file).arg(list_folders_reg.at(list_folders_reg.size()-2));

    }else{
         out_tubularity_filename = PARA.inimg_file + "_tubularity.visiocyteraw";
    }
...
name_output_visiocyte.p =buf_out_vec;
output.append(name_output_visiocyte);
...
if( (PARA.n_ac_iters)>0){
    VISIOCYTEPluginArgItem name_para_visiocyte;
     name_para_visiocyte.p =buf_para_vec;
    input.append(name_para_visiocyte);
}
