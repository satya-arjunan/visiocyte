...
trace_para.b_estRadii = false;
   ...
   // QString swc_name = PARA.inimg_file + sprintf("_MST_Tracing_Ws_%i_th_%i.swc", PARA.Ws, PARA.pos_thres);//+ "_MST_Tracing.swc";
    ...
    //  out_tubularity_filename = PARA.inimg_file + sprintf("_tubularity_%s.visiocyteraw", PARA.regressor_paths.at(PARA.n_ac_iters).toStdString().c_str());
  ...
  //   nt_final = post_process(nt_final);
    ...
    //return
        ...
        visiocyte_msg(QString(" generated swc fle [%1] is empty !.").arg(swc_name.toStdString().c_str()),bmenu);
