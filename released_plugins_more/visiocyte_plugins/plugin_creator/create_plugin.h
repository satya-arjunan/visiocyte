//last changed by Hanchuan Peng, 2013-08-06. fix a return value bug and also add some visiocyte_msg calls to indicate where the code should be added
...
void create_plugin_pro(PluginTemplate & pt)
{
	...
	ofs<<"VISIOCYTEPATH = "<<pt.VISIOCYTE_PATH<<endl;
	...
	ofs<<"DESTDIR\t= $$VISIOCYTEPATH/bin/plugins/"<<pt.PLUGIN_NAME<<"/"<<endl;
	ofs.close();
}
...
void create_plugin_neuronrec_pro(PluginTemplate & pt)
{
    ...
    ofs<<"TARGET\t= $$qtLibraryTarget("<<pt.PLUGIN_NAME<<")"<<endl;
    ofs<<"DESTDIR\t= $$VISIOCYTEPATH/bin/plugins/bigneuronhackathon/"<<pt.PLUGIN_NAME<<"/"<<endl;
    ofs.close();
}

void create_plugin_neuronML_pro(PluginTemplate & pt)
{
    cout<<"create "<<pt.PRO_FILE<<" ... "<<endl;
    ofstream ofs(pt.PRO_FILE.c_str());
    ofs<<""<<endl;
...
void create_plugin_cpp(PluginTemplate & pt)
{
	...
	ofs<<"}"<<endl;
	ofs<<""<<endl;
	ofs<<"void "<<pt.PLUGIN_CLASS<<"::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)"<<endl;
	ofs<<"{"<<endl;
	ofs<<"\tif (menu_name == tr(\""<<pt.MENUS[0]<<"\"))"<<endl;
	ofs<<"\t{"<<endl;
    ofs<<"\t\tvisiocyte_msg(\"To be implemented.\");"<<endl;
	ofs<<"\t}"<<endl;
	...
	ofs<<"\telse"<<endl;
	ofs<<"\t{"<<endl;
	ofs<<"\t\tvisiocyte_msg(tr(\""<<pt.PLUGIN_DESCRIPTION<<". \"\n\t\t\t\"Developed by "<<pt.PLUGIN_AUTHOR<<", "<<pt.PLUGIN_DATE<<"\"));"<<endl;
	ofs<<"\t}"<<endl;
	ofs<<"}"<<endl;
	ofs<<""<<endl;

	ofs<<"bool "<<pt.PLUGIN_CLASS<<"::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)"<<endl;
	ofs<<"{"<<endl;
...
void create_plugin_neuronrec_cpp(PluginTemplate & pt)
{
    ...
    ofs<<" "<<endl;
    string line;
    string template_path = pt.VISIOCYTE_PATH + "/../visiocyte_tools/released_plugins/visiocyte_plugins/plugin_creator/neuronrec_template.cpp";
    ...
    if (templatefile.is_open())
    {
        ...
        ofs<<"\t\tprintf(\"visiocyte -x " << pt.PLUGIN_NAME <<" -f " << pt.FUNCS[0] << " -i <inimg_file> -p <channel> <other parameters>\\n\");\n";
...
void create_plugin_neuronML_cpp(PluginTemplate & pt)
{
    cout<<"create "<<pt.PLUGIN_CPP<<" ... "<<endl;
    ofstream ofs(pt.PLUGIN_CPP.c_str());
    ofs<<"/* "<<pt.PLUGIN_CPP<<endl;
    ...
    ofs<<" "<<endl;
    string line;
    string template_path = pt.VISIOCYTE_PATH + "/../visiocyte_tools/released_plugins/visiocyte_plugins/plugin_creator/neuronML_template.cpp";
    ifstream templatefile (template_path.c_str());
    if (templatefile.is_open())
    {
        for(int d = 0; d< 4; d++)
        ...
        ofs<<"\t\tprintf(\"**** Usage of " << pt.PLUGIN_NAME <<" tracing **** \\n\");\n";
        ofs<<"\t\tprintf(\"visiocyte -x " << pt.PLUGIN_NAME <<" -f " << pt.FUNCS[0] << " -i <inimg_file> -p <channel> <swc_file> <marker_file> <other parameters>\\n\");\n";

        for(int d = 43;d< 143; d++)
        {
            getline (templatefile,line);
            ofs<<line<<endl;
        }
        ofs<<"\tQString swc_name = PARA.inimg_file + \"_" << pt.PLUGIN_NAME <<".swc\";\n";
        ofs<<"\tnt_output.name = \"" << pt.PLUGIN_NAME <<"\";\n";
        ofs<<"\tQString marker_name = PARA.inimg_file + \"_" << pt.PLUGIN_NAME <<".marker\";\n";

        for(int d = 143;d< 170; d++)
        {
            getline (templatefile,line);
            ofs<<line<<endl;
        }
...
void create_plugin_header(PluginTemplate & pt)  // PLUGIN_HEADER
{
	...
	ofs<<""<<endl;
	ofs<<"class "<<pt.PLUGIN_CLASS<<" : public QObject, public VISIOCYTEPluginInterface2_1"<<endl;
	ofs<<"{"<<endl;
	ofs<<"\tQ_OBJECT"<<endl;
...
string demo_plugin_template()
{
	...
	oss<<"VISIOCYTE_PATH = \"../../work/visiocyte_external\""<<endl;
	oss<<endl;
...
void create_plugin_neuronML_all(PluginTemplate & pt)
{
    create_plugin_header(pt);
    create_plugin_neuronML_cpp(pt);
    create_plugin_neuronML_pro(pt);
}
...
PluginTemplate read_plugin_template(string plugin_tmpl_file)
{
	...
	while(ifs.good())
	{
		...
		pt.VISIOCYTE_PATH = (name == "VISIOCYTE_PATH") ? value : pt.VISIOCYTE_PATH;
