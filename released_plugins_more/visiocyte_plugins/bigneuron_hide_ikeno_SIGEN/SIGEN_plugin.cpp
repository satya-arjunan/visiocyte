...
void SigenPlugin::domenu(
  ...
  if (menu_name == tr("Trace")) {
    ...
    visiocyte_msg(tr("This plugin is porting of SIGEN (https://sites.google.com/site/sigenproject/) for Visiocyte."));
  } else {
    assert(false);
  }
}

// static std::string GetArgString(const VISIOCYTEPluginArgList &input, int index, int index2) {
...
bool SigenPlugin::dofunc(
  ...
  if (func_name == tr("trace")) {
    ...
    printf("visiocyte -x SIGEN -f trace -i <inimg_file> -p <channel> <other parameters>\n");
...
static bool getConfig(QWidget *parent, sigen::interface::Options *options) {
  ...
  // visiocyte_msg(vt_lineEdit->text() + "/" + dt_lineEdit->text() + "/" + sm_lineEdit->text() + "/" + cl_lineEdit->text(), true);
...
void reconstruction_func(
  ...
  visiocyte_msg(QString("Now you can drag and drop the generated swc fle [%1] into Visiocyte.").arg(swc_name.toStdString().c_str()), via_gui);
}
