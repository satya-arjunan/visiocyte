...
void MovieFrom3Dviewer(VISIOCYTEPluginCallback2 & visiocyte, QWidget * parent);
void SnapShoot3Dviewer(VISIOCYTEPluginCallback2 & visiocyte, QWidget * parent);
...
void MovieFrom3DviewerPlugin::domenu(const QString & menu_name,	VISIOCYTEPluginCallback2 & visiocyte, QWidget * parent)
{
	if (menu_name == tr("make movie from 3D viewer"))
	{
		MovieFrom3Dviewer(visiocyte, parent);
	}
	else if (menu_name == tr("take a snapshot of 3D viewer"))
	{
		SnapShoot3Dviewer(visiocyte, parent);
	}
}
...
void controlPanel::_slot_start()
{
	...
	{
		visiocyte_msg("Output dir is empty or do not exist!");
		return;
	}
...
void controlPanel::_slot_timerupdate()
{
	...
	visiocytehandle curwin = m_visiocyte.currentImageWindow();
...
void SnapShoot3Dviewer(VISIOCYTEPluginCallback2 & visiocyte, QWidget * parent)
{
	...
	if (!curwin)
	{
		visiocyte_msg("You don't have any image opened in the main window.");
		return;
	}
	visiocyte.open3DWindow(curwin);
