...
// func declare
int clonalselecting(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
...
void ClonalSelectPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
bool ClonalSelectPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    if (func_name == tr("help"))
    {
        //print Help info
        printf("\nUsage: \tvisiocyte -x clonalSelect -h \n\n");
    ...
    else if (func_name == tr("i2p"))
    {
        ...
        if(input.size()<1 || (input.size()==1 && output.size()<1) ) // no inputs
        {
            //print Help info
            printf("\nNo Inputs!\n");
            printf("\nUsage: \tvisiocyte -x clonalSelect -f i2p -i <input_image> -o <output_pointcloud> \n\n");
            return true;
        }
