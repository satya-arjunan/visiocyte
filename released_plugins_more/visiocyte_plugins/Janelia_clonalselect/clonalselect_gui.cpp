...
ColonalSelectWidget::ColonalSelectWidget(VISIOCYTEPluginCallback &callback, QWidget *parentWidget)
{
    //
    m_callback = &callback;
    ...
    visiocytehandle wincurr = callback.currentImageWindow(); // focused image
...
void ColonalSelectWidget::update()
{
    ...
    visiocytehandle wincurr = m_callback->currentImageWindow(); // focused image
...
void ColonalSelectWidget::evaluateClonal()
{
    ...
    if(!cmFileList.empty())
    {
        ...
        QString outImgNameMask = curImgNamePath + "/" + curImgNameBase + "_evaluated.visiocyteraw";
        ...
        VISIOCYTELONG sc_out = 2; // assuming only computing in the first 2 signals
        ...
        VISIOCYTELONG tolpxl = pagesz*sc_out;

        VISIOCYTELONG offset_c = tolpxl; // ref
        ...
        for(int i = 0; i<listWidget->count(); i++)
        {
            if(listWidget->item(i)->checkState())
            {
                ...
                if(!pMask)
                {
                    y_new<unsigned char, VISIOCYTELONG>(pMask,tolpxl);
                }
                ...
                for(long k=0; k<pc.points.size(); k++)
                {
                    ...
                    {
                        VISIOCYTELONG idx = pagesz + p.z*sx*sy + p.y*sx + p.x;
        ...
        else
        {
            ...
            visiocytehandle newwin = m_callback->newImageWindow();
...
void ColonalSelectWidget::selectClonal()
{
    ...
    if(!cmFileList.empty())
    {
        ...
        QString curImgNamePath=QFileInfo(curImgName).path();
        QString curImgNameBase = QFileInfo(curImgName).baseName();
        QString outImgNameSelected = curImgNamePath + "/" + curImgNameBase + "_selected.visiocyteraw";
        QString outImgNameResidue = curImgNamePath + "/" + curImgNameBase + "_residue.visiocyteraw";
        QString outImgNameSelectedMIP = curImgNamePath + "/" + curImgNameBase + "_selectedMIP.visiocyteraw";
        QString outImgNameResidueMIP = curImgNamePath + "/" + curImgNameBase + "_residueMIP.visiocyteraw";
        ...
        VISIOCYTELONG slicesz = sx*sy;
        VISIOCYTELONG pagesz = slicesz*sz;
        VISIOCYTELONG tolpxl = pagesz*sc;
        VISIOCYTELONG mippxl = slicesz*sc;
        ...
        for(int i = 0; i<listWidget->count(); i++)
        {
            if(listWidget->item(i)->checkState())
            {
                ...
                if(!pSel)
                {
                    y_new<unsigned char, VISIOCYTELONG>(pSel,tolpxl);
                }

                if(!pRes)
                {
                    y_new<unsigned char, VISIOCYTELONG>(pRes,tolpxl);
                }
                ...
                for(long k=0; k<pc.points.size(); k++)
                {
                    ...
                    if(p.x<sx && p.y<sy && p.z<sz && p.v>threshold)
                    {
                        VISIOCYTELONG idx = p.z*sx*sy + p.y*sx + p.x;
        ...
        {
            // computing residue
            for(VISIOCYTELONG c=0; c<sc_out; c++)
            {
            ...
            for(VISIOCYTELONG c=sc_out; c<sc; c++)
            {
                VISIOCYTELONG offsets = c*pagesz;
                for(VISIOCYTELONG i=0; i<pagesz; i++)
                {
            ...
            y_new<unsigned char, VISIOCYTELONG>(pSelMIP, mippxl);
            y_new<unsigned char, VISIOCYTELONG>(pResMIP, mippxl);

            for(VISIOCYTELONG i=0; i<slicesz; i++)
            {
                for(VISIOCYTELONG z=0; z<sz; z++)
                {
                    for(VISIOCYTELONG c=0; c<sc; c++)
                    {
                        VISIOCYTELONG idx = c*pagesz + z*slicesz + i;
                        VISIOCYTELONG idxout = c*slicesz + i;
            ...
            visiocytehandle newwinRes = m_callback->newImageWindow();
            ...
            visiocytehandle newwinSelMIP = m_callback->newImageWindow();
            ...
            visiocytehandle newwinResMIP = m_callback->newImageWindow();
