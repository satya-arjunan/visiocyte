...
crop_dialog::crop_dialog(VISIOCYTEPluginCallback2 *cb)
{
    callback=cb;
    image1Dc_in=0;
}
...
bool crop_dialog::csv_out()
{
    ...
    QString tmp_dir = QFileDialog::getExistingDirectory(0, QObject::tr("Select Directory to Save Results"),
        ...
        QSettings settings("VISIOCYTE plugin","CropImageTrace");
...
void crop_dialog::crop()
{
    ...
    VISIOCYTELONG size_page=crop_sz[0]*crop_sz[1]*crop_sz[2];
    ...
    for (VISIOCYTELONG dx=x_start;dx<=x_end;dx++){
        for (VISIOCYTELONG dy=y_start;dy<=y_end;dy++){
            for (VISIOCYTELONG dz=z_start;dz<=z_end;dz++){
                ...
                VISIOCYTELONG pos1=xyz2pos(dx-x_start,dy-y_start,dz-z_start,crop_sz[0],crop_sz[0]*crop_sz[1]);
    ...
    QString filename=QDir(folder_output).filePath("cropped_image.visiocyteraw");
    //qDebug()<<"image name:"<<image_name<<" filename:"<<filename;
    if (!simple_saveimage_wrapper(*callback,filename.toStdString().c_str(),cropped_image,crop_sz,VISIOCYTE_UINT8))
    {
        visiocyte_msg("Errors occur when saving the image");
        return;
    }
    ...
    image4d.setData(image_tmp,crop_sz[0],crop_sz[1],crop_sz[2],crop_sz[3],VISIOCYTE_UINT8);
    visiocytehandle window=callback->newImageWindow(WINNAME_LOCAL);
               ...
               VISIOCYTELONG sz_img[4],NeuronTree &neuron,visiocytehandle &curwin)
{
    ...
    if (visiocytehandleList_current.size()==0){
        visiocyte_msg("Please load image and swc file");
        return false;
    }
    else if (visiocytehandleList_current.size()==1)  //One window open
    {
        ...
        swc_list=callback->getHandleNeuronTrees_3DGlobalViewer(visiocytehandleList_current[0]);
        ...
        {
            visiocyte_msg("Please load swc file");
            return false;
        }
        else
        {
            visiocyte_msg("Multiple swc files are found. Please load one");
            return false;
        }
    }
    ...
    if (pixeltype!=1)
    {
        visiocyte_msg("This image format cannot be handled. We only support UINT8 image now.");
        return false;
    }
    if (!p4DImage){
