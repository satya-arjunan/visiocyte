...
template<class T>
bool q_imresize_3D(const T *p_img_input,const VISIOCYTELONG sz_img_input[4],const int mode,
		const VISIOCYTELONG sz_img_output[4],T *&p_img_output)
{
	//check paras
	if(p_img_input==0)
	{
	...
	{
		VISIOCYTELONG l_npixels=sz_img_output[0]*sz_img_output[1]*sz_img_output[2]*sz_img_output[3];
		for(VISIOCYTELONG i=0;i<l_npixels;i++)	p_img_output[i]=p_img_input[i];
		return true;
	}
	...
	if(mode==2)
	{
		for(VISIOCYTELONG x=0;x<sz_img_output[0];x++)
			for(VISIOCYTELONG y=0;y<sz_img_output[1];y++)
				for(VISIOCYTELONG z=0;z<sz_img_output[2];z++)
				{
					double cur_pos[3];
					...
					for(VISIOCYTELONG c=0;c<sz_img_output[3];c++)
		...
		for(VISIOCYTELONG x=0;x<sz_img_output[0];x++)
			for(VISIOCYTELONG y=0;y<sz_img_output[1];y++)
				for(VISIOCYTELONG z=0;z<sz_img_output[2];z++)
				{
					...
					VISIOCYTELONG x_l_b,x_r_b,y_l_b,y_r_b,z_l_b,z_r_b;
					...
					{
						double avg=0;
						VISIOCYTELONG npixles=0;
						for(VISIOCYTELONG xx=x_l_b;xx<=x_r_b;xx++)
							for(VISIOCYTELONG yy=y_l_b;yy<=y_r_b;yy++)
								for(VISIOCYTELONG zz=z_l_b;zz<=z_r_b;zz++)
								{
