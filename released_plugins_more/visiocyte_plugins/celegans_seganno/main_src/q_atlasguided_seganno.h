...
public:
	...
	VISIOCYTELONG 	l_niter_pertemp;
	//refine align
	bool 	b_ref_simplealign;
	double 	d_ref_T;								//fixed temperature for refined align
	VISIOCYTELONG 	l_ref_cellradius;						//cell radius (after downsample) used to compute the energy
	VISIOCYTELONG 	l_ref_maxiter;
	...
	bool	b_stepwise;
	//work mode
	VISIOCYTELONG 	l_mode;
	QWidget *qw_rootparent;

	CParas()
	{
		b_imgfromVISIOCYTE=0;
		b_atlasupsidedown=0;
		l_refchannel=1;
		d_downsampleratio=4;
		d_fgthresh_factor=3;
...
public:
	CControlPanel(VISIOCYTEPluginCallback &_visiocyte, QWidget *parent=0);
	~CControlPanel();

public:
	static CControlPanel* panel;
	VISIOCYTEPluginCallback &visiocyte;
	bool b_stop;
