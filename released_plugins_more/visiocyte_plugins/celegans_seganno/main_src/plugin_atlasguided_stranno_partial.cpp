...
void AtlasGuidedStrAnnoPartial(VISIOCYTEPluginCallback2 &callback, QWidget *parent, int mode);

bool AtlasGuidedStrAnnoPartial(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 &callback, int mode);

bool readCelloi_file(const QString &qs_filename,QList<QString> &ql_celloi);

bool do_AtlasGuidedStrAnno ( VISIOCYTEPluginCallback2 &callback,
...
void AtlasGuidedStrAnnoPartialPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
bool AtlasGuidedStrAnnoPartialPlugin::dofunc(const QString &func_name, const VISIOCYTEPluginArgList &input, VISIOCYTEPluginArgList &output, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
          cout<<"Usage : visiocyte -x atlasguided -f seganno -i <inimg_file> <inmarker_file> <inatlas_file> <incell_file> [<incell2_file>] -o <outatlas_file> <outseglab_file> -p <refchannel> <ratio> <fgthresh> <initemp> <minitemp> <ann_rate> <iter> <useCell2>"<<endl;
		cout<<endl;
          cout<<"inimg_file      name of input image file"<<endl;
        cout<<"inmarker_file   name of input marker file (the markers will define initial head and tail marker position from VISIOCYTE) (should be specified as NULL if non-defined)" <<endl;
		...
		cout<<endl;
          cout<<"e.g. visiocyte -x atlasguided -f seganno -i inimg_file.raw inmarker_file.marker inatlas_file.apo incell_file.txt -o myoutatlas.apo outseglab.raw -p 2 4 3 20 0.2 0.95 1 0"<<endl;
          cout<<endl;
		return true;
	}
}
...
bool AtlasGuidedStrAnnoPartial(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 &callback, int mode)
{
	...
	paras_anno.l_niter_pertemp=1;
     ...
     if (input.size()>=1)
     {
          ...
          {
               visiocyte_msg("There are not enough input files.\n", 0);
               return false;
          }
          ...
          {
               visiocyte_msg("There are not enough output files.\n", 0);
               return false;
          }
     ...
     // set for dofunc() because VISIOCYTE view is no use for dofunc()
     paras_str.b_imgfromVISIOCYTE=0;
	paras_str.b_markerfromVISIOCYTE=0;
	//visualization
	...
	paras_anno.qw_rootparent= 0; //parent;
     ...
     {
          visiocyte_msg("Invalid input and output parameters.\n", 0);
          return false;
     }
     ...
     {
          visiocyte_msg("Error in calling do_AtlasGuidedStrAnno()", 0);
          return false;
     }
...
void AtlasGuidedStrAnnoPartial(VISIOCYTEPluginCallback2 &callback, QWidget *parent, int mode)
{
	...
	CSParas paras_str;
	//image and marker
	paras_str.b_imgfromVISIOCYTE=1;
	paras_str.b_markerfromVISIOCYTE=1;
	QString qs_filename_img="";
	QString qs_filename_marker_input="";
	...
	paras_anno.qw_rootparent=parent;
     ...
     {
          visiocyte_msg("Error in calling do_AtlasGuidedStrAnno()");
          return;
     }
     visiocyte_msg("Program exit successfully!\n");
     ...
     QString &qs_filename_atals_output, QString &qs_filename_seglabel_output ) // output files
{
	...
	if(!paras_str.b_imgfromVISIOCYTE)
	{
	...
	else
	{
		...
		if(h_wndlist.size()<1)
		{
			visiocyte_msg(QString("Make sure there are at least 1 image in VISIOCYTE!"));
			return false;
		}
	...
	if(!p_img_8u)
	{
		...
		if(p_img_input && !paras_str.b_imgfromVISIOCYTE) 	{delete []p_img_input;		p_img_input=0;}
	...
	if(!paras_str.b_markerfromVISIOCYTE)
	{
        if(qs_filename_marker_input.isEmpty() || qs_filename_marker_input.toUpper()=="NULL")
		{
            visiocyte_msg(QString("Invalid marker path! Ignore it!"), 0);
            vec2d_markers.clear();
	...
	{
		visiocyte_msg(QString("Given atlas file is empty or invalid!"));
		if(p_img_8u) 								{delete []p_img_8u;			p_img_8u=0;}
		if(p_img_input && !paras_str.b_imgfromVISIOCYTE) 	{delete []p_img_input;		p_img_input=0;}
        //if(sz_img_input) 							{delete []sz_img_input;		sz_img_input=0;}
		return false;
	...
	for(VISIOCYTELONG i=0;i<ql_celloi_name.size();i++)
	...
	else
	{
		...
		VISIOCYTELONG l_width=paras_str.l_radius_cuttingplane*2+1;
		QList<ImageMarker> ql_marker;
		for(unsigned VISIOCYTELONG i=0;i<vec2d_markers.size();i++)
		{
			ImageMarker tmp;
	...
	if(outputinfo.b_rotate90)
		visiocyte_msg("The deformed atlas maybe wrongly 90 degree rotated!\nCheck result!\n");
	...
	if( b_use_celloi_2 || !qs_filename_celloi_2_input.isEmpty()) // b_use_celloi_2 = DLG_stranno.checkBox_celloi_2->isChecked()
	{
		...
		for(VISIOCYTELONG i=0;i<ql_atlasapo.size();i++)
		{
			...
			for(VISIOCYTELONG j=0;j<ql_celloi_name.size();j++)
			{
		...
		for(VISIOCYTELONG i=0;i<ql_atlasapo.size();i++)
		{
			...
			for(VISIOCYTELONG j=0;j<ql_celloi2_name.size();j++)
			{
		...
		for(VISIOCYTELONG i=0;i<ql_cellio2_ori.size();i++)
		{
			x_ori(i+1,1)=1.0;
		...
		for(VISIOCYTELONG i=0;i<ql_cellio2_tps.size();i++)
		{
	...
	if(vec2d_markers.size()>=2)
	{
		for(VISIOCYTELONG i=0;i<ql_musclecell_output.size();i++)
		{
			VISIOCYTELONG x=ql_musclecell_output[i].x;
			VISIOCYTELONG y=ql_musclecell_output[i].y;
			VISIOCYTELONG z=ql_musclecell_output[i].z;
...
//		visiocytehandle curwin=callback.getImageWindowList()[0];
	...
	{
		VISIOCYTELONG sz_seglabelimg[4]={sz_img_input[0],sz_img_input[1],sz_img_input[2],1};
