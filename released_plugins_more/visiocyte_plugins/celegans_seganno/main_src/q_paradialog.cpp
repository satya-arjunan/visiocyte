...
ParaDialog::ParaDialog(VISIOCYTEPluginCallback &callback,QWidget *parent):QDialog(parent)
{
	setupUi(this);
	IniDialog(callback);
}
...
void ParaDialog::IniDialog(VISIOCYTEPluginCallback &callback)
{
	...
	comboBox_imgvisiocyte->addItems(items);
	//visualization
	this->checkBox_showVISIOCYTE_img->setCheckState(Qt::Unchecked);
	this->checkBox_showVISIOCYTE_pts->setCheckState(Qt::Unchecked);
	//initial align paras
