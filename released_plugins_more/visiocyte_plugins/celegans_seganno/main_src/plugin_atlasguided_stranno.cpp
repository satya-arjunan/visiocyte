...
void AtlasGuidedStrAnno(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
...
void AtlasGuidedStrAnno(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	//visualization
	paras_str.b_showinVISIOCYTE_pts=0;
	//skeleton detection
	...
	//visualization
	paras_anno.b_showinVISIOCYTE_img=0;
	paras_anno.b_showinVISIOCYTE_pts=0;
	//initial align
	...
	if(datatype_input!=1)
	{
		visiocyte_msg(QString("Need image datatype==1(Uint8) !"));
		return;
	}
	...
	if(!paras_str.b_markerfromVISIOCYTE)
	{
		...
		{
			visiocyte_msg(QString("Need right at least 2 markers!"));
			if(p_img_input && !paras_str.b_imgfromVISIOCYTE) 	{delete []p_img_input;		p_img_input=0;}
			if(sz_img_input)						 	{delete []sz_img_input;		sz_img_input=0;}
			return;
		}
