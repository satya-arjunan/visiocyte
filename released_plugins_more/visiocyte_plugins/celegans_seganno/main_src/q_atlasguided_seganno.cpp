...
#include "../../visiocyte_main/basic_c_fun/basic_memory.cpp"//note: should not include .h file, since they are template functions
#include "q_atlasguided_seganno.h"

CControlPanel::CControlPanel(VISIOCYTEPluginCallback &_visiocyte, QWidget *parent) : QDialog(parent),visiocyte(_visiocyte)
{
	panel=this;
	b_stop=1;
...
bool q_atlas2image(const CParas &paras,VISIOCYTEPluginCallback &callback,
		const unsigned char *p_img8u,const VISIOCYTELONG sz_img[4],const QList<CellAPO> &ql_atlasapo,const QList<QString> &ql_celloi,
		QList<CellAPO> &ql_musclecell_output,unsigned char *&p_img8u_seglabel,COutputInfo &outputinfo)
{
	...
	VISIOCYTELONG sz_img_1c[4]={sz_img[0],sz_img[1],sz_img[2],1};
	VISIOCYTELONG l_npixels_1c=sz_img_1c[0]*sz_img_1c[1]*sz_img_1c[2];
	...
	VISIOCYTELONG pgsz_y=sz_img[0];
	VISIOCYTELONG pgsz_xy=sz_img[0]*sz_img[1];
	VISIOCYTELONG pgsz_xyz=sz_img[0]*sz_img[1]*sz_img[2];
	for(VISIOCYTELONG x=0;x<sz_img[0];x++)
		for(VISIOCYTELONG y=0;y<sz_img[1];y++)
			for(VISIOCYTELONG z=0;z<sz_img[2];z++)
			{
				VISIOCYTELONG ind_1c=pgsz_xy*z+pgsz_y*y+x;
				for(VISIOCYTELONG c=0;c<3;c++)
					if(arr_validchannel[c])
					{
						VISIOCYTELONG ind_ref=pgsz_xyz*c+ind_1c;
...
//	saveImage("img_1c.raw",p_img_1c,sz_img_1c,1);
	...
	VISIOCYTELONG sz_img_s[4]={1,1,1,1};//w,h,z,c
...
//	saveImage("/Users/qul/work/visiocyte_2.0/sub_projects/atlasguided_seganno/img_s.raw",p_img_s,sz_img_s,1);

	//resize atlas
	QList<CellAPO> ql_atlasapo_s(ql_atlasapo);
	for(VISIOCYTELONG i=0;i<ql_atlasapo_s.size();i++)
	{
...
//	writeAPO_file("atlas_s.apo",ql_atlasapo);
	...
	for(VISIOCYTELONG i=0;i<ql_atlasapo_s.size();i++)
	{
		...
		for(VISIOCYTELONG j=0;j<ql_celloi.size();j++)
		{
...
//	writeAPO_file("/Users/qul/work/visiocyte_2.0/sub_projects/atlasguided_seganno/atlas_muscle.apo",ql_musclecell);
	...
	if(paras.b_atlasupsidedown)
	{
		...
		for(VISIOCYTELONG i=0;i<ql_musclecell.size();i++)
		{
...
//	writeAPO_file("atlas_muscle.apo",ql_musclecell);
	...
	vector<VISIOCYTELONG> 		vec_fg_ind;
	vector<point3D64F> 	vec_fg_xyz;
	VISIOCYTELONG l_npixel_s=sz_img_s[0]*sz_img_s[1]*sz_img_s[2]*sz_img_s[3];

	{
	//compute the mean and std of image
	double d_img_mean=0.0,d_img_std=0.0;
	for(VISIOCYTELONG i=0;i<l_npixel_s;i++)
...
//	double d_thresh_fg=d_img_mean;
	...
	VISIOCYTELONG pgsz_y=sz_img_s[0];
	VISIOCYTELONG pgsz_xy=sz_img_s[0]*sz_img_s[1];
	for(VISIOCYTELONG x=0;x<sz_img_s[0];x++)
		for(VISIOCYTELONG y=0;y<sz_img_s[1];y++)
			for(VISIOCYTELONG z=0;z<sz_img_s[2];z++)
			{
				VISIOCYTELONG ind=pgsz_xy*z+pgsz_y*y+x;
	...
	//------------------------------------------------------------------------------------------------------------------------------------
	//creat new image window in VISIOCYTE and show the downsampled image
	if(paras.b_showatlas || paras.b_showsegmentation)
	{
		...
		Image4DSimple tmp;
		tmp.setData(newdata1d,sz_img_s[0],sz_img_s[1],sz_img_s[2],sz_img_s[3],VISIOCYTE_UINT8);
	...
	vector< vector<unsigned VISIOCYTELONG> > vec2d_cellpixelgroups(vec_musclecell.size(),vector<unsigned VISIOCYTELONG>());
	double d_celldiameter_max=paras.l_ref_cellradius/paras.d_downsampleratio*2;
	for(unsigned VISIOCYTELONG i=0;i<vec_fg_ind.size();i++)
	{
		//for a given pixel, find the cell index with highest prob
		double d_maxprob=0;
		unsigned VISIOCYTELONG l_maxprob_ind=0;
		for(unsigned VISIOCYTELONG j=0;j<vec_musclecell.size();j++)
		{
...
//	QList<CellAPO> ql_tmp(ql_musclecell);
//	RGBA8 color;	color.r=255; 	color.g=0;	color.b=0;
//	for(VISIOCYTELONG i=0;i<vec_musclecell_affine.size();i++)
//	{
...
////		ql_tmp[i].color=color;
////	}
////	writeAPO_file("/Users/qul/work/visiocyte_2.0/sub_projects/atlasguided_seganno/tmp_noaffine.apo",ql_tmp);
////	}
...
//
//	for(VISIOCYTELONG i=0;i<vec_musclecell_ref.size();i++)
//	{
...
//	}
	...
	RGBA8 color;	color.r=255; 	color.g=0;	color.b=0;
	for(VISIOCYTELONG i=0;i<ql_musclecell_output.size();i++)
	{
...
//	writeAPO_file("/Users/qul/work/visiocyte_2.0/sub_projects/atlasguided_seganno/tmp_final.apo",ql_musclecell_output);

	//------------------------------------------------------------------------------------------------------------------------------------
	printf("(10). Post-precess (force the VP constraint). \n");
	//get muscle cell 4 bundle index
	vector< vector<VISIOCYTELONG> > vec2d_ind_bundlecell(4,vector<VISIOCYTELONG>());	//0:VR, 1:VL, 2:DR, 3:DL
	for(VISIOCYTELONG i=0;i<ql_musclecell_output.size();i++)
	{
	...
	{
		b_swap=0;
		for(VISIOCYTELONG i=0;i<4;i++)
			for(unsigned VISIOCYTELONG j=0;j<vec2d_ind_bundlecell[i].size()-1;j++)
			{
...
bool q_atlas2image_partial(const CParas &paras,VISIOCYTEPluginCallback &callback,
		const unsigned char *p_img8u,const VISIOCYTELONG sz_img[4],const QList<CellAPO> &ql_atlasapo,const QList<QString> &ql_celloi,
		QList<CellAPO> &ql_musclecell_output,COutputInfo &outputinfo)
{
	...
	for(VISIOCYTELONG x=0;x<sz_img[0];x++)
		for(VISIOCYTELONG y=0;y<sz_img[1];y++)
			...
			{
				VISIOCYTELONG ind_1c=pgsz_xy*z+pgsz_y*y+x;
				VISIOCYTELONG ind_ref=pgsz_xyz*(paras.l_refchannel-1)+ind_1c;
...
//	for(VISIOCYTELONG i=0;i<vec_fg_ind.size();i++)
//		p_img_tmp[vec_fg_ind[i]]=255;
//	saveImage("/Users/qul/work/visiocyte_2.0/sub_projects/atlasguided_seganno/img_fg.raw",p_img_tmp,sz_img_s,1);
//	delete []p_img_tmp;
//	}
	...
	if(paras.l_mode==0 || paras.l_mode==1 || paras.l_mode==3)
	{
		...
		for(VISIOCYTELONG x=0;x<nx;x++)
			for(VISIOCYTELONG y=0;y<ny;y++)
				for(VISIOCYTELONG z=0;z<nz;z++)
				{
					point3D64F tmp;
		...
		for(unsigned VISIOCYTELONG i=0;i<vec_musclecell_ref.size();i++)
		{
	...
	unsigned VISIOCYTELONG l_estcellnum=0;
	if(paras.l_mode==0 || paras.l_mode==1 || paras.l_mode==3)
	{
		//group cells and estimate cell number
		VISIOCYTELONG l_cellradius=8;
		double d_mingroupdis=l_cellradius;
		vector< vector<VISIOCYTELONG> > vec2d_groupind;
		...
		for(unsigned VISIOCYTELONG i=0;i<vec2d_groupind.size();i++)
		{
			printf("\tgroup[%ld]: ",i);
			for(unsigned VISIOCYTELONG j=0;j<vec2d_groupind[i].size();j++)
		...
		for(unsigned VISIOCYTELONG i=0;i<vec2d_groupind.size();i++)
		{
			...
			for(unsigned VISIOCYTELONG j=0;j<vec2d_groupind[i].size();j++)
			{
				VISIOCYTELONG ind=vec2d_groupind[i][j];
		...
		if(paras.b_showatlas)
		{
			...
			for(unsigned VISIOCYTELONG i=0;i<vec_musclecell_tmp.size();i++)
			{
			...
			q_push2VISIOCYTE_pts(callback,vec_musclecell_tmp,10);
		}
		...
		if(paras.l_mode==1)	//grid onto image
		{
			...
			tmp.color=color;
			tmp.volsize=50;
			for(unsigned VISIOCYTELONG i=0;i<vec_musclecell_groupavg.size();i++)
			{
	...
	vector<VISIOCYTELONG> vec_muscleind_crop2ori;
	...
	{
		double d_minx=1e+5;
		VISIOCYTELONG l_minind;

		for(unsigned VISIOCYTELONG i=0;i<vec_musclecell.size();i++)
		{
			bool b_existed=0;
			for(unsigned VISIOCYTELONG j=0;j<vec_muscleind_crop2ori.size();j++)
	...
	RGBA8 color;	color.r=255; 	color.g=0;	color.b=0;
	ql_musclecell_output.clear();
	for(unsigned VISIOCYTELONG i=0;i<vec_muscleind_crop2ori.size();i++)
	{
...
//	writeAPO_file("/Users/qul/work/visiocyte_2.0/sub_projects/atlasguided_seganno/tmp_final.apo",ql_musclecell_output);
	...
	vector< vector<VISIOCYTELONG> > vec2d_ind_bundle2cell(4,vector<VISIOCYTELONG>(0,0));
	vector< vector<VISIOCYTELONG> > vec2d_ind_cell2bundle(ql_musclecell_output.size(),vector<VISIOCYTELONG>(2,-1));//0:bundle ind, 1: ind in current bundle
	for(VISIOCYTELONG i=0;i<ql_musclecell_output.size();i++)
	{
		QString qs_cellname=ql_musclecell_output[i].name;
		qs_cellname=qs_cellname.simplified();
	...
	for(unsigned VISIOCYTELONG i=0;i<vec2d_ind_bundle2cell.size();i++)
	{
		if(i==0)	printf("\tbundle[BWMVR]: ");
		if(i==1)	printf("\tbundle[BWMVL]: ");
		if(i==2)	printf("\tbundle[BWMDR]: ");
		if(i==3)	printf("\tbundle[BWMDL]: ");
		for(unsigned VISIOCYTELONG j=0;j<vec2d_ind_bundle2cell[i].size();j++)
	...
	vector< vector<VISIOCYTELONG> > vec2d_annochange;
	for(unsigned VISIOCYTELONG i=0;i<vec2d_groupind.size();i++)
	{
		for(unsigned VISIOCYTELONG j=1;j<vec2d_groupind[i].size();j++)
		{
			VISIOCYTELONG l_indbundle=vec2d_ind_cell2bundle[vec2d_groupind[i][j]][0];			//ind of bundle need to change anno
			VISIOCYTELONG l_indcellinbundle=vec2d_ind_cell2bundle[vec2d_groupind[i][j]][1];		//cell ind in current bundle
			//backward push the anno behind the current cell in current bundle
			vector<VISIOCYTELONG> vec_ind_cur2new(2,-1);	//0: cur ind, 1: new ind
			for(unsigned VISIOCYTELONG k=l_indcellinbundle;k<vec2d_ind_bundle2cell[l_indbundle].size();k++)
			{
	...
	for(unsigned VISIOCYTELONG i=0;i<vec2d_annochange.size();i++)
	...
	for(VISIOCYTELONG i=0;i<ql_musclecell_refine.size();i++)
	{
		...
		vector< vector<VISIOCYTELONG> > &vec2d_groupind)
{
	...
	for(unsigned VISIOCYTELONG i=0;i<vec_cell.size();i++)
	{
		double d_mindis=1e+5;
		VISIOCYTELONG l_groupind_mindis=0;

		for(unsigned VISIOCYTELONG g=0;g<vec2d_groupind.size();g++)
			for(unsigned VISIOCYTELONG k=0;k<vec2d_groupind[g].size();k++)
			{
				VISIOCYTELONG ind=vec2d_groupind[g][k];
		...
		if(d_mindis>d_mingroupdis)		//current cell do not belong to any group
			vec2d_groupind.push_back(vector<VISIOCYTELONG>(1,i));
...
bool q_atlas2image_prior(const CParas &paras,VISIOCYTEPluginCallback &callback,
		const unsigned char *p_img8u,const VISIOCYTELONG sz_img[4],const QList<CellAPO> &ql_atlasapo,const LandmarkList &ml_makers,
		QList<CellAPO> &ql_musclecell_output)
{
	...
	VISIOCYTELONG pgsz_xyz=sz_img[0]*sz_img[1]*sz_img[2];
	printf("\t>>extract channel:[%ld]\n",paras.l_refchannel);
	VISIOCYTELONG ind_start=pgsz_xyz*(paras.l_refchannel-1);
	for(VISIOCYTELONG i=0;i<l_npixels_1c;i++)
	{
		VISIOCYTELONG ind_1c=i;
		VISIOCYTELONG ind_ref=ind_start+ind_1c;
		p_img_1c[ind_1c]=p_img8u[ind_ref];
	}
//	saveImage("img_1c.raw",p_img_1c,sz_img_1c,1);
	}
...
//	}
	...
	vector<VISIOCYTELONG> vec_ind_anchor2atlas(ml_makers_s.size(),-1);

	{
	//find the atals points position that corresponding to the anchor points
	for(VISIOCYTELONG i=0;i<ml_makers_s.size();i++)
	{
		...
		for(VISIOCYTELONG j=0;j<ql_musclecell.size();j++)
		{
	...
	VISIOCYTELONG n_anchor=ml_makers_s.size();
	//fill matrix A
	//
	//	  | sx, 0 , 0 , cx |    |x1|
	...
	VISIOCYTELONG row=1;
	for(VISIOCYTELONG i=0;i<n_anchor;i++)
	{
...
//	writeAPO_file("/Users/qul/work/visiocyte_2.0/sub_projects/atlasguided_seganno/atlas_rigid.apo",ql_musclecell_rigid);
	}
	...
	for(VISIOCYTELONG i=0;i<ql_musclecell_rigid.size();i++)
	{
		point3D64F tmp;
	...
	for(VISIOCYTELONG iter=0;iter<5000;iter++)
	{
		...
		double d_maxdis=0;
		for(unsigned VISIOCYTELONG i=0;i<vec_pts_atlas_rigid.size();i++)
			for(unsigned VISIOCYTELONG j=0;j<vec_pts_anchor.size();j++)
			{
		...
		for(unsigned VISIOCYTELONG i=0;i<vec_pts_anchor.size();i++)
		{
			...
			for(unsigned VISIOCYTELONG j=0;j<vec_pts_atlas_rigid.size();j++)
			{
...
//	writeAPO_file("/Users/qul/work/visiocyte_2.0/sub_projects/atlasguided_seganno/atlas_nonrigid.apo",ql_musclecell_nonrigid);
	}
	...
	vector<VISIOCYTELONG> vec_ind_valid2atlas;

	{
	//find the valid atlas points (within image area)
	vector<VISIOCYTELONG> vec_ind_anchor2valid(ml_makers_s.size(),-1);
	for(unsigned VISIOCYTELONG i=0;i<vec_pts_atlas_nonrigid.size();i++)
	{
		   ...
		   vec_pts_atlas_nonrigid[i].z>=0 && vec_pts_atlas_nonrigid[i].z<sz_img_s[2])
		{
			...
			for(VISIOCYTELONG j=0;j<ml_makers_s.size();j++)
	...
	for(unsigned VISIOCYTELONG i=0;i<vec_musclecell_valid_ref.size();i++)
	{
	...
	for(VISIOCYTELONG i=0;i<ql_musclecell_ref_mshift.size();i++)
	{
...
//			vec_musclecell_valid_mshift))
	...
	for(unsigned VISIOCYTELONG i=0;i<vec_musclecell_valid_mshift.size();i++)
	{
...
//	writeAPO_file("/Users/qul/work/visiocyte_2.0/sub_projects/atlasguided_seganno/atlas_ref.apo",ql_musclecell_output);
...
bool q_atlas2image_musclecell_ini_affine(const CParas &paras,VISIOCYTEPluginCallback &callback,
		const unsigned char *p_img8u,const VISIOCYTELONG sz_img[4],const vector<point3D64F> &vec_musclecell,
		const vector<VISIOCYTELONG> &vec_fg_ind,const vector<point3D64F> &vec_fg_xyz,
		vector<point3D64F> &vec_musclecell_output,vector< vector<double> > &vec2d_labelprob,COutputInfo &outputinfo)
{
	...
	VISIOCYTELONG n_maxiter_inner=paras.l_niter_pertemp;
	double d_lamda=5000;
	...
	bool b_stopiter=0;
	for(VISIOCYTELONG iter=0;iter<1000;iter++)
	{
		for(VISIOCYTELONG iter_inner=0;iter_inner<n_maxiter_inner;iter_inner++)
		{
//			vector< vector<double> > vec2d_fitcost(vec_fg_ind.size(),vector<double>(vec_musclecell.size(),0));//for fit and entropy energy
			...
			for(unsigned VISIOCYTELONG i=0;i<vec_musclecell.size();i++)
				for(unsigned VISIOCYTELONG j=0;j<vec_fg_ind.size();j++)
				{
...
//			}
			...
			for(unsigned VISIOCYTELONG i=0;i<vec_musclecell_affine.size();i++)
			{
				x_ori(i+1,1)=1.0;
...
//		printf("[%4d]: T=%f, poschange=%f, energy=%f\n",iter,d_T,d_totalposchange,d_energy);
		...
		if(paras.b_showatlas && !paras.b_showsegmentation)
		{
			...
			VISIOCYTELONG l_makradius=10;
			q_push2VISIOCYTE_pts(callback,vec_musclecell_affine,l_makradius);
		}
		else if(paras.b_showsegmentation)
		{
			...
			NeuronSWC ns;
			ns.r=1;
			for(unsigned VISIOCYTELONG i=0;i<vec2d_cellpixelgroups.size();i++)
			{
				...
				VISIOCYTELONG parentind=nt_cell2validpix.listNeuron.size()-1;
				for(unsigned VISIOCYTELONG j=0;j<vec2d_cellpixelgroups[i].size();j++)
				{
					ns.n=nt_cell2validpix.listNeuron.size();
					ns.type=2;
...
bool q_atlas2image_musclecell_ref_DAWC(const CParas &paras,VISIOCYTEPluginCallback &callback,
		const unsigned char *p_img8u,const VISIOCYTELONG sz_img[4],const vector<point3D64F> &vec_musclecell,
		const vector<VISIOCYTELONG> &vec_fg_ind,const vector<point3D64F> &vec_fg_xyz,
		vector<point3D64F> &vec_musclecell_output)
{
	...
	VISIOCYTELONG l_cellradius=paras.l_ref_cellradius/paras.d_downsampleratio+0.5;
	VISIOCYTELONG l_maxiter=paras.l_ref_maxiter;
	...
	for(VISIOCYTELONG iter=0;iter<l_maxiter;iter++)
	{
		if(paras.b_showatlas) q_push2VISIOCYTE_pts(callback,vec_musclecell_tmp,10);
...
//		if(paras.b_showinVISIOCYTE_pts)
//			for(VISIOCYTELONG o=0;o<10;o++)
//				QCoreApplication::processEvents();
...
//		callback.screenShot3DWindow(curwin, BMPfilename);
	...
	if(paras.b_showatlas) q_push2VISIOCYTE_pts(callback,vec_musclecell_output,10);

	return true;
}

bool q_atlas2image_musclecell_ref_DAWC_prior(const CParas &paras,VISIOCYTEPluginCallback &callback,
		const unsigned char *p_img8u,const VISIOCYTELONG sz_img[4],const vector<point3D64F> &vec_musclecell,const vector<VISIOCYTELONG> vec_ind_anchor,
		const vector<VISIOCYTELONG> &vec_fg_ind,const vector<point3D64F> &vec_fg_xyz,
		vector<point3D64F> &vec_musclecell_output)
{
	//check paras
...
//				d_sum_z+=vec2d_labelprob[j][i]*vec_fg_xyz[j].z;
		...
		for(unsigned VISIOCYTELONG i=0;i<vec_ind_anchor.size();i++)
		{
...
bool q_atlas2image_musclecell_ref_DATPS(const CParas &paras,VISIOCYTEPluginCallback &callback,
		const unsigned char *p_img8u,const VISIOCYTELONG sz_img[4],const vector<point3D64F> &vec_musclecell,
		const vector<VISIOCYTELONG> &vec_fg_ind,const vector<point3D64F> &vec_fg_xyz,
		vector<point3D64F> &vec_musclecell_output)
{
...
//	n_maxiter_inner=5;
	for(VISIOCYTELONG iter=0;iter<100;iter++)
	{
		for(VISIOCYTELONG iter_inner=0;iter_inner<n_maxiter_inner;iter_inner++)
		{
			//------------------------------------------------------------------
...
//				break;
		...
		if(paras.b_showatlas) q_push2VISIOCYTE_pts(callback,vec_musclecell_affine,10);
...
//refine the warped atlas (LP: locally horizental 3/4 dulside perturbation)
bool q_atlas2image_musclecell_ref_LHHV(const CParas &paras,VISIOCYTEPluginCallback &callback,
		const unsigned char *p_img8u,const VISIOCYTELONG sz_img[4],const vector<point3D64F> &vec_musclecell,
		const vector<VISIOCYTELONG> &vec_fg_ind,const vector<point3D64F> &vec_fg_xyz,
		vector<point3D64F> &vec_musclecell_output)
{
	...
	vector< vector<VISIOCYTELONG> > vec2d_musclecellind_4strip;
	//VL
	VISIOCYTELONG tmp_VL[]={3,4,10,14,17,23,27,29,33,37,42,46,50,55,57,58,69,72,75};
	vector<VISIOCYTELONG> vec_musclecellind_VL(tmp_VL,tmp_VL+sizeof(tmp_VL)/sizeof(VISIOCYTELONG));
	vec2d_musclecellind_4strip.push_back(vec_musclecellind_VL);
	//VR
	VISIOCYTELONG tmp_VR[]={0,7,11,12,19,20,24,28,32,39,41,45,51,52,56,59,66,70,74,78};
	vector<VISIOCYTELONG> vec_musclecellind_VR(tmp_VR,tmp_VR+sizeof(tmp_VR)/sizeof(VISIOCYTELONG));
	vec2d_musclecellind_4strip.push_back(vec_musclecellind_VR);
	//DL
	VISIOCYTELONG tmp_DL[]={1,6,9,13,18,21,26,30,34,38,40,47,48,54,61,62,65,68,73,77,80};
	vector<VISIOCYTELONG> vec_musclecellind_DL(tmp_DL,tmp_DL+sizeof(tmp_DL)/sizeof(VISIOCYTELONG));
	vec2d_musclecellind_4strip.push_back(vec_musclecellind_DL);
	//DR
	VISIOCYTELONG tmp_DR[]={2,5,8,15,16,22,25,31,35,36,43,44,49,53,60,63,64,67,71,76,79};
	vector<VISIOCYTELONG> vec_musclecellind_DR(tmp_DR,tmp_DR+sizeof(tmp_DR)/sizeof(VISIOCYTELONG));
	...
	double d_avgdis=0;	VISIOCYTELONG n_dis=0;
	for(unsigned VISIOCYTELONG i=0;i<vec2d_musclecellind_4strip.size();i++)
	{
		vec_dis.clear();
		for(unsigned VISIOCYTELONG j=0;j<vec2d_musclecellind_4strip[0].size()-1;j++)
		{
			VISIOCYTELONG ind1=vec2d_musclecellind_4strip[i][j];
			VISIOCYTELONG ind2=vec2d_musclecellind_4strip[i][j+1];
...
//	for(VISIOCYTELONG i=0;i<vec_musclecell_best.size();i++)
//	{
...
//	}
//	writeAPO_file("/Users/qul/work/visiocyte_2.0/sub_projects/atlasguided_seganno/tmp_best.apo",ql_tmp);}

	//------------------------------------------------------------------------------------------------------------------------------------
	//shift every cell to its 3/4 left and right side
	//the shift of the neighbor cells satisfy Gaussian distribution
	for(unsigned VISIOCYTELONG strip=0;strip<vec2d_musclecellind_4strip.size();strip++)
	{
		for(unsigned VISIOCYTELONG i=1;i<vec2d_musclecellind_4strip[strip].size()-1;i++)
		{
			VISIOCYTELONG ind=vec2d_musclecellind_4strip[strip][i];
			for(VISIOCYTELONG lr=0;lr<2;lr++)//0:left, 1:right
			{
...
//				double pi=3.1415926;
				for(unsigned VISIOCYTELONG j=0;j<vec2d_musclecellind_4strip[strip].size();j++)
				{
					VISIOCYTELONG ind2=vec2d_musclecellind_4strip[strip][j];
...
//				q_push2VISIOCYTE_pts(callback,vec_musclecell_tmp);
				...
				VISIOCYTELONG l_radius=paras.l_ref_cellradius/paras.d_downsampleratio+0.5;
				if(!q_compute_energy(p_img8u,sz_img,vec_musclecell_ref,
						vec_fg_ind,vec_fg_xyz,
						l_radius,
						d_energy))
...
bool q_atlas2image_musclecell_ref_mshift(const CParas &paras,VISIOCYTEPluginCallback &callback,
		const unsigned char *p_img8u,const VISIOCYTELONG sz_img[4],const vector<point3D64F> &vec_musclecell,
		vector<point3D64F> &vec_musclecell_output)
{
	//check paras
	...
	VISIOCYTELONG l_wndradius=paras.l_ref_cellradius/2.0+0.5;
	VISIOCYTELONG l_maxiter=100;
	double d_mintotalposchannge=0.001;

	//build sphere xyz range array
	vector<point3D64F> vec_sphereoffset;
	for(VISIOCYTELONG x=-l_wndradius;x<=l_wndradius;x++)
		for(VISIOCYTELONG y=-l_wndradius;y<=l_wndradius;y++)
			for(VISIOCYTELONG z=-l_wndradius;z<=l_wndradius;z++)
			{
	...
	for(VISIOCYTELONG iter=0;iter<l_maxiter;iter++)
	{
		for(unsigned VISIOCYTELONG i=0;i<vec_musclecell.size();i++)
		{
			...
			point3D64F pt_new;
			for(unsigned VISIOCYTELONG j=0;j<vec_sphereoffset.size();j++)
			{
				...
				VISIOCYTELONG x_s,x_b,y_s,y_b,z_s,z_b;
	...
	if(paras.b_showatlas)
	{
		...
		for(unsigned VISIOCYTELONG i=0;i<vec_musclecell_output.size();i++)
		{
			vec_musclecell_tmp[i].x/=paras.d_downsampleratio;
			vec_musclecell_tmp[i].y/=paras.d_downsampleratio;
			vec_musclecell_tmp[i].z/=paras.d_downsampleratio;
...
bool q_atlas2image_musclecell_ref_mshift_adaptive(const CParas &paras,VISIOCYTEPluginCallback &callback,
		const unsigned char *p_img8u,const VISIOCYTELONG sz_img[4],const vector<point3D64F> &vec_musclecell,const double d_thresh_fg,
		vector<point3D64F> &vec_musclecell_output)
{
	//check paras
	if(p_img8u==0)
	...
	VISIOCYTELONG l_wndradius=paras.l_ref_cellradius/2+0.5;
	VISIOCYTELONG l_wndradius_bk=l_wndradius;
	VISIOCYTELONG l_wnddiameter=l_wndradius_bk*2+1;
	VISIOCYTELONG l_wndvolume=l_wnddiameter*l_wnddiameter*l_wnddiameter;

	//reposition the cells to the mass center of the corresponding sphere
	vector<point3D64F> vec_musclecell_new(vec_musclecell),vec_musclecell_last(vec_musclecell);
	VISIOCYTELONG pgsz_y=sz_img[0];
	VISIOCYTELONG pgsz_xy=sz_img[0]*sz_img[1];
	for(VISIOCYTELONG i=0;i<vec_musclecell.size();i++)
	{
		VISIOCYTELONG l_iter=0;
		while(l_iter<l_maxiter)
		{
			double d_intensitysum=0;
			double d_intensitymax=0;
			VISIOCYTELONG l_nfgpts=0;
			point3D64F pt_new;
			for(VISIOCYTELONG xx=-l_wndradius;xx<=l_wndradius;xx++)
				for(VISIOCYTELONG yy=-l_wndradius;yy<=l_wndradius;yy++)
					for(VISIOCYTELONG zz=-l_wndradius;zz<=l_wndradius;zz++)
					{
...
bool q_compute_energy(const unsigned char *p_img8u,const VISIOCYTELONG sz_img[4],const vector<point3D64F> &vec_musclecell,
		const vector<VISIOCYTELONG> &vec_fg_ind,const vector<point3D64F> &vec_fg_xyz,
		const VISIOCYTELONG &l_radius,
		double &d_energy)
{
	//check paras
	if(p_img8u==0)
...
bool q_compute_rigidmatrix_3D(const vector<point3D64F> &vec_A,const vector<point3D64F> &vec_B,Matrix &x4x4_rigidmatrix)
{
	...
	for(VISIOCYTELONG i=0;i<n_point;i++)
	{
		...
		Matrix &x4x4_d,Matrix &xnx4_c,Matrix &xnxn_K)
{
	...
	VISIOCYTELONG n_pts=vec_sub.size();
	...
	for(VISIOCYTELONG i=0;i<n_pts;i++)
	{
		Q(i+1,1)=X(i+1,1)=1;
...
//		if(!b_flipdetected)
//		{
//			visiocyte_msg("miss detect flip\n");
//		}
...
bool q_TPS_k(const vector<point3D64F> &vec_sub,const vector<point3D64F> &vec_basis,Matrix &xmxn_K)
{
	...
	//compute K=-r=-|xi-xj|
	double d_x,d_y,d_z;
	for(unsigned VISIOCYTELONG i=0;i<vec_sub.size();i++)
		for(unsigned VISIOCYTELONG j=0;j<vec_basis.size();j++)
		{
...
bool q_push2VISIOCYTE_pts(VISIOCYTEPluginCallback &callback,vector<point3D64F> &vec_pts,VISIOCYTELONG l_makradius)
{
	//show deformed atlas pts in VISIOCYTE
	visiocytehandle curwin = callback.currentImageWindow();
	callback.open3DWindow(curwin);
		...
		Matrix &wa)
{
	...
	for(VISIOCYTELONG j=0;j<n_marker;j++)
	  for(VISIOCYTELONG i=0;i<n_marker;i++)
	  {
		s=0.0;
		...
		point3D64F &pt_sub2tar)
{
	...
	for(VISIOCYTELONG n=0;n<n_marker;n++)
	{
		s=0;
	...
	s=0;
	for(VISIOCYTELONG p=0;p<n_marker+4;p++)	s+=Ua[p]*wa_sub2tar(p+1,1);
	pt_sub2tar.x=s;

	s=0;
	for(VISIOCYTELONG p=0;p<n_marker+4;p++)	s+=Ua[p]*wa_sub2tar(p+1,2);
	pt_sub2tar.y=s;

	s=0;
	for(VISIOCYTELONG p=0;p<n_marker+4;p++)	s+=Ua[p]*wa_sub2tar(p+1,3);
	pt_sub2tar.z=s;
...
bool q_align_dapicells(const CParas &paras,VISIOCYTEPluginCallback &callback,
		const unsigned char *p_img8u,const VISIOCYTELONG sz_img[4],const QList<CellAPO> &ql_atlasapo,const QList<QString> &ql_celloi,
		QList<CellAPO> &ql_musclecell_output,COutputInfo &outputinfo)
{
	//------------------------------------------------------------------------------------------------------------------------------------
...
//	writeAPO_file("atlas_s.apo",ql_atlasapo);
	...
	vector< vector <VISIOCYTELONG> > vec2d_muslcecell_ind;
	for(VISIOCYTELONG i=0;i<ql_atlasapo_s.size();i++)
	{
		QString qs_cellname=ql_atlasapo_s[i].name;
		...
		{
			vector<VISIOCYTELONG> vec_muslcecell_ind(2,0);
...
//	}
	...
	for(VISIOCYTELONG i=0;i<ql_insterestcell.size();i++)
	...
	VISIOCYTELONG n_maxiter_inner=1;
	double d_lamda=0;
	...
	for(VISIOCYTELONG iter=0;iter<1000;iter++)
	{
		for(VISIOCYTELONG iter_inner=0;iter_inner<n_maxiter_inner;iter_inner++)
		{
			...
			for(VISIOCYTELONG i=0;i<vec2d_muslcecell_ind.size();i++)
			{
...
//		if(d_T<=d_T_min) d_lamda*=d_annealingrate;
	...
	//+(arr_downratio_actual[0]+0.5) is for compensate the downsample error (image origin offset error)
	RGBA8 color;	color.r=255; 	color.g=0;	color.b=0;
	for(VISIOCYTELONG i=0;i<ql_interestcell_output.size();i++)
	{
