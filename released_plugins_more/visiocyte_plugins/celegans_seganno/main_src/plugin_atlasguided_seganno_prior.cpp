...
void AtlasGuidedSegAnnoPrior(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
...
void AtlasGuidedSegAnnoPriorPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
void AtlasGuidedSegAnnoPrior(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	if(paraDlg.exec()==QDialog::Accepted)
	{
		...
		if(ml_makers.size()<3)
		{
			visiocyte_msg(QString("Need at least 3 markers!"));
			if(p_img_input && !paras.b_imgfromVISIOCYTE) 		{delete []p_img_input;		p_img_input=0;}
			if(sz_img_input)						 	{delete []sz_img_input;		sz_img_input=0;}
			return;
		}
