...
void AtlasGuidedSegAnnoCombine(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
...
void AtlasGuidedSegAnnoCombinePlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
void AtlasGuidedSegAnnoCombine(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	if(paraDlgCombine.exec()==QDialog::Accepted)
	{
		...
		//visualization
		paras.b_showinVISIOCYTE_pts=paraDlgCombine.checkBox_showVISIOCYTE_pts->isChecked();
		paras.b_showinVISIOCYTE_img=paras.b_showinVISIOCYTE_pts;
		//initial align
		...
		printf("1. Import image and markers from VISIOCYTE. \n");
		long l_imgind=paraDlgCombine.comboBox_imgvisiocyte->currentIndex();
		if(l_imgind<0)
		{
		...
		if(datatype_input!=1)
		{
			visiocyte_msg(QString("Image type need to be UINT8!"));
		...
		{
			LocationSimple s;
			if(b_annomethod==0 && paras.b_showinVISIOCYTE_pts==0)
			{
