...
bool prune_branch(NeuronTree nt, NeuronTree & result, double prune_size)
{
	...
	vector<VISIOCYTELONG> branches(siz,0); //number of branches on the pnt: 0-tip, 1-internal, >=2-branch
...
double calculate_diameter(NeuronTree nt, vector<VISIOCYTELONG> branches)
{
	VISIOCYTELONG siz = nt.listNeuron.size();
	vector<vector<double> > longest_path(siz, vector<double>(2,0));//the 1st and 2nd longest path to each node in a rooted tree
	vector<VISIOCYTELONG> chd(siz, -1);//immediate child of the current longest path
	for (VISIOCYTELONG i=0;i<siz;i++)
	{
		if (branches[i]!=0) continue;
		VISIOCYTELONG cur = i;
