...
#ifdef DISABLE_VISIOCYTE_MSG
	#define DISABLE_VISIOCYTE_MSG
#endif
...
int global_retrieve_main(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output)
{
	...
	vector<vector<VISIOCYTELONG> > retrieved_all;

	for  (int i=0;i<feature_codes.size();i++)
	{
		for (int j=0;j<norm_codes.size();j++)
		{
			vector<VISIOCYTELONG> retrieved_tmp;
	...
	vector<VISIOCYTELONG>	retrieved_id;
...
void printHelp_global_retrieve()
{
	...
	printf("Example: visiocyte -x blastneuron -f global_retrieve -p \"#d myfeaturebase.nfb #q query.swc #n 10 #o result.ano #m 1,2,4\"\n\n");
}
