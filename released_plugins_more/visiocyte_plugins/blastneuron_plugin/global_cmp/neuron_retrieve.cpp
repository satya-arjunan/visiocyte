...
bool neuron_retrieve(NeuronTree query, QList<double*> & feature_list, vector<VISIOCYTELONG>  & result, VISIOCYTELONG cand, int method_code, int norm_code)
{
	...
	VISIOCYTELONG neuronNum = feature_list.size();
	...
	for (VISIOCYTELONG i=0;i<cand;i++)
...
bool read_matrix(const char* file_name, QList<double*>&  matrix, VISIOCYTELONG line, VISIOCYTELONG width)
{
	...
	for (VISIOCYTELONG i=0;i<line;i++)
	{
		double * new_feature = new double[width];
		for (VISIOCYTELONG j=0;j<width;j++)
		{
			float tmp;
			fscanf(fp, "%f", &tmp);
			new_feature[j] = tmp;
...
bool compute_intersect(vector<vector<VISIOCYTELONG> > & list, vector<VISIOCYTELONG> & result, int number, int rej_thres)
{
	result.clear();
	VISIOCYTELONG stratNum = list.size();
	VISIOCYTELONG neuronNum = list[0].size();
	...
	for (VISIOCYTELONG i=0;i<neuronNum;i++)
		rank_sum[i] = 0;
	for (VISIOCYTELONG i=0;i<neuronNum;i++)
		for (VISIOCYTELONG j=0;j<stratNum;j++)
		{
			int ind = list[j][i];
	...
	pick_max_n(idx, rank_sum, number, neuronNum, score);
	*/
/*	for (VISIOCYTELONG siz=1;siz<=l1.size();siz++)
	{
		result.clear();
		for (VISIOCYTELONG i=0;i<siz;i++)
		{
			for (VISIOCYTELONG j=0;j<siz;j++)
			{
				if (l1[i]==l2[j])
				{
