...
void BlastNeuronPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	visiocyte_msg(tr("BlastNeuron plugin for rapid retrieve and alignment of neuron morphologies, also includes pre-processing, inverse projection and batch feature computation.\n\nDomenu is not supported yet. Please run the program from either of the following ways:\n1. Run indivisual function from dofunc: type \"visiocyte -x blastneuron -f help\" for detailed instruction\n2. Run a query search from the neuron feature database: try \"sh blastneuron_plugin.sh\" for detailed instruction\n\nDeveloped by Yinan Wan, 2016-06-03"));
}

bool BlastNeuronPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
	...
	else
	{
		...
		printf("Example: \n");
        printf("\t visiocyte -x blastneuron -f pre_processing -p  \"#i input.swc #o result.swc #l 3 #s 2 #r 1 \"\n");
		printf("\t visiocyte -x blastneuron -f batch_compute -p \"#i mydatabase.ano #o features.nfb\"\n");
		printf("\t visiocyte -x blastneuron -f global_retrieve -p \"#d features.nfb #q query.swc #n 10 #m 1,2,4 #o retrieve.ano\"\n");
		printf("\t visiocyte -x blastneuron -f local_alignment -p \"#t target.swc #s subject.swc #o match.swc\"\n");
		printf("\t visiocyte -x blastneuron -f inverse_projection -p \"#i target.swc #o target_invp.swc\"\n\n");
		printf("Type visiocyte -x blastneuron -f <function_name> for instructions to run individual functions.\n");
