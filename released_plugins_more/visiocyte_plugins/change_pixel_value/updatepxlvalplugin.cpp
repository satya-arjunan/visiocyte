...
void UpdatePxlValPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
bool UpdatePxlValPlugin::dofunc(const QString &func_name, const VISIOCYTEPluginArgList &input, VISIOCYTEPluginArgList &output, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    {
        cout<<"Usage : visiocyte -x dllname -f change_pixel_value -i <inimg_file> -o <outimg_file> -p <nx> <ny> <nz> <chval1> [<chval2> ...] "<<endl;
        cout<<endl;
        ...
        cout<<endl;
        cout<<"e.g. visiocyte -x dll_name -f change_pixel_value -i input.raw -o output.raw -p 1 2 3 0"<<endl;
        cout<<endl;
        return true;
    }
}
...
bool processImage(VISIOCYTEPluginCallback2 &callback, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output)
{
    ...
    VISIOCYTELONG val_num = paras.size()-3;
    if(val_num>=sc)
    {
        for(int i=0; i<sc; i++)
        {
    ...
    for(VISIOCYTELONG c=0; c<sc; c++)
    {
        ...
        VISIOCYTELONG indLoop = offsetc + nz*M*N + ny*N + nx;
...
void ChangePxlValDialog::update()
{
    ...
    {
        VISIOCYTELONG offsetc = c*pagesz;
        VISIOCYTELONG ind = offsetc + nz*M*N + ny*N + nx;
