...
void GNFPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
		visiocyte_msg("(version 2.0) Compute global features for a certain neuron. Developed by Yinan Wan, 2012-02-20.");
	}
	return;
}


bool GNFPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback, QWidget * parent)
{	
	...
	if (func_name==tr("help"))
	{
		...
		cout<<"Usage: visiocyte -x global_neuron_feature -f compute_feature -i test.swc \n"<<endl;
		return true;
	}
