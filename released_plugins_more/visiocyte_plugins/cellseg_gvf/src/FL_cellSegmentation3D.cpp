...
bool FL_cellseg(Vol3DSimple <unsigned char> *img3d, Vol3DSimple <unsigned short int> *outimg3d, const segParameter & segpara)
{
	...
	VISIOCYTELONG data_sz[3];
	data_sz[0] = img3d->sz0(); data_sz[1] = img3d->sz1(); data_sz[2] = img3d->sz2();
	VISIOCYTELONG len = data_sz[0] * data_sz[1]*data_sz[2];
	...
	if (segpara.adpatThreMethod == 0) // method 0: local adaptive thresholding 
	{
		...
		VISIOCYTELONG kernelsz[3], kernelstp[3];
	...
	else
	{
		...
		else 
		{
			if (segpara.adpatThreMethod == 2) // method 2: global threshold in each 2D slice, fix problematic threshold on the top and bottom slices
			{
				...
				float *hist = 0;
				VISIOCYTELONG *pp = new VISIOCYTELONG [data_sz[2]]; //initial threshold of each 2D slice
				VISIOCYTELONG *qq = new VISIOCYTELONG [data_sz[2]]; //fixed threshold of each 2D slice

				VISIOCYTELONG i,j,k;
				VISIOCYTELONG len1 = data_sz[0]*data_sz[1];
				...
				VISIOCYTELONG starti, endi;
				VISIOCYTELONG startmax = -999, endmax = -999;
				
				
				for (i=0; i<(data_sz[2]-1)/3; i++)
				{	
					VISIOCYTELONG tmp = abs(double(pp[i+1]-pp[i])); //2010-05-20. by PHC. force convert from VISIOCYTELONG to double for MSVC complier
					if (tmp>startmax) 
					{
						startmax = tmp;
						starti = i;
				...
				if (p_num>n_num)
				{
                    for (i=(VISIOCYTELONG)(double(data_sz[2]-1)*2/3 + 0.5); i<data_sz[2]-1; i++)
					{	
						VISIOCYTELONG tmp = pp[i+1]-pp[i];
						if ((abs(double(tmp))>endmax)&(tmp<0))  //2010-05-20. by PHC. force convert from VISIOCYTELONG to double for MSVC complier
						{
							endmax = abs(double(tmp));  //2010-05-20. by PHC. force convert from VISIOCYTELONG to double for MSVC complier
							endi = i;
						}
					}
				}
				...
				VISIOCYTELONG lowestThre = (VISIOCYTELONG) (mean+std);
	...
	if (segpara.segMethod == 1) //distance transform + shape-based watershed  
	{
		...
		VISIOCYTELONG *pixindex = new VISIOCYTELONG [len];
	...
	else //intensity-based watershed on thresholded image
	{
		...
		{	
			if (outdata1d[i] ==0) //outdata1d is the output of the thresholding step
				...
				tmpdata1d[i] = (maxval+10) - sqrt(double(tmpdata1d[i]));  //2010-05-20. by PHC. force convert from VISIOCYTELONG to double for MSVC complier
		}
