...
// parse array input
void parse_Radii_Paremeter(char * optarg, double * &radius, VISIOCYTELONG & len_Radii)
{
	...
	while (k<n)
	{
		...
		for (int i=cb;i<=len-2;i++)
		{
			if (optarg[i]==',')
			{
				...
				if (ce-cb+1>3)
				{
					printf("You have specified a number with more than 3 digits VISIOCYTELONG, which is unlikely the correct input. Exit!");
					exit(1);
				}
				else
				{
...
////	segpara.method = 1;
	...
	VISIOCYTELONG * sz = 0; /* note that this variable must be initialized as NULL. */
	int datatype = 0;
...
//	dfactor[0] = 2; dfactor[1]=2; dfactor[2] = 2;
	...
	//VISIOCYTELONG *data_sz = new VISIOCYTELONG [3];
	data_sz = new VISIOCYTELONG [3];
...
//	b_res = 1;
	...
	// overlay boundary of segmenation result on top of the original image, generating a 4d stack for visiocyte to visualize
