...
template <class T1> bool min_and_max(T1 * vec1d, VISIOCYTELONG cnt, T1 & minval, T1 &maxval)
{
	if (!vec1d || cnt<0)
		return false;
...
template <class T1, class T2> bool adaptiveThre3d(T1 ***indata3d, T2 *** outdata3d, VISIOCYTELONG *sz, VISIOCYTELONG * kernelsz, VISIOCYTELONG * kernelstp)
{
	
	VISIOCYTELONG i,j,k;
	VISIOCYTELONG m,n,p;
		
	unsigned VISIOCYTELONG len, cnt, cnt0;
	VISIOCYTELONG rr[3];
	...
	VISIOCYTELONG cntk, cntj, cnti;
	VISIOCYTELONG m1, m2, n1, n2, p1, p2;
	
	VISIOCYTELONG szsmall[3];
	...
	double dfactor[3];	
	VISIOCYTELONG sznew[3];

	for (i=0; i<3; i++)
	{
		dfactor[i] = (double)sz[i]/(double)szsmall[i];
		sznew[i] = VISIOCYTELONG(ceil(dfactor[i]*szsmall[i]));
	}
	...
	VISIOCYTELONG k1 = sz[2]<sznew[2]?sz[2]:sznew[2];
	VISIOCYTELONG j1 = sz[1]<sznew[1]?sz[1]:sznew[1];
	VISIOCYTELONG i1 = sz[0]<sznew[0]?sz[0]:sznew[0];
...
template <class T1, class T2> bool adaptiveThre3d(Vol3DSimple <T1> * inimg, Vol3DSimple <T2> * outimg, VISIOCYTELONG * kernelsz, VISIOCYTELONG * kernelstp)
{
...
//	VISIOCYTELONG m,n,p;
//		
//	VISIOCYTELONG *sz = new VISIOCYTELONG [3];
//	sz[0] = inimgsmall->sz0();  
//	sz[1] = inimgsmall->sz1();
//	sz[2] = inimgsmall->sz2(); 
//
//	unsigned VISIOCYTELONG len, cnt, cnt0;
//	VISIOCYTELONG rr=(2*kernelsz+1);
//	len = rr*rr*rr;
//		
//	
//	float *vec1d = new float[len];	
...
//	
//	for (VISIOCYTELONG i=0; i< outimg->sz0()*outimg->sz1()*outimg->sz2(); i++)
//	{
