...
template< class T > class Sequence
{
 private:
    ...
    T *fArray;
    VISIOCYTELONG fSequence_length; 
    VISIOCYTELONG fArray_length;    
    VISIOCYTELONG fHead;            // item # of start of sequence

    void expand();
    inline T *getItemAddress(VISIOCYTELONG i);
    inline T get(VISIOCYTELONG i);
 ...
 public:
    ...
    VISIOCYTELONG getSequenceLength() 
    { 
        return fSequence_length; 
    }

    void initialize(VISIOCYTELONG h);	
    inline void setItem(VISIOCYTELONG i, T x);
    inline T getItem(VISIOCYTELONG i); 
...
template< class T> inline T *Sequence<T>::getItemAddress(VISIOCYTELONG i)
{
	if (fArray==0)
	{
...
{	
    ...
    VISIOCYTELONG old_length;
    T *newArray;

    old_length = fArray_length;
    VISIOCYTELONG new_length = 2 * fArray_length;
...
template<class T> inline T Sequence<T>::get(VISIOCYTELONG i)
{
	if (fArray == NULL)
...
template< class T> void Sequence<T>::initialize(VISIOCYTELONG hint)
{
     if (hint < 0)
	 {
...
template<class T> inline T Sequence<T>::getItem(VISIOCYTELONG i)
{
...
template<class T> inline void Sequence<T>::setItem(VISIOCYTELONG i, T x)
{
	if (fArray == NULL)
	{
...
template<class T> inline void Sequence<T>::insertAtEnd(T x)
{
    ...
    VISIOCYTELONG len = fSequence_length++;
    setItem(len, x);
}
...
template<class T> void Sequence<T>::copyToBuf(T *a)
{
    ...
    {
        for (VISIOCYTELONG k = 0; k < fSequence_length; k++)
        {
            a[k] = getItem(k);
        }
    }
