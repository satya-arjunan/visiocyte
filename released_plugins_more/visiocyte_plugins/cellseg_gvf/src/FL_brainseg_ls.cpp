...
void rectifyShape(unsigned short int **&ulabel, float **u, VISIOCYTELONG *sznew, VISIOCYTELONG m, unsigned short int ***labelimgdata3d, float **utmpdata2d)
{
...
//	}
	...
	VISIOCYTELONG tmplen = sznew[0] * sznew[1];
	for (i=0; i<tmplen; i++)
	{
...
//				rgnprops = regionProps(ulabel, sznew);
		...
		for (i=0;i<maxval; i++)
		{
			VISIOCYTELONG thisarea = rgnprops.getArea(ulabel,i+1,sznew);
//					printf("thisarea = %d\n", thisarea);
				...
				Vol3DSimple <unsigned short int> *outimg3d, const levelset_segParameter & segPara)
{
	...
	VISIOCYTELONG *pixindex = new VISIOCYTELONG [sznew[0]*sznew[1]]; //used in distance transform
    ...
    unsigned char **overlaprgn = new unsigned char * [(VISIOCYTELONG)(segPara.regionnum)]; //added by PHC, 2010-05-20
	unsigned char * overlaprgn1d = new unsigned char [(VISIOCYTELONG)(segPara.regionnum) * segPara.regionnum]; //added by PHC, 2010-05-20
	for (i=0;i<(VISIOCYTELONG)(segPara.regionnum);i++) //added by PHC, 2010-05-20
		overlaprgn[i] = overlaprgn1d + i*(VISIOCYTELONG)(segPara.regionnum);
