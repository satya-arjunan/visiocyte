...
template <class T1, class T2> bool gaussfilt3d(T1 ***indata3d, T2 ***outdata3d, VISIOCYTELONG *sz, int kernelsz, float sigma)
{
	VISIOCYTELONG i,j,k;
	VISIOCYTELONG m,n,p;

	unsigned VISIOCYTELONG len, cnt;
   ...
   for (k=0; k<rr; k++)
   { 
		VISIOCYTELONG k1 = k-kernelsz;
		
		for (j=0; j<rr; j++)
		{
			VISIOCYTELONG j1 = j-kernelsz;
			
			for (i=0; i<rr; i++)
			{
				VISIOCYTELONG i1 = i-kernelsz;
...
template <class T1, class T2> bool medfilt3d(T1 ***indata3d, T2 ***outdata3d, VISIOCYTELONG *sz, int kernelsz)
{
	VISIOCYTELONG i,j,k;
	VISIOCYTELONG m,n,p;

	//unsigned 
	VISIOCYTELONG len, cnt;
	VISIOCYTELONG rr=(2*kernelsz+1);
	len = rr*rr*rr;
    VISIOCYTELONG midlen = (len-1)/2; //this is not exactly the midpoint, but should be fine for now. Noted by PHC 20130719
