...
template <class T> bool distTrans3d(Vol3DSimple <T> *inimg, Vol3DSimple <float> *distimg, Vol3DSimple <VISIOCYTELONG> *indeximg, unsigned char tag)
{	
	...
	VISIOCYTELONG *pix_index = indeximg->getData1dHandle();
...
template <class T1, class T2> bool distTrans3d(Vol3DSimple <T1> *inimg, Vol3DSimple <T2> *distimg, Vol3DSimple <VISIOCYTELONG> *indeximg, unsigned char tag)
{	
	if (!inimg || !inimg->valid() || !distimg || !distimg->valid() || !indeximg || !indeximg->valid())
	{	printf("invalid image in distance transform \n");
		return false;
