...
int swc2eswc_io(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	NeuronTree neuron;
	vector<VISIOCYTELONG> segment_id, segment_layer;
	if (fileOpenName.endsWith(".swc") || fileOpenName.endsWith(".SWC"))
	{
		...
		{
			visiocyte_msg("Cannot convert swc to eswc.\n");
			return -1;
		}
	}
	else {
		visiocyte_msg("The file type you specified is not supported. Please check.");
		return -1;
	}
	...
	{
		visiocyte_msg("fail to write the output eswc file.");
		return -1;
	}
	return 1;
}
...
bool swc2eswc_io(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output)
{
	...
	if (fileOpenName.endsWith(".swc") || fileOpenName.endsWith(".SWC"))
	{
		...
		if (!swc2eswc(neuron,segment_id, segment_layer))
		{
			visiocyte_msg("Cannot convert swc to eswc.", 0);
			return false;
		}
	}
	else {
		visiocyte_msg("The file type you specified is not supported. Please check.", 0);
		return false;
	}
	
	if (!export_eswc(neuron,segment_id,segment_layer,qPrintable(fileOpenName),qPrintable(fileSaveName)))
	{
		visiocyte_msg("fail to write the output eswc file.", 0);
		return false;
	}
	return true;
}
...
int eswc2swc_io(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	if (fileOpenName.endsWith(".eswc") || fileOpenName.endsWith(".ESWC"))
	{
		...
		if (!swc2eswc(neuron,segment_id, segment_layer))
		{
			visiocyte_msg("Cannot convert eswc to swc.\n");
			return -1;
		}
	}
	else {
...
bool eswc2swc_io(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output)
{
	...
	if (check_eswc(neuron, segment_id, segment_layer)==-1)
	{
		visiocyte_msg("WARNING: Something is wrong with the eswc file", 0);
		visiocyte_msg("But we still put convert it to swc format", 0);
	}
	...
	if (!export_swc(neuron,qPrintable(fileSaveName)))
	{
		visiocyte_msg("fail to write the output swc file.", 0);
		return false;
	}
	return true;
}
...
int check_eswc_io(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	if (check_result==-1 || check_result==0)
	{
		...
		visiocyte_msg(QString("%1 has been updated!").arg(fileOpenName));
		}
		else return -1;
	}
	else
		visiocyte_msg("Your eswc file is in correct format!");
...
int swc2eswc_toolbox(const VISIOCYTEPluginArgList & input)
{
	...
	if (fileOpenName.endsWith(".swc") || fileOpenName.endsWith(".SWC"))
	{
		...
		visiocyte_msg("The file type you specified is not in swc format. Please check.");
		return -1;
	}
...
int eswc2swc_toolbox(const VISIOCYTEPluginArgList & input)
{
	...
	if (fileOpenName.endsWith(".eswc") || fileOpenName.endsWith(".ESWC"))
	{
		...
		visiocyte_msg("The file type you specified is not in .eswc format. Please check.");
		return -1;
	}
	int ret = QMessageBox::Yes;
	if (check_eswc(neuron, segment_id, segment_layer)==-1)
...
void printHelp()
{
	...
	cout<<"Usage: visiocyte -x eswc_converter -f <function_name> -i <input_file> -o <output_file>"<<endl;
	...
	cout<<"Example: visiocyte -x eswc_converter -f swc_to_eswc -i 1.swc -o 1.eswc\n"<<endl;
}
