...
void processImage_selective(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
void processImage_adaptive(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
void processImage_adaptive_auto(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
void processImage_adaptive_auto_2D(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
void processImage_adaptive_auto_blocks(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
void processImage_detect_soma(VISIOCYTEPluginCallback2 &callback, QWidget *parent);

bool processImage_adaptive_auto(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback);
bool processImage_adaptive_auto_2D(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback);
bool processImage_adaptive_auto_blocks(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback);
bool processImage_adaptive_auto_blocks_indv(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback);
bool processImage_adaptive_auto_blocks_indv_v2(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback);
bool processImage_adaptive_auto_blocks_indv_multithread(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback);
bool processImage_adaptive_auto_blocks_indv_multithread_v2(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback);

bool processImage_detect_soma(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback);
bool processImage_detect_soma_2D(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback);
...
template <class T> void callGaussianPlugin(VISIOCYTEPluginCallback2 &callback,
...
template <class T> void callgsdtPlugin(VISIOCYTEPluginCallback2 &callback,const T* data1d,
                                       VISIOCYTELONG *in_sz,
                                       unsigned int c,
                                       double th_global,
                                       T* &outimg,
...
template <class T> void enhancementWithsoma(VISIOCYTEPluginCallback2 &callback,
                                            const T* data1d,
                                            T* data1d_enhanced,
                                            VISIOCYTELONG *in_sz,
                                            unsigned int c,
...
template <class T> void somalocation(VISIOCYTEPluginCallback2 &callback,
                                     const T* data1d,
                                     VISIOCYTELONG *in_sz,
                                     unsigned int c,
                                     int &soma_x,
...
template <class T> double getdtmax(VISIOCYTEPluginCallback2 &callback,const T* data1d,VISIOCYTELONG *in_sz);
...
template <class SDATATYPE> int pwi_fusing(SDATATYPE *data1d, SDATATYPE *subject1d, VISIOCYTELONG *sz_subject, SDATATYPE *target1d, VISIOCYTELONG *sz_target, VISIOCYTELONG *offset, VISIOCYTELONG new_sz0, VISIOCYTELONG new_sz1, VISIOCYTELONG new_sz2);

//FL added
template <class T> void fusing(T *data1d, T *block1d, VISIOCYTELONG xb, VISIOCYTELONG xe, VISIOCYTELONG yb, VISIOCYTELONG ye, VISIOCYTELONG *sz_data1d, unsigned int ws, bool tag_leftmost_block);

//FL added
template <class T> void fusing2D(unsigned int *data, unsigned char *counterImg, T *blockSection2D, VISIOCYTELONG xb, VISIOCYTELONG xe, VISIOCYTELONG yb, VISIOCYTELONG ye, VISIOCYTELONG *sz_enhanced2d);
...
void selectiveEnhancement::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    {
        visiocyte_msg(QString("Multi-scale enhancement of an image with fiber structures. v%1"
                        "Developed by Zhi Zhou and Hanchuan Peng (2013)").arg(getPluginVersion()));
    }
}

bool selectiveEnhancement::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    ...
    {
        cout<<"Usage : visiocyte -x dllname -f adaptive_auto -i <inimg_file> -o <outimg_file> -p <scale> <ch> <ratio> <soma> <bulr>"<<endl;
        cout<<endl;
        ...
        cout<<"Usage : visiocyte -x dllname -f adaptive_auto_block -i <inimg_file> -o <outimg_file> -p <ws> <ch> <ratio> <soma>" <<endl;
        cout<<endl;
        ...
        cout<<"Usage : visiocyte -x dllname -f adaptive_auto_block_indv -i <inimg_file> -o <outimg_folder> -p <ws> <ch> <ratio>" <<endl;
        cout<<endl;
        cout<<"ws          block window size (pixel #), default 1000"<<endl;
        cout<<"ch          the input channel value, start from 1, default 1"<<endl;
        cout<<"ratio       the window size calibration ratio, default 1"<<endl;
        ...
        cout<<"Usage : visiocyte -x dllname -f adaptive_auto_block_indv_multithread -i <inimg_file> -o <outimg_folder> -p <ws> <ch> <ratio> <threads> <2Dsections>" <<endl;
        cout<<endl;
        cout<<"ws          block window size (pixel #), default 1000"<<endl;
        cout<<"ch          the input channel value, start from 1, default 1"<<endl;
        cout<<"ratio       the window size calibration ratio, default 1"<<endl;
        ...
        cout<<"Usage : visiocyte -x dllname -f soma_detection -i <inimg_file> -p <marker_file> <tc_file>" <<endl;
        cout<<endl;
...
void processImage_selective(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    QString temp_raw = QString(p4DImage->getFileName()) + "_temp.visiocyteraw";
    QString temp_gf = QString(p4DImage->getFileName()) + "_gf.visiocyteraw";
//    QString temp_gsdt = QString(p4DImage->getFileName()) + "_gsdt.visiocyteraw";
//    QString temp_gsdt_v2 = QString(p4DImage->getFileName()) + "_gsdt_v2.visiocyteraw";
//    QString temp_wogf = QString(p4DImage->getFileName()) + "_woGf.visiocyteraw";
//
//
//    QString temp_soma = QString(p4DImage->getFileName()) + "_soma.visiocyteraw";
//    QString temp_gsdtsoma  = QString(p4DImage->getFileName()) + "_gsdtsoma.visiocyteraw";
//    QString temp_ds = QString(p4DImage->getFileName()) + "_ds.visiocyteraw";
//    QString temp_gsdtds = QString(p4DImage->getFileName()) + "_gsdtds.visiocyteraw";
    ...
    catch(...)  {visiocyte_msg("cannot allocate memory for EnahancedImage_final."); return;}
   ...
   //set up output image
    ...
    new4DImage->setData((unsigned char *)EnahancedImage_final,in_sz[0], in_sz[1], in_sz[2], 1, VISIOCYTE_UINT8);
...
void processImage_adaptive_auto(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    Image4DSimple * new4DImage = new Image4DSimple();
    new4DImage->setData((unsigned char *)data1d_uint8,in_sz[0], in_sz[1], in_sz[2], 1, VISIOCYTE_UINT8);
    visiocytehandle newwin = callback.newImageWindow();
    callback.setImage(newwin, new4DImage);
...
void processImage_adaptive_auto_2D(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    VISIOCYTELONG pagesz_3d = p4DImage->getTotalUnitNumberPerChannel();
    ImagePixelType pixeltype = p4DImage->getDatatype();
    ...
    VISIOCYTELONG offsetc = (c-1)*pagesz_3d;
    unsigned char *EnahancedImage_final_3D = 0;
    try {EnahancedImage_final_3D = new unsigned char [pagesz_3d];}
    catch(...)  {visiocyte_msg("cannot allocate memory for EnahancedImage_final_3D."); return;}
...
//					callgsdtPlugin(callback,(unsigned char *)data1d_gf, in_sz, 1,0,(unsigned char* &)gsdtld);
    ...
    Image4DSimple * new4DImage = new Image4DSimple();
    new4DImage->setData((unsigned char *)EnahancedImage_final_3D,N, M, P, 1, VISIOCYTE_UINT8);
    visiocytehandle newwin = callback.newImageWindow();
    callback.setImage(newwin, new4DImage);
...
void processImage_adaptive_auto_blocks(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    VISIOCYTELONG szSub_y[4];
    VISIOCYTELONG szTar_y[4];

    for(VISIOCYTELONG iy = 0; iy < M; iy = iy+Ws-Ws/10)
    {
        unsigned char* subject1d = 0;
        unsigned char* target1d = 0;

        VISIOCYTELONG szSub[4];
        VISIOCYTELONG szTar[4];
        VISIOCYTELONG new_sz0 = 0;
        VISIOCYTELONG new_sz1 = 0;
        VISIOCYTELONG new_sz2 = 0;
        VISIOCYTELONG yb = iy;
        VISIOCYTELONG ye = iy+Ws-1; if(ye>=M-1) ye = M-1;

        int count = 0;
        for(VISIOCYTELONG ix = 0; ix < N; ix = ix+Ws-Ws/10)
        {
            VISIOCYTELONG xb = ix;
            VISIOCYTELONG xe = ix+Ws-1; if(xe>=N-1) xe = N-1;

            unsigned char *blockarea=0;
            VISIOCYTELONG blockpagesz = (xe-xb+1)*(ye-yb+1)*P;
            ...
            {
                VISIOCYTELONG offsetk = iz*M*N;
                for(VISIOCYTELONG iy = yb; iy < ye+1; iy++)
                {
                    VISIOCYTELONG offsetj = iy*N;
                    for(VISIOCYTELONG ix = xb; ix < xe+1; ix++)
                    {
            ...
            VISIOCYTELONG block_sz[4];
            block_sz[0] = xe-xb+1; block_sz[1] = ye-yb+1; block_sz[2] = P; block_sz[3] = 1;

//            temp_raw = QString(p4DImage->getFileName()) + "_temp.visiocyteraw";
//            temp_gf = QString(p4DImage->getFileName()) + "_gf.visiocyteraw";
//            temp_gsdt = QString(p4DImage->getFileName()) + "_gsdt.visiocyteraw";
            ...
            catch(...)  {visiocyte_msg("cannot allocate memory for localEnahancedArea."); return;}
            double sigma = 0;
...
//                    callgsdtPlugin(callback,(unsigned char *)data1d_gf, block_sz, 1,th_global,(unsigned char* &)gsdtld);
                ...
                if (scale==0)
                   ...
                   {
                       for(VISIOCYTELONG i = 0; i<blockpagesz; i++)
                       {
            ...
            if (count==0)
            {
                VISIOCYTELONG targetsize = block_sz[0]*block_sz[1]*block_sz[2];
            ...
            else
            {
                ...
                VISIOCYTELONG subjectsize = block_sz[0]*block_sz[1]*block_sz[2];
                ...
                VISIOCYTELONG totalplxs = new_sz0*new_sz1*new_sz2;
                ...
                VISIOCYTELONG targetsize = new_sz0*new_sz1*new_sz2;
                target1d = new unsigned char [targetsize];
        ...
        if (county==0)
        {
            ...
            VISIOCYTELONG targetsize_y = new_sz0*new_sz1*new_sz2;
        ...
        {
            VISIOCYTELONG subjectsize_y = new_sz0*new_sz1*new_sz2;
    ...
    //set up output image
    Image4DSimple * new4DImage = new Image4DSimple();
    new4DImage->setData((unsigned char *)data1d_uint8,N, M, P, 1, VISIOCYTE_UINT8);
    visiocytehandle newwin = callback.newImageWindow();
    callback.setImage(newwin, new4DImage);
...
void processImage_detect_soma(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    p4DImage_name.append("_soma.visiocyteraw");
    ...
    {
        visiocyte_msg("The size of these two image is not the same. Please try it again!");
        return;
...
bool processImage_adaptive_auto(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback)
{
    ...
    QString temp_raw = QString(inimg_file) + "_temp.visiocyteraw";
    QString temp_gf = QString(inimg_file) + "_gf.visiocyteraw";
    QString temp_gsdt = QString(inimg_file) + "_gsdt.visiocyteraw";
    QString temp_gsdt_v2 = QString(inimg_file) + "_gsdt_v2.visiocyteraw";
    QString temp_wogf = QString(inimg_file) + "_woGf.visiocyteraw";


    QString temp_soma = QString(inimg_file) + "_soma.visiocyteraw";
    QString temp_gsdtsoma  = QString(inimg_file) + "_gsdtsoma.visiocyteraw";
    QString temp_ds = QString(inimg_file) + "_ds.visiocyteraw";
    QString temp_gsdtds = QString(inimg_file) + "_gsdtds.visiocyteraw";
    ...
    unsigned char *EnahancedImage_final=0;
    try {EnahancedImage_final = new unsigned char [pagesz];}
    catch(...)  {visiocyte_msg("cannot allocate memory for EnhancedImage_final."); return false;}
...
//            callgsdtPlugin(callback,(unsigned char *)data1d_gf, in_sz, 1,0,(unsigned char* &)gsdtld);
            ...
            AdpThresholding_adpwindow((unsigned char *)data1d_gf, in_sz, 1,sigma,(unsigned char* &)EnahancedImage, gsdtld,3,ratio);
            break;
        default: visiocyte_msg("Invalid data type. Do nothing."); return false;
        }
    ...
    if(q==1)
    {
        simple_saveimage_wrapper(callback,temp_wogf.toStdString().c_str(), (unsigned char *)Enhancement_soma, in_sz, 1);
        VISIOCYTEPluginArgItem arg;
        VISIOCYTEPluginArgList input_gf;
        VISIOCYTEPluginArgList output_gf;
...
bool processImage_adaptive_auto_2D(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback)
{
    ...
    VISIOCYTELONG pagesz_3d = subject->getTotalUnitNumberPerChannel();
    VISIOCYTELONG offsetc = (c-1)*pagesz_3d;

    unsigned char *EnahancedImage_final_3D = 0;
    try {EnahancedImage_final_3D = new unsigned char [pagesz_3d];}
    catch(...)  {visiocyte_msg("cannot allocate memory for EnahancedImage_final_3D."); return false;}
...
bool processImage_adaptive_auto_blocks(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback)
{
    ...
    for(VISIOCYTELONG iy = 0; iy < M; iy = iy+Ws-Ws/10)
    {
        ...
        for(VISIOCYTELONG ix = 0; ix < N; ix = ix+Ws-Ws/10)
        {
            ...
            unsigned char *localEnahancedArea=0;
            try {localEnahancedArea = new unsigned char [blockpagesz];}
            catch(...)  {visiocyte_msg("cannot allocate memory for localEnahancedArea."); return false;}
            double sigma = 0;
...
//    QString temp_gsdtds = QString(inimg_file) + "_gsdtds.visiocyteraw";
   ...
   // VISIOCYTELONG offsetc = (c-1)*pagesz;
    ...
    VISIOCYTELONG tilenum = (floor(N/(0.9*Ws))+1.0)*(floor(M/(0.9*Ws))+1.0);
    int count = 2;
	...
	for(VISIOCYTELONG iy = 0; iy < M; iy = iy+Ws-Ws/10)
	{
        ...
        //	blockarea = new unsigned char [blockpagesz];
		   ...
		   /* unsigned char *blockarea_median=0;
			...
			switch (datatype)
			{
			case VISIOCYTE_UINT8: median_filter(blockarea, block_sz, ws, ws, ws, c,(unsigned char* &)blockarea_median);
				 break;
				 default: visiocyte_msg("Invalid data type. Do nothing."); return false;
			}
...
//						callgsdtPlugin(callback,(unsigned char *)data1d_gf, block_sz, 1,th_global,(unsigned char* &)gsdtld);
					...
					break;
					default: visiocyte_msg("Invalid data type. Do nothing."); return false; //FL comment out, openMP does not allow exit from blocked structure
...
//    temp_raw = QString(inimg_file) + "_temp.visiocyteraw";
//    temp_gf = QString(inimg_file) + "_gf.visiocyteraw";
//    temp_gsdt = QString(inimg_file) + "_gsdt.visiocyteraw";
//    temp_gsdt_v2 = QString(inimg_file) + "_gsdt_v2.visiocyteraw";
//    temp_wogf = QString(inimg_file) + "_woGf.visiocyteraw";
//
//
//    temp_soma = QString(inimg_file) + "_soma.visiocyteraw";
//    temp_gsdtsoma  = QString(inimg_file) + "_gsdtsoma.visiocyteraw";
//    temp_ds = QString(inimg_file) + "_ds.visiocyteraw";
//    temp_gsdtds = QString(inimg_file) + "_gsdtds.visiocyteraw";
...
//    	EnhancedStack = new unsigned char [N*M*P]; // for saving enhanced 2D section purpose
	...
	for(VISIOCYTELONG iy = 0; iy < M; iy = iy+Ws-Ws/10)
//	for(VISIOCYTELONG iy = 0; iy < 10; iy = iy+Ws-Ws/10)

	{
		...
		for(VISIOCYTELONG ix = 0; ix < N; ix = ix+Ws-Ws/10)
//		for(VISIOCYTELONG ix = 0; ix < 10; ix = ix+Ws-Ws/10)
...
//			printf("ix=%d, iy=%d\n", ix, iy);
			...
			QString temp_raw = QString(inimg_file).append(QString("_x_%1_%2_y_%3_%4.raw").arg(xb).arg(xe).arg(yb).arg(ye)) + "_temp.visiocyteraw";
			QString temp_gf = QString(inimg_file).append(QString("_x_%1_%2_y_%3_%4.raw").arg(xb).arg(xe).arg(yb).arg(ye)) + "_gf.visiocyteraw";
			QString temp_gsdt = QString(inimg_file).append(QString("_x_%1_%2_y_%3_%4.raw").arg(xb).arg(xe).arg(yb).arg(ye)) + "_gsdt.visiocyteraw";
//			QString temp_gsdt_v2 = QString(inimg_file).append(QString("_x_%1_%2_y_%3_%4.raw").arg(xb).arg(xe).arg(yb).arg(ye)) + "_gsdt_v2.visiocyteraw";
//			QString temp_wogf = QString(inimg_file).append(QString("_x_%1_%2_y_%3_%4.raw").arg(xb).arg(xe).arg(yb).arg(ye)) + "_woGf.visiocyteraw";
//		
//			QString temp_soma = QString(inimg_file).append(QString("_x_%1_%2_y_%3_%4.raw").arg(xb).arg(xe).arg(yb).arg(ye)) + "_soma.visiocyteraw";
//			QString temp_gsdtsoma  = QString(inimg_file).append(QString("_x_%1_%2_y_%3_%4.raw").arg(xb).arg(xe).arg(yb).arg(ye)) + "_gsdtsoma.visiocyteraw";
//			QString temp_ds = QString(inimg_file).append(QString("_x_%1_%2_y_%3_%4.raw").arg(xb).arg(xe).arg(yb).arg(ye)) + "_ds.visiocyteraw";
//			QString temp_gsdtds = QString(inimg_file).append(QString("_x_%1_%2_y_%3_%4.raw").arg(xb).arg(xe).arg(yb).arg(ye)) + "_gsdtds.visiocyteraw";
            ...
            unsigned char *localEnahancedArea=0;
            try {localEnahancedArea = new unsigned char [blockpagesz];}
//            catch(...)  {visiocyte_msg("cannot allocate memory for localEnahancedArea."); return false;} // FL comment out, openMP does not allow exit from blocked structure

            catch(...)  {visiocyte_msg("cannot allocate memory for localEnahancedArea.",0); falsetag = false;} //FL
...
//						callgsdtPlugin(callback,(unsigned char *)data1d_gf, block_sz, 1,th_global,(unsigned char* &)gsdtld);
						...
						break;
	 //                   default: visiocyte_msg("Invalid data type. Do nothing."); return false; //FL comment out, openMP does not allow exit from blocked structure
                        default: visiocyte_msg("Invalid data type. Do nothing.",0); falsetag = false; //FL
    ...
    else
    {
		...
		if (saveEnhanced2DSections==1)
		{
			...
			for (long k=0; k<in_sz[2]; k++)
			{
				...
				VISIOCYTELONG *sz_block3D = 0;
				VISIOCYTELONG *sz_block_section2D = 0;
...
//    VISIOCYTELONG N = in_zz[0];
//    VISIOCYTELONG M = in_zz[1];
//    VISIOCYTELONG P = in_zz[2];
...
//    VISIOCYTELONG blockpagesz = (vim.lut[index_tile].end_pos[1] - vim.lut[index_tile].start_pos[1] + 1) * (vim.lut[index_tile].end_pos[0] - vim.lut[index_tile].start_pos[0] + 1) * P;
//    try {data1d_region = new unsigned char [blockpagesz];}
//    catch(...)  {visiocyte_msg("cannot allocate memory for data1d_region."); return false;}
...
//    {
//        VISIOCYTELONG offsetk = iz*M*N;
//        for(VISIOCYTELONG iy = vim.lut[index_tile].start_pos[1]; iy <= vim.lut[index_tile].end_pos[1]; iy++)
//        {
//            VISIOCYTELONG offsetj = iy*N;
//            for(VISIOCYTELONG ix = vim.lut[index_tile].start_pos[0]; ix <= vim.lut[index_tile].end_pos[0]; ix++)
//            {
...
//    QString temp_raw = inimg_file + "_temp.visiocyteraw";
//    QString temp_gf = inimg_file + "_gf.visiocyteraw";
//    QString temp_gsdt = inimg_file + "_gsdt.visiocyteraw";
//    QString temp_gsdt_v2 = inimg_file + "_gsdt_v2.visiocyteraw";
//    QString temp_wogf = inimg_file + "_woGf.visiocyteraw";


    QString temp_soma = inimg_file + "_soma.visiocyteraw";
    QString temp_gsdtsoma  = inimg_file + "_gsdtsoma.visiocyteraw";
    QString temp_ds = inimg_file + "_ds.visiocyteraw";
    QString temp_gsdtds = inimg_file + "_gsdtds.visiocyteraw";
...
bool processImage_detect_soma_2D(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output,VISIOCYTEPluginCallback2 &callback)
{
    ...
    VISIOCYTELONG xb = soma_x-200; if(xb<0) xb = 0;
    VISIOCYTELONG xe = soma_x+200; if(xe>=N-1) xe = N-1;
    VISIOCYTELONG yb = soma_y-200; if(yb<0) yb = 0;
    VISIOCYTELONG ye = soma_y+200; if(ye>=M-1) ye = M-1;

    for(VISIOCYTELONG iz = 0; iz < P; iz++)
    {
        VISIOCYTELONG offsetk = iz*M*N;
        for(VISIOCYTELONG iy = yb; iy < ye; iy++)
        {
            VISIOCYTELONG offsetj = iy*N;
            for(VISIOCYTELONG ix = xb; ix < xe; ix++)
            {
                                             ...
                                             T* &outimg)
{
    if (!in_sz)
    {
        visiocyte_msg("Invalid input size information");
        return;
    }

    if (c>in_sz[3] || c<1)
    {
        visiocyte_msg("Invalid channel information");
        return;
    }
                                                  ...
                                                  double ratio)
{
    ...
    double LOG2 = log(2.0);
//    VISIOCYTELONG DSN = floor(in_sz[0]/4);
//    VISIOCYTELONG DSM = floor(in_sz[1]/4);
//    VISIOCYTELONG DSP = floor(in_sz[2]/4);

//    VISIOCYTELONG pageszDS = DSN*DSM*DSP;

    for(VISIOCYTELONG iz = 0; iz < P; iz++)
    {
        printf("\r Enhancement : %d %% completed ", int(float(iz)/P*100)); fflush(stdout);
        VISIOCYTELONG offsetk = iz*M*N;
      //  VISIOCYTELONG dsoffsetk = floor(iz/4)*DSM*DSN;

        for(VISIOCYTELONG iy = 0; iy < M; iy++)
        {
            VISIOCYTELONG offsetj = iy*N;
        //    VISIOCYTELONG dsoffsetj = floor(iy/4)*DSN;
            ...
            for(VISIOCYTELONG ix = 0; ix < N; ix++)
            {
              ...
              /*  double pixelWin = 0;
                ...
                VISIOCYTELONG xb = ix-3; if(xb<0) xb = 0;
                VISIOCYTELONG xe = ix+3; if(xe>=N-1) xe = N-1;
                VISIOCYTELONG yb = iy-3; if(yb<0) yb = 0;
                VISIOCYTELONG ye = iy+3; if(ye>=M-1) ye = M-1;
                VISIOCYTELONG zb = iz-3; if(zb<0) zb = 0;
                VISIOCYTELONG ze = iz+3; if(ze>=P-1) ze = P-1;
...
//                VISIOCYTELONG DSindex_pim = dsoffsetk+dsoffsetj+floor(ix/4);
//                T GsdtValue;
...
template <class T> double getdtmax(VISIOCYTEPluginCallback2 &callback,const T* data1d,VISIOCYTELONG *in_sz)
{
    ...
    VISIOCYTELONG P = in_sz[2];
    VISIOCYTELONG pagesz = N*M*P;
    simple_saveimage_wrapper(callback, "temp_enhanced.visiocyteraw", (unsigned char *)data1d, in_sz, 1);
    ...
    for(VISIOCYTELONG iz = 0; iz < P; iz++)
    {
        ...
        for(VISIOCYTELONG iy = 0; iy < M; iy++)
        {
            ...
            for(VISIOCYTELONG ix = 0; ix < N; ix++)
            {
                ...
                VISIOCYTELONG PixelVaule = data1d[offsetk + offsetj + ix];
                PixelSum = PixelSum + PixelVaule;
    ...
    arg.type = "random";std::vector<char*> args1;
    args1.push_back("temp_enhanced.visiocyteraw"); arg.p = (void *) & args1; input<< arg;
    arg.type = "random";std::vector<char*> args;
    ...
    arg.type = "random";std::vector<char*> args2;args2.push_back("gtImage.visiocyteraw"); arg.p = (void *) & args2; output<< arg;
    ...
    remove("temp_enhanced.visiocyteraw");
    remove("gtImage.visiocyteraw");
    double maxfl = 0;
    for(VISIOCYTELONG i=0; i<pagesz; i++)
    {
...
int pwi_fusing(SDATATYPE *data1d, SDATATYPE *subject1d, VISIOCYTELONG *sz_subject, SDATATYPE *target1d, VISIOCYTELONG *sz_target, VISIOCYTELONG *offset,VISIOCYTELONG new_sz0, VISIOCYTELONG new_sz1, VISIOCYTELONG new_sz2)
{
    //
            ...
            for(VISIOCYTELONG j=ty_start; j<ty_end; j++)
            {
                ...
                for(VISIOCYTELONG i=tx_start; i<tx_end; i++)
                {
                    ...
                    if(data1d[idx])
                    {
                        if(offset[0] > 0)
                        {
                            VISIOCYTELONG overlap_range = tx - offset[0];
                            VISIOCYTELONG tar_range= i - offset[0]+1;
                        ...
                        {
                            VISIOCYTELONG overlap_range = ty - offset[1];
                            VISIOCYTELONG tar_range= j - offset[1]+1;
                            double tar_ratio = (double)tar_range/overlap_range;
                            data1d[idx] = (SDATATYPE) ( tar_ratio*data1d[idx] + (1-tar_ratio)*target1d[offsets_j + i - tx_start] );
...

{
    ...
    VISIOCYTELONG yb = soma_y-200; if(yb<0) yb = 0;
    VISIOCYTELONG ye = soma_y+200; if(ye>=M-1) ye = M-1;
    VISIOCYTELONG somasz = (xe-xb)*(ye-yb)*P;
                                     ...
                                     QString temp_gsdtds)
{
    ...
    for(VISIOCYTELONG i=0; i<pageszDS; i++)
        datald_downsample[i] = 0;

    double th = 0;
    VISIOCYTELONG dsiz=0;
    for(VISIOCYTELONG iz = 0; iz < P; iz = iz+4)
    {
        double PixelSum = 0;
        VISIOCYTELONG offsetk = iz*M*N;
        VISIOCYTELONG dsoffsetk = dsiz*DSM*DSN;
        VISIOCYTELONG dsiy=0;
        for(VISIOCYTELONG iy = 0; iy <  M; iy = iy+4)
        {
            VISIOCYTELONG offsetj = iy*N;
            VISIOCYTELONG dsoffsetj = dsiy*DSN;
            VISIOCYTELONG dsix=0;
            for(VISIOCYTELONG ix = 0; ix < N; ix = ix+4)
            {

                VISIOCYTELONG index_pim = offsetc+offsetk + offsetj + ix;
                VISIOCYTELONG DSindex_pim = dsoffsetk+dsoffsetj+dsix;
                if(DSindex_pim < pageszDS)
                {
    ...
    double std = 0;
    for(VISIOCYTELONG dsiz = 0; dsiz < DSP; dsiz++)
    {
        VISIOCYTELONG dsoffsetk = dsiz*DSM*DSN;
        double stdSum=0;
        for(VISIOCYTELONG dsiy = 0; dsiy < DSM; dsiy++)
        {
            VISIOCYTELONG dsoffsetj = dsiy*DSN;
            for(VISIOCYTELONG dsix = 0; dsix < DSN; dsix++)
            {
...

{
    ...
    {
        int i = 0;
        VISIOCYTELONG xb = x-200; if(xb<0) xb = 0;
        VISIOCYTELONG xe = x+200; if(xe>=N-1) xe = N-1;
        VISIOCYTELONG yb = y-200; if(yb<0) yb = 0;
        VISIOCYTELONG ye = y+200; if(ye>=M-1) ye = M-1;
        for(VISIOCYTELONG iz = 0; iz < P; iz++)
        {
            VISIOCYTELONG offsetk = iz*M*N;
            for(VISIOCYTELONG iy = yb; iy < ye; iy++)
