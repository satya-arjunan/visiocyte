...
void image_profiling::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
bool image_profiling::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
	...
	else if (func_name == tr("seg_separate"))
	{
		...
		string inputImgName = imageRoot + caseNum + "_ds4.visiocyteraw";
		...
		for (int i = 0; i < swcList.size(); ++i)
		{
			...
			if (!swcFileBaseName.compare(swcFileBaseName.length() - 3, 3, "ds4") || !swcFileSuffix.compare("visiocyteraw")) continue;
	...
	else if (func_name == tr("segmentBasedProfile"))
	{
		...
		string caseNum = folderPath.substr(folderPath.length() - 9, 9);
		//cout << caseNum << endl;
		string inputImgName = folderPath + "\\" + caseNum + "_ds4.visiocyteraw";
		const char* inputImgNameC = inputImgName.c_str();
		char* inputImgNameChar = new char[inputImgName.length()];
		strcpy(inputImgNameChar, inputImgNameC);
		Image4DSimple* inputImgPtr = callback.loadImage(inputImgNameChar);
