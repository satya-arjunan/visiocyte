...
shape_dialog::shape_dialog(VISIOCYTEPluginCallback2 *cb)
{
    callback=cb;
    image1Dc_in=0;
    image1Dc_out=0;
...
void shape_dialog::core()
{
    ...
    qDebug()<<"size:"<<visiocytehandleList_current.size();
...
void shape_dialog::extract()
{
    ...
    VISIOCYTELONG sumrsize=0;
    //volume.clear();
    for (int j=0;j<poss_landmark.size();j++){
        qDebug()<<"j:"<<j;
        if (label[j]>0) continue;
        VISIOCYTELONG rsize=shape_ext_obj.extract(x_all, y_all,z_all,label,poss_landmark[j],
                                       convolute_iter,bg_thr,j+1);
        if(rsize<=1) continue;

        GetColorRGB(rgb,j);
        VISIOCYTELONG finalpos;
    ...
    if (sumrsize>0)
    {
        ...
        image4d.setData(label_input, sz_img[0], sz_img[1], sz_img[2], 1, VISIOCYTE_UINT8);
        image4d_color.setData(image1Dc_input,sz_img[0], sz_img[1], sz_img[2],sz_img[3],VISIOCYTE_UINT8);

        visiocytehandle visiocytehandle_color=callback->newImageWindow();
        callback->setImage(visiocytehandle_color, &image4d);
        callback->setLandmark(visiocytehandle_color, LList);
        callback->setImageName(visiocytehandle_color, "extractor_index_"+QString(callback->getImageName(curwin)));
        callback->updateImageWindow(visiocytehandle_color);

        visiocytehandle visiocytehandle_main=callback->newImageWindow();
        callback->setImage(visiocytehandle_main, &image4d_color);
        callback->setLandmark(visiocytehandle_main, LList);
        callback->setImageName(visiocytehandle_main, "extractor_"+QString(callback->getImageName(curwin)));
...
void shape_dialog::convert2UINT8(unsigned short *pre1d, unsigned char *pPost, VISIOCYTELONG imsz)
{
...
void shape_dialog::convert2UINT8(float *pre1d, unsigned char *pPost, VISIOCYTELONG imsz)
{
