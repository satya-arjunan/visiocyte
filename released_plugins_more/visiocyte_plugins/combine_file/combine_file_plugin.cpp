...
void generatorcombined4FilesInDir(VISIOCYTEPluginCallback2 &callback, QWidget *parent, int method_code);
bool generatorcombined4FilesInDir(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output);
...
void combine_file::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    else if (menu_name == tr("separate a SWC file by type"))
    {
        ...
        visiocyte_msg("Done with SWC separation!");
	...
	else if (menu_name == tr("separate a SWC file by type (without sorting)"))
	{
		...
		map<int, QList<NeuronSWC> > multi_neurons;
		map<int, set<VISIOCYTELONG> > typeNodeSetMap;
		for (VISIOCYTELONG i = 0; i<nt.listNeuron.size(); i++)
		{
			multi_neurons[nt.listNeuron[i].type].append(nt.listNeuron[i]);
...
//    }
	...
	{
        visiocyte_msg(tr("this is a plugin used to combine two or more swc files together.Firstly, you need put all the swc or eswc files to one folder.. "
                   "Developed by Zhi Zhou (2015-2-27) and redesigned by Shengdian Jiang (2018-07-31)"));
	}
}

bool combine_file::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
	...
	{
           cout<<"Usage : visiocyte -x combine_file -f combine -i <in_folder> -o <combined_file> -p <method_code> "<<endl;
           cout<<endl;
...
void generatorcombined4FilesInDir(VISIOCYTEPluginCallback2 &callback, QWidget *parent, int method_code)
{
    if(method_code == 1)
    {
        ...
        for(VISIOCYTELONG i = 0; i < swcList.size(); i++)
        {
            ...
            for(VISIOCYTELONG d = 0; d < mergeswc.size(); d++)
            {
