                        ...
                        bool b_returnDTable)
{
	...
	VISIOCYTELONG tsz0 = subject->sz0(), tsz1 = subject->sz1(), tsz2 = subject->sz2();
	...
	VISIOCYTELONG is = hardLimit(xs, VISIOCYTELONG(0), tsz0-1), ie = hardLimit(xe, VISIOCYTELONG(0), tsz0-1);
	VISIOCYTELONG js = hardLimit(ys, VISIOCYTELONG(0), tsz1-1), je = hardLimit(ye, VISIOCYTELONG(0), tsz1-1);
	VISIOCYTELONG ks = hardLimit(zs, VISIOCYTELONG(0), tsz2-1), ke = hardLimit(ze, VISIOCYTELONG(0), tsz2-1);
    
	VISIOCYTELONG xlen = ie-is+1, ylen = je-js+1, zlen = ke-ks+1;
	VISIOCYTELONG cubelen = (VISIOCYTELONG)xlen*ylen*zlen;
    
	const VISIOCYTELONG myCUBECOLUMNNUM=15;
...
bool getSmoothedWarpParameter(Vol3DSimple<WarpParameterAffine3D> * wp_new, Vol3DSimple<WarpParameterAffine3D> * wp_old, int halfwin)
{
	...
	VISIOCYTELONG i,j,k;
	VISIOCYTELONG ib,ie,jb,je,kb,ke;
	...
	VISIOCYTELONG sz0 = wp_old->sz0();
	VISIOCYTELONG sz1 = wp_old->sz1();
	VISIOCYTELONG sz2 = wp_old->sz2();
...
void Warp3D::initData()
{
	...
	my_len3d = 0;
	for (VISIOCYTELONG i=0; i<3;i++) my_sz[i] = 0;
											...
											string file0_warped)
{
	...
	new4dpointer(img0_target4d, (VISIOCYTELONG)img0_sz_target[0], (VISIOCYTELONG)img0_sz_target[1], (VISIOCYTELONG)img0_sz_target[2], (VISIOCYTELONG)img0_sz_target[3], img0_target1d);
	...
	new4dpointer(img0_subject4d, (VISIOCYTELONG)img0_sz_subject[0], (VISIOCYTELONG)img0_sz_subject[1], (VISIOCYTELONG)img0_sz_subject[2], (VISIOCYTELONG)img0_sz_subject[3], img0_subject1d);
...
template <class T> bool Warp3D::applyDFtoChannel(
	...
	VISIOCYTELONG startpos_subject [3],
...
)
{
	...
	VISIOCYTELONG msz0=cur_df->sz0(), msz1=cur_df->sz1(), msz2=cur_df->sz2();
...
bool newGradientData(Vol3DSimple<MYFLOAT_JBA> * & dfx,
							 ...
							 VISIOCYTELONG tsz0, VISIOCYTELONG tsz1, VISIOCYTELONG tsz2)
{
    if (dfx || dfy || dfz || dft)
	{
...
bool Warp3D::downsample3dvol(Vol3DSimple<MYFLOAT_JBA> * &outimg, Vol3DSimple<MYFLOAT_JBA> * inimg, double dfactor)
{
	...
	VISIOCYTELONG cur_sz0 = (VISIOCYTELONG)(floor(double(inimg->sz0()) / double(dfactor)));
	VISIOCYTELONG cur_sz1 = (VISIOCYTELONG)(floor(double(inimg->sz1()) / double(dfactor)));
	VISIOCYTELONG cur_sz2 = (VISIOCYTELONG)(floor(double(inimg->sz2()) / double(dfactor)));
	...
	for (VISIOCYTELONG k=0;k<outimg->sz2();k++)
	{
		VISIOCYTELONG k2low=(VISIOCYTELONG)(floor(k*dfactor)), k2high=(VISIOCYTELONG)(floor((k+1)*dfactor-1));
		if (k2high>inimg->sz2()) k2high = inimg->sz2();
		VISIOCYTELONG kw = k2high - k2low + 1;

		for (VISIOCYTELONG j=0;j<outimg->sz1();j++)
		{
			VISIOCYTELONG j2low=(VISIOCYTELONG)(floor(j*dfactor)), j2high=(VISIOCYTELONG)(floor((j+1)*dfactor-1));
			if (j2high>inimg->sz1()) j2high = inimg->sz1();
			VISIOCYTELONG jw = j2high - j2low + 1;

			for (VISIOCYTELONG i=0;i<outimg->sz0();i++)
			{
				VISIOCYTELONG i2low=(VISIOCYTELONG)(floor(i*dfactor)), i2high=(VISIOCYTELONG)(floor((i+1)*dfactor-1));
...
bool Warp3D::upsample3dvol(Vol3DSimple<MYFLOAT_JBA> * & outimg, Vol3DSimple<MYFLOAT_JBA> * inimg, double dfactor)
{
	...
	VISIOCYTELONG tsz0o = (VISIOCYTELONG)(ceil(dfactor*tsz0)), tsz1o = (VISIOCYTELONG)(ceil(dfactor*tsz1)), tsz2o = VISIOCYTELONG(ceil(dfactor*tsz2));
	...
	VISIOCYTELONG totallen = tsz0o * tsz1o * tsz2o; //8 times because every dim is 2 times bigger
...
bool Warp3D::upsampleDisplaceField(Vol3DSimple<DisplaceFieldF3D> * & out_df, Vol3DSimple<DisplaceFieldF3D> * in_df, double dfactor)
{
	...
	bool b_noerror=true;
	VISIOCYTELONG tsz0 = in_df->sz0(), tsz1 = in_df->sz1(), tsz2 = in_df->sz2();
	VISIOCYTELONG csz0 = (VISIOCYTELONG)(ceil(dfactor*tsz0)), csz1 = (VISIOCYTELONG)(ceil(dfactor*tsz1)), csz2 = (VISIOCYTELONG)(ceil(dfactor*tsz2));
	VISIOCYTELONG i,j,k;
...
Vol3DSimple<DisplaceFieldF3D> *  Warp3D::do_global_transform(const BasicWarpParameter & bwp) 
{
	...
	VISIOCYTELONG rx,ry,rz;
	rx = VISIOCYTELONG(ceil((double)img_target->sz0() * (sqrt(2.0)-1.0) / 2.0));
	ry = VISIOCYTELONG(ceil((double)img_target->sz1() * (sqrt(2.0)-1.0) / 2.0));
	rz = VISIOCYTELONG(ceil((double)img_target->sz2() * (sqrt(2.0)-1.0) / 2.0));
	...
	double err_best; VISIOCYTELONG ck_best;
	...
	VISIOCYTELONG NCK = 180; double angle_nck = 360.0/NCK;
	VISIOCYTELONG ck;
...
vector<Coord3D_JBA> readPosFile(string posFile)
{
  ...
  VISIOCYTELONG xpos, ypos, zpos;  xpos=ypos=zpos=-1;
  VISIOCYTELONG k=0;
  while(!file_op.eof())
  {
...
bool readPosFile_3dpair(string posFile, vector<Coord3D_JBA> & pos1, vector<Coord3D_JBA> & pos2)
{
  ...
  VISIOCYTELONG xpos, ypos, zpos;  xpos=ypos=zpos=-1;
  VISIOCYTELONG s_xpos, s_ypos, s_zpos;  s_xpos=s_ypos=s_zpos=-1;
  VISIOCYTELONG k=0, KK=0;
  while(!file_op.eof())
  {
	file_op.getline(curline, 2000);
	k++;
...
Vol3DSimple<DisplaceFieldF3D> * Warp3D::do_landmark_warping(const BasicWarpParameter & bwp1)
{
	...
	VISIOCYTELONG d0=img_subject->sz0(), d1=img_subject->sz1(), d2=img_subject->sz2();

	unsigned VISIOCYTELONG n, i, j, k;
	...
	{
		fprintf(tmp_fp, "%ld,%ld,%ld\n", VISIOCYTELONG(matchSubjectPos.at(n).x), VISIOCYTELONG(matchSubjectPos.at(n).y), VISIOCYTELONG(matchSubjectPos.at(n).z));
	}
	fclose(tmp_fp);
    ...
    fprintf(tmp_fp, "x,y,z\n");
   	for (n=0; n<matchTargetPos.size(); n++)
	{
		fprintf(tmp_fp, "%ld,%ld,%ld\n", VISIOCYTELONG(matchTargetPos.at(n).x), VISIOCYTELONG(matchTargetPos.at(n).y), VISIOCYTELONG(matchTargetPos.at(n).z));
	}
	fclose(tmp_fp);
	...
	int b_verbose_print)
{
	...
	VISIOCYTELONG sz0 = img_subject->sz0(), sz1 = img_subject->sz1(), sz2 = img_subject->sz2();
...
bool diffxyz(MYFLOAT_JBA ***dx, MYFLOAT_JBA ***dy, MYFLOAT_JBA ***dz, MYFLOAT_JBA *** dt,
					 ...
					 VISIOCYTELONG d0, VISIOCYTELONG d1, VISIOCYTELONG d2)
{
    VISIOCYTELONG tlen = d0*d1*d2;
...
#define LEN_KERNEL 5
	...
	convisiocyte_1dvec(dx, tmp3d, d0, d1, d2, kernel_d_xyz_m, LEN_KERNEL, 1);
	convisiocyte_1dvec(dx, dx,    d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 2);
	convisiocyte_1dvec(dx, dx,    d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 3);

	convisiocyte_1dvec(dy, tmp3d, d0, d1, d2, kernel_d_xyz, LEN_KERNEL, 2);
	convisiocyte_1dvec(dy, dy,    d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 3);
	convisiocyte_1dvec(dy, dy,    d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 1);

	convisiocyte_1dvec(dz, tmp3d, d0, d1, d2, kernel_d_xyz_m, LEN_KERNEL, 3);
	convisiocyte_1dvec(dz, dz,    d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 1);
	convisiocyte_1dvec(dz, dz,    d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 2);

	vol3d_minus(tmp3d, cur_img3d_subject, cur_img3d_target, d0, d1, d2);

	convisiocyte_1dvec(dt, tmp3d, d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 1);
	convisiocyte_1dvec(dt, dt,    d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 2);
	convisiocyte_1dvec(dt, dt,    d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 3);
								...
								bool b_returnDTable) 
{
	...
	const VISIOCYTELONG myCUBECOLUMNNUM=12;
...
bool df_warp(Vol3DSimple<MYFLOAT_JBA> * img, Vol3DSimple<DisplaceFieldF3D> * p, bool bni)
{
	...
	VISIOCYTELONG tsz0 = img->sz0();
	VISIOCYTELONG tsz1 = img->sz1();
	VISIOCYTELONG tsz2 = img->sz2();

	DisplaceFieldF3D *** tp3d = p->getData3dHandle();

	VISIOCYTELONG totallen = tsz0 * tsz1 * tsz2;
	Coord3D_JBA * c = new Coord3D_JBA [totallen];
	MYFLOAT_JBA * v = new MYFLOAT_JBA [totallen];
...
bool df_add(Vol3DSimple<DisplaceFieldF3D> * addee, Vol3DSimple<DisplaceFieldF3D> * adder, bool b_nearest_interp)
{
	...
	VISIOCYTELONG tsz0 = adder->sz0(), tsz1 = adder->sz1(), tsz2 = adder->sz2();
	VISIOCYTELONG i,j,k;
...
bool df_copy(Vol3DSimple<DisplaceFieldF3D> * copyee, Vol3DSimple<DisplaceFieldF3D> * copyer)
{
	...
	VISIOCYTELONG tsz0 = copyer->sz0(), tsz1 = copyer->sz1(), tsz2 = copyer->sz2();
	VISIOCYTELONG i,j,k;
...
void shift_transform(Coord3D_JBA * c, VISIOCYTELONG numCoord, DisplaceFieldF3D * p, Coord3D_JBA * offset)
{
    ...
    for (VISIOCYTELONG i=0;i<numCoord;i++)
	{
	    x = c[i].x - offset->x;
		y = c[i].y - offset->y;
		z = c[i].z - offset->z;
...
template <class T> bool convisiocyte_1dvec(MYFLOAT_JBA ***outimg3d, T *** inimg3d, VISIOCYTELONG d0, VISIOCYTELONG d1, VISIOCYTELONG d2, MYFLOAT_JBA *vec, VISIOCYTELONG lenvec, int dim_of_conv)
{
	...
	if (lenvec<=0 || VISIOCYTELONG(floor(lenvec/2.0)*2)==lenvec)
	{
	...
	VISIOCYTELONG r = (lenvec-1)/2;

	//allocate memory
	VISIOCYTELONG d0big = d0+2*r;
	VISIOCYTELONG d1big = d1+2*r;
	VISIOCYTELONG d2big = d2+2*r;
	...
	VISIOCYTELONG totallen = d0big*d1big*d2big;
...
template <class T> bool Warp3D::doWarpUsingDF(T * &outimg_warped, VISIOCYTELONG * &out_sz_warped, T * inimg_subject, VISIOCYTELONG * inimg_sz, const int datatype_warped, Vol3DSimple <DisplaceFieldF3D> * cur_df, bool bni)
{
	...
	VISIOCYTELONG i,totalpix;

	out_sz_warped = new VISIOCYTELONG [4];
...
template <class T> bool Warp3D::getWarpedData(T * &outimg_warped, VISIOCYTELONG * &out_sz_warped, const int datatype_warped)
{
    if (outimg_warped || out_sz_warped)
	{
		fprintf(stderr, "The handle of warped data is not initilized as empty! Exit without copying data!\n");
...
string gen_file_name(string basename, int layer, string otherinfo)
{
	...
	a = a + layerstr + "_" + otherinfo + ".visiocyteraw";
	return a;
}
...
template <class T> bool find3DGradient(Vol3DSimple <T> * & g, Vol3DSimple <T> *subject)
{
	...
	VISIOCYTELONG d0=subject->sz0(), d1=subject->sz1(), d2=subject->sz2();
...
#define LEN_KERNEL 5
	...
	convisiocyte_1dvec(tmp3d, indata3d, d0, d1, d2, kernel_d_xyz_m, LEN_KERNEL, 1);
	convisiocyte_1dvec(tmp3d, tmp3d,  d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 2);
	convisiocyte_1dvec(tmp3d, tmp3d,  d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 3);
    vol3d_square(tmp3d, tmp3d,  d0, d1, d2);
	vol3d_plus(g3d, g3d, tmp3d, d0, d1, d2);

	convisiocyte_1dvec(tmp3d, indata3d, d0, d1, d2, kernel_d_xyz, LEN_KERNEL, 2);
	convisiocyte_1dvec(tmp3d, tmp3d,  d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 3);
	convisiocyte_1dvec(tmp3d, tmp3d,  d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 1);
    vol3d_square(tmp3d, tmp3d,  d0, d1, d2);
	vol3d_plus(g3d, g3d, tmp3d, d0, d1, d2);

	convisiocyte_1dvec(tmp3d, indata3d, d0, d1, d2, kernel_d_xyz_m, LEN_KERNEL, 3);
	convisiocyte_1dvec(tmp3d, tmp3d,  d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 1);
	convisiocyte_1dvec(tmp3d, tmp3d,  d0, d1, d2, kernel_p_xyz, LEN_KERNEL, 2);
    vol3d_square(tmp3d, tmp3d,  d0, d1, d2);
	vol3d_plus(g3d, g3d, tmp3d, d0, d1, d2);
					   ...
					   bool b_returnDTable) 
{
	...
	const VISIOCYTELONG myCUBECOLUMNNUM=4;
...
Vol3DSimple<DisplaceFieldF3D> * Warp3D::do_local_topdown_blocktps_fast(const BasicWarpParameter & bwp)
{
	...
	VISIOCYTELONG dsz0=img_subject->sz0(), dsz1=img_subject->sz1(), dsz2=img_subject->sz2();
	...
	const VISIOCYTELONG MIN_LOCALBLK_XWID = 48, MIN_LOCALBLK_YWID = 48, MIN_LOCALBLK_ZWID = 24;
    
    Vol3DSimple<DisplaceFieldF3D> * df_local = 0;

	WarpParameterAffine3D cur_wp;
	DisplaceFieldF3D cur_df;
	VISIOCYTELONG layer, i, j, k;
	...
	for (layer=NLayer-1; layer>=0; layer--)
	{
		...
		VISIOCYTELONG KGRID=(VISIOCYTELONG)(pow(2.0,double(NLayer-layer))), JGRID=(VISIOCYTELONG)(2*pow(2.0,double(NLayer-layer))), IGRID=(VISIOCYTELONG)(2*pow(2.0,double(NLayer-layer)));
		...
		VISIOCYTELONG KGRID2, JGRID2, IGRID2;
		...
		for (n_refinetps=0; n_refinetps<N_refinetps;n_refinetps++) 
		{
			...
			{
				VISIOCYTELONG kb, ke;
				if (b_useOverlappingGrid) {kb=VISIOCYTELONG(floor(k*double(sz2)/KGRID/2)); ke=VISIOCYTELONG(floor((k+2)*double(sz2)/KGRID/2))-1;} 
				else {kb=VISIOCYTELONG(floor(k*double(sz2)/KGRID)); ke=VISIOCYTELONG(floor((k+1)*double(sz2)/KGRID))-1;}

				double cur_center_z = (double(kb)+ke)/2.0;
				if (b_use_MINBLKSZ)
				{
					if (cur_center_z - MIN_LOCALBLK_ZWID/2 < kb) {kb=VISIOCYTELONG(floor(cur_center_z - MIN_LOCALBLK_ZWID/2)); if (kb<0) kb=0;}
					if (cur_center_z + MIN_LOCALBLK_ZWID/2 > ke) {ke=VISIOCYTELONG(ceil(cur_center_z + MIN_LOCALBLK_ZWID/2)); if (ke>=sz2) ke=sz2-1;}
				}

				for (j=0;j<JGRID2;j++)
				{
					VISIOCYTELONG jb,je;
					if (b_useOverlappingGrid) {jb=VISIOCYTELONG(floor(j*double(sz1)/JGRID/2)); je=VISIOCYTELONG(floor((j+2)*double(sz1)/JGRID/2))-1;}
					else {jb=VISIOCYTELONG(floor(j*double(sz1)/JGRID)); je=VISIOCYTELONG(floor((j+1)*double(sz1)/JGRID))-1;}

					double cur_center_y = (double(jb)+je)/2.0;
					if (b_use_MINBLKSZ)
					{
						if (cur_center_y - MIN_LOCALBLK_YWID/2 < jb) {jb=VISIOCYTELONG(floor(cur_center_y - MIN_LOCALBLK_YWID/2)); if (jb<0) jb=0;}
						if (cur_center_y + MIN_LOCALBLK_YWID/2 > je) {je=VISIOCYTELONG(ceil(cur_center_y + MIN_LOCALBLK_YWID/2)); if (je>=sz1) je=sz1-1;}
					}

					for (i=0;i<IGRID2;i++)
					{
						VISIOCYTELONG ib,ie;
						if (b_useOverlappingGrid) {ib=VISIOCYTELONG(floor(i*double(sz0)/IGRID/2)); ie=VISIOCYTELONG(floor((i+2)*double(sz0)/IGRID/2))-1;}
						else {ib=VISIOCYTELONG(floor(i*double(sz0)/IGRID)); ie=VISIOCYTELONG(floor((i+1)*double(sz0)/IGRID))-1;}
					
                        double cur_center_x = (double(ib)+ie)/2.0;
						if (b_use_MINBLKSZ)
						{
							if (cur_center_x - MIN_LOCALBLK_XWID/2 < ib) {ib=VISIOCYTELONG(floor(cur_center_x - MIN_LOCALBLK_XWID/2)); if (ib<0) ib=0;}
							if (cur_center_x + MIN_LOCALBLK_XWID/2 > ie) {ie=VISIOCYTELONG(ceil(cur_center_x + MIN_LOCALBLK_XWID/2)); if (ie>=sz0) ie=sz0-1;}
						}
                        ...
                        VISIOCYTELONG nnz_targetgmap_blk, nnz_subjectgmap_blk;
			...
			if (b_filterUnnecessaryCPoints)
			{
				...
				for (k=0;k<KGRID2;k++)
				{
					VISIOCYTELONG kbn, ken;
					kbn=k-1; if (kbn<0) kbn=0;
					ken=k+1; if (ken>=KGRID2) ken=KGRID2-1;

					VISIOCYTELONG kb, ke;
					if (b_useOverlappingGrid) {kb=VISIOCYTELONG(floor(k*double(sz2)/KGRID/2)); ke=VISIOCYTELONG(floor((k+2)*double(sz2)/KGRID/2))-1;} //add the overlapping blocks
					else {kb=VISIOCYTELONG(floor(k*double(sz2)/KGRID)); ke=VISIOCYTELONG(floor((k+1)*double(sz2)/KGRID))-1;}

					for (j=0;j<JGRID2;j++)
					{
						VISIOCYTELONG jbn,jen;
...
bool assign_maskimg_val( Vol3DSimple <UINT8_JBA> * maskImg, Vol3DSimple <MYFLOAT_JBA> * targetImg,  Vol3DSimple <MYFLOAT_JBA> * subjectImg)
{
	...
	VISIOCYTELONG csz0=maskImg->sz0(), csz1=maskImg->sz1(), csz2=maskImg->sz2();
	VISIOCYTELONG nValidPix=0;
	for (k=0;k<csz2; k++)
	  for (j=0;j<csz1;j++)
	    for (i=0;i<csz0;i++)
		{
...
bool ebs3d_interface(DisplaceFieldF3D & wp_current,  Vol3DSimple <MYFLOAT_JBA> * targetImg,  Vol3DSimple <MYFLOAT_JBA> * subjectImg, int nloops)
{
	...
	VISIOCYTELONG csz0=targetImg->sz0(), csz1=targetImg->sz1(), csz2=targetImg->sz2();
	...
	unsigned VISIOCYTELONG tmpi; double minv=err_history.at(0); unsigned VISIOCYTELONG min_i=0;
...
Vol3DSimple<DisplaceFieldF3D> * cdum(const vector <Coord3D_JBA> & matchTargetPos, const vector <Coord3D_JBA> & matchSubjectPos, VISIOCYTELONG sz0, VISIOCYTELONG sz1, VISIOCYTELONG sz2, DFComputeMethodType method)
{
	switch (method)
	{
		case DF_GEN_TPS:
													  ...
													  DFComputeMethodType interp_method) 
{
	...
	VISIOCYTELONG p;
	VISIOCYTELONG gsz0 = df_subv->sz0(), gsz1 = df_subv->sz1(), gsz2 = df_subv->sz2();
																 ...
																 VISIOCYTELONG gfactor_x, VISIOCYTELONG gfactor_y, VISIOCYTELONG gfactor_z)
{
	...
	VISIOCYTELONG gsz0 = (VISIOCYTELONG)(ceil((double(sz0)/gfactor_x)))+1, gsz1 = (VISIOCYTELONG)(ceil((double(sz1)/gfactor_y)))+1, gsz2 = (VISIOCYTELONG)(ceil((double(sz2)/gfactor_z)))+1;
...
{
	VISIOCYTELONG k, j, i, k1, j1, i1, kk, jj, ii;
	...
	VISIOCYTELONG n0 = df_regular_grid->sz0()-3, n1 = df_regular_grid->sz1()-3, n2 = df_regular_grid->sz2()-3;
	if (n0<=0 || n1<=0 || n2<=0)
	{
