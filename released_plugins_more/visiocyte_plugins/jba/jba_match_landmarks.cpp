...
template <class T> double cmpFeaturesonTwoImages(Vol3DSimple<T>* img_target, VISIOCYTELONG txpos, VISIOCYTELONG typos, VISIOCYTELONG tzpos,
												 Vol3DSimple<T>* img_subject, VISIOCYTELONG sxpos, VISIOCYTELONG sypos, VISIOCYTELONG szpos,
												 VISIOCYTELONG R_compare, const KernelSet * KS) 
{
	if (!img_target || !img_target->valid() || !img_subject || !img_subject->valid())
		return -1; 
	
	VISIOCYTELONG TSZ2=img_target->sz2(), TSZ1=img_target->sz1(), TSZ0=img_target->sz0();
	VISIOCYTELONG SSZ2=img_subject->sz2(), SSZ1=img_subject->sz1(), SSZ0=img_subject->sz0();
	...
	double s=0;
	VISIOCYTELONG i,j,k, kt,jt,it, ks,js,is;
	VISIOCYTELONG cnt=0;
...
template <class T> double cmpFeaturesonTwoImages(Vol3DSimple<T>* img_target, VISIOCYTELONG txpos, VISIOCYTELONG typos, VISIOCYTELONG tzpos,
												 Vol3DSimple<T>* img_subject, VISIOCYTELONG sxpos, VISIOCYTELONG sypos, VISIOCYTELONG szpos,
												 VISIOCYTELONG R_compare, const PointMatchMethodType mMethod, Vector1DSimple<double> * matchingScore) 
{
#define USE_SPHERE_WIN
    ...
    if (mMethod==MATCH_MEANCIRCLEINTENSITY) 
	{
		...
		VISIOCYTELONG * cir_cnt = new VISIOCYTELONG [R_compare+1];
		...
		for (k=-R_compare; k<=R_compare; k+=1)
		{
			...
			for (j=-R_compare; j<=R_compare; j+=1) 
			{
				...
				for (i=-R_compare; i<=R_compare; i+=1)
				{
					...
					VISIOCYTELONG tmp_r=VISIOCYTELONG(floor(sqrt(tdis3)));
		...
		for (k=-R_compare; k<=R_compare; k+=4)
		{
			...
			for (j=-R_compare; j<=R_compare; j+=4) 
			{
				...
				for (i=-R_compare; i<=R_compare; i+=4)
				{
					...
					ind_t = VISIOCYTELONG(floor(cur_t*ILEVEL)); if (ind_t<0) ind_t=0; if (ind_t>=ILEVEL) ind_t=ILEVEL-1;
					ind_s = VISIOCYTELONG(floor(cur_s*ILEVEL)); if (ind_s<0) ind_s=0; if (ind_s>=ILEVEL) ind_s=ILEVEL-1;
											  ...
											  )
{
	...
	VISIOCYTELONG R_search = VISIOCYTELONG(floor(img_target->sz0()/16.0));
	VISIOCYTELONG R_compare = 48*(double(img_target->sz0())/512);
	...
	const VISIOCYTELONG Step_Search_coarse = (VISIOCYTELONG)(floor(R_search/2.0)), Step_Search_fine = 1;
	
	matchTargetPos.clear();
	matchSubjectPos.clear();
	
	unsigned VISIOCYTELONG n;
	...
	if (b_use_target_mrange)
	{
		...
		{
			VISIOCYTELONG px = VISIOCYTELONG(priorTargetPos.at(n).x);
			VISIOCYTELONG py = VISIOCYTELONG(priorTargetPos.at(n).y);
			VISIOCYTELONG pz = VISIOCYTELONG(priorTargetPos.at(n).z);
	...
	{
		dis_prior[n][n]=0;
		VISIOCYTELONG txpos_1 = (VISIOCYTELONG)(priorTargetPos.at(n).x), typos_1 = (VISIOCYTELONG)(priorTargetPos.at(n).y), tzpos_1 = (VISIOCYTELONG)(priorTargetPos.at(n).z);
		for (m=n+1; m<priorTargetPos.size(); m++)
		{
			VISIOCYTELONG txpos_2 = (VISIOCYTELONG)(priorTargetPos.at(m).x), typos_2 = (VISIOCYTELONG)(priorTargetPos.at(m).y), tzpos_2 = (VISIOCYTELONG)(priorTargetPos.at(m).z);
	...
	VISIOCYTELONG n_added=0; 
    for (n=0; n<priorTargetPos.size(); n++)
	{
		VISIOCYTELONG txpos_target = (VISIOCYTELONG)(priorTargetPos.at(n).x), typos_target = (VISIOCYTELONG)(priorTargetPos.at(n).y), tzpos_target = (VISIOCYTELONG)(priorTargetPos.at(n).z);
		VISIOCYTELONG txpos_subject = (VISIOCYTELONG)(matchSubjectPos_searchCenter.at(n).x), typos_subject = (VISIOCYTELONG)(matchSubjectPos_searchCenter.at(n).y), tzpos_subject = (VISIOCYTELONG)(matchSubjectPos_searchCenter.at(n).z);
			...
			for (j=typos_subject-R_search; j<=typos_subject+R_search; j+=4)
			{
				...
				for (i=txpos_subject-R_search; i<=txpos_subject+R_search; i+=4)
				{
					...
					if (priorTargetPos.size()>1)
					{
						...
						for (VISIOCYTELONG itmp=0;itmp<priorTargetPos.size(); itmp++) 
						{
							if (itmp==n) continue;
		...
		for (kk=0; kk<N_USE_METHOD; kk++)
		{
			...
			for (k=(VISIOCYTELONG)bestCoordCoarse.getData1dHandle()[kk].z-3; k<=(VISIOCYTELONG)bestCoordCoarse.getData1dHandle()[kk].z+3; k+=Step_Search_fine)
			{
				if (k<0 || k>=img_subject->sz2()) continue; 
				for (j=(VISIOCYTELONG)bestCoordCoarse.getData1dHandle()[kk].y-xy_fine_radius; j<=(VISIOCYTELONG)bestCoordCoarse.getData1dHandle()[kk].y+xy_fine_radius; j+=Step_Search_fine)
				{
					if (j<0 || j>=img_subject->sz1()) continue; 
					for (i=(VISIOCYTELONG)bestCoordCoarse.getData1dHandle()[kk].x-xy_fine_radius; i<=(VISIOCYTELONG)bestCoordCoarse.getData1dHandle()[kk].x+xy_fine_radius; i+=Step_Search_fine)
					{
						if (i<0 || i>=img_subject->sz0()) continue; 
						if (b_use_subject_mrange) 
						{
		...
		else
		{
			...
			{
				kk1=0; double tmp_best_score=-1, tmp_score; VISIOCYTELONG tmp_ind;
				for (kk=kk1;kk<N_USE_METHOD;kk++)
				{
					cmpFeaturesonTwoImages(img_target, txpos_target, typos_target, tzpos_target,
										   img_subject, VISIOCYTELONG(bestCoordFine.getData1dHandle()[kk].x), VISIOCYTELONG(bestCoordFine.getData1dHandle()[kk].y), VISIOCYTELONG(bestCoordFine.getData1dHandle()[kk].z),
		...
		if (b_add_to_list==true)
		{
			printf("target [%ld][%ld][%ld] -> subject[%ld][%ld][%ld]\n",
				   ...
				   VISIOCYTELONG(optimal_subject_point.z), VISIOCYTELONG(optimal_subject_point.y), VISIOCYTELONG(optimal_subject_point.x));
			...
			for (n=n_added-1; n>=0; n--)
			{
				if (distMove[n] > distMean + t_thres*distStd)
				{
					printf("Remove target [%ld][%ld][%ld] -> subject[%ld][%ld][%ld] dist[%ld]=%5.3f > %5.3f\n",
						   VISIOCYTELONG(matchTargetPos.at(n).z), VISIOCYTELONG(matchTargetPos.at(n).y), VISIOCYTELONG(matchTargetPos.at(n).x),
						   VISIOCYTELONG(matchSubjectPos.at(n).z), VISIOCYTELONG(matchSubjectPos.at(n).y), VISIOCYTELONG(matchSubjectPos.at(n).x),
									   ...
									   vector<Coord3D_JBA> & matchSubjectPos, PointMatchScore & matchScore)
{
	...
	unsigned VISIOCYTELONG i,j,k,n;
	...
	for (k=0;k<matchTargetPos.size(); k++)
	{
		printf("cc[%ld]=%5.3f\t target=[%ld,%ld,%ld] subject=[%ld,%ld,%ld]\n", k+1, cc[k],
			   VISIOCYTELONG(matchTargetPos.at(k).x), VISIOCYTELONG(matchTargetPos.at(k).y), VISIOCYTELONG(matchTargetPos.at(k).z),
			   VISIOCYTELONG(matchSubjectPos.at(k).x), VISIOCYTELONG(matchSubjectPos.at(k).y), VISIOCYTELONG(matchSubjectPos.at(k).z));
	}
