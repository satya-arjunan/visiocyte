...
void printHelp()
{
    ...
    printf("\n===================================================================================================\n");
    printf("\nUsage: ./visiocyte -x brainaligner -f brainaligner -p \"#t <input_target_image> #s <input_subject_image> #o <aligned/warped_subject_image> #w <registration_type> ... <other_arguments>\"\n");
    printf("\n");
    printf("\t [-t <input_target_image>]          .raw/.tif/.lsm file containing 3D template/target stack to which a subject image will be warped/mapped. The .raw file is in VISIOCYTE's RAW format.\n");
    ...
    printf("\t [-L <landmark_file_target>]        a plain text marker file (in VISIOCYTE's .marker format) containing target image landmarks (if not specified, JBA defines these landmarks automatically based on the target image's content).\n");
    printf("\n");
    printf("\t -s <input_subject_image>           .raw/.tif/.lsm file containing 3D subject which will deform/warp to best appriximate the geometry of the template/target. The .raw file is in VISIOCYTE's RAW format.\n");
    ...
    printf("\t [-l <landmark_file_subject>]       a plain text marker file (in VISIOCYTE's .marker format) containing subject image landmarks. If un-specified, then JBA searches the best corresponding landmarks using the RLM algoriothm.\n");
    printf("\n");
    printf("\t -o <aligned/warped_subject_image>  .raw/.tif file containing the aligned result stack. (Highly recommended to use VISIOCYTE's RAW format)\n");
    printf("\n");
    ...
    printf("\n");
    printf("\t [-F <displacement_field_file>]     output the displacement field to file(s). VISIOCYTE's RAW format files (4-byte float) containing the displacement field (DF) will be produced in separate files with _sx, _sy, and _sz tags. \n");
...
void JBAPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    if (menu_name == tr("about"))
    {
        visiocyte_msg(tr("This is a simple plugin to port Janelia BrainAligner developed by Hanchuan Peng (Nature Methods 2011) to Visiocyte.\n\n"
"At this moment, this plugin can be executed only via command-line use of Visiocyte. The GUI access of the plugin will be added later.\n\n"                   
...
bool JBAPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    if (func_name == tr("brainaligner"))
    {
        ...
        {
            //print Help info
            cout<<endl<<"Visiocyte JBA-Plugin"<<endl;
            printHelp();
            return false;
        }
        ...
        VISIOCYTELONG XYPlaneMaxSize=512; //the -B option
        ...
        VISIOCYTELONG * sz_target = 0; /* note that this variable must be initialized as NULL. */
        int datatype_target = 0;

        unsigned char * img_subject = 0; /* note that this variable must be initialized as NULL. */
        VISIOCYTELONG * sz_subject = 0; /* note that this variable must be initialized as NULL. */
        int datatype_subject = 0;

        unsigned char * img_warped = 0;
        VISIOCYTELONG * sz_warped = 0;
        ...
        if (b_onlyconvertformat==true)
        {
            ...
            if (b_attach_channel_to_subject)
            {
                ...
                {
                    VISIOCYTELONG chansz = sz_subject[0]*sz_subject[1]*sz_subject[2];
                    VISIOCYTELONG subject_sz_old[4]; for (int ii=0; ii<4; ii++) subject_sz_old[ii] = sz_subject[ii];
