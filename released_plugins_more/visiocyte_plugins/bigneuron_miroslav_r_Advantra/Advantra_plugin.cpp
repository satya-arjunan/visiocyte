...
static VISIOCYTELONG channel = 1; // default channel
...
void Advantra::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
//        dialog.get_num("Percentile [50, 100].", PARA.perc);
        ...
        if(PARA.bratio <= 0 || PARA.bratio > 1.0){visiocyte_msg(QObject::tr("Background ratio is out of range")); return;}
//        if(PARA.perc < 50  || PARA.perc > 100){visiocyte_msg(QObject::tr("Percentile is out of range")); return;}
        ...
        if(PARA.angSig < 20 || PARA.angSig>90){visiocyte_msg(QObject::tr("Angular sigma is out of range")); return;}
	...
	{
		visiocyte_msg(tr("Tool for automatic neuron reconstruction from microscopy image stacks.. "
...
void print_help(){
    ...
    printf("visiocyte -x Advantra -f advantra_func -i <inimg_file> -p <scal bratio znccTh Ndir angSig Ni Ns zDist>\n");
...
bool Advantra::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
//        PARA.perc       = (paras.size() >= k+1)   ? atoi(paras[k])              : perc;      k++;
        ...
        // check user input
        if(PARA.scal <= 5  || PARA.scal>20){visiocyte_msg(QObject::tr("Scale is out of range")); return 0;}
        if(PARA.bratio <= 0 || PARA.bratio > 1.0){visiocyte_msg(QObject::tr("Background ratio is out of range"));return 0;}
//        if(PARA.perc < 50  || PARA.perc > 100){visiocyte_msg(QObject::tr("Percentile is out of range")); return 0;}
        if(PARA.znccTh < 0.5 || PARA.znccTh >= 1.0){visiocyte_msg(QObject::tr("Correlation is out of range"));return 0;}
        if(PARA.Ndir<5|| PARA.Ndir>20){visiocyte_msg(QObject::tr("# directions is out of range")); return 0;}
        if(PARA.angSig<20|| PARA.angSig>90){visiocyte_msg(QObject::tr("Angular sigma is out of range")); return 0;}
        if(PARA.Ni < 2 || PARA.Ni>50){visiocyte_msg(QObject::tr("# iterations is out of range")); return 0;}
        if(PARA.Ns <=  0 || PARA.Ns>20){visiocyte_msg(QObject::tr("# states is out of range")); return 0;}
        if(PARA.zDist < 1 || PARA.zDist>4){visiocyte_msg(QObject::tr("zdist is out of range")); return 0;}
...
// another set of methods/classes used in reconstruction_func
