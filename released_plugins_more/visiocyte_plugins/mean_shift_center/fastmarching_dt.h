#ifndef FASTMARCHING_DT_H
#define FASTMARCHING_DT_H
 ...
 *
 * Notice :
 * 1. the input pixel number should not be larger than 2G if sizeof(long) == 4 //this should have been solved by changing "long" to "VISIOCYTELONG". see changes introduced in version 634
 * 2. The background point is of intensity 0
 * *****************************************************************************/

template<class T> bool fastmarching_dt(T * inimg1d, float * &phi, VISIOCYTELONG sz0, VISIOCYTELONG sz1, VISIOCYTELONG sz2, int cnn_type = 3, int bkg_thresh = 0, double z_thickness = 1.0)
{
    ...
    VISIOCYTELONG bkg_count = 0;                          // for process counting
    VISIOCYTELONG bdr_count = 0;                          // for process counting
    ...
    map<VISIOCYTELONG, HeapElem*> elems;

    // init heap
    {
        VISIOCYTELONG i = -1, j = -1, k = -1;
        for(VISIOCYTELONG ind = 0; ind < tol_sz; ind++)
        {
...
//    cout<<"elems.size() = "<<elems.size()<<endl;
    // loop
    VISIOCYTELONG time_counter = bkg_count;
    double process1 = 0;
    while(!heap.empty())
    {
        ...
        {
            //cout<<"\r"<<((VISIOCYTELONG)process2)/1000.0<<"%";cout.flush();
            process1 = process2;
