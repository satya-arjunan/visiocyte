...
void mean_shift_plugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
bool mean_shift_plugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
void mean_shift_plugin::all_method_comp_func(VISIOCYTEPluginCallback2 & callback, const VISIOCYTEPluginArgList & input,
...
void mean_shift_plugin::all_method_comp(VISIOCYTEPluginCallback2 *callback)
{
    ...
    else if(pixeltype == 4) //VISIOCYTE_FLOAT32;
    {
...
//                mass_center[1]+1,mass_center[2]+1,0,1,"gradient","",255,255,0);
    ...
    visiocyte_msg("Computation complete. Markers loaded");
}

void mean_shift_plugin::load_image_marker(VISIOCYTEPluginCallback2 & callback,const VISIOCYTEPluginArgList & input,
                        unsigned char * & image_data,LandmarkList &LList,int &intype,VISIOCYTELONG sz_img[4])
{
    ...
    if (LList.count()<=0)
    {
        visiocyte_msg("Did not find any valid markers\n",0);
        return;
    }
    qDebug()<<"LList size:"<<LList.count();
}

void mean_shift_plugin::gradient(VISIOCYTEPluginCallback2 & callback, const VISIOCYTEPluginArgList & input,
                                 VISIOCYTEPluginArgList & output)
{
    mean_shift_fun fun_obj;
    vector<char*> infiles, inparas, outfiles;
...
void mean_shift_plugin::ray_shoot(VISIOCYTEPluginCallback2 & callback, const VISIOCYTEPluginArgList & input,
                                  VISIOCYTEPluginArgList & output)
{
    mean_shift_fun fun_obj;
    vector<char*> infiles, inparas, outfiles;
    ...
    if (inparas.size()==1)
    {
        ...
        if (tmp>=0 && tmp<=255)
        {
            ...
            visiocyte_msg("The parameter of window radius is not valid, the program will use default value of 15",0);
    }
...
void mean_shift_plugin::mean_shift_center(VISIOCYTEPluginCallback2 & callback, const VISIOCYTEPluginArgList & input,
                                          VISIOCYTEPluginArgList & output,int methodcode)
{
    //Input,output check
    mean_shift_fun fun_obj;
    vector<char*> infiles, inparas, outfiles;
...
void mean_shift_plugin::printHelp()
{
    ...
    printf("Usage visiocyte -x mean_shift_center_finder -f mean_shift -i <input.visiocyteraw> <input.visiocyteraw.marker> "
           "[-p <int search_window_radius>(2-30,default 15)] [-o <output_image.marker>]\n");
    printf("Usage visiocyte -x mean_shift_center_finder -f mean_shift_with_constraints -i <input.visiocyteraw> <input.visiocyteraw.marker> "
           "[-p <int search_window_radius>(2-30,default 15)] [-o <output_image.marker>]\n");
    printf("Usage visiocyte -x mean_shift_center_finder -f ray_shoot -i <input.visiocyteraw> <input.visiocyteraw.marker> "
           "[-p <int background_thresh>(0-255,default 70)] [-o <output_image.marker>]\n");
    printf("Usage visiocyte -x mean_shift_center_finder -f gradient_distance_transform -i <input.visiocyteraw> <input.visiocyteraw.marker> "
...
QList <ImageMarker> mean_shift_plugin::readMarker_file(const QString & filename)
{
    ...
    {
#ifndef DISABLE_VISIOCYTE_MSG
        visiocyte_msg(QString("open file [%1] failed!").arg(filename),0);
#endif
        return tmp_list;
    }
