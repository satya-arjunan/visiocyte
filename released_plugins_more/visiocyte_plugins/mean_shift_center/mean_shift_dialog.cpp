...
mean_shift_dialog::mean_shift_dialog(VISIOCYTEPluginCallback2 *cb,int method)
{
    callback=cb;
    image1Dc_in=0;
    pixeltype=VISIOCYTE_UNKNOWN;
...
//        fprintf(fp,"%1d %5.1f %5.1f %5.1f\n",j+1,mass_center[0],mass_center[1],mass_center[2]);
    ...
    QList<NeuronTree> * mTreeList = callback->getHandleNeuronTrees_3DGlobalViewer(visiocytehandleList_current[0]);
    if(mTreeList->size() == 0)
    {
        //visualize
        ...
        callback->setLandmark(visiocytehandle_main,LList_new_center);
        if (methodcode==2)
            callback->setImageName(visiocytehandle_main, "mean_shift_constraints_"+callback->getImageName(curwin));
        else
            callback->setImageName(visiocytehandle_main, "mean_shift_"+callback->getImageName(curwin));
    ...
    }else
    {
        ...
        QString swc_out_name = callback->getImageName(visiocytehandleList_current[0]) + "_meanshifted.swc";
        writeSWC_file(swc_out_name,nt);
        visiocyte_msg(QString("Now you can drag and drop the meanshifted swc fle [%1] into Visiocyte.").arg(swc_out_name));
    }
...
//  Image4DSimple image4d;
//  visiocytehandle visiocytehandle_main=callback->newImageWindow();
//  image4d.setData(intensity,100,200,100,1,VISIOCYTE_UINT8);
//  callback->setImage(visiocytehandle_main, &image4d);
//  callback->open3DWindow(visiocytehandle_main);
//}
