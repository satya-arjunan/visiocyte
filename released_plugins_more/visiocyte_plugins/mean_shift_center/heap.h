...
{
	int    heap_id;
    VISIOCYTELONG   img_ind;
	double value;
    HeapElem(VISIOCYTELONG _ind, double _value)
	{
		heap_id = -1;
		img_ind = _ind;
		value = _value;
...
{
    VISIOCYTELONG prev_ind;  // previous img ind
    HeapElemX(VISIOCYTELONG _ind, double _value) : HeapElem(_ind, _value){prev_ind = -1;}
};
