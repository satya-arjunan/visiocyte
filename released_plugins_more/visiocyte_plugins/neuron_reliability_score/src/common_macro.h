...
#define INIT_IMAGE_VARIABLE string infile = parser.filelist.size() >= 1 ? parser.filelist.at(0) : string(""); \
                            ...
                            VISIOCYTELONG* in_sz = 0, * out_sz = 0; int datatype = VISIOCYTE_UINT8; int channel = 0;  
#endif

#ifndef LOAD_FIRST_IMAGE
#define LOAD_FIRST_IMAGE if(infile != "" && is_img_file(infile) && !my_load_image((char*) infile.c_str(), indata1d, in_sz, datatype)) {cout<<"Unable to load image"<<endl; return false;} if(infile != "" &&  indata1d && datatype != VISIOCYTE_UINT8){cerr<<"Can not deal with 12bit or 16bit data."<<endl; return false;}
#endif
...
#ifdef __VISIOCYTE_CONVERT_PLUGIN__
#ifndef SAVE_OUTPUT_IMAGE
#define SAVE_OUTPUT_IMAGE if(outdata1d) \
	{ \
		if(out_sz == 0) {out_sz = new VISIOCYTELONG[4]; out_sz[0] = in_sz[0]; out_sz[1] = in_sz[1]; out_sz[2] = in_sz[2]; out_sz[3] = in_sz[3];} \
...
{ \
	...
	my_save_image(outfile.c_str(), phiimg, out_sz, VISIOCYTE_UINT8);\
