...
void neuronScore::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	if (menu_name == tr("calculate_score"))
	{
        ...
        if(myDlg.isaccept){
            ...
            visiocyte_msg("Done! files saved to "+fname_output+"_score.txt");
        }
	}
	...
	{
		visiocyte_msg(tr("This plugin will generate the reliability score of neuron reconstruction.. "
			"Developed by Hanbo Chen, Hang Xiao, Hanchuan Peng, 2015-4-27"));
	}
}

bool neuronScore::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
void doCalculateScore(VISIOCYTEPluginCallback2 &callback, QString fname_img, QString fname_swc, QString fname_output, int score_type=1, float radius_factor=2, bool is_gui=0)
{
    //load image
    ...
    if(!simple_loadimage_wrapper(callback, fname_img.toStdString().c_str(), p_img1d, sz_img, type_img)){
        if(is_gui){
            visiocyte_msg("ERROR: failed to load image file "+fname_img);
        }else{
    ...
    if(sz_img[3]>1){
        if(is_gui){
            visiocyte_msg("WARNING: image has more than 1 color channel. Only the first channel will be used.");
        }else{
    ...
    if(neuronTree.size()==0){
        if(is_gui){
            visiocyte_msg("ERROR: failed to load swc file "+fname_swc);
        }else{
    ...
    if(score_type <2) //topology segment
        if(type_img==1)
            ...
            if(is_gui){
                visiocyte_msg("ERROR: image formate not supported. Only support: UINT8, UINT16, and FLOAT32.");
            }else{
    ...
    if(!fp.is_open()){
        if(is_gui){
            visiocyte_msg("ERROR: cannot open file to save: "+fname_outscore);
        }else{
    ...
    fp.close();
    for(VISIOCYTELONG i = 0; i<neuronTree.size(); i++){
...
NeuronTree calculateScoreTerafly(VISIOCYTEPluginCallback2 &callback,QString fname_img, NeuronTree nt, int score_type=1, float radius_factor=2)
{
    NeuronTree result;
    ...
    for(VISIOCYTELONG i=0; i<seg_list.size();i++)
    {
        ...
        start_z = seg->at(0)->z;
        end_z = seg->at(0)->z;
        for(VISIOCYTELONG j=1; j<seg->size();j++)
        {
        ...
        for(VISIOCYTELONG j=0; j<seg->size();j++)
        {
            seg->at(j)->x -= start_x;
            seg->at(j)->y -= start_y;
            seg->at(j)->z -= start_z;
...
//        QString imageSaveString = fname_img + QString("/x_%1_y%2_z%3.visiocyteraw").arg(seg->at(0)->x).arg(seg->at(0)->y).arg(seg->at(0)->z);
//        simple_saveimage_wrapper(callback, "test.visiocyteraw",(unsigned char *)total1dData, mysz, 1);
...
//        saveSWC_file(fname_tmp.toStdString(), *seg);
    ...
    tree.clear();
    map<MyMarker*, VISIOCYTELONG> index_map;
    for (VISIOCYTELONG i=0;i<seg_list.size();i++)
        for (VISIOCYTELONG j=0;j<seg_list[i]->size();j++)
        {
            tree.push_back(seg_list[i]->at(j));
            index_map.insert(pair<MyMarker*, VISIOCYTELONG>(seg_list[i]->at(j), tree.size()-1));
        }

//    visiocyte_msg(QString("tree size is %1").arg(tree.size()));
...
void printHelp()
{
    qDebug()<<"visiocyte -x libname -f calculate_score -i <input_image> <input_swc> -o <output> -p [<score type> [<radius_factor>]]";
...
neuronScoreDialog::neuronScoreDialog(VISIOCYTEPluginCallback2 &cb, QWidget *pt)
{
    callback=&cb;
    parent=pt;
...
{
    QSettings settings("VISIOCYTE plugin","neuronReliablilityScore");
