...
void nt_selfcorrect_func::correct_tracing(QString fname_img, QString fname_swc, QString fname_score, QString fname_output, VISIOCYTEPluginCallback2* cb)
{
    taskID = 1;
    callback=cb;
...
void nt_selfcorrect_func::tracing_correct(QString fname_img, QString fname_output, VISIOCYTEPluginCallback2* cb)
{
    taskID = 2;
    callback=cb;
...
void nt_selfcorrect_func::smart_tracing(QString fname_img, QString fname_output, VISIOCYTEPluginCallback2* cb)
{
    taskID = 3;
    callback=cb;
...
void nt_selfcorrect_func::smart_tracing(QString fname_img, QString fname_output, VISIOCYTEPluginCallback2* cb, int ch)
{
    taskID = 3;
    callback=cb;
...
//    for(VISIOCYTELONG i = 0; i<ntmarkers.size(); i++){
...
bool nt_selfcorrect_func::getTrainingSample()
{
    ...
    float minScore=-1;
    for(VISIOCYTELONG nid=0; nid<ntmarkers.size(); nid++){
    ...
    VISIOCYTELONG negativeCount=0;
    VISIOCYTELONG positiveCount=0;
    for(VISIOCYTELONG nid=0; nid<ntmarkers.size(); nid++){
        MyMarker* current = ntmarkers.at(nid);
        MyMarker* parent = current->parent;
        if(parent<=(MyMarker*)0){
    ...
    VISIOCYTELONG sampleNum=MIN(positiveCount,param.sample_maxNumber);
    ...
    VISIOCYTELONG positiveRec=0, negativeRec=0;
    double positiveLine=0, negativeLine=0;
    for(VISIOCYTELONG idx=0; idx<sz_img[0]*sz_img[1]*sz_img[2]; idx++){
...
//        qDebug()<<"negative: "<<i<<":"<<x<<":"<<y<<":"<<z;
    ...
    if(FLAG_TEST){
        ...
        for(VISIOCYTELONG i=0; i<sampleNum*(featureNum+1); i++){
            fp<<train_data[i];
...
bool nt_selfcorrect_func::predictExisting()
{
    ...
    for(int i=0; i<ntmarkers.size(); i++){
        double label = predictWindow((VISIOCYTELONG)ntmarkers.at(i)->x,(VISIOCYTELONG)ntmarkers.at(i)->y,(VISIOCYTELONG)ntmarkers.at(i)->z);
        if(label==VAL_FG){
...
bool nt_selfcorrect_func::correctExisting()
{
    ...
    map<MyMarker*, VISIOCYTELONG> compId;
    VISIOCYTELONG tmp_id=0;
    for(VISIOCYTELONG i=0; i<ntmarkers.size(); i++){
    ...
    vector<VISIOCYTELONG> compSize(tmp_id,0);
    for(VISIOCYTELONG i=0; i<ntmarkers.size(); i++){
    ...
    multimap<VISIOCYTELONG, VISIOCYTELONG, std::greater<VISIOCYTELONG> > sizeSort;
    for(int i=0; i<compSize.size(); i++){
        sizeSort.insert(std::pair<VISIOCYTELONG,VISIOCYTELONG>(compSize[i],i) );
    }
    multimap<VISIOCYTELONG, VISIOCYTELONG, std::greater<VISIOCYTELONG> >::iterator mapiter=sizeSort.begin();
    while(mapiter!=sizeSort.end() && (mapiter->first)>param.correct_sizeThr){
        VISIOCYTELONG cid=mapiter->second;
    ...
    for(VISIOCYTELONG cid=0; cid<components.size(); cid++){
        for(VISIOCYTELONG nid=0; nid<(components[cid]).size(); nid++){
    ...
    for(VISIOCYTELONG cid0=0; cid0<components.size(); cid0++){
        qDebug()<<"fragment: "<<cid0;
        //find the top nearest neighbors
        multimap<double, VISIOCYTELONG> disSort;
        if(param.correct_neighborNumber>=components.size()-1){ //total number of neibor is less than maximum neibor allowed
            for(VISIOCYTELONG cid1=cid0+1; cid1<components.size(); cid1++){
                disSort.insert(pair<double, VISIOCYTELONG>((double)cid1,cid1));
            }
        }else{
            for(VISIOCYTELONG cid1=0; cid1<components.size(); cid1++){
                if(cid1==cid0) continue;
                double dis=getMarkersDistance(components[cid0],components[cid1]);
                disSort.insert(pair<double, VISIOCYTELONG>(dis,cid1));
            }
        }
        VISIOCYTELONG ccid=-1; //the id of component it can be connected
        vector<MyMarker*> linkage; //markers linking two components
        multimap<double, VISIOCYTELONG>::iterator iter=disSort.begin();
        for(int tmpid=0; tmpid<param.correct_neighborNumber && iter!=disSort.end(); tmpid++, iter++){
            VISIOCYTELONG cid1 = iter->second;
            ...
            //    continue;
            //examin the new path
            VISIOCYTELONG bg_count=0,bg_count_tmp=0;
            for(VISIOCYTELONG i = 0; i<tmp_linkage.size(); i++){
                double tmp_label=predictWindow((VISIOCYTELONG) tmp_linkage[i]->x,(VISIOCYTELONG) tmp_linkage[i]->y,(VISIOCYTELONG) tmp_linkage[i]->z);
...
//            }
        ...
        if(ccid!=-1){
            ...
            MyMarker * p_mm_tmp;
            for(VISIOCYTELONG nid=0; nid<components[ccid].size(); nid++){
                double tmpdis=0;
            ...
            mindis=MAX_DOUBLE;
            for(VISIOCYTELONG nid=0; nid<components[cid0].size(); nid++){
                double tmpdis=0;
            ...
            for(VISIOCYTELONG nid=0; nid<linkage.size(); nid++){
    ...
    for(map<MyMarker *, set<MyMarker *> >::iterator mapiter = neuronGraph.begin(); mapiter!=neuronGraph.end(); mapiter++){
        if(mm_mask[mapiter->first]==0){
            ...
            for(VISIOCYTELONG sid=0; sid<mm_stack.size(); sid++){
...
bool nt_selfcorrect_func::smartTracing_regionstart(){
        ...
        for(VISIOCYTELONG y=0; y<sz_img[1]; y++){
            for(VISIOCYTELONG z=0; z<sz_img[2]; z++){
                if(p_label[x+y*sz_img[0]+z*sz_img[0]*sz_img[1]]!=VAL_FG)
                    continue;
                bool is_seed = false;
                for(VISIOCYTELONG xx=x-1; xx<=x+1; xx++){
                    if(xx<0||xx>=sz_img[0]) continue;
                    for(VISIOCYTELONG yy=y-1; yy<=y+1; yy++){
                        if(yy<0||yy>=sz_img[1]) continue;
                        for(VISIOCYTELONG zz=z-1; zz<=z+1; zz++){
                ...
                if(is_seed){
                    //vector<VISIOCYTELONG> tmp; tmp.push_back(x); tmp.push_back(y); tmp.push_back(z);
                    //seeds.push_back(tmp);
    ...
    //growing by seeds
//    for(VISIOCYTELONG sid=0; sid<seeds.size(); sid++){
//        x=seeds[sid][0];
//        y=seeds[sid][1];
//        z=seeds[sid][2];
    VISIOCYTELONG count_newseed=count_seeds;
    while(count_newseed!=0){
        count_newseed=0;
        for(VISIOCYTELONG sid=0;sid<sz_img[0]*sz_img[1]*sz_img[2];sid++){
            if(seeds_mask[sid]!=1)
                continue;
            ind2sub(x,y,z,sid);
            seeds_mask[sid]=3; //seeds already checked
            for(int dx=-param.grow_neighbor; dx<=param.grow_neighbor; dx++){
                VISIOCYTELONG xx=x+dx; if(xx<0||xx>=sz_img[0]) continue;
                for(int dy=-param.grow_neighbor; dy<=param.grow_neighbor; dy++){
                    VISIOCYTELONG yy=y+dy; if(yy<0||yy>=sz_img[1]) continue;
                    for(int dz=-param.grow_neighbor; dz<=param.grow_neighbor; dz++){
                        VISIOCYTELONG zz=z+dz; if(zz<0||zz>=sz_img[2]) continue;
                        VISIOCYTELONG ind=sub2ind(xx,yy,zz);
                        if(p_label[ind]!=0){
                            continue;
                        }
    ...
    for(VISIOCYTELONG ind=0; ind<sz_img[0]*sz_img[1]*sz_img[2]; ind++){
...
bool nt_selfcorrect_func::smartTracing_seedstart(){
    ...
    //growing from center
    //vector<vector<VISIOCYTELONG> > seeds;
    //vector<VISIOCYTELONG> start; start.push_back(x); start.push_back(y); start.push_back(z);
    //seeds.push_back(start);
    unsigned char * seeds_mask = new unsigned char[sz_img[0]*sz_img[1]*sz_img[2]];
    memset(seeds_mask,0,sz_img[0]*sz_img[1]*sz_img[2]);
    seeds_mask[sub2ind(x,y,z)]=1;
    VISIOCYTELONG count_seeds = 1, count_newseeds = 1;
...
{
    QString full_plugin_name = "Visiocyte_Neuron2";
...
{
    double minDis=MAX_DOUBLE;
    for(VISIOCYTELONG i=0; i<m1.size(); i++){
        for(VISIOCYTELONG j=0; j<m2.size(); j++){
...
double nt_selfcorrect_func::predictWindow(VISIOCYTELONG x, VISIOCYTELONG y, VISIOCYTELONG z)
{
    vector<float> wave;
...
bool nt_selfcorrect_func::getWindowWavelet(VISIOCYTELONG x, VISIOCYTELONG y, VISIOCYTELONG z, vector<float>& wave)
{
...
//    //for test
//    cur_winSize=1;
//    VISIOCYTELONG tmp_sz[4]={1,1,1,1};
...
void nt_selfcorrect_func::ind2sub(VISIOCYTELONG &x, VISIOCYTELONG &y, VISIOCYTELONG &z, VISIOCYTELONG ind)
{
...
long nt_selfcorrect_func::sub2ind(VISIOCYTELONG x, VISIOCYTELONG y, VISIOCYTELONG z)
{
