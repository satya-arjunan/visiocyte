...
void smartTrace_plugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
		visiocyte_msg(tr("This plugin will automatically trace and correct neuron reconstructions.. "
			"Developed by Hanbo Chen, 2015-4-21"));
	}
}

bool smartTrace_plugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    ...
    else if (func_name == tr("help"))
    {
        ...
        printf("visiocyte -x smartTrace -f selfCorrection -i <inimg_file> <inswc_file> -o <output> -p <score.txt> \n");
        ...
        printf("\n\nvisiocyte -x smartTrace -f smartTrace -i <inimg_file> -p <channel>\n");
        printf("inimg_file       The input image\n");
...
void reconstruction_func(VISIOCYTEPluginCallback2 &callback, QWidget *parent, input_PARA &PARA, bool bmenu)
{
    ...
    if(datatype!=1){
        visiocyte_msg(QString("Now only support image type UINT8."),bmenu);
        return;
    }
    ...
    if(tracefunc.error_code==0)
    {
        ...
        case 1:
            error_msg="Failed to call APP2 tracing function. Please check if plugin Visiocyte_Neuron2 is installed correctly.";
            break;
        case 2:
