...
using namespace std;
// Adapted from the msfm3d from D.Kroon University of Twente (June 2009)
...
// Entry function for msfm
double* msfm(double *F, VISIOCYTELONG* in_sz, int* SourcePoints, bool usesecond, bool usecross, bool Ed) {
    ...
    // SourcePoints must be a 3xn matrix.
    /* Get the sizes of the  SourcePoints */
    // dims_sp[0]=sp_sz[1]; dims_sp[1]=sp_sz[0]; dims_sp[2]=1;
    
    /* Create the distance output array */
    /*Distance image, also used to store the index of narrowband pixels */
    /*during marching process */
    try {T = new double [npixels];} 
    catch(...) {cout<<"cannot allocate memory for T."<<endl; return NULL;};

    if(Ed) {
        try {Y = new double [npixels];} 
        catch(...) {cout<<"cannot allocate memory for T."<<endl; return NULL;};
    }
