...
void show_logo();
void reconstruction_func(VISIOCYTEPluginCallback2 &callback, QWidget *parent,
...
void RivuletPlugin::domenu(const QString &menu_name,
                           VISIOCYTEPluginCallback2 &callback, QWidget *parent) {
  cout<<"in domenu"<<endl;
  if (menu_name == tr("tracing")) {
    ...
    if (callback.getImageWindowList().empty()) {
      visiocyte_msg("Oops... No image opened in VISIOCYTE...");
      return;
    }
    PARA_RIVULET p;
...
bool RivuletPlugin::dofunc(const QString &func_name,
  ...
  if (func_name == tr("tracing_func")) {
    ...
    printf(
        "visiocyte -x Rivulet2 -f tracing_func -i your_image.visiocyteraw [-o out.swc] -p "
...
void reconstruction_func(VISIOCYTEPluginCallback2 &callback, QWidget *parent,
  ...
  long *in_sz3 = new VISIOCYTELONG[3];
  in_sz3[0] = in_sz[0];
  in_sz3[1] = in_sz[1];
  in_sz3[2] = in_sz[2];
