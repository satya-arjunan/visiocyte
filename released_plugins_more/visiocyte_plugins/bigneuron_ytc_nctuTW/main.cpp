...
void ImagePreprocessforTracing(char *outFilnename = NULL, const float BIN_THRESHOLD = 0.8, const int CC_2D_THRESHOLD = 9, const int CC_3D_THRESHOLD = 30)
{
    ...
    //std::ofstream outBinarized("tmp_binarized", std::ios::binary);
    //std::string sTmpFile = g_sAppDir + "tmp_binarized";
    //std::ofstream outBinarized(sTmpFile.data(), std::ios::binary);
    //outBinarized.write( (char*) imgBuf, Vol);
...
void do_PreprocessforTracing()
{
    ...
    //char filename[80]="test_data_83x102x95.raw";	//input ****raw name****
...
//    std::ifstream inImg(g_sInputRawFilePath.data(), std::ios::binary );		//input  ****raw name****
//    inImg.read( (char*) imgBuf, imgV );
    memcpy(imgBuf, imgBuf_raw, imgV);

    unsigned char *imgBin = new unsigned char [ imgV ];
//  std::string sTmp = g_sAppDir + "tmp_binarized";
//  std::ifstream inBinary(sTmp.data(), std::ios::binary );		//input tmp_binarized
//  inBinary.read( (char*) imgBin, imgV );
    memcpy(imgBin, tmp_binarized, imgV);

    VectorCordinate somaPos = SomaDetection( imgBuf, imgBin, width, height, zSize );

//    std::cout<<somaPos<<'\n';
//    std::ofstream out(g_sImageInfoPath.data(), std::ios::app);
//    out<<somaPos<<'\n';
    g_nSomaX = somaPos.x;
    g_nSomaY = somaPos.y;
    g_nSomaZ = somaPos.z;
...
//    std::string sFilePath = g_sAppDir + "tmp_ForTracing.bin";
    //std::ifstream inFile("tmp_ForTracing.bin", std::ios::binary); //input : raw data
//    std::ifstream inFile(sFilePath.data(), std::ios::binary); //input : raw data
	
//    std::ifstream inFile_info(g_sImageInfoPath.data());
//	int width, height, zSize;
//	inFile_info>>width>>height>>zSize;
//	inFile_info.close();
...
//	inFile.read( (char*) imgBuffer, imgVolume);
...
//    sFilePath = g_sAppDir + "Repaired.raw";
//    //outFile.open("Repaired.raw", std::ios::binary);
//    outFile.open(sFilePath.data(), std::ios::binary);
//	outFile.write( (char*) imgBuffer, imgVolume );
...
void do_Tracing()
{
	...
	time( &total_start );
//    std::string sFilePath = g_sAppDir + "tmp_binarized";
//    //std::ifstream inImg("tmp_binarized", std::ios::binary);		//input image stack
//    std::ifstream inImg(sFilePath.data(), std::ios::binary);		//input image stack
	
//	int width, height, zSize;
//    std::ifstream inFile(g_sImageInfoPath.data());
//	inFile>>width>>height>>zSize;
...
//  sFilePath = g_sAppDir + "Repaired.raw";
    //std::ifstream inRepair("Repaired.raw", std::ios::binary);
//  std::ifstream inRepair(sFilePath.data(), std::ios::binary);
//	inRepair.read((char*) repairedBuf, Vol);
    ...
    strcpy(filename,"tmp_binarized");
    strcat(filename,"_Reconstruction");
...
//	inFile.close();
    ...
    DT_Tracing_Ver2( inBuf, skeleBuf, repairedBuf, CostDT,
                     width, height, zSize, SrcX, SrcY, SrcZ, avgR,
...
void do_PolygonalApproximation()
{
    ...
    filepath = g_sAppDir + "tmp_binarized_Reconstruction";
    ...
    outpath = g_sAppDir + std::string("tmp_binarized_Reconstruction");
...
void convert_to_SWC()
{
    //std::ofstream fout_SWC("binarized_Reconstruction.swc");			//output file name
    std::ofstream fout_SWC(g_sOutSwcFilePath.data());			//output file name
	std::ifstream fin_poly;
    std::string sTmp = g_sAppDir + "tmp_binarized_Reconstruction_poly";
...
int NeuronTracingMain()
{
    ...
    delete [] tmp_binarized;
    delete [] tmp_ForTracing;
    delete [] Repaired;
    return 1;
