...
void nctuTW::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
        visiocyte_msg(QString("This is the implementation of the tracing algorithm published in PLoS Computational Biology, \"High-throughput Computer Method for 3D Neuronal Structure Reconstruction from the Image Stack of the Drosophila Brain and Its Applications\", September 13, 2013. "
            "Implemented by P. C. Lee, K. W. He, C. C. Cheng, Y. T. Ching, 2015-5-23. Version %1").arg(getPluginVersion()));
	}
}

bool nctuTW::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    ...
    else if (func_name == tr("help"))
    {
        ...
        printf("visiocyte -x nctuTW -f tracing_func -i <inimg_file> -p <inmarker_file> <threshold>\n");
...
/*
    ...
    if(nParaState == QDialog::Accepted){
        ...
        visiocyte_msg(QString(sPara.c_str()));
    }
*/
    QDlgPara* pqDlgPara;
