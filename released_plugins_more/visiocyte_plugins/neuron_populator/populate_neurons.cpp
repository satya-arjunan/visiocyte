...
VISIOCYTELONG boundValue(VISIOCYTELONG x, VISIOCYTELONG m_min, VISIOCYTELONG m_max)
{
    x = MAX(x, m_min);
    x = MIN(x, m_max);
    return x;
...
NeuronTree populate_transform (NeuronTree sampleNeuron,
                               VISIOCYTELONG x, VISIOCYTELONG y, VISIOCYTELONG z, float rotation_y )
{
...
QList<NeuronTree> populate_neurons(NeuronTree sampleNeuron, int num_neurons,
                                   float maxRotation, VISIOCYTELONG siz_x,  VISIOCYTELONG siz_y,  VISIOCYTELONG siz_z)
{
    QList<NeuronTree> neurons;

    for (int i =0; i< num_neurons; i++) {
        VISIOCYTELONG x = randomPosition1D(siz_x);
        VISIOCYTELONG y = randomPosition1D(siz_y);
        VISIOCYTELONG z = randomPosition1D(siz_z);
...

{
    ...
    for (id_x = MAX(0,center.x - closeness); id_x <=  MIN(sz_x-1,center.x + closeness ); id_x++)
        for (id_y = MAX(0,center.y - closeness); id_y <= MIN(sz_y-1,center.y + closeness) ; id_y++)
            ...
            {
                VISIOCYTELONG idx = id_z * (sz_x*sz_y) + id_y * sz_x + id_x;
                         ...
                         Point3D offset, NeuronTree nt, int type,  int typeValue, float closeness)
{
    ...
    for (int i = 0; i < nt.listNeuron.size(); i++)
    {
        ...
        VISIOCYTELONG id_x = (node.x-offset.x) +0.5; //round up
        VISIOCYTELONG id_y = (node.y-offset.y) +0.5;
        VISIOCYTELONG id_z = (node.z-offset.z) +0.5;
                         ...
                         Point3D offset,   NeuronTree nt, int type,  int typeValue, float closeness)
{
    ...
    {
        NeuronSWC node = nt.listNeuron.at(i);
        VISIOCYTELONG id_x = (node.x-offset.x)/closeness -1 +0.5; //round up
        VISIOCYTELONG id_y = (node.y-offset.y)/closeness -1 +0.5;
        VISIOCYTELONG id_z = (node.z-offset.z)/closeness -1 +0.5;
                                            ...
                                            float closeness,VISIOCYTEPluginCallback2 &callback)
{
    ...
    */
            marker.x = (nodeA.x+min_node.x)/2+1;  // +1 due to visiocyte convention: landmarkers are 1-based
    ...
    float closeness,VISIOCYTEPluginCallback2 &callback)
{
    ...
    VISIOCYTELONG  sz_x = ceil((bbUnion.max_x - bbUnion.min_x ) / closeness); //+0.5 to round up from float to VISIOCYTELONG
    VISIOCYTELONG  sz_y = ceil((bbUnion.max_y - bbUnion.min_y ) / closeness);
    VISIOCYTELONG  sz_z = ceil((bbUnion.max_z - bbUnion.min_z ) / closeness);
    VISIOCYTELONG  tol_sz = sz_x * sz_y * sz_z;
    //else
    ...
    for(VISIOCYTELONG i = 0; i < tol_sz; i++) img1d_A[i] = 0;
    ...
    // image->setData(img1d_A, sz_x, sz_y, sz_z, 1, VISIOCYTE_UINT8);
    // callback.saveImage(image, "./test_maskA.visiocyteraw");

    //mask B
    unsigned char * img1d_B= new unsigned char[tol_sz];
    for(VISIOCYTELONG i = 0; i < tol_sz; i++) img1d_B[i] = 0;
    ...
    if (img1d_A[i] == (A1+B2)){
    ImageMarker mark;
    VISIOCYTELONG z = i / (sz_x*sz_y)            ;
    VISIOCYTELONG y = (i -z *(sz_x*sz_y) ) / sz_x   ;
    VISIOCYTELONG x = i - z *(sz_x*sz_y) - y*sz_x   ;
    //cout << "Find contact at:" << x <<" " << y <<" "<< z<<endl;
    mark.x = (x +0.5) * closeness   + offset.x ;
    mark.y = (y +0.5) * closeness  + offset.y ;
    mark.z = (z +0.5) * closeness  + offset.z ;  // +1 landmarks are 1-based in visiocyte
    ...
    //image->setData(img1d_A, sz_x, sz_y, sz_z, 1, VISIOCYTE_UINT8);
    //callback.saveImage(image, "./test_Add.visiocyteraw");
                 ...
                 float closeness,VISIOCYTEPluginCallback2 &callback)
{
    ...
    for(int j = 0;j< detected_contacts.size();j++){
        ...
        VISIOCYTELONG id_x = (curMarker.x-1-offset.x)/closeness +0.5-1; //round up
        VISIOCYTELONG id_y = (curMarker.y-1-offset.y)/closeness +0.5-1;
        VISIOCYTELONG id_z = (curMarker.z-1-offset.z)/closeness +0.5-1;
    ...
    for(int jj =0; jj<tol_sz;jj++){
        if (img1d[jj]>0){
            ImageMarker mark;
            VISIOCYTELONG z = jj/ (sz_x*sz_y)            ;
            VISIOCYTELONG y = (jj -z *(sz_x*sz_y) ) / sz_x   ;
            VISIOCYTELONG x = jj - z *(sz_x*sz_y) - y*sz_x   ;
            //cout << "Find contact at:" << x <<" " << y <<" "<< z<<endl;
            mark.x = (x +0.5) * closeness  + offset.x ;
            mark.y = (y +0.5) * closeness  + offset.y ;
