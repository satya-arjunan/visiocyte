...
void linker_terafly::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	if (menu_name == tr("linker"))
	{
        ...
        visiocyte_msg(QString("Save the linker file to: %1 Complete!").arg(linker_name));
    }else if  (menu_name == tr("linker_folder"))
    {
        ...
        for(VISIOCYTELONG ii = 0; ii < NTILES; ii++)
        {
...
bool linker_terafly::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
