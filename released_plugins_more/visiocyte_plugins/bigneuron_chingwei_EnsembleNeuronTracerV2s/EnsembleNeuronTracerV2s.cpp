...
#include "EnsembleNeuronTracerV2s.h"
...
unsigned char *Bernsen(unsigned char *inimg1d, VISIOCYTELONG x, VISIOCYTELONG y, VISIOCYTELONG z);
unsigned char *Mean(unsigned char *inimg1d, VISIOCYTELONG x, VISIOCYTELONG y, VISIOCYTELONG z);
unsigned char *Median(unsigned char *inimg1d, VISIOCYTELONG x, VISIOCYTELONG y, VISIOCYTELONG z);
unsigned char *MidGrey(unsigned char *inimg1d, VISIOCYTELONG x, VISIOCYTELONG y, VISIOCYTELONG z);
unsigned char* binarization_method (unsigned char* image, VISIOCYTELONG size, input_PARA PARA, int c_threshold);
unsigned char* binarization_method2 (unsigned char* image, VISIOCYTELONG size, input_PARA PARA, int c_threshold);
unsigned char* binarization_method3 (unsigned char* image, VISIOCYTELONG size, input_PARA PARA, int c_threshold);
unsigned char* binarization_method4 (unsigned char* image, VISIOCYTELONG size, input_PARA PARA, int c_threshold);
unsigned char* Contrast(unsigned char* image, VISIOCYTELONG size, int threshold);
...
//	file << "#Supervisor: Prof Ching-Wei Wang (cweiwang@mail.ntust.edu.tw)\n#Authors: Ching-Wei Wang, M. Hilmil Muchtar Aditya Pradana (hilmil.041@gmail.com), Cheng-Ta Huang (chengta.huang@mail.ntust.edu.tw)\n#Institute: Medical Image Research Center, National Taiwan University of Science & Technology\n#Web: http://www-o.ntust.edu.tw/~cweiwang/\n#License: This plugin uses the same license as Visiocyte.\n";
	...
	for (int iteration=0;iteration<3;iteration++)
	{
		...
		visiocyte_msg("start to use happ test.\n", 0);
...
visiocyte_msg("-500test start-.\n", 0);
		...
		visiocyte_msg("-516test start-.\n", 0);
	}
	
	double real_thres = 40;
	int method_radius_est = 2;
	VISIOCYTELONG szOriginalData[4] = {N, M, P, 1};
	visiocyte_msg("-523test start-.\n", 0);
	//Combine image
	for (int a=0;a<3;a++)
	{
		for(VISIOCYTELONG i = 0; i < outswc[a].size(); i++)
		{
			outswc[a][i]->radius = markerRadius(data1d_1ch, szOriginalData, *(outswc[a][i]), real_thres, method_radius_est);
		}
	}
	visiocyte_msg("-532test start-.\n", 0);
/*	QString swc_name2 = PARA.inimg_file + "_baseTracer1.swc";
    QString swc_name3 = PARA.inimg_file + "_baseTracer2.swc";
    QString swc_name4 = PARA.inimg_file + "_baseTracer3.swc";
	...
	for(VISIOCYTELONG i = 0; i < result.size(); i++)
	{
		result[i]->radius = markerRadius(data1d_1ch, szOriginalData, *(result[i]), real_thres, method_radius_est);
	}
	
	//Output
	QString swc_name1 = PARA.inimg_file + "_EnsembleNeuronTracerV2s.swc";
	...
	visiocyte_msg(QString("Now you can drag and drop the generated swc fle [%1] into Visiocyte.").arg(swc_name1.toStdString().c_str()),bmenu);
...
unsigned char *Bernsen(unsigned char *inimg1d, VISIOCYTELONG x, VISIOCYTELONG y, VISIOCYTELONG z)
{
	...
	for (VISIOCYTELONG i=1;i<(x*y*z);i++)
	{
...
{
	int c = 0;
/*	VISIOCYTELONG * temp = new VISIOCYTELONG[z];
	for (int j=0;j<z;j++)
	{
		temp[j] = 0;
		for (VISIOCYTELONG i=0;i<(x*y);i++)
		{
			temp[j] = temp + inimg1d[(j*x*y)+i];
		}
		temp[j]
	}*/
	VISIOCYTELONG temp = 0;
	unsigned long mean;
	for (VISIOCYTELONG i=0;i<(x*y*z);i++)
	{
		temp = temp + inimg1d[i];
	}
	mean = temp/(x*y*z);
...
unsigned char* binarization_method (unsigned char* image, VISIOCYTELONG size, input_PARA PARA, int c_threshold) //OTSU
{
	...
	for (VISIOCYTELONG i=0;i<size;i++) image[i] = img[i];
	
	return image;
}

unsigned char* binarization_method2 (unsigned char* image, VISIOCYTELONG size, input_PARA PARA, int c_threshold) //Phansalkar
{
	int *img = new int[size];
	for (VISIOCYTELONG i=0;i<size;i++)
	{
...
unsigned char* binarization_method3 (unsigned char* image, VISIOCYTELONG size, input_PARA PARA, int c_threshold) //Sauvola
{
	int *img = new int[size];
	for (VISIOCYTELONG i=0;i<size;i++)
	{
...
unsigned char* binarization_method4 (unsigned char* image, VISIOCYTELONG size, input_PARA PARA, int c_threshold) //Niblack
{
	int *img = new int[size];
	for (VISIOCYTELONG i=0;i<size;i++)
	{
