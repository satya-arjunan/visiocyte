...
void SyncZ::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    {
        visiocyte_msg(tr("This plugin syncs the z-cut of the image and the surface."
...
void SynTwoImage(VISIOCYTEPluginCallback2 &visiocyte, QWidget *parent)
{
    ...
    if (win_list.size()<1)
    {
        visiocyte_msg("You need one opened image to get 3D view data!");
        return;
    }
    ...
    {
        panel = new lookPanel(visiocyte, parent); //this shouldn't happen.
...
lookPanel::lookPanel(VISIOCYTEPluginCallback2 &_visiocyte, QWidget *parent) : //do it here!!
    QDialog(parent), m_visiocyte(_visiocyte)
{
    win_list = m_visiocyte.getImageWindowList(); //this is NOT for the 3D window

    //first check to see if there is an image
    list_3dviewer = m_visiocyte.getListAll3DViewers();

    curwin = m_visiocyte.currentImageWindow();
    view = m_visiocyte.getView3DControl(curwin);\
    ...
    nt_list = m_visiocyte.getHandleNeuronTrees_3DGlobalViewer(curwin); //curwin is only for the main window

if(nt_list->size()==0){
        visiocyte_msg("You have no surface loaded in the 3D window. Cut is only for the image.");
    }
    ...
    Image4DSimple* subject = _visiocyte.getImage(curwin);
    sz2 = subject->getZDim();
    sz22 = (float)sz2;
...
void MyComboBox::updateList()
{
    ...
    QList <VisiocyteR_MainWindow *> cur_list_3dviewer = m_visiocyte->getListAll3DViewers(); //this should work...
           ...
           if (win_list[i1])//ensure the 3d viewer window is open; if not, then open it
           {
                ...
                if (view)
                {
                    ...
                    if(min_max_dim!=0&&min_max_dim!=sz2){
                        visiocyte_msg("Warning! Cut will not sync for ROI cropped in the z-dimension. Please select an ROI from the triview window.");
                    }
