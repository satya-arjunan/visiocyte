...
bool neuron_dist_toolbox(const VISIOCYTEPluginArgList & input, VISIOCYTEPluginCallback2 & callback)
{
	...
	VisiocyteR_MainWindow * win = paras->win;
	QList<NeuronTree> * nt_list = callback.getHandleNeuronTrees_Any3DViewer(win);
	NeuronTree nt = paras->nt;
	if (nt_list->size()<=1)
	{
		visiocyte_msg("You should have at least 2 neurons in the current 3D Viewer");
		return false;
	}
	...
	for (VISIOCYTELONG i=0;i<nt_list->size();i++)
	{
...
int neuron_dist_mask(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    for(VISIOCYTELONG i = 0; i <nt2.listNeuron.size(); i++)
    ...
    sy_2=y_max_2;
    sz_2=z_max_2;
    VISIOCYTELONG stacksz_2 = sx_2*sy_2*sz_2;
    ...
    for (VISIOCYTELONG k1 = 0; k1 < sz_2; k1++){
       for(VISIOCYTELONG j1 = 0; j1 < sy_2; j1++){
           for(VISIOCYTELONG i1 = 0; i1 < sx_2; i1++){
    ...
    VISIOCYTELONG AandB = 0, AorB = 0;
    for(VISIOCYTELONG i = 0; i < nx*ny*nz; i++)
    {
        if(pData[i] > 0) AorB++;
    ...
    visiocyte_msg(QString("score is %1, %2").arg(AandB).arg(AorB));
    Image4DSimple tmp;
...
void printHelp()
{
    ...
    cout<<"Usage: visiocyte -x neuron_distance -f neuron_distance -i <input_filename1> <input_filename2> -p <dist_thres> -o <output_file>"<<endl;
