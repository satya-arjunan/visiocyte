...
void NeuronChaser::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
//        dialog.get_num("Gaussian Cross Section sigma (1-10) [pix]", PARA.gcsSig);
        ...
        // check input
        if(PARA.channel <= 0)                           {visiocyte_msg(QObject::tr("Channel is out of range")); return;}
        if(PARA.scal <= 5       || PARA.scal>20)        {visiocyte_msg(QObject::tr("Scale is out of range")); return;}
//        if(PARA.perc < 50       || PARA.perc > 100)     {visiocyte_msg(QObject::tr("Percentile is out of range")); return;}
        if(PARA.znccTh < 0.5    || PARA.znccTh >= 1.0)  {visiocyte_msg(QObject::tr("Correlation is out of range")); return;}
        if(PARA.Ndir<5          || PARA.Ndir>20)        {visiocyte_msg(QObject::tr("# directions is out of range")); return;}
        if(PARA.angSig < 1      || PARA.angSig>90)      {visiocyte_msg(QObject::tr("Angular sigma is out of range")); return;}
//        if(PARA.gcsSig < 1      || PARA.gcsSig>10)      {visiocyte_msg(QObject::tr("Gaussian Cross Section sigma is out of range")); return;}
        if(PARA.Ni < 2          || PARA.Ni>50)          {visiocyte_msg(QObject::tr("# iterations is out of range")); return;}
        if(PARA.Ns <=  0        || PARA.Ns>20)          {visiocyte_msg(QObject::tr("# states is out of range")); return;}
        if(PARA.zDist < 1       || PARA.zDist>4    )    {visiocyte_msg(QObject::tr("zdist is out of range")); return;}
        if(PARA.saveMidres<0    || PARA.saveMidres>1)   {visiocyte_msg(QObject::tr("saveMidres has to be 0 or 1")); return;}
	...
	{
        visiocyte_msg(tr("Neuron reconstruction plugin. "
...
void print_help(){
    ...
    printf("visiocyte -x NeuronChaser -f nc_func -i <inimg_file> -p <channel> <scal znccTh Ndir angSig Ni Ns zDist saveMidres>\n");
...
bool NeuronChaser::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
//        PARA.gcsSig     = (paras.size() >= k+1)   ? QString(paras[k]).toFloat() : gcsSig;    k++;
        ...
        // check input
        if(PARA.channel <= 0)                            {visiocyte_msg(QObject::tr("Channel is out of range")); return false;}
        if(PARA.scal <= 5       || PARA.scal>20)         {visiocyte_msg(QObject::tr("Scale is out of range")); return false;}
//        if(PARA.perc < 50       || PARA.perc > 100)      {visiocyte_msg(QObject::tr("Percentile is out of range")); return false;}
        if(PARA.znccTh < 0.5    || PARA.znccTh >= 1.0)   {visiocyte_msg(QObject::tr("Correlation is out of range")); return false;}
        if(PARA.Ndir<5          || PARA.Ndir>20)         {visiocyte_msg(QObject::tr("# directions is out of range")); return false;}
        if(PARA.angSig < 1      || PARA.angSig>90)       {visiocyte_msg(QObject::tr("Angular sigma is out of range")); return false;}
//        if(PARA.gcsSig < 1      || PARA.gcsSig>10)      {visiocyte_msg(QObject::tr("Gaussian Cross Section sigma is out of range")); return false;}
        if(PARA.Ni < 2         || PARA.Ni>50      )      {visiocyte_msg(QObject::tr("# iterations is out of range")); return false;}
        if(PARA.Ns <= 0         || PARA.Ns>20        )   {visiocyte_msg(QObject::tr("# states is out of range")); return false;}
        if(PARA.zDist < 1       || PARA.zDist>4    )    {visiocyte_msg(QObject::tr("Z layer dist is out of range")); return false;}
        if(PARA.saveMidres<0    || PARA.saveMidres>1)    {visiocyte_msg(QObject::tr("saveMidres has to be 0 or 1")); return false;}
...
//                scr.push_back(scrxyz);
    ...
    if (PARA.saveMidres) {
        ...
        simple_saveimage_wrapper(callback, of.toStdString().c_str(), erd1d, in_sz, VISIOCYTE_UINT8);
        of = PARA.inimg_file + "_scr1.tif";
        simple_saveimage_wrapper(callback, of.toStdString().c_str(), scr1, in_sz, VISIOCYTE_UINT8);
    }
    ...
    if (PARA.saveMidres) {
        ...
        simple_saveimage_wrapper(callback, of.toStdString().c_str(), somaseg, in_sz, VISIOCYTE_UINT8);
    }
    ...
    // 2    : node<2>   (blob)
    // ...
    // A    : node<A>   (gpnt) // so the gpnts are automatically in the reconstruction which Advantra will try to avoid
    // A+1  : node<A+1> (gpnt)
    // ...
    // B    : node<B>   (trc)
    // B+1  : node<B+1> (trc)
...
//    unsigned char th1 = otsu_th(data1d_temp, cntvals);
    ...
    if (PARA.saveMidres) {
        ...
        simple_saveimage_wrapper(callback, of.toStdString().c_str(), temp, in_sz, VISIOCYTE_UINT8);
...
//    delete labimg; labimg = 0;
    ...
    if (PARA.saveMidres) {
        ...
        outimg1.setData((unsigned char *)tagmapcopy, in_sz[0], in_sz[1], in_sz[2], 1, VISIOCYTE_FLOAT32);
