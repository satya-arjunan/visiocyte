...
void RigidAffineRegistration(VISIOCYTEPluginCallback &callback, QWidget *parent,const int i_regtype);
void FFDNonrigidRegistration(VISIOCYTEPluginCallback &callback, QWidget *parent);
...
void ImageRegistrationPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    {
        visiocyte_msg(QString("This is a 3D SSD registration program developed in Hanchuan Peng lab. Version %1.").arg(version_str));
    }
}
...
bool iNormalizer(Tdata *p, Tidx sz, double *&pOutput)
{
    ...
    if (max_v==0)
    {
        visiocyte_msg("You data consists of all-the-same values. You don't need registration, do you??\n", 0);
        freeMemory<double>(pOutput);
        return false;
    }
                   ...
                   Tdata *&pOutput)
{
    ...
    if(VISIOCYTELONG(vec4D_grid_int.size()) != VISIOCYTELONG(sz_img_input[1]) || 
            VISIOCYTELONG(vec4D_grid_int[0].size()) != VISIOCYTELONG(sz_img_input[0]) ||
            VISIOCYTELONG(vec4D_grid_int[0][0].size()) != VISIOCYTELONG(sz_img_input[2]) ||
            VISIOCYTELONG(vec4D_grid_int[0][0][0].size()) != 3)
    {
    ...
    freeMemory<Tdata>(pOutput);
    VISIOCYTELONG totalplxs = sz_img_input[3]*pagesz;
    try
    {
        pOutput = new Tdata [totalplxs];
...
void errorPrint()
{
    printf("\nUsage: visiocyte -x imagereg.dylib (or equivalent dll name) "
    ...
    visiocyte_msg(QString("This is a 3D SSD registration program developed in Hanchuan Peng lab. Version %1.").arg(version_str), 0);
}

// function call

//bool wrapperLoadImage(char imgSrcFile[], unsigned char *& data1d, VISIOCYTELONG * &sz, int & datatype)
//{
//	visiocyte_msg("wrapperLoadImage", 0);
	
//	if (!imgSrcFile || data1d || sz)
//	{
//		visiocyte_msg("wrong input to wrapperLoadImage().",0);
//		return false;
//	}
...
//	if (!(tmpimg->valid()))
//	{
//		visiocyte_msg(QString("Fail to load image [%1]").arg(imgSrcFile),0);
//		return false;
//	}
...
bool ImageRegistrationPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    if (func_name == tr("rigidreg"))
    {
        ...
        if(paras)
        {
            ...
            {
                qs_filename_img_sub2tar=qs_pathname_output+"/"+qs_basename_input+".visiocyteraw";
            }
        ...
        VISIOCYTELONG sz_img_tar_input[4], sz_img_sub_input[4];
        unsigned char *p_img_tar_input=0,*p_img_sub_input=0;
        double *p_img64f_tar=0,*p_img64f_sub=0;
        ...
        VISIOCYTELONG sz_img[4]={sz_img_tar_input[0],sz_img_tar_input[1],sz_img_tar_input[2],1};
        ...
        for(int i=0;i<3;i++)
        {
            ...
            {
                sz_img[i]=(VISIOCYTELONG)((double)sz_img_tar_input[i]/(double)(l_downsampleratio) + 0.5);
            }
        ...
        if(sz_img_tar_input[0]!=sz_img[0] || sz_img_tar_input[1]!=sz_img[1] || sz_img_tar_input[2]!=sz_img[2])
        {
            ...
            case 1:
            {
                if(!iSampler<unsigned char, VISIOCYTELONG>(p_img_tar_input+offsets_tar, sz_img_tar_input, p_tar, sz_img, sz_img[3]))
                {
            ...
            case 2:
            {
                ...
                if(!iSampler<unsigned short int, VISIOCYTELONG>((unsigned short int *)p_img_tar_input+offsets_tar, sz_img_tar_input, pTmp, sz_img, sz_img[3]))
                {
                    printf("\nError: fail to call iSampler function!\n");
                    return false;
                }
            ...
            case 4:
            {
                ...
                if(!iSampler<float, VISIOCYTELONG>((float *)p_img_tar_input+offsets_tar, sz_img_tar_input, pTmp, sz_img, sz_img[3]))
                {
                    printf("\nError: fail to call iSampler function!\n");
                    return false;
                }
        ...
        if(sz_img_sub_input[0]!=sz_img[0] || sz_img_sub_input[1]!=sz_img[1] || sz_img_sub_input[2]!=sz_img[2])
        {
            ...
            case 1:
            {
                if(!iSampler<unsigned char, VISIOCYTELONG>(p_img_sub_input+offsets_sub, sz_img_sub_input, p_sub, sz_img, sz_img[3]))
                {
                    printf("\nError: fail to call iSampler function!\n");
                    return false;
                }
            ...
            case 2:
            {
                ...
                if(!iSampler<unsigned short int, VISIOCYTELONG>((unsigned short int *)p_img_sub_input+offsets_sub, sz_img_sub_input, pTmp, sz_img, sz_img[3]))
                {
                    printf("\nError: fail to call iSampler function!\n");
                    return false;
                }
            ...
            case 4:
            {
                ...
                if(!iSampler<float, VISIOCYTELONG>((float *)p_img_sub_input+offsets_sub, sz_img_sub_input, pTmp, sz_img, sz_img[3]))
                {
                    printf("\nError: fail to call iSampler function!\n");
                    return false;
                }
        ...
        //--------------------------------------------------------------------------------------------------
        VISIOCYTELONG pagesz_img = sz_img[0]*sz_img[1]*sz_img[2];
        switch (datatype_sub_input)
        {
        case 1:
        {
            if(!saturateImage<unsigned char, VISIOCYTELONG>(p_sub, sz_img[0], sz_img[1], sz_img[2], 0.98))
            {
        ...
        case 2:
        {
            ...
            if(!saturateImage<unsigned short, VISIOCYTELONG>(pTmp, sz_img[0], sz_img[1], sz_img[2], 0.98))
            {
                printf("\nError: fail to call saturateImage function!\n");
                return false;
            }
        ...
        case 1:
        {
            if(!iNormalizer<unsigned char, VISIOCYTELONG>(p_tar, pagesz_img, p_img64f_tar))
            {
        ...
        case 2:
        {
            if(!iNormalizer<unsigned short int, VISIOCYTELONG>((unsigned short int*)p_tar, pagesz_img, p_img64f_tar))
            {
                printf("\nError: fail to call iNormalizer function!\n");
                return false;
            }
        ...
        case 4:
        {
            if(!iNormalizer<float, VISIOCYTELONG>((float *)p_tar, pagesz_img, p_img64f_tar))
            {
                printf("\nError: fail to call iNormalizer function!\n");
                return false;
            }
        ...
        case 1:
        {
            if(!iNormalizer<unsigned char, VISIOCYTELONG>(p_sub, pagesz_img, p_img64f_sub))
            {
                printf("\nError: fail to call iNormalizer function!\n");
                return false;
            }
        ...
        case 2:
        {
            if(!iNormalizer<unsigned short int, VISIOCYTELONG>((unsigned short int*)p_sub, pagesz_img, p_img64f_sub))
            {
                printf("\nError: fail to call iNormalizer function!\n");
                return false;
            }
        ...
        case 4:
        {
            if(!iNormalizer<float, VISIOCYTELONG>((float *)p_sub, pagesz_img, p_img64f_sub))
            {
                printf("\nError: fail to call iNormalizer function!\n");
                return false;
            }
        ...
        if(qs_filename_img_sub2tar!=NULL)
        {
            ...
            VISIOCYTELONG sz_img_output[4] = {sz_img_tar_input[0], sz_img_tar_input[1], sz_img_tar_input[2], sz_img_sub_input[3]};
            ...
            if(sz_img_sub_input[0]!=sz_img_output[0] || sz_img_sub_input[1]!=sz_img_output[1] || sz_img_sub_input[2]!=sz_img_output[2])
            {
                ...
                {
                    if(!iSampler<unsigned char, VISIOCYTELONG>(p_img_sub_input, sz_img_sub_input, p_img_sub_resize, sz_img_output, sz_img_sub_input[3]))
                    {
                        printf("\nError: fail to call iSampler function!\n");
                        return false;
                    }
                ...
                else if(datatype_sub_input == 2)
                {
                    ...
                    if(!iSampler<unsigned short, VISIOCYTELONG>((unsigned short *)p_img_sub_input, sz_img_sub_input, pTmp, sz_img_output, sz_img_sub_input[3]))
                    {
                        printf("\nError: fail to call iSampler function!\n");
                        return false;
                    }
                ...
                else if(datatype_sub_input == 4)
                {
                    ...
                    if(!iSampler<float, VISIOCYTELONG>((float *)p_img_sub_input, sz_img_sub_input, pTmp, sz_img_output, sz_img_sub_input[3]))
                    {
                        printf("\nError: fail to call iSampler function!\n");
                        return false;
                    }
