...
#include "../../../released_plugins/visiocyte_plugins/neurontracing_vn2/app1/visiocyteneuron_gd_tracing.h"
...
#include "stackutil.h"
#include "../../../released_plugins/visiocyte_plugins/neurontracing_vn2/app1/gd.h"
...
template <class T> QList<NeuronSWC> seed_detection(VISIOCYTEPluginCallback2 &callback,T* data1d,
                                      VISIOCYTELONG *in_sz,
                                      unsigned int Ws,
                                      unsigned int c,
                                      double th);
...
void neurontracing_mst::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
bool neurontracing_mst::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    ...
    else if (func_name == tr("help"))
    {
        ...
        printf("visiocyte -x plugin_name -f trace_mst -i <inimg_file> -p <channel> <window size>\n");
...
void autotrace_mst(VISIOCYTEPluginCallback2 &callback, QWidget *parent, MST_PARA &PARA, bool bmenu)
{
   ...
   // return;
    ...
    VISIOCYTELONG id = 1;
    for(VISIOCYTELONG i = 1; i <nt_seed.size(); i++)
    {
       // visiocyte_msg(QString("mst_%1.swc").arg(i));
       ...
       // pp.push_back(tmpp);
        ...
        VISIOCYTELONG xb = cent_x - 2*Ws; if(xb < 0) xb = 0;
        VISIOCYTELONG xe = cent_x + 2*Ws-1; if(xe>N-1) xe = N-1;
        VISIOCYTELONG yb = cent_y - 2*Ws; if(yb < 0) yb = 0;
        VISIOCYTELONG ye = cent_y + 2*Ws-1; if(ye>M-1) ye = M-1;
        VISIOCYTELONG zb = cent_z - 2*Ws; if(zb < 0) zb = 0;
        VISIOCYTELONG ze = cent_z + 2*Ws-1; if(ze>P-1) ze = P-1;

        unsigned char *localarea=0;
        VISIOCYTELONG blockpagesz = (xe-xb+1)*(ye-yb+1)*(ze-zb+1);
        localarea = new unsigned char [blockpagesz];
        VISIOCYTELONG d = 0;
        for(VISIOCYTELONG iz = zb; iz < ze + 1; iz++)
        {
            VISIOCYTELONG offsetk = iz*M*N;
...
//    VISIOCYTELONG neuronNum = nt_sorted_prund_2nd.listNeuron.size();
//    childs = QVector< QVector<VISIOCYTELONG> >(neuronNum, QVector<VISIOCYTELONG>() );
//    for (VISIOCYTELONG i=0;i<neuronNum;i++)
//    {
//        VISIOCYTELONG par = nt_sorted_prund_2nd.listNeuron[i].pn;
//        if (par<0) continue;
...
//    NeuronTree nt_2nd = visiocyteneuron_GD_tracing(p4d_entire, sz_tracing,
//                              p0, pp,
//                              trace_para, weight_xy_z);
                                      ...
                                      double th)
{
    ...
    for(VISIOCYTELONG iz = 0; iz < P; iz = iz + Ws)
    {
      //  VISIOCYTELONG offsetk = iz*M*N;
        VISIOCYTELONG zb = iz;
        VISIOCYTELONG ze = iz+Ws-1; if(ze>=P-1) ze = P-1;
        for(VISIOCYTELONG iy = 0; iy < M; iy = iy + Ws)
        {
          //  VISIOCYTELONG offsetj = iy*N;
            VISIOCYTELONG yb = iy;
            VISIOCYTELONG ye = iy+Ws-1; if(ye>=M-1) ye = M-1;
            for(VISIOCYTELONG ix = 0; ix < N; ix = ix + Ws)
            {
                VISIOCYTELONG xb = ix;
                ...
                xm /= s; ym /=s; zm /=s;
                VISIOCYTELONG seed_index = (int)zm*M*N + (int)ym*N +(int)xm;
                if(s >0 && data1d[seed_index] <= th + 30 && loc_points_list.size()>1) //find medoid point
                {
                    double dist_min = INF;
                    VISIOCYTELONG medoid_index = -1;
...
//                VISIOCYTELONG xb = cent_x - 3*Ws; if(xb < 0) xb = 0;
//                VISIOCYTELONG xe = cent_x + 3*Ws-1; if(xe>N-1) xe = N-1;
//                VISIOCYTELONG yb = cent_y - 3*Ws; if(yb < 0) yb = 0;
//                VISIOCYTELONG ye = cent_y + 3*Ws-1; if(ye>M-1) ye = M-1;
//                VISIOCYTELONG zb = cent_z - 3*Ws; if(zb < 0) zb = 0;
//                VISIOCYTELONG ze = cent_z + 3*Ws-1; if(ze>P-1) ze = P-1;
...
//                    for(VISIOCYTELONG nn = 0; nn < nt.listNeuron.size() ;nn++)
//                    {

//                        VISIOCYTELONG ii = nt.listNeuron.at(nn).x;
//                        VISIOCYTELONG jj = nt.listNeuron.at(nn).y;;
//                        VISIOCYTELONG kk = nt.listNeuron.at(nn).z;
