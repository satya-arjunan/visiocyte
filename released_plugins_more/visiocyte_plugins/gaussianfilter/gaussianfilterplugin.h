...
//
//int GaussianFilter(float *pImage, VISIOCYTELONG N, VISIOCYTELONG M, VISIOCYTELONG P, unsigned int Wx, unsigned int Wy, unsigned int Wz);
...
class GaussianFilterPlugin : public QObject, public VISIOCYTEPluginInterface2_1
{
     Q_OBJECT
...
class GaussianFilterDialog : public QDialog
    ...
    public:
        GaussianFilterDialog(VISIOCYTEPluginCallback2 &cb, QWidget *parent)
        {
            ...
            {
                visiocyte_msg("The image is not valid yet. Check your data.");
                return;
            }
