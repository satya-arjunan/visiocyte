...
//visiocyte preprocessing method for histogram equalization
...
unsigned char * Preprocess(unsigned char* data1d, long N, long M, long P, long sc, int& threshold)
{
	...
	if (threshold == 0 || threshold == 998)
	{
		...
		{
			//visiocyte_msg("Fail to allocate memory when applying adaptive thresholding for simple axis anlyzer.", 0);
		...
		delete[] data1d;
		data1d = pData;
		//visiocyte_msg("Done thresholding", 0);
		...
		//visiocyte_msg(QString("Total percentage of foreground pixels after adaptive thresholding:%1").arg(((float)num) / (N*M*P)), 0);
		...
		//visiocyte_msg(QString("Histogram equalization done"), 0);
	}
	...
	if (threshold == 999 || threshold == 0 || threshold == 998)
	{
		...
		//visiocyte_msg(QString("Total foreground points:%1").arg(sum1), 0);
		//visiocyte_msg(QString("Total percentage of foreground points after binarization:%1").arg(((float)num1) / (N*M*P)), 0);
