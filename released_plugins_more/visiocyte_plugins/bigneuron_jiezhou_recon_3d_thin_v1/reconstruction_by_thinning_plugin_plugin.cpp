...
void reconstruction_by_thinning_plugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
        visiocyte_msg(tr("NeuronAxisAnalyser -- Input: Gray-level Image (or Binary Image); Output: Reconstructed swc\n"
            "Developed by Jie Zhou and Edward Hottendorf, 2015-6-1"));
	}
}

bool reconstruction_by_thinning_plugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    ...
    else if (func_name == tr("help"))
    {
        ...
        printf("visiocyte -x SimpleAxisAnalyzer -f medial_axis_analysis -i <inimg_file> -p <parameters>\n");
...
void reconstruction_func(VISIOCYTEPluginCallback2 &callback, QWidget *parent, input_PARA &PARA, bool bmenu)
{
    ...
    // * Visiocyte appears having issue reading a signed int image of only 0 and 1 which was the output skeleton of the thinning algorithm of itk.
...
void reconstructionThinning(itk::Image<signed short, (unsigned) 3> *input, QString qoutputfilename, int reconstructionTh, int treeNumber)
{
    ...
    visiocyte_msg(QString("Start thinning with reconstruction threshold %1 ...").arg(reconstructionTh),0 );
    clock_t begin = clock();

    thinningFilter->Update();

    visiocyte_msg("after Thinning is done ", 0);
    clock_t end = clock();
    double elapsed_secs = double(end - begin) / CLOCKS_PER_SEC;

    visiocyte_msg(QString("time used for thinning in secs  %1").arg(elapsed_secs), 0);

    //12/03/2015 call the trueAnalyze in Reconstruction3D.cxx
     visiocyte_msg(QString("Calling trueAnalyze in Reconstruction3D ..."), 0);
