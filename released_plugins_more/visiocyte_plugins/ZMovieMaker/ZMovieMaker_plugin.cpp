...
bool loadZMovieAnoFile(QString openFileNameLabel, ZMovieFileType & cc);
void MovieFromPoints(VISIOCYTEPluginCallback2 &visiocyte, QWidget *parent);
...
{ \
    ...
    if(curwin)\
{\
    m_visiocyte.updateImageWindow(curwin);\
    }\
    else\
    m_visiocyte.update_3DViewer(surface_win); \
    }
...
{ \
    ...
    timePoint = currentParas.at(27).toInt();\
    }
...
{ \
    ...
    if(curwin)\
    m_visiocyte.screenShot3DWindow(curwin, BMPfilename);\
    else\
    m_visiocyte.screenShot_Any3DViewer(surface_win, BMPfilename);\
    framenum++;\
    }
...
{\
    ...
    if(curwin)\
{\
    m_visiocyte.open3DWindow(curwin);\
    view = m_visiocyte.getView3DControl(curwin);\
    }\
    else\
    return;\
    }\
...
{\
    ...
    visiocyte_msg(QString("current window selected:[%1]").arg(curname), 0);\
...
{\
    ...
    if(surface_win)\
{\
    view = m_visiocyte.getView3DControl_Any3DViewer(surface_win);\
    }\
    else\
    return;\
    break;\
...
void ZMovieMaker::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    {
        visiocyte_msg(tr("This plugin generates a smooth 3D movie using multiple anchor points. "
                   "Developed by Zhi Zhou and Hanchuan Peng, inspired by previous discussion and work with Z. Ruan and C. Bruns. 2012-2013."));
    }
}

bool ZMovieMaker::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    ...
    QDialog(parent), m_visiocyte(_visiocyte)
{
    ...
    QPushButton* btn_Batch = new QPushButton("Batch Process");
    ...
    gridLayout->addWidget(btn_Batch,11,0,1,5);
    ...
    connect(btn_Batch, SIGNAL(clicked()), this, SLOT(_slot_batch()));
...
void controlPanel::_slot_snapshot()
{
    ...
    if(curwin)
        ...
        m_visiocyte.screenShot_Any3DViewer(surface_win, BMPfilename);
    visiocyte_msg(QString("Snapshot is saved in %1.BMP").arg(BMPfilename.toStdString().c_str()));
}
...
void controlPanel::_slot_preview()
{
    ...
    if(list_anchors->count()<=0)
    {
        visiocyte_msg("Please define at least one archor point.");
        return;
    }
                                                  ...
                                                  QMessageBox::Yes, QMessageBox::No))
    {
        ...
        if(d.exec())
        {
            ...
            visiocyte_msg(QString("Video frames have now been saved in the folder [%1], please use the Movie Converter plugin to convert to a single movie (e.g. avi, mpg).").arg(selectedFile));
...
void controlPanel::_slot_delete()
{
    ...
    {
        visiocyte_msg("Please select a valid archor point.");
        return;
    }
...
void controlPanel::_slot_batch()
{
    ...
    {
        visiocyte_msg("Cannot find the linker files in the given directory!\nTry another diretory");
        return;
    }
    for(int i = 0; i <qsl_filelist.size(); i++)
    {
        ...
        if(loadZMovieAnoFile(ano_path, cc))
        {
            ...
            m_visiocyte.open3DViewerForLinkerFile(ano_path);
            view=0;curwin=0;
            list_3dviewer = m_visiocyte.getListAll3DViewers();
            for (int j=0; j<list_3dviewer.count(); j++)
            {
                QString curname = m_visiocyte.getImageName(list_3dviewer[j]).remove("3D View [").remove("]");
                if (curname==ano_path)
                {
...
bool _saveAnchorFile(QString filename, QStringList ParaLists, bool b_append)
{
    ...
    {
        visiocyte_msg("Fail to invoke _saveAnchorFile(), invalid parameter or empty list.",0);
        return false;
    }
    ...
    for(int row = 0; row < ParaLists.size(); row++)
    {
        ...
        {
            visiocyte_msg(QString("Format error in splitting paralist for row [%1] which is [%2]. Ignore this row!").arg(row).arg(ParaLists.at(row)));
            continue;
        }
...
bool controlPanel::saveAnchorFile(QString filename)
{
    ...
    {
        visiocyte_msg("Fail to invoke saveAnchorFile(), maybe the file name is invalid or invalid list_anchors pointer or empty anchor point list.",0);
        return false;
    }
...
bool loadZMovieAnoFile(QString openFileNameLabel, ZMovieFileType & cc)
{
    ...
    for (int i=0;i<tmpList.size(); i++)
    {
        ...
        if (itemList.size()==2)
        {
            QString tss = itemList.at(0).trimmed().toUpper();
            QString tvv = itemList.at(1).trimmed();
            QDir tmpdir;
    ...
    visiocyte_msg(QString("count is%1").arg(cnt));
