...
void RefExtractPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
void errorPrint()
{
    printf("\nUsage: visiocyte -x refExtract -f refExtract -i <input_image> -o <output_image> -p \"#c <refchannel> \"\n");
}
...
bool RefExtractPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
     ...
     else if (func_name == tr("refExtract"))
    {
        ...
        if(paras)
        {
            ...
            {
                qs_filename_img_output=qs_pathname_output+"/"+qs_basename_input+"_8bit.visiocyteraw";
            }
        ...
        //
        VISIOCYTELONG offset_c = channel_ref*sz_relative[0]*sz_relative[1]*sz_relative[2];

        unsigned char *pOutput = NULL;
        if(datatype_tile == VISIOCYTE_UINT8)
        {
            if(extconv<unsigned char, VISIOCYTELONG>((unsigned char *)relative1d + offset_c, sz_relative[0], sz_relative[1], sz_relative[2], pOutput)!=true)
            {
        ...
        {
            if(extconv<unsigned short, VISIOCYTELONG>((unsigned short *)relative1d + offset_c, sz_relative[0], sz_relative[1], sz_relative[2], pOutput)!=true)
            {
                printf("Fail to call function extconv! \n");
                return false;
            }
        ...
        {
            sz_relative[3]=1;
            if(!simple_saveimage_wrapper(callback,qPrintable(qs_filename_img_output),pOutput,sz_relative,VISIOCYTE_UINT8))
            {
    ...
    else if (func_name == tr("ref2mip"))
    {
        ...
        for(VISIOCYTELONG j=0; j<sz_relative[1]; j++)
        {
            VISIOCYTELONG offset_j = j*sz_relative[0];
            for(VISIOCYTELONG i=0; i<sz_relative[0]; i++)
            {
                int val = 0;
                for(VISIOCYTELONG k=0; k<sz_relative[2]; k++)
                {
    ...
    else if (func_name == tr("createTemplate"))
    {
        ...
        else
        {
            ...
            qs_filename_img_output=qs_pathname_output+"/"+qs_basename_input+"_template.visiocyteraw";
        }
