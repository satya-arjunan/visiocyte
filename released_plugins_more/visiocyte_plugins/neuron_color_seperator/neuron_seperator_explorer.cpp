...
using namespace std;
neuron_seperator_explorer::neuron_seperator_explorer(VISIOCYTEPluginCallback2 * cb, QWidget *parent)
{
    creat();
    idx_ext=0;
    idx_img=0;
...
void neuron_seperator_explorer::screenShot()
{
    ...
    if(!simple_saveimage_wrapper(*callback, fname.toStdString().c_str(), image1Dc_in, sz_img, intype)){
        visiocyte_msg("Error:failed to save screenshot to "+fname);
        return;
    }
    ...
    visiocyte_msg("ScreenShot saved to "+fname);
}
...
//        }
    ...
    anopath=fname_input;
    visiocyte_msg("Project file: " + fname_input + " loaded.\nFound "
...
void neuron_seperator_explorer::updateAll()
{
    ...
    if(pre_img!=idx_img){
        ...
        {
          visiocyte_msg("load image "+imgs[idx_img].fnames_extract.at(0)+" error!");
          return;
        }
    ...
    if(idx_ext>0 && idx_ext<imgs[idx_img].fnames_extract.size()){
        ...
        {
          visiocyte_msg("load image "+imgs[idx_img].fnames_extract.at(idx_ext)+" error!");
          return;
        }
        ...
        for(VISIOCYTELONG vid=0; vid<sz_img[0]*sz_img[1]*sz_img[2]; vid++){
        ...
        if(this->imgs.at(idx_img).fnames_extract.size()>1){
            for(int tmp_idx=1; tmp_idx<imgs[idx_img].fnames_extract.size(); tmp_idx++){
                ...
                {
                  visiocyte_msg("load image "+imgs[idx_img].fnames_extract.at(tmp_idx)+" error!");
                  return;
                }
...
void neuron_seperator_explorer::updateExtractWindow(unsigned char *image1Dc, VISIOCYTELONG size[4], int type)
{
    //search in open windows
    bool winfound=false;
    visiocytehandleList visiocytehandleList_current=callback->getImageWindowList();
    for (VISIOCYTELONG i=0;i<visiocytehandleList_current.size();i++)
    {
        if(callback->getImageName(visiocytehandleList_current[i]).contains(NAME_EXTWIN))
        {
            winfound=true;
        }
    }
    if(image1Dc != 0){ //image loaded
        ...
        Image4DSimple image4d;
        image4d.setFileName(this->imgs.at(idx_img).fnames_extract.at(idx_ext).toStdString().c_str());
        VISIOCYTELONG size_page = size[0]*size[1]*size[2]*size[3];
        ...
        if(!winfound){ //open a window if none is open
            visiocytehandle visiocytehandle_main=callback->newImageWindow();
            callback->setImage(visiocytehandle_main, &image4d);
            callback->setImageName(visiocytehandle_main, NAME_EXTWIN);
            callback->updateImageWindow(visiocytehandle_main);
        }else{
            //update the image
            for (VISIOCYTELONG i=0;i<visiocytehandleList_current.size();i++)
