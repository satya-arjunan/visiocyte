...
void neuron_extractor_by_marker::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	if (menu_name == tr("extract_by_markers"))
	{
        ...
        if(selectWindowList.size()<1){
            visiocyte_msg("Cannot find 3D view with SWC file and markers. Please load the SWC file you want to extract in the 3D view and define markers on the structure you want to extract.");
            return;
        ...
        mList = callback.getHandleLandmarkList_Any3DViewer(visiocytewin);
    ...
    else if (menu_name == tr("extract_spines"))
    {
        ...
        if(!export_list2file(spines,fileSaveName, fileOpenName)){
            visiocyte_msg("Cannot open file " + fileSaveName + " for writing!");
        }
	...
	{
		visiocyte_msg(tr("This plugin will extract the neurons selected by markers. The structure connected to each markers will be extracted and saved seperately.. "
			"Developed by Hanbo Chen, 2014-11-2"));
	}
}

bool neuron_extractor_by_marker::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
{
    cout<<"\nUsage: visiocyte -x neuron_extractor -f label_components -i <input.swc> -o <output.swc>"<<endl;
    cout<<"\n";
}
...
void neuron_extractor_by_marker::extract_spine(const NeuronTree& nt, QList<NeuronSWC>& spine, int length)
{
    ...
    VISIOCYTELONG idx=vertexType.indexOf(-2);
    VISIOCYTELONG curtid=0;
    while(idx>=0){
        int count = 0;
        int pid=idx;
        do{
...
{
    if(mList->size()<=0){
        visiocyte_msg("No markers found. Please define a marker first. The program will extract structure connected to each marker.");
        return;
    }

    if(ntList->size()<=0){
        visiocyte_msg("No neuron tree identified. Please load SWC file first.");
        return;
    }
    ...
    for(int nid=0; nid<ntList->size(); nid++){
        QList<int> pList;
        pList.clear();
        for(VISIOCYTELONG i=0; i<ntList->at(nid).listNeuron.size(); i++){
    ...
    //output
    visiocyte_msg("found "+QString::number(treecount)+" from "+
    ...
    for(int i=0; i<allTrees.size(); i++){
        ...
        if(!export_list2file(allTrees[i],fname_neuron, "")){
            visiocyte_msg("Cannot open file " + fname_neuron + " for writing!");
        }
    }
}
...
bool export_list2file(const QList<NeuronSWC>& lN, QString fileSaveName, QString oldFileName)
{
    ...
    myfile<<"# generated by Visiocyte Plugin neuron_extractor"<<endl;
