...
void reconstruction_func(VISIOCYTEPluginCallback2 &callback, QWidget *parent, input_PARA &PARA, QString image_name,bool bmenu)
{
...
void MeanShiftPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
        ...
        {
                visiocyte_msg(tr("This is a demo plugin to perform spanning tree algorithm which combined with breadth First search on the current image.\n"
...
//     The functions may not necessarily be in example_func.cpp, but you are strongly recommended to do so
//     to seperate the Interface from the core functions, and it is consistant with the form in plugin_creator
bool MeanShiftPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
        ...
        else if (func_name == tr("help"))
        {
                ...
                printf("visiocyte -x BJUT_meanshift -f meanshift -i <inimg_file> -p <channel> <prim_distance> <threshold> <percentage> \n");
