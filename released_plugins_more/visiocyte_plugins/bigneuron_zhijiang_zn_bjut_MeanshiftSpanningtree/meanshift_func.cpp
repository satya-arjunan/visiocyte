...
#include "../../../released_plugins/visiocyte_plugins/neurontracing_vn2/app2/heap.h"

#include "../../../../visiocyte_external/visiocyte_main/neuron_editing/v_neuronswc.h"
#include "../../../released_plugins/visiocyte_plugins/sort_neuron_swc/sort_swc.h"
...
struct sphere_model{
    ...
    VISIOCYTELONG up_right_up;
    VISIOCYTELONG up_left_up;
    VISIOCYTELONG up_left_bottom;
    VISIOCYTELONG up_right_bottom;
    VISIOCYTELONG down_right_up;
    VISIOCYTELONG down_left_up;
    VISIOCYTELONG down_left_bottom;
    VISIOCYTELONG down_right_bottom;
...
struct sphere_model_four_directions{
    //the number of nodes which were included by eight directions
    VISIOCYTELONG up_right;
    VISIOCYTELONG up_left;
    VISIOCYTELONG down_right;
    VISIOCYTELONG down_left;
...
QMultiMap<VISIOCYTELONG,QMap<VISIOCYTELONG,QList<Node> > > number_path_vn3;
QList<NeuronSWC > final_listNeuron;
QMap<VISIOCYTELONG,int > cp_SWC;
...
QMap<VISIOCYTELONG, NeuronSWC > original_tree;//keep the original tree and insert the shortest path into each two nodes

QMap<VISIOCYTELONG,int > Map_finalnode;
QMap<VISIOCYTELONG,int > Map_nodes;
QMap<VISIOCYTELONG,VISIOCYTELONG > Map_allnode;
QList <NeuronSWC > result_list;
QList <NeuronSWC > con_tree;
vector<QList<Node*> > v_List;
QMap<VISIOCYTELONG,QList<Node*> > root_class;
QList<Node*> QTest;
int cluster_number=0;
QMap<VISIOCYTELONG,QList<VISIOCYTELONG> > covered_relation;

QMap<VISIOCYTELONG,Node*> node_afterMerge;
QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,QList<Node> > > arrive_path;
QMap<VISIOCYTELONG,int> seed_pos_number;
...
double enlarge_radiusof_single_node_xy_vn2(unsigned char * &img1d,Node node,VISIOCYTELONG sz_x, VISIOCYTELONG sz_y, VISIOCYTELONG sz_z)
{
...
void printSWCByQMap_QMap_vn2(char* path,QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,Node > > nodeMap)
{
    VISIOCYTELONG number=0;
    FILE * fp = fopen(path, "wt");
    if (!fp) return;
    ...
    for(QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,Node > >::iterator iter1 = nodeMap.begin(); iter1 != nodeMap.end(); iter1++)
    {
        QMap<VISIOCYTELONG,Node > temp1=iter1.value();
        for(QMap<VISIOCYTELONG,Node >::iterator iter2=temp1.begin();iter2!=temp1.end();iter2++)
        {
...
void construct_tree_vn2(QMap<int,QList<Node*> > finalclass_node,unsigned char * &img1d,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z)
    ...
    {
        {
            ...
            VISIOCYTELONG marknum = seeds.size();
...
double cal_weight(VISIOCYTELONG curi,VISIOCYTELONG curj,VISIOCYTELONG curk, VISIOCYTELONG x,VISIOCYTELONG y,VISIOCYTELONG z,double inte_nd,double inte_cen,VISIOCYTELONG r)//MeanShiftﾋ羚ｨﾖﾐｼﾆﾋ羞耨・ｭｵ聊ｮｼ莊ﾄﾈｨﾖｵ｣ｬﾀ・ｭｵ耿ｽｽ・ﾄﾈｨﾖｵﾔｽｴｬﾏﾘｺﾍﾔｭｵ耿ｽｽ・ﾄﾈｨﾖｵﾔｽｴ・
{
    //double r=100;
    double I=255;
    double weight=0;
...
unsigned char* Gauss_filter(unsigned char * &img1d,unsigned char* nData1,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z,VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
QList <NeuronSWC> construct_tree_vn5(QMap<int,Node* > roots,unsigned char * &img1d,unsigned char * &img2d,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z,VISIOCYTELONG *in_sz)
{//combined the gradient and distance as weight of spanning to construct every roots
    ...
    for(int loop = 0; loop<marknum;loop++)
    {
        ...
        if(indexi>=0)
        {
            ...
            VISIOCYTELONG loc1=GET_IND(seeds.at(indexi)->x,seeds.at(indexi)->y,seeds.at(indexi)->z);
            VISIOCYTELONG loc2=GET_IND(seeds.at(indexj)->x,seeds.at(indexj)->y,seeds.at(indexj)->z);
            QList<Node> temp22;
            temp22.clear();
            QMultiMap<VISIOCYTELONG,QMap<VISIOCYTELONG,QList<Node> > >::iterator begin=number_path_vn3.lowerBound(loc1);
            QMultiMap<VISIOCYTELONG,QMap<VISIOCYTELONG,QList<Node> > >::iterator end=number_path_vn3.upperBound(loc1);
            //printf("times:::%d\n",times);
            while((begin!=end))
            {
    ...
    marker_MST.listNeuron = listNeuron;
    marker_MST.hashNeuron = hashNeuron;
    //printSWCByQList_Neuron(listNeuron,"C:\\Visiocyte\\list.SWC");
...
bool decide_outrange(VISIOCYTELONG current_x,VISIOCYTELONG current_y,VISIOCYTELONG current_z,Node* start,Node* end)
{

    VISIOCYTELONG con_startX=start->x;
    VISIOCYTELONG con_startY=start->y;
    VISIOCYTELONG con_startZ=start->z;

    VISIOCYTELONG con_endX=end->x;
    VISIOCYTELONG con_endY=end->y;
    VISIOCYTELONG con_endZ=end->z;

    VISIOCYTELONG step_x=ABS(con_startX-con_endX);
    VISIOCYTELONG step_y=ABS(con_startY-con_endY);
    VISIOCYTELONG step_z=ABS(con_startZ-con_endZ);

    VISIOCYTELONG condition_Xa=ABS(current_x-con_startX);
    VISIOCYTELONG condition_Xb=ABS(current_x-con_endX);
    VISIOCYTELONG condition_Ya=ABS(current_y-con_startY);
    VISIOCYTELONG condition_Yb=ABS(current_y-con_endY);
    VISIOCYTELONG condition_Za=ABS(current_z-con_startZ);
    VISIOCYTELONG condition_Zb=ABS(current_z-con_endZ);
...
bool decide_samebox(VISIOCYTELONG offset_x,VISIOCYTELONG offset_y,VISIOCYTELONG offset_z,Node* node,int step)
{
    VISIOCYTELONG start_x=offset_x;
    VISIOCYTELONG start_y=offset_y;
    VISIOCYTELONG start_z=offset_z;

    VISIOCYTELONG end_x=node->x;
    VISIOCYTELONG end_y=node->y;
    VISIOCYTELONG end_z=node->z;

    VISIOCYTELONG step_x=ABS(start_x-end_x);
    VISIOCYTELONG step_y=ABS(start_y-end_y);
    VISIOCYTELONG step_z=ABS(start_z-end_z);
...
QList<NeuronSWC> spanning_without_bf(QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,Node> >  roots){
    ...
    for(QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,Node> > ::iterator iter1=roots.begin();iter1!=roots.end();iter1++){
        QMap<VISIOCYTELONG,Node> temp1=iter1.value();
        for(QMap<VISIOCYTELONG,Node>::iterator iter2=temp1.begin();iter2!=temp1.end();iter2++){
            Node temp2=iter2.value();
            seeds.append(temp2);
...
void bf_vn3(QMap<int,Node*> roots,double **weight_result,unsigned char *&img1d,double average_dis,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z){
    ...
    VISIOCYTELONG up_limit;
    VISIOCYTELONG down_limit;
    int marknum=roots.size();
    int row=-1;
    int iter_step=2;
    QQueue<Node> queue1;
    QMap<VISIOCYTELONG,bool> node_searched;
    QMap<VISIOCYTELONG,Node> parent_path_vn2;
    QList<Node> path_shortest_vn2;
    QMap<VISIOCYTELONG,QList<Node> > unit_path_vn3;
    ...
    for(QMap<int,Node* >::iterator iter=roots.begin();iter!=roots.end();iter++)
    {
        ...
        VISIOCYTELONG start_x=node1->x;
        VISIOCYTELONG start_y=node1->y;
        VISIOCYTELONG start_z=node1->z;
        if(start_x==sz_x)
        {
            start_x=start_x-1;
        }
        ...
        VISIOCYTELONG limit1=GET_IND(start_x,start_y,start_z);
        for(QMap<int,Node* >::iterator iter1=roots.begin();iter1!=roots.end();iter1++)
        {
            ...
            VISIOCYTELONG end_x=node2->x;
            VISIOCYTELONG end_y=node2->y;
            VISIOCYTELONG end_z=node2->z;
            if(end_z==sz_z)
            {
                end_z=end_z-1;

            }
            VISIOCYTELONG limit2=GET_IND(end_x,end_y,end_z);
            ...
            }else
            {
                ...
                VISIOCYTELONG node_number=(ABS(start_x-end_x)+1)*(ABS(start_y-end_y)+1)*(ABS(start_z-end_z)+1);
                VISIOCYTELONG count=0;
                VISIOCYTELONG offset_x=start_x;
                VISIOCYTELONG offset_y=start_y;
                VISIOCYTELONG offset_z=start_z;
                VISIOCYTELONG offset_dis=0;
                ...
                while((!decide_samebox(offset_x,offset_y,offset_z,node2,iter_step)&&(queue1.size()!=0)&&(count<=node_number)))
                {
                    ...
                    for(int times=0;times<26;times++)
                    {
                        ...
                        VISIOCYTELONG index=GET_IND(offset_x,offset_y,offset_z);
                ...
                if(flag)
                {
                    ...
                    {
                        visiocyte_msg(QString("the shortest path is empty, program crash."));
...
{//considering the distance and pixal intensity as weight so as to calculate the markEdge
    ...
    VISIOCYTELONG current_x=result.first().x;
    VISIOCYTELONG current_y=result.first().y;
    VISIOCYTELONG current_z=result.first().z;
    ...
    while(!((current_x==temp->x)&&(current_y==temp->y)&&(current_z==temp->z)))
    {
        ...
        VISIOCYTELONG x1=path_map[GET_IND(current_x,current_y,current_z)].x;
        VISIOCYTELONG y1=path_map[GET_IND(current_x,current_y,current_z)].y;
        VISIOCYTELONG z1=path_map[GET_IND(current_x,current_y,current_z)].z;
...
int meanshift_plugin_vn4(VISIOCYTEPluginCallback2 &callback, QWidget *parent,unsigned char* img1d,VISIOCYTELONG *in_sz, QString &image_name,bool bmenu,input_PARA &PARA)
{
    ...
    VISIOCYTELONG sz_x = in_sz[0];
    VISIOCYTELONG sz_y = in_sz[1];
    VISIOCYTELONG sz_z = in_sz[2];
    ...
    QMultiMap<VISIOCYTELONG,Node> cluster_result;
    QMap<VISIOCYTELONG,QList<Node> > final_cluster_result;

    // printf("### find out rootnode  ###\n");


    /* for(VISIOCYTELONG ii=0;ii<sz_x;ii=ii+nstep)
    {
        for(VISIOCYTELONG jj=0;jj<sz_y;jj=jj+nstep)
        {
            for(VISIOCYTELONG kk=0;kk<sz_z;kk=kk+nstep)
            {
                VISIOCYTELONG ind=GET_IND(ii,jj,kk);
    ...
    QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,QList<Node> > > path_between_each_roots;
    ...
    if(after_merge_final.size()<threshold_node_number){
        ...
        // QMap<VISIOCYTELONG,QList<Node> >  node_adj_node=build_adj_matrix(path_between_each_roots,node_con_node);//output:QMap<VISIOCYTELONG,QList<Node> >  the adjecent node of node A should be included in QList
    ...
    visiocyte_msg(QString("Now you can drag and drop the generated swc fle [%1] into Visiocyte.").arg(outswc_file.toStdString().c_str()),bmenu);
    return -1;
...
QList<NeuronSWC> fulfill_path_between_roots(QList<NeuronSWC> con_tree,QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,QList<Node> > > path_between_each_roots,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z){
    VISIOCYTELONG temp_child;
    VISIOCYTELONG temp_pn;
    QMap<VISIOCYTELONG,VISIOCYTELONG> child_parent;
    QList<NeuronSWC> result;
    QMap<VISIOCYTELONG,NeuronSWC> con_tree_Map;
    QMap<VISIOCYTELONG,int> order_con_tree;
    FILE * fp = fopen("C:\\result\\final6.swc", "wt");
    if (!fp) return result;

    for(int i=0;i<con_tree.size();i++){
        NeuronSWC child_node=con_tree.at(i);
        VISIOCYTELONG child_node_index=GET_IND(child_node.x,child_node.y,child_node.z);
        if(child_node.pn==-1){
            child_parent.insert(child_node_index,-1);

        }else{
            NeuronSWC parent_node=con_tree.at((int)child_node.pn-1);
            VISIOCYTELONG parent_node_index=GET_IND(parent_node.x,parent_node.y,parent_node.z);
    ...
    QMap<VISIOCYTELONG,int> root_index;

    /* for(int i=0;i<con_tree.size();i++){
        //seed_pos_number;
        NeuronSWC temp_child=con_tree.at(i);
        VISIOCYTELONG temp_index_child=GET_IND(temp_child.x,temp_child.y,temp_child.z);
        if(temp_child.pn==-1)
            continue;
        NeuronSWC temp_parent=con_tree.at((int)temp_child.pn-1);
        VISIOCYTELONG temp_index_parent=GET_IND(temp_parent.x,temp_parent.y,temp_parent.z);
        ...
        {//have a path
            if(seed_pos_number.value(temp_index_child)<seed_pos_number.value(temp_index_parent)){
                ...
                for(int i=0;i<temp_list.size();i++)
                {
                    ...
                    VISIOCYTELONG temp2_index=GET_IND(temp2.x,temp2.y,temp2.z);
                    if(temp2.pn==-1){
                ...
                for(int j=0;j<temp_list1.size();j++){
                    ...
                    VISIOCYTELONG temp1_index=GET_IND(temp1.x,temp1.y,temp1.z);
                    if(temp1.pn==-1){
    ...
    int parent_order;
    // NeuronSWC temp_node;
    /*  for(QMap<VISIOCYTELONG,VISIOCYTELONG>::iterator iter1=child_parent.begin();iter1!=child_parent.end();iter1++)
    {
        VISIOCYTELONG parent_index=iter1.value();
        VISIOCYTELONG child_index=iter1.key();
        if(parent_index==-1)
        {
    ...
    Node node_parent;
    Node node_child;
    VISIOCYTELONG index_child;
    VISIOCYTELONG index_parent;
    for(int i=0;i<length;i++)
    {
        NeuronSWC temp_child=con_tree.at(i);
        VISIOCYTELONG index_child=GET_IND(temp_child.x,temp_child.y,temp_child.z);
        if(temp_child.pn==-1){
            continue;
        }
        NeuronSWC temp_parent=con_tree.at((int) temp_child.pn-1);
        VISIOCYTELONG index_parent=GET_IND(temp_parent.x,temp_parent.y,temp_parent.z);\
    ...
    for(QMap<VISIOCYTELONG,NeuronSWC>::iterator iter2=con_tree_Map.begin();iter2!=con_tree_Map.end();iter2++)
    {
        NeuronSWC write_node=iter2.value();
        fprintf(fp, "%ld %d %f %f %f %f %ld\n",write_node.n, write_node.type,  write_node.x,  write_node.y,  write_node.z, 1.0, write_node.pn);
    }
    for(int ii=0;ii<con_tree.size();ii++){
        ...
        VISIOCYTELONG temp_result_index=GET_IND(temp_result.x,temp_result.y,temp_result.z);
...
QMap<VISIOCYTELONG,QList<Node> >  check_node_conection(QMultiMap<VISIOCYTELONG,QMultiMap<VISIOCYTELONG,QList<Node> > > path_between_each_roots,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z){
    QMap<VISIOCYTELONG,QList<Node> >  result;
    for(QMultiMap<VISIOCYTELONG,QMultiMap<VISIOCYTELONG,QList<Node> > >::iterator iter1=path_between_each_roots.begin();iter1!=path_between_each_roots.end();iter1++){
        QMultiMap<VISIOCYTELONG,QList<Node> >  temp=iter1.value();
        QList<Node> connect_list;
        connect_list.clear();
        for(QMultiMap<VISIOCYTELONG,QList<Node> > ::iterator iter2=temp.begin();iter2!=temp.end();iter2++){
            QList<Node> temp_list=iter2.value();
            VISIOCYTELONG node_first=GET_IND(temp_list.first().x,temp_list.first().y,temp_list.first().z);
            VISIOCYTELONG node_terminal=GET_IND(temp_list.last().x,temp_list.last().y,temp_list.last().z);
...
void printf_path_between_roots(QMultiMap<VISIOCYTELONG,QMultiMap<VISIOCYTELONG,QList<Node> > > root_path,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z){
    ...
    fprintf(fp, "#comment\n");
    fprintf(fp, "##n,type,x,y,z,radius,parent\n");
    for(QMultiMap<VISIOCYTELONG,QMultiMap<VISIOCYTELONG,QList<Node> > >::iterator iter1=root_path.begin();iter1!=root_path.end();iter1++){
        QMultiMap<VISIOCYTELONG,QList<Node> >  temp1=iter1.value();
        int number=0;
        for(QMultiMap<VISIOCYTELONG,QList<Node> > ::iterator iter2=temp1.begin();iter2!=temp1.end();iter2++){
            QList<Node> temp2=iter2.value();
...
QList <NeuronSWC > calculate_distance_between_roots(QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,QList<Node> > > path_between_each_roots,QMap<int,Node* > roots,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z){
    ...
    for(QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,QList<Node> > >::iterator iter_item1 =path_between_each_roots.begin(); iter_item1 != path_between_each_roots.end(); iter_item1++){
        QMap<VISIOCYTELONG,QList<Node> >  root_begin=iter_item1.value();
        bool flag=false;
        for(QMap<VISIOCYTELONG,QList<Node> > ::iterator iter_item2=root_begin.begin();iter_item2!=root_begin.end();iter_item2++){
    ...
    for(QMap<int,Node* >::iterator iter1 =roots.begin(); iter1 != roots.end(); iter1++){
        ...
        VISIOCYTELONG temp_index=GET_IND(temp->x,temp->y,temp->z);
    ...
    {
        VISIOCYTELONG temp1_visiocytelong=GET_IND(seeds_temp.at(i).x,seeds_temp.at(i).y,seeds_temp.at(i).z);
        Node temp1=seeds_temp.at(i);
        seed_pos_number.insert(temp1_visiocytelong,i);
        QMap<VISIOCYTELONG,QList<Node> >  node_path;
        node_path.clear();
        for(int j=i+1;j<seeds_temp.size();j++)
        {
            VISIOCYTELONG temp2_visiocytelong=GET_IND(seeds_temp.at(j).x,seeds_temp.at(j).y,seeds_temp.at(j).z);
            Node temp2=seeds_temp.at(j);
            QList<Node> distance_list=path_between_each_roots.value(temp1_visiocytelong).value(temp2_visiocytelong);
            markEdge[i][j]=0;
            if(distance_list.size()==1){
                printf("1111111111111:(%d,%d,%d),(%d,%d,%d)\n",temp1.x,temp1.y,temp1.z,temp2.x,temp2.y,temp2.z);

            }
            if((temp1_visiocytelong==GET_IND(distance_list.first().x,distance_list.first().y,distance_list.first().z))&&(temp2_visiocytelong==GET_IND(distance_list.last().x,distance_list.last().y,distance_list.last().z))){
                ...
                node_path.insert(temp2_visiocytelong,distance_list);
            }else{
        ...
        if(node_path.size()!=0){
            arrive_path.insert(temp1_visiocytelong,node_path);
    ...
    /*  for(QMultiMap<VISIOCYTELONG,QMultiMap<VISIOCYTELONG,QList<Node> > >::iterator iter1 =path_between_each_roots.begin(); iter1 != path_between_each_roots.end(); iter1++)
    {
        QMultiMap<VISIOCYTELONG,QList<Node> >  elem1=iter1.value();
        //  printf("sub node list::%d\n",elem1.count());
        indexj=0;
        for(QMultiMap<VISIOCYTELONG,QList<Node> > ::iterator iter2 =elem1.begin(); iter2!= elem1.end(); iter2++)
        {
...
QList <NeuronSWC> DFS_construct(double** markEdge,QList<Node> seeds,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z){
    ...
    QMap<VISIOCYTELONG,int> new_seeds;
    new_seeds.clear();
    QMap<VISIOCYTELONG,int> root_number;//every root has a number in order (line number in SWC)
...
QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,QList<Node> > > sphere_search(unsigned char * &img1d,QMap<int,Node* > cluster_root,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z,double r,int iteration){
    ...
    QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,QList<Node> > > path_between_roots;
    ...
    for(QMap<int,Node*>::iterator iter1 =cluster_root.begin(); iter1 != cluster_root.end(); iter1++)
    {
        ...
        VISIOCYTELONG index1=GET_IND(elem1->x,elem1->y,elem1->z);
        QMap<VISIOCYTELONG,QList<Node> >  path_to_target;
        ...
        for(QMap<int,Node*>::iterator iter2 =cluster_root.begin(); iter2 != cluster_root.end(); iter2++)
        {
            ...
            VISIOCYTELONG index2=GET_IND(elem2->x,elem2->y,elem2->z);
            if(index1==index2)
            {
                continue;
            }
            ...
            while(times<iteration)
            {
                ...
                VISIOCYTELONG left_x=current_center.x-r;
                VISIOCYTELONG right_x=current_center.x+r;
                VISIOCYTELONG left_y=current_center.y-r;
                VISIOCYTELONG right_y=current_center.y+r;
                VISIOCYTELONG left_z=current_center.z-r;
                VISIOCYTELONG right_z=current_center.z+r;
                ...
                for(VISIOCYTELONG i=left_x;i<=right_x;i++){
                    for(VISIOCYTELONG j=left_y;j<=right_y;j++){
                        for(VISIOCYTELONG k=left_z;k<=right_z;k++){
                            ...
                            VISIOCYTELONG relate_x=i-current_center.x;
                            VISIOCYTELONG relate_y=j-current_center.y;
                            VISIOCYTELONG relate_z=k-current_center.z;
                            ...
                            VISIOCYTELONG beg_ter_x=elem2->x-current_center.x;
                            VISIOCYTELONG beg_ter_y=elem2->y-current_center.y;
                            VISIOCYTELONG beg_ter_z=elem2->z-current_center.z;
...
QMultiMap<VISIOCYTELONG,QMultiMap<VISIOCYTELONG,QList<Node> > > spanning_combined_bf_vn3(unsigned char * &img1d,QMap<int,Node* > cluster_root,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z,double r,int iteration){
    ...
    //second, after get the distance, input it into mst algorithm and get a connection relation
    //finally fix the route between parent and child in SWC based on the bf route got in first step
    QMultiMap<VISIOCYTELONG,QMultiMap<VISIOCYTELONG,QList<Node> > > path_between_roots;
    path_between_roots.clear();
    int count_time=0;
    ...
    for(QMap<int,Node*>::iterator iter1 =cluster_root.begin(); iter1 != cluster_root.end(); iter1++)
    {
        Node* elem1=iter1.value();
        VISIOCYTELONG index1=GET_IND(elem1->x,elem1->y,elem1->z);
        QMultiMap<VISIOCYTELONG,QList<Node> >  path_to_target;
        path_to_target.clear();
    ...
    for(QMap<int,Node*>::iterator iter1 =cluster_root.begin(); iter1 != cluster_root.end(); iter1++)
    {
        ...
        for(QMap<int,Node*>::iterator iter2 =cluster_root.begin(); iter2 != cluster_root.end(); iter2++)
        {
            ...
            while(times<iteration)
            {
                ...
                /* VISIOCYTELONG up_right_up=0;
                VISIOCYTELONG up_left_up=0;
                VISIOCYTELONG up_left_bottom=0;
                VISIOCYTELONG up_right_bottom=0;
                VISIOCYTELONG down_right_up=0;
                VISIOCYTELONG down_left_up=0;
                VISIOCYTELONG down_left_bottom=0;
                VISIOCYTELONG down_right_bottom=0;
                //define the node with maximum pixval value in the eight directions and corresponding pixal
...
//should define a QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,QList<Node> > > which indicate that the QList is the nodes between two roots
...
Node choose_region_vn4(unsigned char * &img1d,struct sphere_model_two_directions sphere_m,Node source_node,Node target_node,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z){
    //two directions
    QList<VISIOCYTELONG> direction_node_number;
    ...
    for(int i=0;i<direction_node.size();i++)
    {
        ...
        for(int j=0;j<sub_all_nodes.size();j++)
        {
            ...
            VISIOCYTELONG vector1_x=source_node.x-sub_node.x;
            VISIOCYTELONG vector1_y=source_node.y-sub_node.y;
            VISIOCYTELONG vector1_z=source_node.z-sub_node.z;

            VISIOCYTELONG vector2_x=source_node.x-target_node.x;
            VISIOCYTELONG vector2_y=source_node.y-target_node.y;
            VISIOCYTELONG vector2_z=source_node.z-target_node.z;
...
Node choose_region(struct sphere_model sphere_m,Node source_node,Node target_node){
    ...
    for(int i=0;i<direction_node.size();i++)
    {
        ...
        VISIOCYTELONG vector1_x=source_node.x-temp.x;
        VISIOCYTELONG vector1_y=source_node.y-temp.y;
        VISIOCYTELONG vector1_z=source_node.z-temp.z;
...
QMap<int,Node*> delete_cluster_node(unsigned char * &img1d,QMap<VISIOCYTELONG,QList<Node> >  final_cluster,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z,VISIOCYTELONG *in_sz,double prim_distance){//every classification node also contained itself
    QMap<int,Node*> result;
    QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,Node> >  mid_result;


    for(QMap<VISIOCYTELONG,QList<Node> >::iterator iter =final_cluster.begin(); iter != final_cluster.end(); iter++){//clean the final_cluster, change its type to QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,Node> > for easy process
        QList<Node> cluster_list=iter.value();
        QMap<VISIOCYTELONG,Node> temp;
        temp.clear();
    ...
    int count_number=0;
    for(QMap<VISIOCYTELONG,QMap<VISIOCYTELONG,Node> > ::iterator iter =mid_result.begin(); iter != mid_result.end(); iter++){

        QMap<VISIOCYTELONG,Node> elem_Map=iter.value();
        QMap<VISIOCYTELONG,Node> after_merge=merge_cluster_node(elem_Map,prim_distance);
        //printf("%d\n",after_merge.count());
        /* if(after_merge.count()==1){
            VISIOCYTELONG key=after_merge.begin().key();
            Node test=after_merge.begin().value();
            //printf("baohan: %d\n",node_afterMerge.contains(key));
            // printf("baohan: %d\n",key==GET_IND(test.x,test.y,test.z));
        }*/
        //change the type of QMap<VISIOCYTELONG,Node> to QMap<int,Node*>
        for(QMap<VISIOCYTELONG,Node>::iterator iter1=after_merge.begin();iter1!=after_merge.end();iter1++){
...
QMap<VISIOCYTELONG,QList<Node> > cluster2newroot(QMap<VISIOCYTELONG,QList<VISIOCYTELONG> > covered,QMultiMap<VISIOCYTELONG,Node> cluster){
    QMap<VISIOCYTELONG,QList<Node> > result;
    for(QMap<VISIOCYTELONG,QList<VISIOCYTELONG> >::iterator iter =covered.begin(); iter != covered.end(); iter++){
        QList<VISIOCYTELONG> subcovered=iter.value();
        if(subcovered.size()==0&&!(node_afterMerge.contains(iter.key())))
            continue;
        QList<Node> temp;
        temp.clear();
        for(int i=0;i<subcovered.size();i++){
            VISIOCYTELONG index=subcovered.at(i);
            QMultiMap<VISIOCYTELONG,Node>::iterator begin=cluster.lowerBound(index);//equal or greater
            QMultiMap<VISIOCYTELONG,Node>::iterator end=cluster.upperBound(index);//greater
            while(begin!=end){
        ...
        QMultiMap<VISIOCYTELONG,Node>::iterator begin=cluster.lowerBound(iter.key());//add the node belongs to itself
        QMultiMap<VISIOCYTELONG,Node>::iterator end=cluster.upperBound(iter.key());
        while(begin!=end){
...
bool determine_noisy(unsigned char * &img1d,VISIOCYTELONG x,VISIOCYTELONG y,VISIOCYTELONG z,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z,double threshold,double percentage,int iter_step=1){
    double number_count=0;
    double node_number=0;
    bool result=false;
    for(int times=0;times<26;times++)
    {
        VISIOCYTELONG offset_x=x+node_table[times].i*iter_step;
        VISIOCYTELONG offset_y=y+node_table[times].j*iter_step;
        VISIOCYTELONG offset_z=z+node_table[times].k*iter_step;
        if(offset_x>=sz_x){
            continue;
        }else if(offset_x<0){
            continue;
...
{
    QList<NeuronSWC> result;
    QHash<VISIOCYTELONG,NeuronSWC> neuron;//for searching the node using VISIOCYTELONG
    QList<VISIOCYTELONG> childs;
    for(int j=0;j<target.size();j++)
    {
        NeuronSWC temp=target.at(j);
        VISIOCYTELONG index=GET_IND(temp.x,temp.y,temp.z);
        neuron.insert(index,temp);
    }
    QHash<VISIOCYTELONG,VISIOCYTELONG> cp=Child_Parent_Wan(target,sz_x,sz_y,sz_z);
    QHash<VISIOCYTELONG,VISIOCYTELONG> pc=Parent_Child_Wan(target,sz_x,sz_y,sz_z);
    ...
    for(int i=0;i<target.size();i++)
    {
        ...
        }else
        {
            ...
            VISIOCYTELONG parent=cp.value(GET_IND(element.x,element.y,element.z));
...
{
    QHash<VISIOCYTELONG,VISIOCYTELONG> result;

    for(int i=1;i<target.size()-1;i++)//ignore the first and end node
    {
        NeuronSWC element_child=target.at(i);
        VISIOCYTELONG child=GET_IND(element_child.x,element_child.y,element_child.z);
        NeuronSWC element_parent=target.at(element_child.parent-1);
        VISIOCYTELONG parent=GET_IND(element_parent.x,element_parent.y,element_parent.z);
...
QMap<VISIOCYTELONG,Node> merge_cluster_node(QMap<VISIOCYTELONG,Node> rootnodes,double distance)
{
    ...
    QMap<VISIOCYTELONG,Node> result;
    for(QMap<VISIOCYTELONG,Node>::iterator iter =rootnodes.begin(); iter != rootnodes.end(); iter++)
    {
        VISIOCYTELONG key=iter.key();
        evidence2.insert(key,0);
    }
    for(QMap<VISIOCYTELONG,Node>::iterator iter1 =rootnodes.begin(); iter1 != rootnodes.end(); iter1++){

        Node elem=(iter1.value());
        // VISIOCYTELONG index1=GET_IND(elem.x,elem.y,elem.z);
        ...
        for(QMap<VISIOCYTELONG,Node>::iterator iter2 =rootnodes.begin(); iter2 != rootnodes.end(); iter2++){
            ...
            // VISIOCYTELONG index2=GET_IND(elem1->x,elem1->y,elem1->z);
    ...
    for(QMap<VISIOCYTELONG,Node>::iterator iter3 =rootnodes.begin(); iter3 != rootnodes.end(); iter3++){
...
QMap<int,Node*> merge_rootnode_vn2(QMap<int,Node*> &rootnodes,unsigned char * &img1d,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z,double distance)
{
    ...
    for(QMap<int,Node*>::iterator iter1 =rootnodes.begin(); iter1 != rootnodes.end(); iter1++){
        QList<VISIOCYTELONG> subnode_index;
        Node* elem=(iter1.value());
        VISIOCYTELONG index1=GET_IND(elem->x,elem->y,elem->z);
        subnode_index.clear();
...
void merge_rootnode(QMap<int,Node*> &rootnodes,unsigned char * &img1d,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z)
{
    ...
    //	printSwcByMap(root,"C:\\Visiocyte\\finalroot.swc");
...
QList<Node*> trim_nodes(QList<Node*> seed,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z)
{
    ...
    QMultiMap<VISIOCYTELONG,VISIOCYTELONG> parent_node;
    QMap<VISIOCYTELONG,int> flag;
    ...
    for(int i=0;i<seed.size();i++)
    {
        ...
        VISIOCYTELONG key=GET_IND(elem1->x,elem1->y,elem1->z);
        seed_radius.insert(elem1->r,elem1);
        flag.insert(GET_IND(elem1->x,elem1->y,elem1->z),0);
        for(int j=0;j<seed.size();j++)
        {
            Node* elem=seed.at(j);
            VISIOCYTELONG key1=GET_IND(elem->x,elem->y,elem->z);
            if((elem1->x==elem->x)&&(elem1->y==elem->y)&&(elem1->z==elem->z))
                continue;
            double dis=(double)sqrt((double)(elem->x-elem1->x)*(elem->x-elem1->x)+(double)(elem->y-elem1->y)*(elem->y-elem1->y)+(double)(elem->z-elem1->z)*(elem->z-elem1->z));
    ...
    for(QMultiMap<double,Node*>::iterator iter=seed_radius.end();iter!=seed_radius.begin();iter--)
    {
        if(iter==seed_radius.end())
            continue;
        Node* temp=iter.value();
        VISIOCYTELONG key2=GET_IND(temp->x,temp->y,temp->z);
        if(flag[GET_IND(temp->x,temp->y,temp->z)]==0)
        {
            result_seeds.append(temp);
            QMultiMap<VISIOCYTELONG,VISIOCYTELONG>::iterator begin=parent_node.lowerBound(key2);
            QMultiMap<VISIOCYTELONG,VISIOCYTELONG>::iterator end=parent_node.upperBound(key2);
            //printf("times:::%d\n",times);
            while((begin!=end))
            {
                flag[begin.value()]=1;
...
void prepare_write(QList<NeuronSWC> marker_MST_sorted)
{
    ...
    VISIOCYTELONG n_temp=0;
    VISIOCYTELONG n_parent=0;
...
bool found_final_vn3(unsigned char * &img1d,VISIOCYTELONG x,VISIOCYTELONG y,VISIOCYTELONG z,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z,VISIOCYTELONG r)
{
    ...
    VISIOCYTELONG xe=x-r;if(xe<0) xe=0;
    VISIOCYTELONG xb=x+r;if(xb>sz_x) xb=sz_x;
    VISIOCYTELONG ye=y-r;if(ye<0) ye=0;
    VISIOCYTELONG yb=y+r;if(yb>sz_y) yb=sz_y;
    VISIOCYTELONG ze=z-r;if(ze<0) ze=0;
    VISIOCYTELONG zb=z+r;if(zb>sz_z) zb=sz_z;
    for(VISIOCYTELONG i=xe;i<xb;i++)
    {
        for(VISIOCYTELONG j=ye;j<yb;j++)
        {
    ...
    VISIOCYTELONG next_x=(long)(sum1_x/sum2_x);
    VISIOCYTELONG next_y=(long)(sum1_y/sum2_y);
    VISIOCYTELONG next_z=(long)(sum1_z/sum2_z);
...
long meanshift_vn7(unsigned char * &img1d,VISIOCYTELONG x,VISIOCYTELONG y,VISIOCYTELONG z,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z,VISIOCYTELONG r,int iteration,VISIOCYTELONG *in_sz)
{
    ...
    while(iter<iteration)
    {
        ...
        VISIOCYTELONG xb=cur_center.x+r;if(xb>sz_x) xb=sz_x;
        VISIOCYTELONG ye=cur_center.y-r;if(ye<0) ye=0;
        VISIOCYTELONG yb=cur_center.y+r;if(yb>sz_y) yb=sz_y;
        VISIOCYTELONG ze=cur_center.z-r;if(ze<0) ze=0;
        VISIOCYTELONG zb=cur_center.z+r;if(zb>sz_z) zb=sz_z;
    ...
    //VISIOCYTELONG index=GET_IND(final_node->x,final_node->y,final_node->z);
    ...
    if(flag_zero||iter==iteration){
        ...
        VISIOCYTELONG ind=(VISIOCYTELONG)GET_IND(pre_node->x,pre_node->y,pre_node->z);
...
long meanshift_vn6(unsigned char * &img1d,VISIOCYTELONG x,VISIOCYTELONG y,VISIOCYTELONG z,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z,VISIOCYTELONG r,int iteration,VISIOCYTELONG *in_sz)
{
    ...
    }else if(Map_finalnode.contains((VISIOCYTELONG)GET_IND(pre_node->x,pre_node->y,pre_node->z))){
        VISIOCYTELONG index_final=(VISIOCYTELONG)GET_IND(pre_node->x,pre_node->y,pre_node->z);
        delete center;
        delete pre_node;
...
Node choose_next_node(unsigned char * &img1d,Node* pre,VISIOCYTELONG next_x,VISIOCYTELONG next_y,VISIOCYTELONG next_z,QList<Node> choose,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z)
{
    ...
    for(int i=0;i<a;i++){
        ...
        VISIOCYTELONG relative_x=pre->x-cur.x;
        VISIOCYTELONG relative_y=pre->y-cur.y;
        VISIOCYTELONG relative_z=pre->z-cur.z;
        // printf("%ld %ld %ld \n",cur->x,cur->y,cur->z);
...
void meanshift_vn5(unsigned char * &img1d,VISIOCYTELONG x,VISIOCYTELONG y,VISIOCYTELONG z,VISIOCYTELONG sz_x,VISIOCYTELONG sz_y,VISIOCYTELONG sz_z,VISIOCYTELONG r,int iteration,VISIOCYTELONG *in_sz)
{
    int iter=0;
    int same_times=0;
    int situation=0;
    ...
    while(1)
    {
        ...
        }else
        {
            ...
            VISIOCYTELONG temp_x=cur_center->x;
            ...
            {
                VISIOCYTELONG next_x=(sum1_x/sum2_x)+0.5;//may shift to the node whose intensity equals zero, and may shfit to the node which was not included in the roots found in the first step
                VISIOCYTELONG next_y=(sum1_y/sum2_y)+0.5;
                VISIOCYTELONG next_z=(sum1_z/sum2_z)+0.5;
                cur_center->x=next_x;
                cur_center->y=next_y;
    ...
    VISIOCYTELONG ind2=GET_IND(cur_center->x,cur_center->y,cur_center->z);//here,we take the cur_center as a final root due to it cannot shift in the region
...
void printHelp()
{
    ...
    cout<<"\nvisiocyte -x BJUT_meanshift -f meanshift -i <inimg_file> -p <channel>"<<endl;
