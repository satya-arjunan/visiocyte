...
public:
	MouseMonitorWidget(VISIOCYTEPluginCallback2 & cb, QWidget * parent) : QWidget(parent)
	{
		this->callback = &cb;
...
public slots:
	...
	{
		visiocyte_msg(tr("(%1, %2, %3) is clicked !").arg(loc.x).arg(loc.y).arg(loc.z));
	}
	
	void onWinClosed()
	{
		visiocyte_msg(tr("Stop monitor ! Enjoy!"));
		this->close();
	}

	void onStart()
	{
		if(curwin==0) {visiocyte_msg("No image is opened!"); return;}
		if(monitor==0)
		{
			...
			visiocyte_msg("Try to create landmark in 3d view!");
		}		
