...
volatile VISIOCYTELONG phcDebugPosNum = 0;
volatile VISIOCYTELONG phcDebugRgnNum = 0;
...
//memory management
template <class T> int newIntImage3dPairMatlabProtocol(T *** & img3d,T * & img1d, VISIOCYTELONG imgdep, VISIOCYTELONG imghei,VISIOCYTELONG imgwid)
{
	VISIOCYTELONG totalpxlnum = imghei*imgwid*imgdep;
...
public:
	...
	~RgnGrow3dClass()
	{
		...
		for(VISIOCYTELONG i=0;i<TOTALRGNnum;i++)
		{
			RGN * pnextRgn = 0;
			if (PHCURGN)
			{
				...
				for(VISIOCYTELONG j=0;j<PHCURGN->poslistlen;j++)
				{
					POS *pnextPos = 0;
					if (PHCURGNPOS)
					{
...
public:
	VISIOCYTELONG ImgWid, ImgHei, ImgDep;
	...
	int MAXSTACKSIZE;
	int IFINCREASELABEL;
	VISIOCYTELONG PHCURLABEL;
	int ***PHCLABELSTACK3d, * PHCLABELSTACK1d;
	VISIOCYTELONG PHCLABELSTACKPOS;

	POS * PHCURGNPOS, * PHCURGNPOS_head;
	RGN * PHCURGN, * PHCURGN_head;

	VISIOCYTELONG TOTALPOSnum, TOTALRGNnum;
};

//generating an int image for any input image type
template <class T> void copyvecdata(T * srcdata, VISIOCYTELONG len, UBYTE * desdata, int& nstate, UBYTE &minn, UBYTE &maxx)
{
	if(!srcdata || !desdata)
	{
...
void rgnfindsub(int rowi,int colj, int depk, int direction,int stackinc, RgnGrow3dClass * pRgnGrow)
{
	...
	if (stackinc==1)
	{
		...
		{
			pRgnGrow->PHCURGNPOS->pos = (VISIOCYTELONG) depk*(pRgnGrow->ImgHei * pRgnGrow->ImgWid) + colj*(pRgnGrow->ImgWid) + rowi; //
...
void regiongrowing(VISIOCYTEPluginCallback2 &callback, QWidget *parent);
bool regiongrowing(VISIOCYTEPluginCallback2 &callback, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output);
...
bool RegionGrowPlugin::dofunc(const QString &func_name, const VISIOCYTEPluginArgList &input, VISIOCYTEPluginArgList &output, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
     if (func_name == tr("rg"))
	{
	...
	{
		cout<<"Usage : visiocyte -x regiongrow -f rg -i <inimg_file> -o <outimg_file>  -p <ch> <th_idx> <small> [<volsz> <threshold>]"<<endl;
		cout<<endl;
		...
		cout<<endl;
		cout<<"e.g. visiocyte -x regiongrow -f rg -i input.raw -o output.raw -p 1 2 1 200 0.0"<<endl;
		cout<<endl;
		return true;
	}
}
...
bool regiongrowing(VISIOCYTEPluginCallback2 &callback, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output)
{
	...
	VISIOCYTELONG ch = 1; // channel number start from 1 , should subtract 1 before using
	VISIOCYTELONG th_idx = 0; // threshold method index
	double threshold = 0.0; // user defined threshold
	VISIOCYTELONG volsz = 200; // vol size fore filtering out
	...
	cout<<"outimg_file = "<<outimg_file<<endl;
     ...
     VISIOCYTELONG ch_rgb = ch-1; // for channel start from 0
    ...
    if(!simple_loadimage_wrapper(callback, inimg_file, pSub, in_sz, datatype))
     ...
     {
          visiocyte_msg("Invalid data pointer!", 0);
          return false;
     }
	...
	ImagePixelType datatype_subject;
     if(datatype == 1) datatype_subject=VISIOCYTE_UINT8;
     else if(datatype == 2) datatype_subject=VISIOCYTE_UINT16;
     else if(datatype == 4) datatype_subject=VISIOCYTE_FLOAT32;

	if(datatype_subject != VISIOCYTE_UINT8)
	{
		visiocyte_msg("Currently this program only support 8-bit data.", 0);
		return false;
	}
	...
	VISIOCYTELONG offset_sub = ch_rgb*pagesz_sub;
	...
	{
		bpos_x = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMax(b_xy.left(), b_zx.left())), sz0-1);
		bpos_y = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMax(b_xy.top(),  b_yz.top())), sz1-1);
		bpos_z = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMax(b_yz.left(), b_zx.top())), sz2-1);

		epos_x = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMin(b_xy.right(), b_zx.right())), sz0-1);
		epos_y = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMin(b_xy.bottom(), b_yz.bottom())), sz1-1);
		epos_z = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMin(b_yz.right(), b_zx.bottom())), sz2-1);
	}
	else if(!vxy && vyz && vzx) // 2 of 3
	{
		bpos_x = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMax(0, b_zx.left())), sz0-1);
		bpos_y = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMax(0,  b_yz.top())), sz1-1);
		bpos_z = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMax(b_yz.left(), b_zx.top())), sz2-1);

		epos_x = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMin((VISIOCYTELONG)(sz0-1), (VISIOCYTELONG)b_zx.right())), sz0-1);
		epos_y = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMin((VISIOCYTELONG)(sz1-1), (VISIOCYTELONG)b_yz.bottom())), sz1-1);
		epos_z = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMin(b_yz.right(), b_zx.bottom())), sz2-1);
	}
	else if(vxy && !vyz && vzx)
	{
		bpos_x = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMax(b_xy.left(), b_zx.left())), sz0-1);
		bpos_y = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMax(b_xy.top(),  0)), sz1-1);
		bpos_z = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMax(0, b_zx.top())), sz2-1);

		epos_x = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMin(b_xy.right(), b_zx.right())), sz0-1);
		epos_y = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMin((VISIOCYTELONG)b_xy.bottom(), (VISIOCYTELONG)(sz1-1))), sz1-1);
		epos_z = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMin((VISIOCYTELONG)(sz2-1), (VISIOCYTELONG)b_zx.bottom())), sz2-1);
	}
	else if(vxy && vyz && !vzx)
	{
		bpos_x = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMax(b_xy.left(), 0)), sz0-1);
		bpos_y = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMax(b_xy.top(),  b_yz.top())), sz1-1);
		bpos_z = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMax(b_yz.left(), 0)), sz2-1);

		epos_x = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMin((VISIOCYTELONG)b_xy.right(), (VISIOCYTELONG)(sz0-1))), sz0-1);
		epos_y = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMin(b_xy.bottom(), b_yz.bottom())), sz1-1);
		epos_z = qBound(VISIOCYTELONG(0), VISIOCYTELONG(qMin((VISIOCYTELONG)b_yz.right(), (VISIOCYTELONG)(sz2-1))), sz2-1);
	}
	...
	//ROI extraction
	VISIOCYTELONG sx = (epos_x-bpos_x)+1;
    VISIOCYTELONG sy = (epos_y-bpos_y)+1;
    VISIOCYTELONG sz = (epos_z-bpos_z)+1;
	VISIOCYTELONG sc = sz3; // 0,1,2
	...
	try
	{
		...
		for(VISIOCYTELONG k=bpos_z; k<=epos_z; k++)
		{
			VISIOCYTELONG offset_z = k*sz0*sz1;
			VISIOCYTELONG offset_crop_z = (k-bpos_z)*sx*sy;
			for(VISIOCYTELONG j=bpos_y; j<=epos_y; j++)
			{
				VISIOCYTELONG offset_y = j*sz0 + offset_z;
				VISIOCYTELONG offset_crop_y = (j-bpos_y)*sx + offset_crop_z;
				for(VISIOCYTELONG i=bpos_x; i<=epos_x; i++)
				{
    ...
    VISIOCYTELONG neighbors = 26;
	VISIOCYTELONG neighborhood_26[26] = {-1, 1, -offset_y, offset_y, -offset_z, offset_z,
	...
	for(VISIOCYTELONG k = 0; k < sz; k++)
	{
		...
		for(VISIOCYTELONG j = 0;  j < sy; j++)
		{
			...
			for(VISIOCYTELONG i = 0, idx = idxj; i < sx;  i++, idx++)
			{
				...
				if(bw[idx])
				{
					...
					{
						VISIOCYTELONG n_idx = idx + neighborhood_26[ineighbor];
	...
	//p4Dbw.setData((unsigned char*)bw, sx, sy, sz, 1, VISIOCYTE_UINT8);

	// 3D region growing
	//----------------------------------------------------------------------------------------------------------------------------------

	//declaration
	VISIOCYTELONG totalpxlnum=pagesz;
	...
	for(int j=minlevel;j<=maxlevel;j++)
	{
		...
		BYTE *** flagImg = pRgnGrow->PHCDONEIMG3d;
		for (VISIOCYTELONG tmpi=0; tmpi<totalpxlnum; tmpi++)
		{
			...
			for(colj=0; colj<pRgnGrow->ImgHei; colj++)
				for(rowi=0; rowi<pRgnGrow->ImgWid; rowi++)
				{
					if (flagImg[depk][colj][rowi]==1)
					{
						...
						{
							pRgnGrow->IFINCREASELABEL = 1;
							VISIOCYTELONG posbeg = pRgnGrow->PHCLABELSTACKPOS;
							VISIOCYTELONG mypos = posbeg;
							...
							while (mypos>=0)
							{
								...
								VISIOCYTELONG posend = pRgnGrow->PHCLABELSTACKPOS;
	...
	VISIOCYTELONG nrgncopied = 0; //num of rgn output
	...
	while(curRgn && curRgn->next)
	{
		...
		staRegion->desposlist = new VISIOCYTELONG [curRgn->poslistlen-1];
	...
	// result pointer
	VISIOCYTELONG length;

	VISIOCYTELONG n_rgn = stclList.size(); // qMin(5, nrgncopied);
	...
	if(n_rgn>65534)
	{
		...
		for(int ii=0; ii<n_rgn; ii++)
		{
			...
			// find idx
			VISIOCYTELONG *cutposlist = stclList.at(ii).desposlist;
			...
			for(int i=0; i<length; i++)
			{
				...
				VISIOCYTELONG idx = cutposlist[i];

				VISIOCYTELONG k1 = idx/(sx*sy);
				VISIOCYTELONG j1 = (idx - k1*sx*sy)/sx;
				VISIOCYTELONG i1 = idx - k1*sx*sy - j1*sx;
			...
			if (si>0)
			{
				VISIOCYTELONG ncx = scx/si + 0.5 +1;
				VISIOCYTELONG ncy = scy/si + 0.5 +1;
				VISIOCYTELONG ncz = scz/si + 0.5 +1;
		...
		// display
		//p4DImage.setData((unsigned char*)pRGCL, sx, sy, sz, 1, VISIOCYTE_FLOAT32);
	...
	else if(n_rgn>254)
	{
		...
		// display
		//p4DImage.setData((unsigned char*)pRGCL, sx, sy, sz, 1, VISIOCYTE_UINT16);
	...
	else
	{
		...
		// display
		//p4DImage.setData((unsigned char*)pRGCL, sx, sy, sz, 1, VISIOCYTE_UINT8);
...
void RegionGrowPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
...
void regiongrowing(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	visiocytehandle newwinbw = callback.newImageWindow();
