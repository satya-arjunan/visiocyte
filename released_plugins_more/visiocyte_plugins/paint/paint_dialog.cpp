...
Paint_Dialog::Paint_Dialog(VISIOCYTEPluginCallback2 *cb, QWidget *parent) :
    QDialog(parent)
{
    callback=cb;
...
void Paint_Dialog::fetch()
{
    ...
    if(pixeltype!=1)
    {
        if (pixeltype == 2) //VISIOCYTE_UINT16;
        {
            Paint_Dialog::convert2UINT8((unsigned short*)image1Dc_in, image1Dc_in, size_tmp);
        }
...
bool Paint_Dialog::pushback()
{
    ...
    if (datasource==1)
    {
        QMessageBox::information(0, "", "Cannot push back. Please load the image in Visiocyte main");
        return false;
    }
    if (zoominflag)
    {
    ...
    image4D.setData(image1Dc_out,sz_img[0],sz_img[1],sz_img[2],3,VISIOCYTE_UINT8);
...
{
    QString initialPath = QDir::currentPath() + "/untitled" + ".visiocyteraw";
...
void Paint_Dialog::convert2UINT8(unsigned short *pre1d, unsigned char *pPost, VISIOCYTELONG imsz)
{
...
void Paint_Dialog::convert2UINT8(float *pre1d, unsigned char *pPost, VISIOCYTELONG imsz)
{
...
void Paint_Dialog::help()
{
    ...
    tr("<p>The <b>Paint</b> plug-in is designed to help users do simple paintings on 2D images.<br>"
               ...
               " Visiocyte main window by using 'load' or 'Fetch' button in the pop-up list of 'open' button.<br>"
               ...
               "<b>Pushback</b> -- Pushback function is enabled only if the image is fetched from current Visiocyte main"
               "window. Once the pushback button clicked, the image together with the painting will be sent back to the current"
               "Visiocyte main window.<br>"
