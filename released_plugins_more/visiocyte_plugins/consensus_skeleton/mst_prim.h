...
int PrimMSTClass::allocatememory(VISIOCYTELONG nodenum) 
{
	...
	if (nnode>0) {
		...
		try{
			...
			nodeParent = new VISIOCYTELONG[nnode];
		}
		catch(...){
			b_memory = 0;
		}
...
void mst_prim(double* adj_matrix, VISIOCYTELONG n_node, VISIOCYTELONG* plist, VISIOCYTELONG rootnode)
{
	//copy data
