...
//
bool q_principalskeleton_detection(VISIOCYTEPluginCallback &callback,
		const unsigned char *p_img_input,const VISIOCYTELONG sz_img_input[2],
		const vector<point3D64F> &vec_cptpos_input,
		const vector< vector<VISIOCYTELONG> > &vecvec_domaincptind_length,const vector<double> &vec_domainweight_length,
		const vector< vector<VISIOCYTELONG> > &vecvec_domaincptind_smooth,const vector<double> &vec_domainweight_smooth,
		...
		vector<point3D64F> &vec_cptpos_output)
{
	...
	vector< vector<VISIOCYTELONG> > vecvec_neighborcptind_length,vecvec_neighborcptind_smooth;
	vector< vector<double> > vecvec_neighborweight_length,vecvec_neighborweight_smooth;
	//initialize neighbor array
	vecvec_neighborcptind_length.assign(vec_cptpos_input.size(),vector<VISIOCYTELONG>(0,0));
	vecvec_neighborweight_length.assign(vec_cptpos_input.size(),vector<double>(0,0));
	vecvec_neighborcptind_smooth.assign(vec_cptpos_input.size(),vector<VISIOCYTELONG>(0,0));
	vecvec_neighborweight_smooth.assign(vec_cptpos_input.size(),vector<double>(0,0));
	//fill neighbor control point array
	for(VISIOCYTELONG k=0;k<vec_cptpos_input.size();k++)
	{
		//lenght constraint domain
		for(VISIOCYTELONG i=0;i<vecvec_domaincptind_length.size();i++)
			for(VISIOCYTELONG j=0;j<vecvec_domaincptind_length[i].size();j++)
			{
				VISIOCYTELONG index=vecvec_domaincptind_length[i][j];
				if(index==k)
				{
		...
		for(VISIOCYTELONG i=0;i<vecvec_domaincptind_smooth.size();i++)
			for(VISIOCYTELONG j=0;j<vecvec_domaincptind_smooth[i].size();j++)
			{
				VISIOCYTELONG index=vecvec_domaincptind_smooth[i][j];
				if(index==k)
				{
					if(j>0)										//left neighbor exist
					{
	...
	vector<VISIOCYTELONG> vec_skeletonend_ind;
	vector< vector<VISIOCYTELONG> > vecvec_skeletonend_2neighbor_ind;
	for(VISIOCYTELONG i=0;i<vecvec_neighborcptind_length.size();i++)
	{
		if(vecvec_neighborcptind_length[i].size()==1)
		{
			vec_skeletonend_ind.push_back(i);

			vector<VISIOCYTELONG> vec_skeletonend_2neighbor_ind;
			VISIOCYTELONG neighbor1_ind=vecvec_neighborcptind_length[i][0];
	...
	for(VISIOCYTELONG i=0;i<vecvec_neighborcptind_length.size();i++)
	{
		...
		for(VISIOCYTELONG j=0;j<vecvec_neighborcptind_length[i].size();j++)
			printf("[%d,%.2f,L];",vecvec_neighborcptind_length[i][j],vecvec_neighborweight_length[i][j]);
		for(VISIOCYTELONG j=0;j<vecvec_neighborcptind_smooth[i].size();j++)
	...
	for(VISIOCYTELONG i=0;i<vec_skeletonend_ind.size();i++)
	{
		printf("\t\tend pt index: [%d], neighbor index: ",vec_skeletonend_ind[i]);
		for(VISIOCYTELONG j=0;j<vecvec_skeletonend_2neighbor_ind[i].size();j++)
	...
	VISIOCYTELONG n_imgpixel=sz_img_input[0]*sz_img_input[1];

	//compute the mean and std of image
	double d_img_mean=0.0,d_img_std=0.0;
	for(VISIOCYTELONG i=0;i<n_imgpixel;i++)
	...
	for(VISIOCYTELONG y=0;y<sz_img_input[1];y++)
		...
		{
			VISIOCYTELONG index=sz_img_input[0]*y+x;
...
//	}
	...
	vector<VISIOCYTELONG> vec_foregroundpixel_x,vec_foregroundpixel_y;
	...
	sz_img_test[2]=1;	//z	dim
	sz_img_test[3]=1;	//c	dim
//	saveImage("/groups/peng/home/qul/work/straightened/baseimg.tif",(unsigned char *)p_img_base,sz_img_test,1);
//	saveImage("/groups/peng/home/qul/work/straightened/baseimg.tif",(unsigned char *)p_img_foreground_co,sz_img_test,1);
...
//		VISIOCYTELONG skeleton_left,skeleton_right,skeleton_top,skeleton_bottom;
...
//		for(VISIOCYTELONG i=0;i<vec_foregroundpixel_x.size();i++)
//		{
...
//		float f_zoomfactor_skeleton=0.5;//larva:0.1, vnc:1, celegans:0.1
//		for(VISIOCYTELONG i=0;i<vec_cptpos_ini.size();i++)
//		{
...
//	}
	...
	if(paras_input.b_showinVISIOCYTE_pts)
	{
		unsigned char* p_img=p_img_base;
//		unsigned char* p_img=p_img_foreground;
		VISIOCYTELONG sz_img[4]={sz_img_input[0],sz_img_input[1],1,1};

		VISIOCYTELONG l_npixel_s=sz_img[0]*sz_img[1];
		unsigned char* newdata1d=new unsigned char[l_npixel_s]();
		memcpy(newdata1d,p_img,l_npixel_s);
		Image4DSimple tmp;
		tmp.setData(newdata1d,sz_img[0],sz_img[1],sz_img[2],sz_img[3],VISIOCYTE_UINT8);
	...
	VISIOCYTELONG n_maxiter=paras_input.l_maxitertimes;
	for(VISIOCYTELONG iter=0;iter<n_maxiter;iter++)
	{
		//------------------------------------------------------------------------------------------------------------------------------------
		//compute the voronoi region for each control point in the skeleton
		vector< vector<VISIOCYTELONG> > vecvec_voronoi_x(vec_cptpos_ini.size(),vector<VISIOCYTELONG>(0,0));
		vector< vector<VISIOCYTELONG> > vecvec_voronoi_y(vec_cptpos_ini.size(),vector<VISIOCYTELONG>(0,0));

		//compute the distance matrix of every foreground pixel to every control point
		vector< vector<VISIOCYTELONG> > vecvec_dismatrix_pixel2ctlpt(vec_foregroundpixel_x.size(),vector<VISIOCYTELONG>(vec_cptpos_ini.size(),0));
		for(VISIOCYTELONG p=0;p<vec_foregroundpixel_x.size();p++)
			for(VISIOCYTELONG m=0;m<vec_cptpos_ini.size();m++)
			{
				VISIOCYTELONG dif_x=vec_foregroundpixel_x[p]-vec_cptpos_last[m].x;
				VISIOCYTELONG dif_y=vec_foregroundpixel_y[p]-vec_cptpos_last[m].y;
		...
		for(VISIOCYTELONG p=0;p<vec_foregroundpixel_x.size();p++)
		{
			...
			VISIOCYTELONG l_nearestctlpt_ind=0;
			for(VISIOCYTELONG m=0;m<vec_cptpos_ini.size();m++)
		...
		for(VISIOCYTELONG ind_cpt=1;ind_cpt<vec_cptpos_ini.size()-1;ind_cpt++)
		{
			double E_image[2]={0.0,0.0},E_length[2]={0.0,0.0};

			//compute the image term (mass center of current control point's voronoi region)
			VISIOCYTELONG n_voronoipixel=vecvec_voronoi_x[ind_cpt].size();
			double sum_intensity=0;
			if(n_voronoipixel!=0)
			{
				for(VISIOCYTELONG p=0;p<n_voronoipixel;p++)
				{
					VISIOCYTELONG index=sz_img_input[0]*vecvec_voronoi_y[ind_cpt][p]+vecvec_voronoi_x[ind_cpt][p];
			...
			for(VISIOCYTELONG i=0;i<vecvec_neighborcptind_length[ind_cpt].size();i++)
			{
				VISIOCYTELONG l_neighbor_ind=vecvec_neighborcptind_length[ind_cpt][i];
			...
			for(VISIOCYTELONG i=0;i<vecvec_neighborcptind_smooth[ind_cpt].size();i++)
			{
				VISIOCYTELONG l_neighbor_ind=vecvec_neighborcptind_smooth[ind_cpt][i];
			...
			for(VISIOCYTELONG i=0;i<vec_skeletonend_ind.size();i++)
			{
				...
				{
					VISIOCYTELONG neighbor1_ind=vecvec_skeletonend_2neighbor_ind[i][0];
					VISIOCYTELONG neighbor2_ind=vecvec_skeletonend_2neighbor_ind[i][1];
		...
		if(paras_input.b_showinVISIOCYTE_pts)
		{
			...
			for(int i=0;i<vec_cptpos_last.size();i++)
			{
				LocationSimple s;
				s.x=vec_cptpos_last[i].x+1;//note: marker coord start from 1 instead of 0
				s.y=vec_cptpos_last[i].y+1;//note: marker coord start from 1 instead of 0
				s.z=1;//note: marker coord start from 1 instead of 0
				s.radius=10;
