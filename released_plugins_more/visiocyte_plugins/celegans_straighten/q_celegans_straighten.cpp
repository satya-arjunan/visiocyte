...
#include "../../visiocyte_main/basic_c_fun/stackutil.h"
#include "../../visiocyte_main/basic_c_fun/basic_memory.cpp"
        ...
        unsigned char *&p_img_output,VISIOCYTELONG sz_img_output[4])
{
	...
	VISIOCYTELONG l_cuttingplane_width=paras.l_radius_cuttingplane*2+1;
        ...
        unsigned char *&p_img_output,VISIOCYTELONG sz_img_output[4])
{
	...
	VISIOCYTELONG sz_img_1c[4]={sz_img_input[0],sz_img_input[1],sz_img_input[2],1};
	...
	{
		VISIOCYTELONG pgsz_x=sz_img_1c[0];
		VISIOCYTELONG pgsz_xy=sz_img_1c[0]*sz_img_1c[1];
		VISIOCYTELONG pgsz_xyz=sz_img_1c[0]*sz_img_1c[1]*sz_img_1c[2];
		for(VISIOCYTELONG z=0;z<sz_img_1c[2];z++)
			for(VISIOCYTELONG y=0;y<sz_img_1c[1];y++)
				for(VISIOCYTELONG x=0;x<sz_img_1c[0];x++)
				{
					VISIOCYTELONG index=pgsz_xyz*paras.l_refchannel+pgsz_xy*z+pgsz_x*y+x;
					VISIOCYTELONG index_1c=pgsz_xy*z+pgsz_x*y+x;
		...
		for(VISIOCYTELONG z=0;z<sz_img_1c[2];z++)
			for(VISIOCYTELONG y=0;y<sz_img_1c[1];y++)
				for(VISIOCYTELONG x=0;x<sz_img_1c[0];x++)
				{
					...
					{
						VISIOCYTELONG index=pgsz_xyz*c+pgsz_xy*z+pgsz_x*y+x;
						if(p_img_input[index]>d_maxintensity)
							d_maxintensity=p_img_input[index];
					}

					p_img_1c[index_1c]=d_maxintensity;
				}
	}
	else
	{
		visiocyte_msg("ERROR: Invalid input interesting channel!\n");
		if(p_img_1c) 					{delete []p_img_1c;				p_img_1c=0;}
		return false;
	}

//	saveImage("/Users/qul/work/visiocyte_2.0/sub_projects/celegant_straighten/1c.raw",(unsigned char *)p_img_1c,sz_img_1c,1);

	//------------------------------------------------------------------------------------------------------------------------------------
	printf("(2). Resize image. \n");
	unsigned char *p_img_s=0;
	VISIOCYTELONG sz_img_s[4]={1,1,1,1};
...
//	saveImage("/Users/qul/work/visiocyte_2.0/sub_projects/celegant_straighten/s.raw",(unsigned char *)p_img_s,sz_img_s,1);

	//------------------------------------------------------------------------------------------------------------------------------------
	printf("(3). Straighten in XY plane. \n");
	unsigned char *p_img_sub2tar_xy=0;
	VISIOCYTELONG *sz_img_sub2tar_xy=0;

	//------------------------------------------------------------------
	printf("\t>>generate mip_xy image. \n");
	unsigned char *p_img_mip_xy=0;
	VISIOCYTELONG sz_img_mip_xy[4]={sz_img_s[0],sz_img_s[1],1,sz_img_s[3]};
	...
	VISIOCYTELONG pgsz_x=sz_img_s[0];
	VISIOCYTELONG pgsz_xy=sz_img_s[0]*sz_img_s[1];
	VISIOCYTELONG pgsz_xyz=sz_img_s[0]*sz_img_s[1]*sz_img_s[2];
	...
	for(VISIOCYTELONG y=0;y<sz_img_s[1];y++)
		for(VISIOCYTELONG x=0;x<sz_img_s[0];x++)
		{
			...
			for(VISIOCYTELONG z=0;z<sz_img_s[2];z++)
				for(VISIOCYTELONG c=0;c<sz_img_s[3];c++)
				{
					VISIOCYTELONG index=pgsz_xyz*c+pgsz_xy*z+pgsz_x*y+x;
			...
			for(VISIOCYTELONG c=0;c<sz_img_s[3];c++)
			{
				VISIOCYTELONG index_MIP=pgsz_xy*c+pgsz_x*y+x;
				p_img_mip_xy[index_MIP]=u_MIP_rgb[c];
			}
		}
	}
//	saveImage("/Users/qul/work/visiocyte_2.0/sub_projects/celegant_straighten/mip_xy.raw",(unsigned char *)p_img_mip_xy,sz_img_mip_xy,1);
	...
	double d_length=0;
	for(VISIOCYTELONG i=0;i<vec2d_markers.size()-1;i++)
	{
		double dis,difx,dify,difz;
		difx=vec2d_markers[i][0]-vec2d_markers[i+1][0];
		dify=vec2d_markers[i][1]-vec2d_markers[i+1][1];
		difz=vec2d_markers[i][2]-vec2d_markers[i+1][2];
		dis=sqrt(difx*difx+dify*dify+difz*difz);
		vec_skeletonsegdis.push_back(dis);
		d_length+=dis;
	}
	double d_step=d_length/(paras.l_ctlpts_num-1);
	...
	for(VISIOCYTELONG i=0;i<vec2d_markers.size();i++)
	{
		...
		VISIOCYTELONG n_seg=vec_skeletonsegdis[i]/d_step;
		for(VISIOCYTELONG j=0;j<n_seg;j++)
		{
			...
			tmp.z=(vec2d_markers[i+1][2]-vec2d_markers[i][2])*(d_step*(j+1)/vec_skeletonsegdis[i])+vec2d_markers[i][2];
			tmp.x/=paras.d_downsampleratio;
			tmp.y/=paras.d_downsampleratio;
			tmp.z/=paras.d_downsampleratio;
			vec_cptpos_input.push_back(tmp);
		}
	}
	}
	...
	{
	vector<VISIOCYTELONG> tmp;
	for(VISIOCYTELONG i=0;i<vec_cptpos_input.size();i++)
		tmp.push_back(i);
	...
	paras_input.l_maxitertimes=100;
	...
	paras_input.b_showinVISIOCYTE_pts=paras.b_showinVISIOCYTE_pts;
	...
	for(VISIOCYTELONG i=0;i<vec_cptpos_output.size();i++)
	{
		ImageMarker tmp;
	...
	printf("\t>>do straighten in xy plane. \n");
	{
	VISIOCYTELONG l_cuttingplane_width=(paras.l_radius_cuttingplane/paras.d_downsampleratio)*2+1;;
	if(!q_celegans_restacking_xy(
			p_img_s,sz_img_s,
...
//	saveImage("/Users/qul/work/visiocyte_2.0/sub_projects/celegant_straighten/sub2tar_xy.raw",(unsigned char *)p_img_sub2tar_xy,sz_img_sub2tar_xy,1);

	//------------------------------------------------------------------------------------------------------------------------------------
	printf("(4). Straighten in XZ plane. \n");
	unsigned char *p_img_sub2tar_xz=0;
	VISIOCYTELONG *sz_img_sub2tar_xz=0;

	//------------------------------------------------------------------
	printf("\t>>generate mip_xz image. \n");
	unsigned char *p_img_mip_xz=0;
	VISIOCYTELONG sz_img_mip_xz[4]={sz_img_sub2tar_xy[0],sz_img_sub2tar_xy[2],1,sz_img_sub2tar_xy[3]};
	...
	VISIOCYTELONG pgsz_x=sz_img_sub2tar_xy[0];
	VISIOCYTELONG pgsz_xy=sz_img_sub2tar_xy[0]*sz_img_sub2tar_xy[1];
	VISIOCYTELONG pgsz_xz=sz_img_sub2tar_xy[0]*sz_img_sub2tar_xy[2];
	VISIOCYTELONG pgsz_xyz=sz_img_sub2tar_xy[0]*sz_img_sub2tar_xy[1]*sz_img_sub2tar_xy[2];
	unsigned char u_MIP_rgb[3];
	for(VISIOCYTELONG z=0;z<sz_img_sub2tar_xy[2];z++)
		for(VISIOCYTELONG x=0;x<sz_img_sub2tar_xy[0];x++)
		{
			u_MIP_rgb[0]=u_MIP_rgb[1]=u_MIP_rgb[2]=0;
			for(VISIOCYTELONG y=0;y<sz_img_sub2tar_xy[1];y++)
				for(VISIOCYTELONG c=0;c<sz_img_sub2tar_xy[3];c++)
				{
					VISIOCYTELONG index=pgsz_xyz*c+pgsz_xy*z+pgsz_x*y+x;
			...
			for(VISIOCYTELONG c=0;c<sz_img_sub2tar_xy[3];c++)
			{
				VISIOCYTELONG index_MIP=pgsz_xz*c+pgsz_x*z+x;
				p_img_mip_xz[index_MIP]=u_MIP_rgb[c];
			}
		}
	if(p_img_sub2tar_xy) 			{delete []p_img_sub2tar_xy;		p_img_sub2tar_xy=0;}
	}
//	saveImage("/Users/qul/work/visiocyte_2.0/sub_projects/celegant_straighten/mip_xz.raw",(unsigned char *)p_img_mip_xz,sz_img_mip_xz,1);
	...
	marker_head.x=0;					marker_head.y=0;	marker_head.z=vec2d_markers[0][2]/paras.d_downsampleratio;
	marker_tail.x=sz_img_mip_xz[0]-1;	marker_tail.y=0;	marker_tail.z=vec2d_markers[vec2d_markers.size()-1][2]/paras.d_downsampleratio;
	...
	for(VISIOCYTELONG i=0;i<paras.l_ctlpts_num;i++)
	{
		point3D64F tmp;
		tmp.x=i*d_step;
		tmp.y=a*tmp.x+b;;
	...
	for(VISIOCYTELONG i=0;i<ql_cptpos_output_xy.size();i++)
	{
		ql_cptpos_output_xy[i].x*=paras.d_downsampleratio;
		ql_cptpos_output_xy[i].y*=paras.d_downsampleratio;
	}
	for(VISIOCYTELONG i=0;i<ql_cptpos_output_xz.size();i++)
	{
...
//	saveImage("/Users/qul/work/visiocyte_2.0/sub_projects/celegant_straighten/sub2tar_xz.raw",(unsigned char *)p_img_sub2tar_xz,sz_img_sub2tar_xz,1);
		...
		unsigned char *&p_strimg,VISIOCYTELONG *&sz_strimg)
{
	...
	vec4d_mappingfield_str2ori.clear();
		...
		vector< vector< vector< vector<VISIOCYTELONG> > > > &vec4d_mappingfield_str2ori)
{
	...
	if(vec4d_mappingfield_str2ori.size())
	{
		printf("WARNNING: The vec4d_mappingfield_str2ori is not empty, original data will be deleted. \n");
		vec4d_mappingfield_str2ori.clear();
	}
	...
	if(!q_restacking_alongbcurve_xy(p_inputimg,sz_inputimg,cp_x,cp_y,cp_alpha,cutPlaneNum,l_width,p_strimg,sz_strimg,vec4d_mappingfield_str2ori))
	{
		...
		unsigned char *&p_strimg,VISIOCYTELONG *&sz_strimg)
{
	...
	if(!q_celegans_restacking_xz(
			...
			vec4d_mappingfield_str2ori))
	{
		printf("ERROR: q_celegans_restacking_xz() return false! \n");
		return false;
	}
	vec4d_mappingfield_str2ori.clear();
		...
		vector< vector< vector< vector<VISIOCYTELONG> > > > &vec4d_mappingfield_str2ori)
{
	...
	//------------------------------------------------------------------------------------------------------------------------------------
	//restacking along the smoothed curve
	if(!q_restacking_alongbcurve_xz(p_inputimg,sz_inputimg,cp_x,cp_y,cp_alpha,cutPlaneNum,l_width,p_strimg,sz_strimg,vec4d_mappingfield_str2ori))
	{
		printf("ERROR: q_restacking_alongbcurve_xy() return false! \n");
		if(xpos) 		{delete []xpos; xpos=0;}
		if(ypos) 		{delete []ypos; ypos=0;}
		...
		vector< vector< vector< vector<VISIOCYTELONG> > > > &vec4d_mappingfield_str2ori)
{
	...
	VISIOCYTELONG nx=sz_inputimg[0];
	VISIOCYTELONG ny=sz_inputimg[1];
	VISIOCYTELONG nz=sz_inputimg[2];
	VISIOCYTELONG nc=sz_inputimg[3];
	...
	vec4d_mappingfield_str2ori.assign(sz_strimg[1],vector< vector< vector<VISIOCYTELONG> > >(sz_strimg[0],vector< vector<VISIOCYTELONG> >(sz_strimg[2],vector<VISIOCYTELONG>(3,0))));

	//------------------------------------------------------------------------------------------------------------------------------------
	//straighten image along curve
	int ptspace=1; // default value
	double base0=0;
	VISIOCYTELONG Krad=0;
	...
	for(VISIOCYTELONG col=1;col<length_curve;col++)
	{
		...
		for(VISIOCYTELONG row=0;row<l_width;row++)
		{
			...
			VISIOCYTELONG cpx0=VISIOCYTELONG(floor(curpx));
			VISIOCYTELONG cpx1=VISIOCYTELONG(ceil(curpx));
			VISIOCYTELONG cpy0=VISIOCYTELONG(floor(curpy));
			VISIOCYTELONG cpy1=VISIOCYTELONG(ceil(curpy));
...
bool q_restacking_alongbcurve_xz(
		const unsigned char *p_inputimg,const VISIOCYTELONG *sz_inputimg,
        const double *pos_curve_x,const double *pos_curve_z,const double *alpha_curve,const VISIOCYTELONG length_curve,
		const VISIOCYTELONG l_width,
		unsigned char *&p_strimg, VISIOCYTELONG *&sz_strimg,
		vector< vector< vector< vector<VISIOCYTELONG> > > > &vec4d_mappingfield_str2ori)
{
	...
	for(VISIOCYTELONG col=1;col<length_curve;col++)
	{
		...
		for(VISIOCYTELONG row=0;row<l_width;row++)
		{
			...
			VISIOCYTELONG cpx0=VISIOCYTELONG(floor(curpx));
			VISIOCYTELONG cpx1=VISIOCYTELONG(ceil(curpx));
			VISIOCYTELONG cpz0=VISIOCYTELONG(floor(curpz));
			VISIOCYTELONG cpz1=VISIOCYTELONG(ceil(curpz));
