...
{
	//input
	VISIOCYTELONG		l_maxitertimes;
	double 		d_stopiter_threshold;
	double 		d_foreground_treshold;//times of image mean value
	VISIOCYTELONG 		l_diskradius_openning;
	VISIOCYTELONG 		l_diskradius_closing;
...
bool q_principalskeleton_detection(VISIOCYTEPluginCallback &callback,
		...
		const vector< vector<VISIOCYTELONG> > &vecvec_domain_length_ind,const vector<double> &vec_domain_length_weight,
		const vector< vector<VISIOCYTELONG> > &vecvec_domain_smooth_ind,const vector<double> &vec_domain_smooth_weight,
