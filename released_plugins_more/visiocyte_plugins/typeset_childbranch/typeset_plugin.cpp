...
void TypesetWindow(VISIOCYTEPluginCallback2 &visiocyte, QWidget *parent);
...
void TypesetPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    {
        visiocyte_msg(tr("This plugin sets branch type of traced neuron \n"
            "Developed by Surobhi Ganguly, 8/10/2014"));
    }
}

bool TypesetPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
void controlPanel::_slot_sort()//VISIOCYTEPluginCallback2 &_visiocyte, QWidget *parent)
{
    sort_menu(m_visiocyte); //sort first, then typeset
    //visiocyte_msg("sort function run");
}
...
void controlPanel::_slot_typeset_all()
{
    ...
    typeset_swc_func(m_visiocyte, settype, markers);
    //visiocyte_msg("typeset function was run");
}
...
void controlPanel::_slot_typeset_marker()
{
    ...
    if (list_markers->isItemSelected(list_markers->currentItem()))
    {
        ...
        {
            visiocyte_msg(current_marker); //need to convert this to location simple. somehow.
        }
        else
        {
            QRegExp rx("(\\ |\\,)");
            ...
            //visiocyte_msg(QString("%1, %2, %3").arg(markersplit.at(3).toFloat()).arg(markersplit.at(6).toFloat()).arg(markersplit.at(9).toFloat()));
            ...
            //visiocyte_msg("typeset_marker function run");
        }
    }
    else
    {
        visiocyte_msg("select a marker first");
        return;
    }
}
void controlPanel::_slot_refresh()
{
    QList<ImageMarker> tmp_list = get_markers(m_visiocyte);
    list_markers->clear();
    ...
    {
        for (VISIOCYTELONG i=0; i<tmp_list.size();i++)
        {
            ...
            //visiocyte_msg(QString("x = %13").arg(tmp_list.at(i).x));
...
//    {
//        visiocyte_msg("Please select a valid marker point.");
//        return;
//    }
//    else
//    {
...
//    visiocyte_msg("delete button hopefully works");
...
//{
//    visiocyte_msg("highlight button function to be implimented");
//    CHECK_WINDOWS;
...
{
    final_typeset_save();
    //visiocyte_msg("save button function implemented yaaaayyyy");
}
...
//    {
//        visiocyte_msg("Fail to invoke saveAnchorFile(), maybe the file name is invalid or invalid list_markers pointer or empty anchor point list.",0);
//        return false;
//    }
