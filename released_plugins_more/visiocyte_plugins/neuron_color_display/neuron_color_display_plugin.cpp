...
void SetNeuronDisplayColor::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
		visiocyte_msg(tr("This plugin will set the color of the neuron displayed. "
			"Developed by Hanbo Chen, 2015-6-12"));
	}
}

bool SetNeuronDisplayColor::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
void SetNeuronDisplayColor::do1clickcolor(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    if(selectWindowList.size()<1){
        visiocyte_msg("Cannot find 3D view with SWC file. Please load the SWC files you want to color in the 3D view");
        return;
