...
template <class T>
bool affineTransformImage(T * p_img_in,const VISIOCYTELONG sz_in[4], T * p_img_out,const VISIOCYTELONG sz_out[4], const double amat[16], const VISIOCYTELONG globalShift[3], const int i_interpmethod_img)
{
    ...
    //calculate affine
    for(VISIOCYTELONG z=0;z<sz_out[2];z++){
        for(VISIOCYTELONG y=0;y<sz_out[1];y++){
            printf("\r%2.3f%%",(float)z/(float)sz_out[2]*100);
            for(VISIOCYTELONG x=0;x<sz_out[0];x++)
            {
                ...
                if(i_interpmethod_img==1)
                {
                    ...
                    for(VISIOCYTELONG c=0;c<sz_in[3];c++){
                        VISIOCYTELONG ind=x+y*sz_out[0]+z*sz_out[1]*sz_out[0]+c*sz_out[0]*sz_out[1]*sz_out[2];
                        VISIOCYTELONG ind_sub=pos_sub_nn[0]+pos_sub_nn[1]*sz_in[0]+
                ...
                else if(i_interpmethod_img==0)
                {
                    ...
                    for(VISIOCYTELONG c=0;c<sz_in[3];c++)
                    {
                        ...
                        double higher_slice;
                        VISIOCYTELONG ind_1=x_s+y_s*sz_in[0]+z_s*sz_in[1]*sz_in[0]+c*sz_in[0]*sz_in[1]*sz_in[2];
                        VISIOCYTELONG ind_2=x_b+y_s*sz_in[0]+z_s*sz_in[1]*sz_in[0]+c*sz_in[0]*sz_in[1]*sz_in[2];
                        VISIOCYTELONG ind_3=x_s+y_b*sz_in[0]+z_s*sz_in[1]*sz_in[0]+c*sz_in[0]*sz_in[1]*sz_in[2];
                        VISIOCYTELONG ind_4=x_b+y_b*sz_in[0]+z_s*sz_in[1]*sz_in[0]+c*sz_in[0]*sz_in[1]*sz_in[2];
...
template <class T>
bool affineCombineImage(T * p_img_tar,const VISIOCYTELONG sz_tar[4],T * p_img_sub,const VISIOCYTELONG sz_sub[4], T * &p_img_out, VISIOCYTELONG sz_out[4], VISIOCYTELONG globalShift[3], const double amat[16], const int i_interpmethod_img, const bool b_channelSeperate, const bool b_negativeShift)
{
    ...
    QList<VISIOCYTELONG> box_x, box_y, box_z;
    box_x.append(0); box_x.append(sz_sub[0]);
    box_y.append(1); box_y.append(sz_sub[1]);
    box_z.append(2); box_z.append(sz_sub[2]);
    VISIOCYTELONG negativeShift[3]={0,0,0};
    for(VISIOCYTELONG ix=0; ix<2; ix++)
        for(VISIOCYTELONG iy=0; iy<2; iy++)
            for(VISIOCYTELONG iz=0; iz<2; iz++){
                VISIOCYTELONG newx=amat[0]*box_x[ix]+amat[1]*box_y[iy]+amat[2]*box_z[iz]+amat[3];
                VISIOCYTELONG newy=amat[4]*box_x[ix]+amat[5]*box_y[iy]+amat[6]*box_z[iz]+amat[7];
                VISIOCYTELONG newz=amat[8]*box_x[ix]+amat[9]*box_y[iy]+amat[10]*box_z[iz]+amat[11];
    ...
    for(VISIOCYTELONG x=0; x<sz_tar[0]; x++){
        printf("\r%2.3f%%",(float)x/(float)sz_tar[0]*100);
        for(VISIOCYTELONG y=0; y<sz_tar[1]; y++){
            for(VISIOCYTELONG z=0; z<sz_tar[2]; z++){
                VISIOCYTELONG xwarp=x+globalShift[0];
                VISIOCYTELONG ywarp=y+globalShift[1];
                VISIOCYTELONG zwarp=z+globalShift[2];
                ...
                for(VISIOCYTELONG c=0; c<sz_tar[3]; c++){
                    VISIOCYTELONG ind_tar=x+y*sz_tar[0]+z*sz_tar[1]*sz_tar[0]+c*sz_tar[0]*sz_tar[1]*sz_tar[2];
                    VISIOCYTELONG ind_warp=xwarp+ywarp*sz_out[0]+zwarp*sz_out[1]*sz_out[0]+c*sz_out[0]*sz_out[1]*sz_out[2];
                    if(b_channelSeperate)
