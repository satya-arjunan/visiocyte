...
marker_match_dialog::marker_match_dialog(VISIOCYTEPluginCallback2 * cb, LandmarkList * mList_in)
{
    ...
    if( (m0.size()==0 || m1.size()==0) )
        if( mm0.size() == 0){
            visiocyte_msg("There is no marker to match. Please define marker and link them to neuron first.");
            this->reject();
        }
}
...
void marker_match_dialog::free()
{
    ...
    if(id==-1)
    {
        visiocyte_msg("Please select a matched pair on left.");
        return;
    }
