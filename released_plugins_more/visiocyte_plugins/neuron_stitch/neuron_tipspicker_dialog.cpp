...
void neuron_tipspicker_dialog::checkStatus()
{
    if(cb_tips->count()>mList->size()){
        visiocyte_msg("The number of markers does not match, please click Update or Reload button.");
...
neuron_tipspicker_dialog::neuron_tipspicker_dialog(VISIOCYTEPluginCallback2 * cb, VisiocyteR_MainWindow* inwin)
{
    if(!inwin)
        return;
    visiocytewin=inwin;
    ...
    if(ntList->size()<=0){ //this should not happen
        visiocyte_msg("No neuron in the window!");
        return;
    }
...
void neuron_tipspicker_dialog::search()
{
    ...
    for(int i=0; i<cands.size(); i++){
        VISIOCYTELONG nid=cands.at(i).nid;
        VISIOCYTELONG pid=cands.at(i).pid;
...
//        qDebug()<<"cojoc2:"<<callback->getImageName(visiocytewin);
        if(callback->getImageName(visiocytewin).contains(callback->getImageName(list_triwin.at(i)))){
...
void neuron_tipspicker_dialog::roi_tip()
{
    ...
    for(VISIOCYTELONG i=0; i<list_triwin.size(); i++){
        if(callback->getImageName(visiocytewin).contains(callback->getImageName(list_triwin.at(i)))){
            ...
            View3DControl * visiocytelocalcontrol = callback->getLocalView3DControl(list_triwin.at(i));
