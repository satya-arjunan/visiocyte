...
void file_transform::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	if (menu_name == tr("transform_and_combine_images_by_affine_mat"))
	{
        visiocyte_msg("Please check do_func, GUI to be implemented.");
	}
	...
	{
		visiocyte_msg(tr("This plugin will transform and combine image by given affine matrix. Some functions are based on littleQuickWarper.. "
			"Developed by Hanbo Chen, 2015-3-18"));
	}
}

bool file_transform::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
void file_transform::doAffineImage(VISIOCYTEPluginCallback2 & callback, QString fname_tar,
                                             ...
                                             int interpMethod, bool b_negativeShift, bool b_channelSeperate)
{
    ...
    VISIOCYTELONG sz_sub[4], sz_tar[4], sz_out[4]={0}, globalShift[3]={0};
...
int file_transform::dotransform_swc(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    //input file name
    ...
    {
            visiocyte_msg("error read affine transform matrix.");
            return 0;
    }
...
int file_transform::dotransform_marker(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    //input file name
    QString fileOpenName;
    ...
    //marker affine
    double x,y,z;
    for(VISIOCYTELONG i=0; i<inmarker.size() ; i++)
    {
    ...
    {
        visiocyte_msg("fail to write the output marker file.");
        return 0;
    }
...
{
    qDebug()<<"Usage : visiocyte -x dllname -f affineImage -i <target_img_file> <subject_img_file> <affine_matrix.txt> -p <(0)interpmethod> <(1)negative shifting> <(0)seperate channel> -o <out_img_file>";
