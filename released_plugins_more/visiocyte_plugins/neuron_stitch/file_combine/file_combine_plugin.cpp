...
void file_combine::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
        visiocyte_msg(tr("This plugin is for combine files from different sections.. "
			"Developed by Hanbo Chen, 2014-10-07"));
	}
}

bool file_combine::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
void file_combine::docombine_marker(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    //input file 1
    ...
    //output file
    for(VISIOCYTELONG i=0; i<inmarker_2.size() ; i++){
