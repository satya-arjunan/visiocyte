		...
		vector<point3D64F> &vec_cptpos_output)
{
	...
	for(unsigned VISIOCYTELONG k=0;k<vec_cptpos_input.size();k++)
	{
		//lenght constraint domain
		for(unsigned VISIOCYTELONG i=0;i<vecvec_domaincptind_length.size();i++)
			for(unsigned VISIOCYTELONG j=0;j<vecvec_domaincptind_length[i].size();j++)
			{
				VISIOCYTELONG index=vecvec_domaincptind_length[i][j];
				if(index==(VISIOCYTELONG)k)
				{
		...
		for(unsigned VISIOCYTELONG i=0;i<vecvec_domaincptind_smooth.size();i++)
			for(unsigned VISIOCYTELONG j=0;j<vecvec_domaincptind_smooth[i].size();j++)
			{
	...
	for(unsigned VISIOCYTELONG i=0;i<vecvec_neighborcptind_length.size();i++)
	{
		if(vecvec_neighborcptind_length[i].size()==1)
		{
			...
			{
				if(vecvec_neighborcptind_length[neighbor1_ind][0]!=(VISIOCYTELONG)i)
					vec_skeletonend_2neighbor_ind.push_back(vecvec_neighborcptind_length[neighbor1_ind][0]);
				else if(vecvec_neighborcptind_length[neighbor1_ind][1]!=(VISIOCYTELONG)i)
	...
	for(unsigned VISIOCYTELONG i=0;i<vecvec_neighborcptind_length.size();i++)
	{
		...
		for(unsigned VISIOCYTELONG j=0;j<vecvec_neighborcptind_length[i].size();j++)
			printf("[%ld,%.2f,L];",vecvec_neighborcptind_length[i][j],vecvec_neighborweight_length[i][j]);
		for(unsigned VISIOCYTELONG j=0;j<vecvec_neighborcptind_smooth[i].size();j++)
	...
	for(unsigned VISIOCYTELONG i=0;i<vec_skeletonend_ind.size();i++)
	{
		printf("\t\tend pt index: [%ld], neighbor index: ",vec_skeletonend_ind[i]);
		for(unsigned VISIOCYTELONG j=0;j<vecvec_skeletonend_2neighbor_ind[i].size();j++)
	...
	if(paras_input.b_removeboundaryartifact)
	{
		...
		//find seed
		VISIOCYTELONG pos_seed[2];
		{
		pos_seed[0]=sz_img_input[0]/2.0+0.5;//x
		pos_seed[1]=sz_img_input[1]/2.0+0.5;//y
		for(VISIOCYTELONG i=0;i<sz_img_input[0];i++)
		{
			VISIOCYTELONG index=sz_img_input[0]*pos_seed[1]+pos_seed[0];
...
//	saveImage("baseimg.tif",(unsigned char *)p_img_foreground_co,sz_img_test,1);
...

	{
		...
		for(unsigned VISIOCYTELONG i=0;i<vec_cptpos_input.size();i++)
		{
		...
		for(unsigned VISIOCYTELONG i=0;i<vec_foregroundpixel_x.size();i++)
		{
		...
		for(unsigned VISIOCYTELONG i=0;i<vec_cptpos_ini.size();i++)
		{
...
//		f_zoomfactor_skeleton=paras_input.d_inizoomfactor_skeleton;
	...
	for(VISIOCYTELONG iter=0;iter<n_maxiter;iter++)
	{
		...
		for(unsigned VISIOCYTELONG p=0;p<vec_foregroundpixel_x.size();p++)
			for(unsigned VISIOCYTELONG m=0;m<vec_cptpos_ini.size();m++)
			{
		...
		for(unsigned VISIOCYTELONG ind_cpt=0;ind_cpt<vec_cptpos_ini.size();ind_cpt++)
		{
			...
			for(unsigned VISIOCYTELONG i=0;i<vecvec_neighborcptind_length[ind_cpt].size();i++)
			{
			...
			for(unsigned VISIOCYTELONG i=0;i<vecvec_neighborcptind_smooth[ind_cpt].size();i++)
			{
			...
			for(unsigned VISIOCYTELONG i=0;i<vec_skeletonend_ind.size();i++)
			{
				if((VISIOCYTELONG)ind_cpt==vec_skeletonend_ind[i] && vecvec_skeletonend_2neighbor_ind[i].size()==2)
				{
				...
				if((VISIOCYTELONG)ind_cpt==vec_skeletonend_ind[i] && vecvec_skeletonend_2neighbor_ind[i].size()==1)
				{
