...
void img_cutting(Tdata *pInput, VISIOCYTELONG *szInput, Tdata* &pOutput, VISIOCYTELONG* szOutput)
{
    // cutting blank plane occuring in image boundaries
    VISIOCYTELONG pz=0, nz=0, py=0, ny=0, px=0, nx=0;
    VISIOCYTELONG start_z=0;
    VISIOCYTELONG end_z=szInput[2]-1;
    VISIOCYTELONG start_y=0;
    VISIOCYTELONG end_y=szInput[1]-1;
    VISIOCYTELONG start_x=0;
    VISIOCYTELONG end_x=szInput[0]-1;

    VISIOCYTELONG colordim = szInput[3];

    VISIOCYTELONG pagesz = szInput[0]*szInput[1]*szInput[2];

    bool b_blankplanefound;

    // find NULL channel first
    VISIOCYTELONG nullc = -1;
    for(VISIOCYTELONG c=0; c<szInput[3]; c++) // image 1
    {
        VISIOCYTELONG offset_c = c*pagesz;
        VISIOCYTELONG sumint = 0;
        for (VISIOCYTELONG k=0; k<szInput[2]; k++)
        {
            VISIOCYTELONG offset_k = offset_c + k*szInput[0]*szInput[1];
            for(VISIOCYTELONG j=0; j<szInput[1]; j++)
            {
                VISIOCYTELONG offset_j = offset_k + j*szInput[0];
                for(VISIOCYTELONG i=0; i<szInput[0]; i++)
                {
    ...
    while (b_blankplanefound)
    {
        ...
        for(VISIOCYTELONG k=0; k<szInput[2]; k++)
        {
            VISIOCYTELONG offset_k = k*szInput[1]*szInput[0];

            b_blankplanefound=false;
            for(VISIOCYTELONG c=0; c<colordim; c++)
            {
                sum = 0;

                if(c==nullc) continue;

                VISIOCYTELONG offset_c = c*pagesz + offset_k;
                for(VISIOCYTELONG j=0; j<szInput[1]; j++)
                {
                    VISIOCYTELONG offset_j = offset_c + j*szInput[0];
                    for(VISIOCYTELONG i=0; i<szInput[0]; i++)
                    {
    ...
    while (b_blankplanefound)
    {
        ...
        for(VISIOCYTELONG k=szInput[2]-1; k>0; k--)
        {
            VISIOCYTELONG offset_k = k*szInput[1]*szInput[0];
    ...
    while (b_blankplanefound)
    {
        ...
        for(VISIOCYTELONG j=0; j<szInput[1]; j++)
        {
            VISIOCYTELONG offset_j = j*szInput[0];
            ...
            for(VISIOCYTELONG c=0; c<colordim; c++)
            {
                ...
                VISIOCYTELONG offset_c = c*pagesz + offset_j;
                for(VISIOCYTELONG k=0; k<szInput[2]; k++)
                {
                    VISIOCYTELONG offset_k = offset_c + k*szInput[1]*szInput[0];
    ...
    while (b_blankplanefound)
    {
        ...
        for(VISIOCYTELONG j=szInput[1]-1; j>0; j--)
        {
            VISIOCYTELONG offset_j = j*szInput[0];
    ...
    while (b_blankplanefound)
    {
        ...
        for(VISIOCYTELONG i=szInput[0]-1; i>0; i--)
        {
            b_blankplanefound=false;
            for(VISIOCYTELONG c=0; c<colordim; c++)
            {
    ...
    VISIOCYTELONG sx = end_x - start_x + 1;
    VISIOCYTELONG sy = end_y - start_y + 1;
    VISIOCYTELONG sz = end_z - start_z + 1;
    VISIOCYTELONG pagesz_new = sx*sy*sz;
    VISIOCYTELONG totalplxs = pagesz_new*colordim;
    ...
    try {
        ...
        for(VISIOCYTELONG c=0; c<colordim; c++)
        {
            VISIOCYTELONG offset_c_old = c*pagesz;
            VISIOCYTELONG offset_c_new = c*pagesz_new;
            for(VISIOCYTELONG k=start_z; k<=end_z; k++)
            {
                VISIOCYTELONG offset_k_old = offset_c_old + k*szInput[0]*szInput[1];
                VISIOCYTELONG offset_k_new = offset_c_new + (k-start_z)*sx*sy;
                for(VISIOCYTELONG j=start_y; j<=end_y; j++)
                {
                    VISIOCYTELONG offset_j_old = offset_k_old + j*szInput[0];
                    VISIOCYTELONG offset_j_new = offset_k_new + (j-start_y)*sx;
...
bool ref_aligning(Tdata *p1dImg1, Tdata *p1dImg2, VISIOCYTELONG *szImg, VISIOCYTELONG *&outputoffsets, REAL &coef)
{
    // two images with the same size
    VISIOCYTELONG sx = szImg[0];
    VISIOCYTELONG sy = szImg[1];
    VISIOCYTELONG sz = szImg[2];
    VISIOCYTELONG sc = szImg[3];

    VISIOCYTELONG pagesz_sub = sx*sy*sz;

    VISIOCYTELONG tx = szImg[0];
    VISIOCYTELONG ty = szImg[1];
    VISIOCYTELONG tz = szImg[2];
    VISIOCYTELONG tc = szImg[3];

    VISIOCYTELONG pagesz_tar = tx*ty*tz;

    // anchor position
    REAL pos_score = 0;
    VISIOCYTELONG pos_x = sx-1, pos_y = sy-1, pos_z = sz-1;
    VISIOCYTELONG sx_ori = sx, sy_ori = sy, sz_ori = sz;
    ...
    try
    {
        szPad = new VISIOCYTELONG [3];
        szTar = new VISIOCYTELONG [3];
        szSub = new VISIOCYTELONG [3];
        scale = new REAL [3];
    }
    catch (...)
    {
...
//
bool stitch_paired_images_with_refchan(Image4DSimple &p4DImage1, VISIOCYTELONG ref1, Image4DSimple &p4DImage2, VISIOCYTELONG ref2, bool b_transdet, int &transtype)
{
    ...
    VISIOCYTELONG *szImg = new VISIOCYTELONG [4];
    ...
    VISIOCYTELONG datatype_img = p4DImage1.getUnitBytes();

    VISIOCYTELONG pagesz = szImg[0]*szImg[1]*szImg[2];
    VISIOCYTELONG totalplxs = datatype_img*pagesz*szImg[3];

    VISIOCYTELONG offset1 = ref1*pagesz;
    VISIOCYTELONG offset2 = ref2*pagesz;

    VISIOCYTELONG *offset=NULL;
    unsigned char* pTemImg = NULL;

    try {
        offset = new VISIOCYTELONG [3];
    ...
    // align
    REAL coef;
    if(datatype_img == VISIOCYTE_UINT8)
    {
        if(b_transdet)
        {
            if(!transformations_detection<unsigned char, VISIOCYTELONG, Y_IMG_UINT8>((unsigned char*)(p1dImg1)+offset1, (unsigned char*)(p1dImg2)+offset2, szImg, transtype))
            {
    ...
    else if(datatype_img == VISIOCYTE_UINT16)
    {
        if(b_transdet)
        {
            if(!transformations_detection<unsigned short, VISIOCYTELONG, Y_IMG_UINT16>((unsigned short*)(p1dImg1)+offset1, (unsigned short*)(p1dImg2)+offset2, szImg, transtype))
            {
                cout<<"Fail to detect transformations."<<endl;
                return false;
            }
    ...
    else if(datatype_img == VISIOCYTE_FLOAT32)
    {
        if(b_transdet)
        {
            if(!transformations_detection<float, VISIOCYTELONG, Y_IMG_REAL>((float*)(p1dImg1)+offset1, (float*)(p1dImg2)+offset2, szImg, transtype))
            {
                cout<<"Fail to detect transformations."<<endl;
                return false;
            }
    ...
    VISIOCYTELONG sz_start = offset_sz, sz_end = sz_start + szImg[2]; if(sz_start<0) k_start=0; else k_start=sz_start; if(sz_end>szImg[2]) sz_end=szImg[2];
    VISIOCYTELONG sy_start = offset_sy, sy_end = sy_start + szImg[1]; if(sy_start<0) j_start=0; else j_start=sy_start; if(sy_end>szImg[1]) sy_end=szImg[1];
    VISIOCYTELONG sx_start = offset_sx, sx_end = sx_start + szImg[0]; if(sx_start<0) i_start=0; else i_start=sx_start; if(sx_end>szImg[0]) sx_end=szImg[0];

    for(VISIOCYTELONG c=0; c<szImg[3]; c++)
    {
        VISIOCYTELONG offset_c = c*pagesz;
        for(VISIOCYTELONG k=k_start; k<sz_end; k++)
        {
            VISIOCYTELONG offset_k = offset_c + k*szImg[1]*szImg[0];

            VISIOCYTELONG kk = k - k_start + offset_tz;
            if(kk>=szImg[2]) break;

            VISIOCYTELONG offset_k_t = offset_c + kk*szImg[1]*szImg[0];

            for(VISIOCYTELONG j=j_start; j<sy_end; j++)
            {
                VISIOCYTELONG offset_j = offset_k + j*szImg[0];

                VISIOCYTELONG jj = j-j_start + offset_ty;
                if(jj>=szImg[1]) break;

                VISIOCYTELONG offset_j_t = offset_k_t + jj*szImg[0];

                for(VISIOCYTELONG i=i_start; i<sx_end; i++)
                {
                    VISIOCYTELONG ii = i - i_start + offset_tx;
                    if(ii>=szImg[0]) break;

                    VISIOCYTELONG idx = offset_j + i;
                    VISIOCYTELONG idx_t = offset_j_t + ii;
...
template <class Tdata>
double mi_computing(Tdata *pImg1, Tdata *pImg2, VISIOCYTELONG szImg, int datatype)
{
    ...
    VISIOCYTELONG denomHist = szHist*szHist;
    ...
    {
        jointHistogram[ (VISIOCYTELONG)pImg1[i] ][ (VISIOCYTELONG)pImg2[i] ] ++;
    }

    double jointEntropy=0, img1Entropy=0, img2Entropy=0;

    // normalized joint histogram
    for(VISIOCYTELONG i=0; i<szHist; i++)
    {
        for(VISIOCYTELONG j=0; j<szHist; j++)
        {
...
bool ImageBlendPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & visiocyte, QWidget * parent)
{
    //
    int ref_chans[2] = {-1, -1};
    ...
    if (func_name == tr("multiscanblend"))
    {
        ...
        {
            //print Help info
            printf("\nUsage: visiocyte -x blend_multiscanstacks.dylib -f multiscanblend -i <input_images> -o <output_image> -p \"#s <save_blending_result zero(false)/nonzero(true)> #k <b_morecolorstack_first nonzero(true)/zero(false)> #d <deformed nonzero(true)/zero(false) >\" \n");
        ...
        // color mapping
        VISIOCYTELONG c10=-1, c11=-1, c12=-1;
        VISIOCYTELONG c20=-1, c21=-1;
        ...
        if(!outfile)
            blendImageName = m_InputFileName + "_blended.visiocyteraw";
        else
        ...
        if(b_morecolorstack_first && sz_img1[3]<sz_img2[3])
        {
            ...
            try
            {
                VISIOCYTELONG totalplxs1 = p4DImage1.getTotalBytes();
                p1 = new unsigned char [totalplxs1];
                memcpy(p1, p4DImage1.getRawData(), totalplxs1);

                VISIOCYTELONG totalplxs2 = p4DImage2.getTotalBytes();
        ...
        if( (QFileInfo(m_InputFileName1).suffix().toUpper().compare("LSM") == 0) && (QFileInfo(m_InputFileName2).suffix().toUpper().compare("LSM") == 0) )
        {
            ...
            Y_LSMINFO<VISIOCYTELONG> lsminfo1(m_InputFileName1.toStdString());
            lsminfo1.loadHeader();
            ...
            Y_LSMINFO<VISIOCYTELONG> lsminfo2(m_InputFileName2.toStdString());
            lsminfo2.loadHeader();
        ...
        VISIOCYTELONG ref1=0, ref2=0, nullcolor1 = -1, nullcolor2 = -1;
        ...
        else
        {
            ...
            for(VISIOCYTELONG c=0; c<sz_img1[3]; c++) // image 1
            {
                VISIOCYTELONG offset_c = c*pagesz;
                VISIOCYTELONG sumint1 = 0;
                for (VISIOCYTELONG k=0; k<sz_img1[2]; k++)
                {
                    VISIOCYTELONG offset_k = offset_c + k*sz_img1[0]*sz_img1[1];
                    for(VISIOCYTELONG j=0; j<sz_img1[1]; j++)
                    {
                        VISIOCYTELONG offset_j = offset_k + j*sz_img1[0];
                        for(VISIOCYTELONG i=0; i<sz_img1[0]; i++)
                        {
                            VISIOCYTELONG idx = offset_j + i;
            ...
            for(VISIOCYTELONG c=0; c<sz_img2[3]; c++) // image 2
            {
                VISIOCYTELONG offset_c = c*pagesz;
                VISIOCYTELONG sumint2 = 0;
                for (VISIOCYTELONG k=0; k<sz_img1[2]; k++)
                {
                    VISIOCYTELONG offset_k = offset_c + k*sz_img1[0]*sz_img1[1];
                    for(VISIOCYTELONG j=0; j<sz_img1[1]; j++)
            ...
            for(VISIOCYTELONG c1=0; c1<sz_img1[3]; c1++)
            {
                if(b_img1existNULL)
                {
                     if ( c1 == nullcolor1 )
                ...
                for(VISIOCYTELONG c2=0; c2<sz_img2[3]; c2++)
                {
                    if(b_img2existNULL)
                    {
                        if ( c2 == nullcolor2 )
        ...
        if(b_deformed)
        {
            ...
            sz_deformed = new VISIOCYTELONG [4];
            ...
            if(datatype_img1 == VISIOCYTE_UINT8)
            {
                ...
                if(b_infoloaded)
                {
                    for(VISIOCYTELONG c=0; c<colordim-1; c++)
                    {
                        VISIOCYTELONG offset_c = c*pagesz;

                        VISIOCYTELONG offset_c1=0, offset_c2=0;
                        bool b_img1 = true;

                        for(VISIOCYTELONG ci=0; ci<sz_img1[3]; ci++)
                        {
                            if(c1[ci]==c) offset_c1 = ci*pagesz;
                        }

                        for(VISIOCYTELONG ci=0; ci<sz_img2[3]; ci++)
                        {
                        ...
                        for (VISIOCYTELONG k=0; k<sz_img1[2]; k++)
                        {
                            ...
                            VISIOCYTELONG offset_k1 = offset_k - offset_c + offset_c1;
                            VISIOCYTELONG offset_k2 = offset_k - offset_c + offset_c2;

                            for(VISIOCYTELONG j=0; j<sz_img1[1]; j++)
                            {
                                VISIOCYTELONG offset_j = offset_k + j*sz_img1[0];

                                VISIOCYTELONG offset_j1 = offset_k1 + j*sz_img1[0];
                                VISIOCYTELONG offset_j2 = offset_k2 + j*sz_img1[0];
                ...
                VISIOCYTELONG offset = (colordim-1)*pagesz;
                VISIOCYTELONG offset1 = ref1*pagesz;
            ...
            y_del<VISIOCYTELONG>(sz_deformed);
            y_del<void>(p);
        ...
        if(datatype_img1 == VISIOCYTE_UINT8)
        {
            ...
            if(b_infoloaded)
            {
                for(VISIOCYTELONG c=0; c<colordim-1; c++)
                {
                    ...
                    VISIOCYTELONG offset_c1, offset_c2;
                    bool b_img1 = true;
        ...
        else if(datatype_img1 == VISIOCYTE_FLOAT32) //this section need further improvement, maybe discretization. by PHC, 110810
        {
            //
            float* data1d = NULL;
            try
    ...
    else if (func_name == tr("multiscanblend2"))
    {
        ...
        {
            //print Help info
            printf("\nUsage: visiocyte -x blend_multiscanstacks -f multiscanblend2 -i <input_images> -o <output_image> -p \"#s <save_blending_result zero(false)/nonzero(true)> #d <deformed nonzero(true)/zero(false) >\" \n");
    ...
    else if (func_name == tr("transformdet"))
    {
        ...
        {
            //print Help info
            printf("\nUsage: visiocyte -x blend_multiscanstacks.dylib -f transformdet -i <input_images> -o <output_transform_type> \
...
//        }
    ...
    {
        // extract the reference from 3-color .lsm and save the reference as 'target_ref.visiocyteraw'
        // extract the reference and the signal from 2-color .lsm and save them as 'subject_ref.visiocyteraw' and 'subject_signal.visiocyteraw'
        ...
        {
            //print Help info
            printf("\nUsage: visiocyte -x blend_multiscanstacks.dylib -f extractchannels -i <input_images> \
                   -p \"#k <b_morecolorstack_first nonzero(true)/zero(false)>\" \n");
        ...
        if(datatype_img1 == VISIOCYTE_UINT8)
        {
            ...
            try
            {
                y_new<unsigned char, VISIOCYTELONG>(data1d, pagesz);
            ...
            if(outfile)
            {
                savingName=QFileInfo(qs_filename_output).path().append(SEPCHAR).append("target_ref.visiocyteraw");
            }
            else
            {
                savingName=QFileInfo(m_InputFileName1).path().append(SEPCHAR).append("target_ref.visiocyteraw");;
            }
            ...
            if(outfile)
            {
                savingName=QFileInfo(qs_filename_output).path().append(SEPCHAR).append("subject_ref.visiocyteraw");
            }
            else
            {
                savingName=QFileInfo(m_InputFileName1).path().append(SEPCHAR).append("subject_ref.visiocyteraw");;
            }
            ...
            if(outfile)
            {
                savingName=QFileInfo(qs_filename_output).path().append(SEPCHAR).append("subject_signal.visiocyteraw");
            }
            else
            {
                savingName=QFileInfo(m_InputFileName1).path().append(SEPCHAR).append("subject_signal.visiocyteraw");;
            }
        ...
        else if(datatype_img1 == VISIOCYTE_UINT16)
        {
            ...
            try
            {
                y_new<unsigned short, VISIOCYTELONG>(data1d, pagesz);
        ...
        else if(datatype_img1 == VISIOCYTE_FLOAT32) //this section need further improvement, maybe discretization. by PHC, 110810
        {
            ...
            try
            {
                y_new<float, VISIOCYTELONG>(data1d, pagesz);
...
void ImageBlendPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    if (menu_name == tr("Show LSM Info"))
    {
        ...
        // image header info
        Y_LSMINFO<VISIOCYTELONG> lsminfo(filename);
        lsminfo.loadHeader();
    ...
    else if (menu_name == tr("Multiscan Image Blending"))
    {
        ...
        else if(datatype == VISIOCYTE_FLOAT32)
        {
            ...
            //display
            Image4DSimple p4DImage;
            p4DImage.setData((unsigned char*)data1d, sx, sy, sz, colordim, VISIOCYTE_FLOAT32); //
