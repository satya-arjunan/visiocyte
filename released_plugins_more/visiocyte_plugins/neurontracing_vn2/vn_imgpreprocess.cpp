...
bool downsampling_img_xyz(unsigned char *inimg, VISIOCYTELONG in_sz[4], double dfactor_xy, double dfactor_z, unsigned char *&outimg, VISIOCYTELONG out_sz[4])
{
    ...
    {
        visiocyte_msg("The input parameters of downsampling_xyz() is not valid.");
        return false;
    }
    
    VISIOCYTELONG ntotal = 1;
    for (int i=0; i<4; i++) 
    {
        out_sz[i] = in_sz[i]; 
        ntotal *= out_sz[i];
    ...
    {
        visiocyte_msg("Fail to allocate memory in downsampling_xyz()."); 
        return false;
    }
...
bool scale_img_and_convert28bit(Image4DSimple * p4dImage, int lb, int ub) //lb, ub: lower bound, upper bound
{
    ...
    {
        visiocyte_msg("Your data is invalid in scale_img_and_convert28bit().\n");
        return false;
    }
	...
	VISIOCYTELONG tsz0 = p4dImage->getXDim(), tsz1 = p4dImage->getYDim(), tsz2 = p4dImage->getZDim(), tsz3 = p4dImage->getCDim();
	...
	{
		visiocyte_msg("Fail to allocate memory in scale_img_and_convert28bit().\n");
		return false;
	}
	...
	p4dImage->setData(outvol1d, tsz0, tsz1, tsz2, tsz3, VISIOCYTE_UINT8);
...
bool scaleintensity(Image4DSimple * p4dImage, int channo, double lower_th, double higher_th, double target_min, double target_max) //map the value linear from [lower_th, higher_th] to [target_min, target_max].
{
	...
	VISIOCYTELONG channelPageSize = p4dImage->getTotalUnitNumberPerChannel();
                   ...
                   VISIOCYTELONG c0, VISIOCYTELONG szc)
{
    if (!dstImg || !dstImg->valid() ||
        !srcImg || !srcImg->valid() )
    {
        visiocyte_msg("Invalid parameters for the function subvolumecopy(). 111");
        return false;
    }
    if (x0<0 || szx<1 || szx> dstImg->getXDim() || x0+szx > srcImg->getXDim() )
    {
        visiocyte_msg("Invalid parameters for the function subvolumecopy() 21.");
        return false;
    }
    if (y0<0 || szy<1 || szy> dstImg->getYDim() || y0+szy > srcImg->getYDim() )
    {
        visiocyte_msg("Invalid parameters for the function subvolumecopy() 22.");
        return false;
    }
    if (z0<0 || szz<1 || szz> dstImg->getZDim() || z0+szz > srcImg->getZDim() )
    {
        visiocyte_msg("Invalid parameters for the function subvolumecopy() 23.");
        return false;
    }
    if ( c0<0 || szc<1 || szc> dstImg->getCDim() || c0+szc > srcImg->getCDim() )
    {
        visiocyte_msg("Invalid parameters for the function subvolumecopy() 24.");
        return false;
    }
    if (dstImg->getDatatype() != srcImg->getDatatype() )
    {
        visiocyte_msg("Invalid parameters for the function subvolumecopy() 3.");
        return false;
    }
    ...
    VISIOCYTELONG srcChannelLen = srcImg->getTotalUnitNumberPerChannel();
    VISIOCYTELONG srcPlaneLen = srcImg->getTotalUnitNumberPerPlane();
    VISIOCYTELONG srcLineLen = srcImg->getXDim();
    ...
    switch (dstImg->getDatatype()) 
    {
        ...
        default:
            visiocyte_msg("Invalid data type in subvolumecopy(). do nothing.");
            return false;
    }
                   ...
                   VISIOCYTELONG c0, VISIOCYTELONG szc)
{
    ...
    if(dstImg->getDatatype() == VISIOCYTE_UINT8)
    {
        for (c=0;c<szc;c++)
            for (k=0;k<szz;k++)
                for (j=0;j<szy;j++)
    ...
    {
        visiocyte_msg("Invalid data type in invertedsubvolumecopy(). do nothing.");
        return false;
    }
