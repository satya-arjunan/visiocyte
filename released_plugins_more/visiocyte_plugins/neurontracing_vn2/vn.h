...
#ifndef __VISIOCYTENEURON_PARA_H__
#define __VISIOCYTENEURON_PARA_H__
...
struct PARA_VN //VN - VISIOCYTENeuron
{
    ...
    bool initialize(VISIOCYTEPluginCallback2 &callback)
    {
        visiocytehandle curwin = callback.currentImageWindow();
        p4dImage = callback.getImage(curwin);
        if (!p4dImage)
        {
            visiocyte_msg("The input image is NULL. Do nothing.");
            return false;
        }
        landmarks = callback.getLandmark(curwin);
        VISIOCYTELONG in_sz0[4] = {p4dImage->getXDim(), p4dImage->getYDim(), p4dImage->getZDim(), p4dImage->getCDim()};
        
        VISIOCYTELONG view3d_datasz0, view3d_datasz1, view3d_datasz2;
        View3DControl * view3d = callback.getView3DControl(curwin);
        if (!view3d)
        {
            visiocyte_msg("The view3d pointer is NULL which indicates there is no 3D viewer window open, thus set the cut-box as max.",0);
        ...
        if(landmarks.size()>0)
        {
            ...
            for(int i = 0; i < landmarks.size(); i++)
            {
                ...
                if(t.x<xc0+1 || t.x>xc1+1 || t.y<yc0+1 || t.y>yc1+1 || t.z<zc0+1 || t.z>zc1+1)
                {
                    ...
                    if(i==0)
                    {
                        visiocyte_msg("The first marker is invalid.");
                        return false;
                    }
                    else
