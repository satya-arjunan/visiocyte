...
template<class T> bool swc2topo_segs(vector<MyMarker*> & inswc, vector<HierarchySegment*> & topo_segs, int length_method = INTENSITY_DISTANCE_METHOD, T * inimg1d = 0, long sz0 = 0, long sz1 = 0, long sz2 = 0)
{
    ...
    VISIOCYTELONG tol_num = inswc.size();
    map<MyMarker*, VISIOCYTELONG> swc_map; for(VISIOCYTELONG i = 0; i < tol_num; i++) swc_map[inswc[i]] = i;
	
	vector<MyMarker*> leaf_markers;
	//GET_LEAF_MARKERS(leaf_markers, inswc);
    vector<VISIOCYTELONG> childs_num(tol_num);
	{
        for(VISIOCYTELONG i = 0; i < tol_num; i++) childs_num[i]=0;
        for(VISIOCYTELONG m1 = 0; m1 < tol_num; m1++)
		{
			if(!inswc[m1]->parent) continue;
            VISIOCYTELONG m2 = swc_map[inswc[m1]->parent];
			childs_num[m2]++;
		}
        for(VISIOCYTELONG i = 0; i < tol_num; i++) if(childs_num[i] == 0) leaf_markers.push_back(inswc[i]);
	}
    VISIOCYTELONG leaf_num = leaf_markers.size();
    ...
    for(VISIOCYTELONG i = 0; i < leaf_num; i++)
	{
		...
		while(parent_node)
		{
            VISIOCYTELONG pid = swc_map[parent_node];
...
template<class T>  bool topo_segs2swc(vector<HierarchySegment*> & topo_segs, vector<MyMarker*> & outmarkers, T swc_type = 1)
{
    ...
    for(VISIOCYTELONG i = 0; i < topo_segs.size(); i++)
    {
...
template<class T> bool happ(vector<MyMarker*> &inswc, vector<MyMarker*> & outswc, T * inimg1d, long sz0, long sz1, long sz2, double bkg_thresh = 10.0, double length_thresh = 2.0, double SR_ratio = 1.0/9.0, bool is_leaf_prune = true, bool is_smooth = true)
{
    ...
    for(VISIOCYTELONG i = 0; i < filter_segs.size(); i++)
	{
...
#endif
	{
        ...
        VISIOCYTELONG leaf_num_pruned = 1;
		int iteration = 1;
		vector<bool> is_pruneable(filter_segs.size(), 0);
...
#endif
	{
        ...
        for(VISIOCYTELONG s = 0; s < filter_segs.size(); s++)
		{
		...
		if(1) // start prune leaf nodes
		{
			...
			{
				leaf_num_pruned = 0;
                for(VISIOCYTELONG i = 0; i < post_segs.size(); i++)
				{
					if(iteration > 1 && !is_pruneable[i]) continue;
    ...
    for(VISIOCYTELONG i = 0; i < topo_segs.size(); i++) delete topo_segs[i];
	return true;
}
