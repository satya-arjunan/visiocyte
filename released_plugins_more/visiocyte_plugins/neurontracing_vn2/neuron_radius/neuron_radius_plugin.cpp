...
void SWCRadiusPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    else if (menu_name == tr("neuron_radius_terafly"))
    {
        ...
        for(int i = 0; i < inswc.size(); i++)
        {
            ...
            if(inswc[i]->radius == -1)
            {
                ...
                VISIOCYTELONG *in_sz = new VISIOCYTELONG[4];
	...
	{
		visiocyte_msg(tr("Re-estimate the radius of a swc. "
			"Developed by Hang Xiao, 2012-11-07"));
	}
}

bool SWCRadiusPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    ...
    else if (func_name == tr("neuron_radius_2D"))
    {
        ...
        for(VISIOCYTELONG i = 0; i <inswc.size(); i++)
        {
    ...
    else if(func_name == tr("neuron_radius_terafly"))
    {
        ...
        saveSWC_file(outswc_file, inswc);
        visiocyte_msg(QString("neuron radius is calculated successfully. \n\nThe output swc is saved to %1").arg(outswc_file.c_str()),0);
        for(int i = 0; i < inswc.size(); i++) delete inswc[i];

    }
	else //if (func_name == tr("help"))
	{
		cout<<"visiocyte -x <plugin_dll> -f neuron_radius -i <inimg_file> <inswc_file> -p <threshold> [<is2d>]"
        <<endl
