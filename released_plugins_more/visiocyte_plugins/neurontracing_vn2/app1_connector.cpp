...
bool proc_app1(VISIOCYTEPluginCallback2 &callback, PARA_APP1 &p, const QString & versionStr)
{
    //  bool b_menu = true;
      bool b_dofunc = false;
    ...
    tmpstr =  qPrintable( qtstr.prepend("##Visiocyte-Neuron-APP1 ").append(versionStr) ); infostring.push_back(tmpstr);
...
//        cout << *it <<endl;
    ...
    visiocyte_msg("start to preprocessing.\n", 0);
    ...
    {
        visiocyte_msg("Somehow invalid volume box info is detected. Ignore it. But check your Visiocyte program.");
        return false;
    }
    
    //always just use the buffer data
    unsigned char * indata1d = p4dImageNew->getRawDataAtChannel(0);
    VISIOCYTELONG in_sz[4] = {p4dImageNew->getXDim(), p4dImageNew->getYDim(), p4dImageNew->getZDim(), 1};
    int datatype = p.p4dImage->getDatatype();
    
    double dfactor_xy = 1, dfactor_z = 1;
    if(datatype != VISIOCYTE_UINT8 || in_sz[0]>256 || in_sz[1]>256 || in_sz[2]>256)// && datatype != VISIOCYTE_UINT16)
    {
        ...
        if (!p.b_256cube)
        {
            //visiocyte_msg("p.b_256cube=FALSE!");
            
            dfactor_z = dfactor_xy = p.downsample_factor;
        }
        else 
        {
            //visiocyte_msg("p.b_256cube=TRUE!");
        ...
        if (0 && (dfactor_z>1 || dfactor_xy>1)) //130220. instead of direct downsampling, try to use the original APP1 method
        {
            ...
            {
                visiocyte_msg("Down-sampling fails... and quit.");
    ...
    {
        for(VISIOCYTELONG i = 0; i < p.landmarks.size(); i++)
        {
    ...
    visiocyte_msg("start neuron tracing for the preprocessed image.\n", 0);
    ...
    //NOW START THE RECONSTRUCTION
    {
        ...
        VISIOCYTELONG sz_tracing[4]; sz_tracing[0] = in_sz[0]; sz_tracing[1] = in_sz[1]; sz_tracing[2] = in_sz[2]; sz_tracing[3] = nChannelsToUse;
        ...
        if(inmarkers.empty())
        {
            ...
            if (!phi)
            {
                visiocyte_msg("Fail to run the GSDT code. Exit.", 0);
                goto Label_exit;
            }
        ...
        else
        {
            ...
            LocationSimple t;
            for (VISIOCYTELONG i=1; i<inmarkers.size();i++)
            {
                t.x = inmarkers[i].x;
                t.y = inmarkers[i].y;
                t.z = inmarkers[i].z;
    ...
    if (1)
    {
        ...
        visiocyte_msg(QString("The tracing uses %1 ms (%2 ms for preprocessing and %3 for tracing). Now you can drag and drop the generated swc fle [%4] into Visiocyte."
    ...
    else //save SWC
    {
        ...
        VISIOCYTELONG szOriginalData[4] = {p.p4dImage->getXDim(), p.p4dImage->getYDim(), p.p4dImage->getZDim(), 1};
...
bool PARA_APP1::fetch_para_commandline(const VISIOCYTEPluginArgList &input, VISIOCYTEPluginArgList &output, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
