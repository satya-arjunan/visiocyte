...
double calculate_overlapping_ratio_n1(const V_NeuronSWC_unit & n1, const V_NeuronSWC_unit & n2, unsigned char ***imap,  VISIOCYTELONG sz[3], double trace_z_thickness)
{
	...
	{
		cz = VISIOCYTELONG(n1z+k+0.5); if (cz<0 || cz>=sz[2]) continue;
		curr_k = double(k)*k;
		for (j=-R;j<=R;j++)
		{
			cy = VISIOCYTELONG(n1y+j+0.5); if (cy<0 || cy>=sz[1]) continue;
			if ((curr_j = double(j)*j+curr_k) > r12)
				continue;
			
			for (i=-R;i<=R;i++)
			{
				cx = VISIOCYTELONG(n1x+i+0.5); if (cx<0 || cx>=sz[0]) continue;
	...
	if (n1_totalpixel==0)
	{
		visiocyte_msg("total # sample is 0. This means radius is wrong. Should never see this!!! Check data and program. \n", 0);
		printf("total pixel=%ld, R=%ld n1.r=%5.3f n1x=%5.3f n1y=%5.3f n1z=%5.3f sz[0]=%ld sz[1]=%ld sz[2]=%ld ", VISIOCYTELONG(n1_totalpixel), R, n1.r, n1x, n1y, n1z, sz[0], sz[1], sz[2]);
...
double calculate_overlapping_ratio_n1(const V_NeuronSWC_unit & n1, vector<V_NeuronSWC_unit> & mUnit, map<double,VISIOCYTELONG> & index_map, list<VISIOCYTELONG> & mylist_n, unsigned char ***imap,  VISIOCYTELONG sz[3], double trace_z_thickness)
{
