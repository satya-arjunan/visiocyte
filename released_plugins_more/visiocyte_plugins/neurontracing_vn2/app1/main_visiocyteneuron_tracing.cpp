...
void printHelp()
{
        //VISIOCYTELONG i; int j; short int k; float tf; double td; unsigned char *p;
        //printf("VISIOCYTELONG=%d, int=%d, short=%d, float=%d, double=%d, pointer=%d\n", sizeof(i), sizeof(j), sizeof(k), sizeof(tf), sizeof(td), sizeof(p)); 

	printf("\nNeuronTracer (originally called VISIOCYTE-Neuron/Visiocyte-Neuron Tracing) (version 1.66 09/17/2011; 1.67 03/07/2012. 1.70 04/3/2012, 1.71 02/10/2013. )\n");
	...
	printf("Trace a neuron (or other 3D path) from a 3D image using the VISIOCYTENeuron Tracing method. \n");
...
bool parseChannelString(char *optarg, VISIOCYTELONG * &chan, VISIOCYTELONG &nchan)
{
	...
	curstr[i]=',';
	curstr[i+1]='\0';
	VISIOCYTELONG slen = i+1;
	//printf("The working copy of channel string is [%s]\n", curstr);
	
	vector <VISIOCYTELONG> mychan;
	vector <VISIOCYTELONG> pos; 
	for (i=0;i<slen;i++)
...
int main (int argc, char *argv[])
{
	...
	VISIOCYTELONG channelNo_subject=0;
	VISIOCYTELONG *channelsToUse=0;
	VISIOCYTELONG nChannelsToUse=0;
	...
	{
		nChannelsToUse=1;
		channelsToUse = new VISIOCYTELONG[nChannelsToUse];
		channelsToUse[0] = 0;
	}
	...
	VISIOCYTELONG * sz_subject = 0; // must be initialized as NULL
	int datatype_subject = 0;
	VISIOCYTELONG channel_bytes = 0;
	VISIOCYTELONG total_units=0;
	VISIOCYTELONG total_bytes=0;
	...
	//read file
	{
		...
		{
			fprintf (stdout, "Dst marker file [%s] is empty. Thus for every marker in the src list, VISIOCYTE-Neuron is going to trace to the entire image.  \n", dst_markers_file);
		}
		...
		if (nChannelsToUse>0)
		{
			for (VISIOCYTELONG k=0;k<nChannelsToUse;k++)
			{
		...
		else	
		{
			...
			total_units = (VISIOCYTELONG)sz_subject[0]*(VISIOCYTELONG)sz_subject[1]*(VISIOCYTELONG)sz_subject[2];
			...
			try
			{
				...
				{
					for (VISIOCYTELONG j=0;j<nChannelsToUse;j++)
					{
						unsigned char *datahead = img_subject + (VISIOCYTELONG)channelsToUse[j]*channel_bytes;
				...
				else if (datatype_subject==2)
				{
					for (VISIOCYTELONG j=0;j<nChannelsToUse;j++)
					{
						short int *datahead = (short int *)(img_subject + (VISIOCYTELONG)channelsToUse[j]*channel_bytes);
						unsigned char *targethead = img_new + j*total_units;
						short int minv, maxv; VISIOCYTELONG pos_min, pos_max; 
						...
						else
						{
							...
							for (VISIOCYTELONG i=0;i<total_units;i++)
				...
				else // (datatype_subject==4)
				{
					for (VISIOCYTELONG j=0;j<nChannelsToUse;j++)
					{
						float *datahead = (float *)(img_subject + (VISIOCYTELONG)channelsToUse[j]*channel_bytes);
						unsigned char *targethead = img_new + j*total_units;
						float minv, maxv; VISIOCYTELONG pos_min, pos_max; 
						minMaxInVector(datahead, total_units, pos_min, minv, pos_max, maxv);
						if (minv==maxv)
						{
							fprintf (stderr, "The data has the min value equals its max value. Thus the neuron tracing won't make sense. Exit. \n");
		...
		unsigned char ****p4d = 0;
		VISIOCYTELONG sz_tracing[4]; sz_tracing[0] = sz_subject[0]; sz_tracing[1] = sz_subject[1]; sz_tracing[2] = sz_subject[2]; sz_tracing[3] = nChannelsToUse; 
		   ...
		   )
		{
			...
			if (nChannelsToUse==1)
			{
				...
				{
					for (VISIOCYTELONG myii=0;myii<sz_tracing[0]*sz_tracing[1]*sz_tracing[2];myii++)
					...
					if (traceOnDTImg_method==1 || traceOnDTImg_method==2 || traceOnDTImg_method == 3)
					{
						...
						bool b_3d_dt = true; 
						VISIOCYTELONG totallen=sz_subject[0]*sz_subject[1]*sz_subject[2];
						...
						if(traceOnDTImg_method == 3) 
							...
							dt_visiocyteneuron(p_curdata, sz_subject, trace_para.imgTH, b_to_background, b_3d_dt, dd);
				...
				for (VISIOCYTELONG isrc=0; isrc<marker_src.size(); isrc++)
				{
					...
					pp.clear();
					LocationSimple tmpp;
					for (VISIOCYTELONG idst=0; idst<marker_dst.size(); idst++)
					{
					...
					nt = visiocyteneuron_GD_tracing(p4d, sz_tracing, 
											  p0, pp, 
											  trace_para, weight_xy_z);
					
					//save a tmp copy for debug purpose
					VISIOCYTELONG localsz[4]; localsz[0]=sz_subject[0]; localsz[1]=sz_subject[1]; localsz[2]=sz_subject[2]; localsz[3]=1; 
					...
					else
					{
						...
						if (b_produceProjectionPatterns)
						{
							...
							for (VISIOCYTELONG tmpk=0; tmpk<sz_subject[2]; tmpk++)
							{
								if (tmpk==0)
								{	
									VISIOCYTELONG ind=0;
									for (VISIOCYTELONG tmpj=0;tmpj<sz_subject[1]; tmpj++)
										for (VISIOCYTELONG tmpi=0;tmpi<sz_subject[0];tmpi++)
							...
							VISIOCYTELONG sz_img_proj[4]; sz_img_proj[0] = sz_subject[0]; sz_img_proj[1] = sz_subject[1]; sz_img_proj[2] = sz_img_proj[3] = 1; 
			...
			else //nChannels>1
			{
				...
				if (trace_para.sp_graph_background)
				{
					VISIOCYTELONG * chanUse = new VISIOCYTELONG [sz_tracing[3]];
					for (VISIOCYTELONG tmpi=0;tmpi<sz_tracing[3]; tmpi++) chanUse[tmpi]=tmpi;
				...
				printf("\n Run thresholding (th=%5.3f), then median filter (when filter radius >=1), and distance transform (if specified -q option)...\n", trace_para.imgTH);
				{
					for (VISIOCYTELONG myii=0;myii<sz_tracing[0]*sz_tracing[1]*sz_tracing[2]*sz_tracing[3];myii++)
						if (img_new[myii]<=trace_para.imgTH) img_new[myii]=0; 
					if (medianf_rr>=1)
					{
						for (VISIOCYTELONG i=0;i<sz_tracing[3];i++)
