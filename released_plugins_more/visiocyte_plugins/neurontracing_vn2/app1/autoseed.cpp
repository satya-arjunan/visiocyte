...
QList <ImageMarker> autoMarkerFromImg(unsigned char ***img, VISIOCYTELONG sz[4], float zthickness);
...
template <class T> bool downsample3dimg(T *** inimg, VISIOCYTELONG insz[3], double dfactor, T *** outimg, VISIOCYTELONG outsz[3]);

QList <ImageMarker> autoSeed(unsigned char ***img, VISIOCYTELONG sz[4], double weight_xy_z)
{
...
QList <ImageMarker> autoMarkerFromImg(unsigned char ***img, VISIOCYTELONG sz[4], float zthickness)
{
    ...
    double dfactor = 4;
	VISIOCYTELONG xb=0,yb=0,zb=0,xe=sz[0]-1,ye=sz[1]-1,ze=sz[2]-1;
	VISIOCYTELONG vsz0=xe-xb+1, vsz1=ye-yb+1, vsz2=ze-zb+1;
	VISIOCYTELONG vsz0out=vsz0/dfactor, vsz1out=vsz1/dfactor, vsz2out=vsz2/dfactor;
    VISIOCYTELONG szout[4]; szout[0] = vsz0out; szout[1] = vsz1out; szout[2] = vsz2out; szout[3]=1; 
...
//	segpara.t_rgnval=qMax(double(20.0), mean_val+1.0*std_val);
	...
	else
	{
		...
		{
			visiocyte_msg("Some errors happen during the computation of image objects' statistics. The annotation is not generated.", 0);
			return ql;
		}

		double largestMass; VISIOCYTELONG iLargestMass = -1;
...
template <class T> bool downsample3dimg(T *** inimg, VISIOCYTELONG insz[3], double dfactor, T *** outimg, VISIOCYTELONG outsz[3])
{
	...
	VISIOCYTELONG cur_sz0 = (VISIOCYTELONG)(floor(double(insz[0]) / double(dfactor)));
	VISIOCYTELONG cur_sz1 = (VISIOCYTELONG)(floor(double(insz[1]) / double(dfactor)));
	VISIOCYTELONG cur_sz2 = (VISIOCYTELONG)(floor(double(insz[2]) / double(dfactor)));
