...
bool ImageTip::Initialize(unsigned char* data, VISIOCYTELONG sz[3]) //do padding
{
	try
	{
		if (!data || !sz || sz[0]<=0 || sz[1]<=0 || sz[2]<=0)
		{
			visiocyte_msg("Invalid data in SetImageInfo()", 0);
			return false;
		}
	}
	catch (...)
	...
	VISIOCYTELONG i,j,k;
	VISIOCYTELONG pagesz = realszx*realszy;
	...
	for (VISIOCYTELONG k=1; k<realszz-1; k++)
		for (VISIOCYTELONG j=1; j<realszy-1; j++)
			for (VISIOCYTELONG i=1; i<realszx-1; i++)
			{
...
void ImageTip::Compute_DFS()
{
	...
	VISIOCYTELONG m_pagesz = realszx*realszy; 
...
{
	VISIOCYTELONG i, j, k, l;
	VISIOCYTELONG index, index_nei;
	
//	DPoint_t last;
//	vector<VISIOCYTELONG> len;// 
	...
	for(VISIOCYTELONG num = 0; num < m_vdfbptSurface.size(); ++num)
	{
		...
		for (int m = 0; m < 3; ++m)
		{
			for (int n = 0; n < 9; ++n)
			{
				VISIOCYTELONG z = k + nDz[m];
				VISIOCYTELONG y = j + nDy[n];
				VISIOCYTELONG x = i + nDx[n];
...
vector<DPoint_t> tip_detection(unsigned char *p, VISIOCYTELONG sz[3])
{
	...
	bool b_find=false;
	for (VISIOCYTELONG n=0; n<mytip.totallen; n++)
	...
	if (b_find==false)
	{
		visiocyte_msg("You image contains only background!! Do nothing.", 0);
		return vv;
	}
