//visiocyteneuron_tracing_shortestpath.cpp
// by Hanchuan Peng
// 2008-2010
// separated from main VISIOCYTE program on May 24, 2010 for command line program. by Hanchuan Peng

#include "visiocyteneuron_gd_tracing.h"
...
#define CHECK_DATA_GD_TRACING(AA) \
	...
	{ \
		visiocyte_msg("Invalid image and sz parameters.\n", 0); \
		throw("Invalid image and sz parameters.\n"); \
		return AA; \
	} 

#define _CHECK_PRUNING_PARAMETERS_() \
    VISIOCYTELONG npruned = 0; \
    if (mmUnit.size()<=0 || mmUnit[0].size()<=0 || !imap || !dmap || !sz || sz[0]<=0 || sz[1]<=0 || sz[2]<=0) \
    { \
        visiocyte_msg("Wrong parameters in a pruning function.\n", 0); \
        return npruned; \
    } 

#define TRACED_NAME "APP1_Tracing"
#define TRACED_FILE "visiocyte_traced_neuron"



NeuronTree visiocyteneuron_GD_tracing(unsigned char ****p4d, VISIOCYTELONG sz[4], 
								...
								CurveTracePara & trace_para, double trace_z_thickness) 
{
	...
	{
		fprintf(stderr, "Invalid image or sz for visiocyteneuron_GD_tracing().\n");
		return nt;
	}
	...
	{		
		if (0 && pp.size()<=0) //disable the immatured tip-detection
		{
			...
			if (tt.size()>0)
			{
				for (VISIOCYTELONG i=0;i<tt.size();i++)
				{
		...
		if ( trace_para.b_post_trimming )
		{
			...
			dt_visiocyteneuron(p4d[trace_para.channo][0][0], sz, trace_para.imgTH, b_to_background, b_3d_dt, dd);
	...
	//		for (VISIOCYTELONG i=0;i<ppnew.size();i++)
	...
	//		fclose(f);
			...
			VISIOCYTELONG ninitotalpruned = mmUnit[0].size();
			VISIOCYTELONG ntotalpruned = 0;
			if (1)
			{
				for (int tmp=1; ; tmp++)
				{
					VISIOCYTELONG nnodes = mmUnit[0].size();
			...
			{
				VISIOCYTELONG nnodes = mmUnit[0].size();
				VISIOCYTELONG ninternodepruned = pruning_internodes(mmUnit, p4d[trace_para.channo],  dd, sz, trace_z_thickness);
			...
			{
				VISIOCYTELONG nnodes = mmUnit[0].size();
				VISIOCYTELONG nbranchnodepruned = pruning_branch_nodes(mmUnit, p4d[trace_para.channo],  dd, sz, trace_z_thickness);
            ...
            {
                VISIOCYTELONG nArtificialNodes = pruning_artifacial_branches(mmUnit, p4d[trace_para.channo], dd, sz, trace_z_thickness, trace_para.visible_thresh);
				while (1)
                {
                    VISIOCYTELONG nFinalSingleCoverPruned = pruning_covered_leaf_single_cover(mmUnit, p4d[trace_para.channo],  dd, sz, trace_z_thickness);
		...
		int n_end_nodes = 0;//for (VISIOCYTELONG ii=0;ii<mmUnit[0].size();ii++)	{if (mmUnit[0][ii].nchild==0) n_end_nodes++;}
...
V_NeuronSWC_list trace_one_pt_to_N_points_shortestdist(unsigned char ****p4d, VISIOCYTELONG sz[4], 
															 LocationSimple & p0, vector<LocationSimple> & pp, 
															 CurveTracePara & trace_para, double trace_z_thickness,
															 vector< vector<V_NeuronSWC_unit> > & mmUnit) 
{
	...
	if (trace_bounding_box == NULL_BoundingBox)
	{
		...
		printf("set z1=%ld\n", VISIOCYTELONG(trace_bounding_box.z1));
	}
	printf("z1=%ld\n", VISIOCYTELONG(trace_bounding_box.z1));
	...
	{
		VISIOCYTELONG *chans = new VISIOCYTELONG [sz[3]];
		for(VISIOCYTELONG tmpi=0; tmpi<sz[3]; tmpi++) chans[tmpi]=tmpi; //use all channels (since the data of channels_to_use have actually been copied here)
...
int mergeback_mmunits_to_neuron_path(int n_end_nodes, vector< vector<V_NeuronSWC_unit> > & mmUnit, V_NeuronSWC_list & tNeuron)
{
	...
	{
		VISIOCYTELONG nexist = tNeuron.maxnoden();
	...
	{
		for (VISIOCYTELONG ii=0;ii<mmUnit.size();ii++)
		{
			VISIOCYTELONG nexist = tNeuron.maxnoden();
...
bool proj_trace_smooth_downsample_last_traced_neuron(unsigned char ****p4d, VISIOCYTELONG sz[4], V_NeuronSWC_list & tracedNeuron, CurveTracePara & trace_para, int seg_begin, int seg_end)
{
	visiocyte_msg("proj_trace_smooth_downsample_last_traced_neuron(). \n",0);
	CHECK_DATA_GD_TRACING(false);
	
	VISIOCYTELONG nexist = 0; // re-create index number
	
	// (VneuronSWC_list tracedNeuron).(V_neuronSWC seg[]).(V_nueronSWC_unit row[])
	for(VISIOCYTELONG iseg=0; iseg<tracedNeuron.seg.size(); iseg++)
	{
...
bool proj_trace_smooth_traced_neuron(unsigned char ****p4d, VISIOCYTELONG sz[4], vector< vector<V_NeuronSWC_unit> > & mmUnit, CurveTracePara & trace_para, int seg_begin, int seg_end)
{	
	visiocyte_msg("proj_trace_smooth_traced_neuron(). \n",0);
	CHECK_DATA_GD_TRACING(false);
	
	VISIOCYTELONG nexist = 0; // re-create index number
	
	for(VISIOCYTELONG iseg=0; iseg<mmUnit.size(); iseg++)
	{
		if (iseg <seg_begin || iseg >seg_end) continue; //091023
...
bool proj_trace_compute_radius_of_last_traced_neuron(unsigned char ****p4d, VISIOCYTELONG sz[4], V_NeuronSWC_list & tracedNeuron, CurveTracePara & trace_para, int seg_begin, int seg_end, float myzthickness, bool b_smooth)
{
	visiocyte_msg("proj_trace_compute_radius_of_last_traced_neuron(). \n", 0);
	CHECK_DATA_GD_TRACING(false);
...
bool proj_trace_compute_radius_of_last_traced_neuron(unsigned char ****p4d, VISIOCYTELONG sz[4], vector< vector<V_NeuronSWC_unit> >& mmUnit, CurveTracePara & trace_para, int seg_begin, int seg_end, float myzthickness, bool b_smooth)
{
	visiocyte_msg("proj_trace_compute_radius_of_last_traced_neuron(). \n", 0);
	CHECK_DATA_GD_TRACING(false);
...
bool proj_trace_mergeAllClosebyNeuronNodes(V_NeuronSWC_list & tracedNeuron)
{
	...
	for (seg_id=0; seg_id<NSegs;seg_id++) 
	{
		...
		{
			visiocyte_msg(QString("The %1 neuron segment is not a simple line graph. You should first select Edit to turn it into simpler pieces.").arg(seg_id));
			return false;
		}
	}
...
#define VISIOCYTENEURON_MERGE_TO_BIGSPHERE 1
	...
	for (seg_id=0; seg_id<NSegs;seg_id++)
	{
		...
		VISIOCYTELONG slength = subject_swc.nrows();
		...
		for (j=0; j<slength; j++)
		{
			...
			VISIOCYTELONG ind_best_merge_seg=-1, ind_best_merge_node=-1;	double r_best_merge=-1, dist_best; //set as -1 for initialization
			for (cur_sid=seg_id+1;cur_sid<NSegs;cur_sid++)
			{
				...
				V_NeuronSWC & target_swc = tracedNeuron.seg.at(cur_sid);
				//map <VISIOCYTELONG, VISIOCYTELONG> target_index_map = unique_V_NeuronSWC_nodeindex(target_swc);
				VISIOCYTELONG tlength = target_swc.nrows();
			...
			if (ind_best_merge_seg>=0) // && ind_best_merge_node>=0 && r_best_merge>=0) //only one judgment is enough
			{
				...
				if (VISIOCYTENEURON_MERGE_TO_BIGSPHERE) //I set it to 0, thus don't use this method
				{
	...
	{
		V_NeuronSWC & subject_swc = tracedNeuronNew.seg.at(seg_id);
		map <VISIOCYTELONG, VISIOCYTELONG> subject_index_map = unique_V_NeuronSWC_nodeindex(subject_swc);
		VISIOCYTELONG slength = subject_swc.nrows();
		
		printf("seg=%ld\n", seg_id);
		for (j=0; j<slength; j++)
		{
			printf("j=%ld key=%ld mapped row=%ld node=%ld x=%5.3f y=%5.3f z=%5.3f parent=%ld\n", j, VISIOCYTELONG(subject_swc.row.at(j).n), subject_index_map[VISIOCYTELONG(subject_swc.row.at(j).n)], VISIOCYTELONG(subject_swc.row.at(j).n), subject_swc.row.at(j).x, subject_swc.row.at(j).y, subject_swc.row.at(j).z, VISIOCYTELONG(subject_swc.row.at(j).parent));
		}
		printf("\n\n");
		...
		{
			VISIOCYTELONG c = j;
			VISIOCYTELONG p =  VISIOCYTELONG(subject_swc.row.at(j).parent);
			if (p<0) continue;
			else p = subject_index_map[p];
			
			//printf("c0=%ld p0=%ld c=%ld p=%ld\n", c0, p0, c, p);
			if (p<0 || p>=slength) {visiocyte_msg(QString("detect a strange parent! row(%1) node=%2 parent=%3").arg(j).arg(subject_swc.row.at(j).n).arg(p)); continue;}
			
			VISIOCYTELONG ipos=-1;	for (i=0;i<finalNodePool.size();i++)	{if (subject_swc.row.at(c).get_coord() == finalNodePool.at(i).get_coord())	{ipos=i;break;	}}
			VISIOCYTELONG ippos=-1;	for (i=0;i<finalNodePool.size();i++)	{if (subject_swc.row.at(p).get_coord() == finalNodePool.at(i).get_coord())	{ippos=i;break;	}}
			if (ipos>=0 && ippos>=0)
			{
			...
			{
				visiocyte_msg("Error: this message should never appear. You have a neuron node whose coordinate cannot be detected!");
				return false;
			}
		}
	}
	...
	VISIOCYTELONG *ind_end_nodes = new VISIOCYTELONG [terminiNodePool.size()], ind_startnode;
	...
	VISIOCYTELONG ntotalnodes = finalNodePool.size();
	...
	for (i=0;i<curTraceNeuron.seg.size();i++) //now use the already estimated radius information
	{
		...
		{
			bool b_find=false;
			for (VISIOCYTELONG ipos=0;ipos<finalNodePool.size();ipos++)	
			{
			...
			if (!b_find)
			{
				visiocyte_msg(QString("The %1 seg %2 node has an unmatched node.\n").arg(i).arg(j), 0); 
			}
		}
	}
	...
	visiocyte_msg("merge all successfully", 0);
	return res;
}

bool filterThinSegmentsInImage(unsigned char ****p4d, VISIOCYTELONG sz[4], V_NeuronSWC_list & tracedNeuron, CurveTracePara & trace_para, int seg_begin, int seg_end, float myzthickness)
{
	visiocyte_msg("filterThinSegmentsInImage(). \n", 0);
	CHECK_DATA_GD_TRACING(false);
	
	VISIOCYTELONG chano = trace_para.channo;
	VISIOCYTELONG sp_downsample_step = trace_para.sp_graph_resolution_step;
	...
	for(VISIOCYTELONG iseg=0; iseg<tracedNeuron.seg.size(); iseg++)
	{
		...
		for (VISIOCYTELONG iu = 0; iu<mUnit.size(); iu++)
		{
			V_NeuronSWC_unit & u = mUnit.at(iu);
			//p3d[VISIOCYTELONG(u.z)][VISIOCYTELONG(u.y)][VISIOCYTELONG(u.x)] = 0;
...
bool filterNodesByFindingBestCovers(unsigned char ****p4d, VISIOCYTELONG sz[4], 
									VISIOCYTELONG bx0, VISIOCYTELONG bx1,
									VISIOCYTELONG by0, VISIOCYTELONG by1,
									VISIOCYTELONG bz0, VISIOCYTELONG bz1,
									V_NeuronSWC_list & tracedNeuron, CurveTracePara & trace_para, float myzthickness)
{
	visiocyte_msg("filterNodesByFindingBestCovers(). \n", 0);
	CHECK_DATA_GD_TRACING(false);
	...
	//bounding box volume
	VISIOCYTELONG xmin = bx0, xmax = bx1,
		ymin = by0, ymax = by1,
		zmin = bz0, zmax = bz1;
		
	VISIOCYTELONG nx=((xmax-xmin)/min_step)+1, 	xstep=min_step,
	...
	VISIOCYTELONG num_nodes = nx*ny*nz;
	VISIOCYTELONG i,j,k,n,m;
	
	////////////////////////////////////////////////////////////////////////////////////////////////////////////
#define NODE_FROM_XYZ(x,y,z) 	(VISIOCYTELONG((z+.5)-zmin)/zstep*ny*nx + VISIOCYTELONG((y+.5)-ymin)/ystep*nx + VISIOCYTELONG((x+.5)-xmin)/xstep)
#define NODE_TO_XYZ(j, x,y,z) \
{ \
z = (j)/(nx*ny); 		y = ((j)-VISIOCYTELONG(z)*nx*ny)/nx; 	x = ((j)-VISIOCYTELONG(z)*nx*ny-VISIOCYTELONG(y)*nx); \
...
#define X_SI(i)				 	(VISIOCYTELONG(((i)-xmin)/xstep))
#define Y_SI(i)				 	(VISIOCYTELONG(((i)-ymin)/ystep))
#define Z_SI(i)				 	(VISIOCYTELONG(((i)-zmin)/zstep))
	
	////////////////////////////////////////////////////////////////////////////////////////////////////////////

#define SET_ALLELEMENTS_ZEROS(pp, pplen) \
{ \
for (VISIOCYTELONG ppi=0; ppi<pplen; ppi++) pp[ppi] = 0; \
}
#define PUT_IN_RANGE(x0, x, x1) {if (x<x0) x=x0; else if (x>x1) x=x1; }	
	...
	VISIOCYTELONG stacksz = nx*ny*nz;
	float * maxr_img_1d = 0, *** maxr_img_3d = 0; 
	VISIOCYTELONG * maxid_img_1d = 0, *** maxid_img_3d = 0; 
	try
	{
		maxr_img_1d = new float [stacksz]; SET_ALLELEMENTS_ZEROS(maxr_img_1d, stacksz);
		new3dpointer(maxr_img_3d, nx, ny, nz, maxr_img_1d);
		maxid_img_1d = new VISIOCYTELONG [stacksz]; SET_ALLELEMENTS_ZEROS(maxid_img_1d, stacksz);
		new3dpointer(maxid_img_3d, nx, ny, nz, maxid_img_1d);
	}
	catch(...)
	{
		visiocyte_msg("Fail to allocate memory in filterNodesByFindingBestCovers();\n", 0);
	...
	for(VISIOCYTELONG iseg=0; iseg<tracedNeuron.seg.size(); iseg++)
	{
		...
		for (VISIOCYTELONG iu = 0; iu<mUnit.size(); iu++)
		{
			...
			VISIOCYTELONG curnx = X_SI(VISIOCYTELONG(u.x));
			VISIOCYTELONG curny = Y_SI(VISIOCYTELONG(u.y));
			VISIOCYTELONG curnz = Z_SI(VISIOCYTELONG(u.z));
			
			VISIOCYTELONG i0 = curnx - u.r/xstep; PUT_IN_RANGE(0, i0, nx-1);
			VISIOCYTELONG	i1 = curnx + u.r/xstep; PUT_IN_RANGE(0, i1, nx-1);
			VISIOCYTELONG j0 = curny - u.r/ystep; PUT_IN_RANGE(0, j0, ny-1);
			VISIOCYTELONG	j1 = curny + u.r/ystep; PUT_IN_RANGE(0, j1, ny-1);
			VISIOCYTELONG k0 = curnz - u.r/zstep; PUT_IN_RANGE(0, k0, nz-1);
			VISIOCYTELONG	k1 = curnz + u.r/zstep; PUT_IN_RANGE(0, k1, nz-1);
			
			double d;
			for (VISIOCYTELONG k=k0;k<=k1;k++)
				for (VISIOCYTELONG j=j0;j<=j1;j++)
					for (VISIOCYTELONG i=i0;i<=i1;i++)
					{
...
bool est_pixel_radius(unsigned char *p1d, VISIOCYTELONG sz[3], float imgTH, float myzthickness, bool b_est_in_xyplaneonly, float *p1dr) //p1dr the detected radius
{
	visiocyte_msg("est_pixel_radius(). \n", 0);
	if (!p1d || !sz || sz[0]<1 || sz[1]<1 || sz[2]<1 || myzthickness<=0 || !p1dr)
	{
		visiocyte_msg("Invalid parameters.\n",0);
		return false;
	}
	...
	{
		visiocyte_msg("Fail to allocate memory in est_pixel_radius().\n", 0);
...
bool dt_visiocyteneuron(unsigned char *p1d, VISIOCYTELONG sz[3], float imgTH, bool b_to_background, bool b_3d_dt, float *p1dt) //imgTH specify the foreground
{
	visiocyte_msg("dt_visiocyteneuron(). \n", 0);
	...
	VISIOCYTELONG len = sz[0]*sz[1]*sz[2];
	VISIOCYTELONG *label = 0;
	try {
		label = new VISIOCYTELONG [len];
	}
	catch (...)
	{
		visiocyte_msg("Fail to allocate memory in dt_visiocyteneuron().\n", 0);
		return false;
	}
...
//		for (VISIOCYTELONG i=0;i<len; i++) label_float[i] = label[i];
//		if (b_3d_dt)
//		{
...
vector<LocationSimple> find_valid_tip_via_ignore_short_branches(vector< vector<V_NeuronSWC_unit> >& mmUnit, unsigned char *imap,  float *dmap, VISIOCYTELONG sz[3])
{
	vector<LocationSimple> mytip;
	if (mmUnit.size()<=0 || mmUnit[0].size()<=0 || !dmap || !sz || sz[0]<=0 || sz[1]<=0 || sz[2]<=0)
	{
		visiocyte_msg("Wrong parameters in find_valid_tip_via_ignore_short_branches().\n", 0);
		return mytip;
	}
	...
	vector<VISIOCYTELONG> leafArray, branchPointArray;
	VISIOCYTELONG tnodes = mUnit.size();
	for (i=0;i<tnodes; i++)
	{
	...
	{
		VISIOCYTELONG ci = leafArray.at(j); 
		//printf("ini ci=%ld nchild=%ld cp=%ld\n", ci, VISIOCYTELONG(mUnit.at(ci).nchild), VISIOCYTELONG(mUnit.at(ci).parent));
		VISIOCYTELONG cnodes = 0;
		double tlen = 0, tsum = 0;
		while (mUnit.at(ci).nchild<2 && mUnit.at(ci).parent>=0 && cnodes<tnodes)
		{
			V_NeuronSWC_unit & curnode = mUnit.at(ci);
			VISIOCYTELONG cp = VISIOCYTELONG(mUnit.at(ci).parent);
			...
			tsum += imap[VISIOCYTELONG(curnode.z)*sz[0]*sz[1] + VISIOCYTELONG(curnode.y)*sz[0] + VISIOCYTELONG(curnode.x)];
			cnodes++;
			ci = cp;
		}
		...
		if (cnodes<tnodes)
		{
			...
			double curd = dmap[VISIOCYTELONG(curpnode.z)*sz[0]*sz[1] + VISIOCYTELONG(curpnode.y)*sz[0] + VISIOCYTELONG(curpnode.x)];
...
bool condense_branches(vector< vector<V_NeuronSWC_unit> >& mmUnit, unsigned char ***imap,  float *dmap, VISIOCYTELONG sz[3], double trace_z_thickness)
{
#define MAXSR 10
#define transformDval2SR(x) ((MAXSR-x < 1) ? 1 : (MAXSR-x))
	...
	if (mmUnit.size()<=0 || mmUnit[0].size()<=0 || !imap || !dmap || !sz || sz[0]<=0 || sz[1]<=0 || sz[2]<=0)
	{
		visiocyte_msg("Wrong parameters in condense_branches().\n", 0);
		return false;
	}
	...
	std::map<double,VISIOCYTELONG> index_map;	index_map.clear();
	vector <unsigned char> visited;
	for (j=0; j<tnodes; j++)
	{
		double ndx   = mUnit[j].n;
		VISIOCYTELONG new_ndx = index_map.size(); //map the neuron node'd id to row number
	...
	for (i=0; i<tnodes; i++)
	{
		VISIOCYTELONG pi = mUnit[i].parent;
		...
		double cur_dval = dmap[VISIOCYTELONG(curnode.z)*sz[0]*sz[1] + VISIOCYTELONG(curnode.y)*sz[0] + VISIOCYTELONG(curnode.x)];
		...
		if (method_code==1)
		{
			...
			while (1)
			{
				...
				cur_dval = dmap[VISIOCYTELONG(curnode.z)*sz[0]*sz[1] + VISIOCYTELONG(curnode.y)*sz[0] + VISIOCYTELONG(curnode.x)];
...
VISIOCYTELONG pruning_covered_leaf_single_cover(vector< vector<V_NeuronSWC_unit> >& mmUnit, unsigned char ***imap,  float *dmap, VISIOCYTELONG sz[3], double trace_z_thickness)
{
    visiocyte_msg("pruning_covered_leaf_single_cover()\n", 0);
	...
	for (j=0; j<tnodes; j++)
	{
		...
		if (mUnit[j].parent<0)
		{
			...
			root_id = VISIOCYTELONG(mUnit[j].n);
			printf("==================== nchild of root [%ld, id=%ld] = %ld\n", j, VISIOCYTELONG(mUnit[j].n), VISIOCYTELONG(mUnit[j].nchild));
		}
	}

	VISIOCYTELONG nleafdelete;
	VISIOCYTELONG nloops = 0;
	while (1)
	{
		nleafdelete = 0;
		nloops++;
		...
		for (i=0; i<tnodes; i++)
		{
			...
			//if (cur_margin > dmap[VISIOCYTELONG(curnode.z)*sz[0]*sz[1] + VISIOCYTELONG(curnode.y)*sz[0] + VISIOCYTELONG(curnode.x)]) cur_margin = dmap[VISIOCYTELONG(curnode.z)*sz[0]*sz[1] + VISIOCYTELONG(curnode.y)*sz[0] + VISIOCYTELONG(curnode.x)];
			//cur_margin = 0;
			while (tmpcnt<10) //to avoid loops
			{
				...
				pi = mUnit[j].parent;
				VISIOCYTELONG pi_rownum = index_map[pi];
				...
				//if (pi_radius < dmap[VISIOCYTELONG(curpnode.z)*sz[0]*sz[1] + VISIOCYTELONG(curpnode.y)*sz[0] + VISIOCYTELONG(curpnode.x)]) pi_radius = dmap[VISIOCYTELONG(curpnode.z)*sz[0]*sz[1] + VISIOCYTELONG(curpnode.y)*sz[0] + VISIOCYTELONG(curpnode.x)];
					...
					calculate_overlapping_ratio_n1(curnode, curpnode, imap,  sz, trace_z_thickness)>0.9)
				{
					...
					if (VISIOCYTELONG(mUnit[index_map[curnode.parent]].n)==root_id)
						printf("root nchild after decreasing 1 = %ld cur node id =%ld, cur node parent=%ld\n", VISIOCYTELONG(mUnit[index_map[curnode.parent]].nchild), VISIOCYTELONG(curnode.n), VISIOCYTELONG(curnode.parent));
					break; 
				}
...
VISIOCYTELONG pruning_covered_leaf_multi_covers(vector< vector<V_NeuronSWC_unit> >& mmUnit, unsigned char ***imap,  float *dmap, VISIOCYTELONG sz[3], double trace_z_thickness)
{
	visiocyte_msg("pruning_covered_leaf_multi_covers(). \n", 0);
    _CHECK_PRUNING_PARAMETERS_();
    
	VISIOCYTELONG nx=sz[0], ny=sz[1], nz=sz[2];
	...
	{
		visiocyte_msg("Fail to allocate memory in pruning_covered_leaf_multi_covers();\n", 0);
		if (maxr_img_3d) {delete3dpointer(maxr_img_3d, nx, ny, nz);};
		if (maxr_img_1d) {delete []maxr_img_1d; maxr_img_1d = 0;}
		if (maxid_img_3d) {delete3dpointer(maxid_img_3d, nx, ny, nz);};
		if (maxid_img_1d) {delete []maxid_img_1d; maxid_img_1d = 0;}
	...
	{
		V_NeuronSWC_unit & curnode = mUnit[i];
		b_pixelusedbynode_3d[VISIOCYTELONG(curnode.z)][VISIOCYTELONG(curnode.y)][VISIOCYTELONG(curnode.x)] = 1;

		double ndx   = mUnit[i].n;
		index_map[ndx] = i; //map the neuron node'd id to row number
	}
	
	std::deque< list<VISIOCYTELONG> > cover_list; //record for the pixel location the covering list
	std::map<VISIOCYTELONG,VISIOCYTELONG> index_map_pixelloc; //map the pixel location to the location in cover_list
	...
	for(i=0; i<tnodes; i++)
	{
		...
		VISIOCYTELONG i0 = curnx - curnode.r; PUT_IN_RANGE(0, i0, nx-1);
		VISIOCYTELONG	i1 = curnx + curnode.r; PUT_IN_RANGE(0, i1, nx-1);
		VISIOCYTELONG j0 = curny - curnode.r; PUT_IN_RANGE(0, j0, ny-1);
		VISIOCYTELONG	j1 = curny + curnode.r; PUT_IN_RANGE(0, j1, ny-1);
		VISIOCYTELONG k0 = curnz - curnode.r; PUT_IN_RANGE(0, k0, nz-1);
		VISIOCYTELONG	k1 = curnz + curnode.r; PUT_IN_RANGE(0, k1, nz-1);
			
		double d, r2 = curnode.r * curnode.r;
		for (VISIOCYTELONG k=k0;k<=k1;k++)
			for (VISIOCYTELONG j=j0;j<=j1;j++)
				for (VISIOCYTELONG ii=i0;ii<=i1;ii++)
				{
					...
					{
						list<VISIOCYTELONG> mylist_n;
						mylist_n.push_back(VISIOCYTELONG(curnode.n));  
					...
					{
						list<VISIOCYTELONG> & mylist_n = cover_list[ index_map_pixelloc[k*nx*ny+j*nx+ii] ];
						list<VISIOCYTELONG>::iterator it = mylist_n.begin();
						for ( it=mylist_n.begin() ; it != mylist_n.end(); it++ )
							if (curnode.r > mUnit[index_map[*it]].r)
							{
								mylist_n.insert(it, VISIOCYTELONG(curnode.n));
								break;
							}
	...
	while (1)
	{
		...
		for (i=0; i<tnodes; i++)
		{
			...
			double cur_maxid = maxid_img_3d[VISIOCYTELONG(curnode.z)][VISIOCYTELONG(curnode.y)][VISIOCYTELONG(curnode.x)];
			...
			list<VISIOCYTELONG> & mylist_n = cover_list[ index_map_pixelloc[VISIOCYTELONG(curnode.z)*nx*ny+VISIOCYTELONG(curnode.y)*nx+VISIOCYTELONG(curnode.x)] ];
			
			double tmpd = sqrt(distL2square(curnode, curmnode));
			
			//printf("curnode (n=%ld) curmnode (n=%ld) index_map best = [%ld]\n", VISIOCYTELONG(curnode.n), VISIOCYTELONG(curmnode.n), VISIOCYTELONG(mUnit[index_map[*mylist_n.begin()]].n)); 
...
bool cmp_VISIOCYTELONGTRIPLE(VISIOCYTELONGTRIPLE a, VISIOCYTELONGTRIPLE b)
{
	return (a.attr < b.attr) ? true : false;
}

VISIOCYTELONG pruning_covered_leaf_closebyfake_branches(vector< vector<V_NeuronSWC_unit> >& mmUnit, unsigned char ***imap,  float *dmap, VISIOCYTELONG sz[3], double trace_z_thickness, double imgTH)
{
	visiocyte_msg("pruning_covered_leaf_closebyfake_branches(). \n", 0);
    _CHECK_PRUNING_PARAMETERS_();
	...
	//create memory for flag and radius
	VISIOCYTELONG stacksz = nx*ny*nz;
	VISIOCYTELONG * ind_pixelusedbynode_1d = 0, *** ind_pixelusedbynode_3d = 0;
	try
	{
		ind_pixelusedbynode_1d = new VISIOCYTELONG [stacksz]; 
	...
	while (1)
	{
		VISIOCYTELONG npruned_branches = 0, npruned_curloop=0;
		...
		{
			V_NeuronSWC_unit & curnode = mUnit[i];
			ind_pixelusedbynode_3d[VISIOCYTELONG(curnode.z)][VISIOCYTELONG(curnode.y)][VISIOCYTELONG(curnode.x)] = curnode.n+1; //use +1 so that the indexing will have no problem for the 0th node
		...
		vector < vector <VISIOCYTELONG> > LeafBranchPath;
		list <VISIOCYTELONGTRIPLE> LeafArray;
		for(i=0; i<tnodes; i++)
		{
			if (mUnit[i].nchild!=0)
				continue;
			
			VISIOCYTELONG cur_ind = i;
			vector <VISIOCYTELONG> branchPath; branchPath.clear();
			while (1)
			{
				V_NeuronSWC_unit & curnode = mUnit[cur_ind];
				branchPath.push_back(curnode.n);
				
				VISIOCYTELONG pi = curnode.parent;
		...
		
		{
			...
			for (it=LeafArray.begin(), li=0; it!=LeafArray.end(); it++, li++)
			{
				...
				V_NeuronSWC_unit & curnode = mUnit[i]; 
				vector <VISIOCYTELONG> & curnodepathindex = LeafBranchPath[it->ind];
				
				if (curnode.nchild!=0) //do not start from a non-leaf node or start from an already deleted node
				{
					printf("li=%ld [node id=%ld] is not a leaf node. sth wrong. Check!\n", li, VISIOCYTELONG(curnode.n));
					continue;
				}
							...
							for (double dy= -ir; dy <= +ir; ++dy)
								for (double dx= -ir; dx <= +ir; ++dx)
								{
									...
									if (r>ir-1 && r<=ir)
									{
										...
										{
											VISIOCYTELONG actual_index = ind_pixelusedbynode_3d[ck][cj][ci] - 1; //-1 because originally + 1
											bool b_exist = false;
											for (VISIOCYTELONG tmpi=0;tmpi<curnodepathindex.size();tmpi++)
											{
											...
											if (b_exist==false) //then this is the nearest reconstruction that is also not on the current branch (and also include the branching point itself)
											{
												...
												for (VISIOCYTELONG tmpi=0; tmpi<dd; tmpi++)
												{
													if (p3d[VISIOCYTELONG(startz+stepz*tmpi+0.5)][VISIOCYTELONG(starty+stepy*tmpi+0.5)][VISIOCYTELONG(startx+stepx*tmpi+0.5)]<=imgTH)
													{
														b_allfg = false;
														break;
													}
													
													realdatavec.push_back(float(p3d[VISIOCYTELONG(startz+stepz*tmpi+0.5)][VISIOCYTELONG(starty+stepy*tmpi+0.5)][VISIOCYTELONG(startx+stepx*tmpi+0.5)]));
...
VISIOCYTELONG pruning_branch_nodes(vector< vector<V_NeuronSWC_unit> >& mmUnit, unsigned char ***imap,  float *dmap, VISIOCYTELONG sz[3], double trace_z_thickness)
{
    visiocyte_msg("pruning branch nodes.\n",0);
    _CHECK_PRUNING_PARAMETERS_();
	
	VISIOCYTELONG i,j;
	vector<V_NeuronSWC_unit> & mUnit = mmUnit.at(0);
	VISIOCYTELONG trows = mUnit.size();
	...
	vector < vector < VISIOCYTELONG > > childrenList; childrenList.clear();
	for (j=0; j<trows; j++)
	{
		vector<VISIOCYTELONG> tmp; tmp.clear();
	...
	//now prune
	VISIOCYTELONG nbranchnodedelete;
	VISIOCYTELONG nloops = 0;
	while (nloops<1)
	{
		nbranchnodedelete = 0;
		...
		for (i=0; i<trows; i++)
		{
			...
			double cur_margin = curnode.r; 
			for (VISIOCYTELONG k=0;k<curnode.nchild;k++)
			{
...
VISIOCYTELONG pruning_internodes(vector< vector<V_NeuronSWC_unit> >& mmUnit, unsigned char ***imap,  float *dmap, VISIOCYTELONG sz[3], double trace_z_thickness)
{
	visiocyte_msg("pruning_internodes(). \n", 0);
    _CHECK_PRUNING_PARAMETERS_();
	...
	vector <VISIOCYTELONG> leaf_branching_set;
	for (i=0; i<tnodes; i++)
	{
		V_NeuronSWC_unit & curnode = mUnit[i]; 
	...
	VISIOCYTELONG ncurdelete = 0; //# of nodes removed in the current loop
	for (j=0; j<leaf_branching_set.size(); j++)
	{
		i = leaf_branching_set[j];
		V_NeuronSWC_unit & curnode = mUnit[i]; 
		
		VISIOCYTELONG cur_row = i;
		while (1) //loop until a root or branching point has been reached
		{
			...
			VISIOCYTELONG parent_row = index_map[pi];
...
VISIOCYTELONG pruning_artifacial_branches(vector< vector<V_NeuronSWC_unit> >& mmUnit, unsigned char ***imap,  float *dmap, VISIOCYTELONG sz[3], double trace_z_thickness, double VISIBLE_THRESHOLD)
{
    visiocyte_msg("pruning_artifacial_branches()\n", 0);
    _CHECK_PRUNING_PARAMETERS_();
				...
				{ 
                    if (totalDarkEdgeLength > 30)
                    {
                        ...
                        while (k!=index_map[curnode.n])
                        {
                            ...
                            mynode.nchild = -1;
                            VISIOCYTELONG mypi = index_map[mynode.parent];
                            mUnit[mypi].nchild--;
                            nleafdelete++;

                            if (VISIOCYTELONG(mUnit[mypi].n)==root_id)
                                printf("root nchild after decreasing 1 = %ld cur node id =%ld, cur node parent=%ld\n", VISIOCYTELONG(mUnit[index_map[curnode.parent]].nchild), VISIOCYTELONG(curnode.n), VISIOCYTELONG(curnode.parent));
