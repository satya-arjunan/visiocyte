...
bool proc_app2(VISIOCYTEPluginCallback2 &callback, PARA_APP2 &p, const QString & versionStr)
{
  //  bool b_menu = true;
    bool b_dofunc = false;
    if (!p.p4dImage || !p.p4dImage->valid())
    {
        ...
        for(int i = 0; i < file_inmarkers.size(); i++)
        {
            ...
            if(t.x<p.xc0+1 || t.x>p.xc1+1 || t.y<p.yc0+1 || t.y>p.yc1+1 || t.z<p.zc0+1 || t.z>p.zc1+1)
            {
                if(i==0)
                {
                    visiocyte_msg("The first marker is invalid.",p.b_menu);
                    return false;
                }
                else continue;
            }
    ...
    {
       visiocyte_msg("You have to select at least two markers if using high intensity background option.",p.b_menu);
       return false;
    }
    ...
    tmpstr =  qPrintable( qtstr.prepend("##Visiocyte-Neuron-APP2 ").append(versionStr) ); infostring.push_back(tmpstr);
    ...
    //p4dImageNew->saveImage(qPrintable(outtmpfile));  visiocyte_msg(QString("save immediate input image to ") + outtmpfile, 0);
...
//            p.bkg_thresh = imgAve; //+0.5*imgStd ; //(imgAve < imgStd)? imgAve : (imgAve+imgStd)*.5;
    ...
    if(inmarkers.empty())
    {
        ...
        switch(datatype)
        {
            case VISIOCYTE_UINT8:
                ...
                break;
            case VISIOCYTE_UINT16:  //this is no longer needed, as the data type has been converted above
                fastmarching_dt_XY((short int*)indata1d, phi, in_sz[0], in_sz[1], in_sz[2],p.cnn_type, p.bkg_thresh);
                break;
        default:
            visiocyte_msg("Unsupported data type");
            break;
        }
    ...
    {
        visiocyte_msg("start to use APP2 program.\n", 0);
    ...
    if(1)
    {
        ...
        if(p.b_brightfiled)
        {
            for(VISIOCYTELONG i = 0; i < p.p4dImage->getTotalUnitNumberPerChannel(); i++)
...
bool PARA_APP2::fetch_para_commandline(const VISIOCYTEPluginArgList &input, VISIOCYTEPluginArgList &output, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
