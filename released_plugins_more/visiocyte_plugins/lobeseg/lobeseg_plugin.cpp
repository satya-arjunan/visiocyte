 ...
 * June 28, 2011 : by Hanchuan Peng: add the info of the paper, etc. Put in the visiocyte_released plugins folder
...
void LobesegPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
		visiocyte_msg(tr("This plugin segments the two optic lobes (OLs) and the center brain (CB) of  "
...
bool LobesegPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
	if (func_name == tr("lobeseg"))
	{
        return lobeseg(callback, input, output);
	}
