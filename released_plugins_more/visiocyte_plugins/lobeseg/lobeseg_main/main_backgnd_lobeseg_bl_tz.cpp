...
#include "../../../../visiocyte_main/basic_c_fun/basic_memory.h"
...
int main (int argc, char *argv[])
{
	...
	VISIOCYTELONG * sz_input = 0; // note that this variable must be initialized as NULL.
	//VISIOCYTELONG sz_output[2];
	...
	//prepare the data space for output
	{
		...
		img_output=0;
		img_output = new unsigned char [(VISIOCYTELONG)sz_input[0]*sz_input[1]*sz_input[2]*3];
		...
		for (VISIOCYTELONG z=0; z<sz_input[2]; z++)
		{
			for (VISIOCYTELONG j=0; j<sz_input[1]; j++)
			{
				for (VISIOCYTELONG i=0; i<sz_input[0]; i++)
				{
					for (VISIOCYTELONG cc=0;cc<2; cc++)
		...
		// 2d xz output
		img_output=0;
		img_output = new unsigned char [(VISIOCYTELONG)sz_input[0]*sz_input[2]];
...
vector<Coord2D> readControlPointFile2d(string posFile)
{
	...
	VISIOCYTELONG xpos, ypos;  xpos=ypos=-1;//set as default
	VISIOCYTELONG k=0;
	while(!file_op.eof())
	{
