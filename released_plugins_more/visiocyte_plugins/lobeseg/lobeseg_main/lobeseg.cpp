...
template <class T> bool maskImageUsingSegBoundary(T *outimg1d, VISIOCYTELONG sz[4], int in_channel_no, int out_channel_no, int ** left_bound, int ** right_bound)
{
...
bool do_lobeseg_bdbminus(unsigned char *inimg1d0, const VISIOCYTELONG sz[4], int datatype_input, unsigned char *outimg1d, int in_channel_no, int out_channel_no, const BDB_Minus_ConfigParameter & mypara)
...
bool do_lobeseg_bdbminus_onesideonly(unsigned char *inimg1d, const VISIOCYTELONG sz[4], int datatype_input, unsigned char *outimg1d, int in_channel_no, int out_channel_no, const BDB_Minus_ConfigParameter & mypara, int ini_x1, int ini_y1, int ini_x2, int ini_y2, int keep_which, int num_ctrls,bool output_surface)
{
...
///	mCoord_Right.push_back(pt);
	...
	for (z=0; z<sz[2]; z++)
	{
		...
		for (k=0; k<kk-1; k++)
		{
			...
			{
				j = j1;
				for(VISIOCYTELONG tmpi = i1; tmpi <= i2; tmpi++) the_bounds[z][j].insert(tmpi);
			}
		}
		//printf("%d ", z);
	}
