...
#include "../../../released_plugins/visiocyte_plugins/resample_swc/resampling.h"
...
void standardize::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    if (menu_name == tr("standardize"))
	{
        visiocyte_msg("Only works on dofunc for now.");
	}
	else
	{
        visiocyte_msg(tr("Developed by Xiaoxiao Liu, 2016-5-28"));
	}
}

bool standardize::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
//        NeuronTree nt_input_rs = resample(nt_input, dis_th);
        ...
        for (VISIOCYTELONG i = 0; i < nt_ref.listNeuron.size(); i++)
        {
        ...
        VISIOCYTELONG neuronNum = nt_input.listNeuron.size();
        QVector<QVector<VISIOCYTELONG> > children_list = QVector< QVector<VISIOCYTELONG> >(neuronNum, QVector<VISIOCYTELONG>() );

        for (VISIOCYTELONG i = 0; i < neuronNum; i++)
        {
            VISIOCYTELONG parent = nt_input.listNeuron[i].pn;
            if (parent < 0)
                continue;
        ...
        VISIOCYTELONG soma_ID = -1;
        VISIOCYTELONG dist_ID = -1;
        int child_num = 0;
       ...
       // set the distance threshold to searching for matching soma node
        ...
        for (VISIOCYTELONG i=0;i<list_neurons.size();i++)
        {
        ...
        //skip the first root
        for (VISIOCYTELONG i = 1;i<result.size();i++)
        {
            if (result[i].pn == -1)
	...
	else if (func_name == tr("help"))
	{
        ...
        cout << "Usage : <visiocyte> -x standardize -f standardize -i <reference_swc_file> <inswc_file> -o <outswc_file> -p <gap_threshold>  <new_neurite_type> "<<endl;
	}
    else
        return false;
