...
void ChannelSplitPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    {
         visiocyte_msg("This plugin is developed by Hanchuan Peng as a simple example for splitting or extracting color channels of an image.");
    }
}
...
bool ChannelSplitPlugin::dofunc(const QString &func_name, const VISIOCYTEPluginArgList &input, VISIOCYTEPluginArgList &output, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
		cout<<"Usage 1: visiocyte -x channelsplit -f split_extract_channels -i <inimg_file> -o <outimg_file> -p <channel>"<<endl;
...
bool processImage(VISIOCYTEPluginCallback2 &callback, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output)
{
    ...
    if (c<0)
    {
        visiocyte_msg("You explicitly or implicitly specified a channel number < 0, which means split-all-channels. \n",0);
    }
    ...
    for (k=cb; k<=ce; k++)
    {
        ...
        switch(datatype)
        {
            ...
            default:
            b_res = false;
            visiocyte_msg("Right now this plugin supports only UINT8/UINT16/FLOAT32 data. Do nothing."); goto Label_exit;
        }

        // save image
        VISIOCYTELONG oldc = in_sz[3]; in_sz[3]=1;
        if (cb==ce)
            simple_saveimage_wrapper(callback, outimg_file, (unsigned char *)outimg, in_sz, datatype);
        else
            simple_saveimage_wrapper(callback,
                                     qPrintable(QString("").setNum(k).prepend("_C").prepend(outimg_file).append(".visiocyteraw")),
