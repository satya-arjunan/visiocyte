...
void GrayScaleDistanceTransformationPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
		visiocyte_msg(tr("Perform distance transformation on grayscale image.. "
			"Developed by Hang Xiao, 2012-03-02"));
	}
}

bool GrayScaleDistanceTransformationPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
	...
	{
		cout<<"Usage : visiocyte -x gsdt -f gsdt -i <inimg_file> -o <outimg_file> -p <bkg_thresh_value> <cnn_type> <channel> <z_thickness>"<<endl;
		cout<<endl;
		...
		cout<<endl;
		cout<<"e.g. visiocyte -x gsdt -f gsdt -i input.raw -o output.raw -p 0 1 0 1.0"<<endl;
		cout<<endl;
		return true;
	}
}
