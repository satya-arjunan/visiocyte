...
bool ref_aligning(Tdata *p1dImg1, Tdata *p1dImg2, VISIOCYTELONG *szImg, VISIOCYTELONG *outputoffsets)
{
    ...
    // anchor position
	REAL pos_score = 0;
	VISIOCYTELONG pos_x = sx, pos_y = sy, pos_z = sz;
...
//
bool stitch_paired_images_with_refchan(Image4DSimple &p4DImage1, VISIOCYTELONG ref1, Image4DSimple &p4DImage2, VISIOCYTELONG ref2)
{
	...
	for(VISIOCYTELONG c=0; c<szImg[3]; c++)
	{
		...
		for(VISIOCYTELONG k=k_start; k<sz_end; k++)
		{
            ...
            VISIOCYTELONG offset_k_t = offset_c + (k-k_start)*szImg[1]*szImg[0];
			for(VISIOCYTELONG j=j_start; j<sy_end; j++)
			{
				VISIOCYTELONG offset_j = offset_k + j*szImg[0];
                VISIOCYTELONG offset_j_t = offset_k_t + (j-j_start)*szImg[0];
...
bool ImageBlendPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & visiocyte, QWidget * parent)
{
     ...
     {
          visiocyte_msg("\nUsage: visiocyte -x blend_multiscanstacks -f multiscanblend -i <input_images> -o <output_image> -p \"#s <save_blending_result zero(false)/nonzero(true)> #k <b_morecolorstack_first nonzero(true)/zero(false)>\" \n", 0);
     ...
     else if (func_name == tr("multiscanblend"))
	{
        ...
        // load images
        Image4DSimple *p4DImage1 = visiocyte.loadImage((char *)qPrintable(m_InputFileName1)),
                *p4DImage2 = visiocyte.loadImage((char *)qPrintable(m_InputFileName2));
...
//        }
        ...
        if(datatype_img1 == VISIOCYTE_UINT8)
        {
            ...
            {
                //save
                if (simple_saveimage_wrapper(visiocyte, blendImageName.toStdString().c_str(), pOutput, szOutput, 1)!=true)
                {
        ...
        else if(datatype_img1 == VISIOCYTE_UINT16)
        {
            ...
            {
                //save
                if (simple_saveimage_wrapper(visiocyte, blendImageName.toStdString().c_str(), (unsigned char *)pOutput, szOutput, 2)!=true)
                {
                    fprintf(stderr, "Error happens in file writing. Exit. \n");
                    return false;
                }
        ...
        else if(datatype_img1 == VISIOCYTE_FLOAT32) //this section need further improvement, maybe discretization. by PHC, 110810
        {
            ...
            {
                //save
                if (simple_saveimage_wrapper(visiocyte, blendImageName.toStdString().c_str(), (unsigned char *)pOutput, szOutput, 4)!=true)
                {
                    fprintf(stderr, "Error happens in file writing. Exit. \n");
                    return false;
                }
