...
void ROI_Editor_Plugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
		visiocyte_msg("Edit Regions of Interest (ROIs) of the image of the current window: copy ROIs from another image or [paste] them to another image; or delete some of them.");
		return;
	}
}
     ...
     else if (method_code == 7)
	{
		...
		printf("%d %d \n",pRoiList[0].size(),win_list.size());
          ...
          if( !filename.isNull() )
          {
               ...
               if (!fp)
               {
                    visiocyte_msg(QString("Fail to open file %1 to write.").arg(filename));
                    return;
               }
