...
bool q_dfblcokinterp_linear(DisplaceFieldF3D ***&pppSubDF,
		const VISIOCYTELONG szBlock_x,const VISIOCYTELONG szBlock_y,const VISIOCYTELONG szBlock_z,
		const VISIOCYTELONG substart_x,const VISIOCYTELONG substart_y,const VISIOCYTELONG substart_z,
...
bool q_dfblcokinterp_bspline(DisplaceFieldF3D ***&pppSubDF,const Matrix &x_bsplinebasis,
		const VISIOCYTELONG sz_gridwnd,const VISIOCYTELONG substart_x,const VISIOCYTELONG substart_y,const VISIOCYTELONG substart_z,
		DisplaceFieldF3D ***&pppDFBlock)
{
...
//use 3d or 4d pointer instead of 1d, since generating 3d or 4d pointer from 1d is time consuming
template <class T>
bool q_imgblockwarp(T ****&p_img_sub_4d,const VISIOCYTELONG *sz_img_sub,DisplaceFieldF3D ***&pppDFBlock,
		const VISIOCYTELONG szBlock_x,const VISIOCYTELONG szBlock_y,const VISIOCYTELONG szBlock_z,const int i_interpmethod_img,
		const VISIOCYTELONG substart_x,const VISIOCYTELONG substart_y,const VISIOCYTELONG substart_z,
		T ****&p_img_warp_4d)
{
	VISIOCYTELONG start_x,start_y,start_z;
	start_x=substart_x*szBlock_x;
	start_y=substart_y*szBlock_y;
	start_z=substart_z*szBlock_z;
	for(VISIOCYTELONG z=0;z<szBlock_z;z++)
		for(VISIOCYTELONG y=0;y<szBlock_y;y++)
			for(VISIOCYTELONG x=0;x<szBlock_x;x++)
			{
				VISIOCYTELONG pos_warp[3];
				...
				{
					for(VISIOCYTELONG c=0;c<sz_img_sub[3];c++)
...
template <class T>
bool imgwarp_smallmemory(const T *p_img_sub,const VISIOCYTELONG *sz_img_sub,
		const QList<ImageMarker> &ql_marker_tar,const QList<ImageMarker> &ql_marker_sub,
		VISIOCYTELONG szBlock_x, VISIOCYTELONG szBlock_y, VISIOCYTELONG szBlock_z, int i_interpmethod_df,int i_interpmethod_img,
		T *&p_img_warp)
{
	//check parameters
	...
	for (VISIOCYTELONG i=0;i<ql_marker_tar.size();i++)
	{
		Coord3D_JBA tmpc;
	...
	{
		for(VISIOCYTELONG substart_z=0;substart_z<pSubDF->sz2()-1;substart_z++)
			for(VISIOCYTELONG substart_y=0;substart_y<pSubDF->sz1()-1;substart_y++)
				for(VISIOCYTELONG substart_x=0;substart_x<pSubDF->sz0()-1;substart_x++)
				{
					//linear interpolate the SubDfBlock to DFBlock
	...
	else						//bspline interpolate the SubDfBlock to DFBlock and do warp block by block
	{
		...
		VISIOCYTELONG sz_gridwnd=szBlock_x;
		...
		for(VISIOCYTELONG substart_z=0;substart_z<pSubDF->sz2()-1-2;substart_z++)
			for(VISIOCYTELONG substart_y=0;substart_y<pSubDF->sz1()-1-2;substart_y++)
				for(VISIOCYTELONG substart_x=0;substart_x<pSubDF->sz0()-1-2;substart_x++)
				{
...
template <class T>
bool imgwarp_smallmemory_padding(const T *p_img_sub,const VISIOCYTELONG *sz_img_sub,
		const QList<ImageMarker> &ql_marker_tar,const QList<ImageMarker> &ql_marker_sub,
		VISIOCYTELONG szBlock_x, VISIOCYTELONG szBlock_y, VISIOCYTELONG szBlock_z,int i_interpmethod_img,
		T *&p_img_warp)
{
	//check parameters
	if(p_img_sub==0 || sz_img_sub==0)
	...
	VISIOCYTELONG sz_img_sub_padding[4],offset_padding[3];
	sz_img_sub_padding[3]=sz_img_sub[3];
	for(int i=0;i<3;i++)
	{
		sz_img_sub_padding[i]=VISIOCYTELONG(ceil(double(sz_img_sub[i])/32))*32;
	...
	for(VISIOCYTELONG c=0;c<sz_img_sub[3];c++)
		for(VISIOCYTELONG z=0;z<sz_img_sub[2];z++)
			for(VISIOCYTELONG y=0;y<sz_img_sub[1];y++)
				for(VISIOCYTELONG x=0;x<sz_img_sub[0];x++)
				...
				for(VISIOCYTELONG z=0;z<szBlock_z;z++)
					for(VISIOCYTELONG y=0;y<szBlock_y;y++)
						for(VISIOCYTELONG x=0;x<szBlock_x;x++)
						{
							...
							{
								for(VISIOCYTELONG c=0;c<sz_img_sub_padding[3];c++)
...
template <class T>
bool imgresize_padding(T *&p_img,VISIOCYTELONG *sz_img,
		VISIOCYTELONG sz_resize_x, VISIOCYTELONG sz_resize_y, VISIOCYTELONG sz_resize_z)
{
	//check parameters
	if(p_img==0 || sz_img==0)
	{
	...
	for(VISIOCYTELONG z=0;z<sz_resize_z;z++)
		for(VISIOCYTELONG y=0;y<sz_resize_y;y++)
			for(VISIOCYTELONG x=0;x<sz_resize_x;x++)
				for(VISIOCYTELONG c=0;c<sz_img[3];c++)
				{
