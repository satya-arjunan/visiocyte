...
void printHelp();
bool littlequickwarp(VISIOCYTEPluginCallback2 & callback,  QString qs_filename_img_sub,QString qs_filename_marker_sub,QString qs_filename_marker_tar,
		int i_interpmethod_df,int i_interpmethod_img,bool b_padding,bool b_resizeimg,VISIOCYTELONG sz_resize_x,VISIOCYTELONG sz_resize_y,VISIOCYTELONG sz_resize_z,
...
void LittleQuickWarpPlugin::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    if (menu_name == tr("littlequickwarp"))
	{
    	...
    	VISIOCYTELONG sz_resize_x=DLG_littlequickwarp.lineEdit_Xdim->text().toLong();
    	VISIOCYTELONG sz_resize_y=DLG_littlequickwarp.lineEdit_Ydim->text().toLong();
    	VISIOCYTELONG sz_resize_z=DLG_littlequickwarp.lineEdit_Zdim->text().toLong();
    	//do warping
    	...
    	{
    		visiocyte_msg(tr("ERROR: littlequickwarp() return false!"));
    		return;
    	}
	}
	else
	{
		visiocyte_msg(tr("Warp image based on given markers (fast and with small memory consumption). "
			"Developed by Lei Qu, 2012-07-16"));
	}
}
bool LittleQuickWarpPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
	if (func_name == tr("littlequickwarp"))
	{
		...
		{
			visiocyte_msg(tr("ERROR: no enough para!"));
			printHelp();
			return false;
		}
		...
		VISIOCYTELONG sz_resize_x=atoi(paras.at(4));
		VISIOCYTELONG sz_resize_y=atoi(paras.at(5));
		VISIOCYTELONG sz_resize_z=atoi(paras.at(6));
...
void printHelp()
{
    printf("\nUsage: visiocyte -x <littlequickwarp> -f littlequickwarp -i <input_image_sub> <input_marker_sub> <input_marker_tar> -o <output_image_file> -p interpmethod_df interpmethod_img dopadding doresize newsize_x newsize_y newsize_z\n");
    ...
    printf("Demo :\t visiocyte -x littlequickwarp -f littlequickwarp -i /Users/qul/Desktop/testdata/output_global.tiff /Users/qul/Desktop/testdata/output_subject.marker /Users/qul/Desktop/testdata/output_target.marker -o /Users/qul/Desktop/testdata/output_warp_littlequick.visiocyteraw -p 1 0 0 0 1 1 1\n");
    return;
}

bool littlequickwarp(VISIOCYTEPluginCallback2 & callback,  QString qs_filename_sub,QString qs_filename_marker_sub,QString qs_filename_marker_tar,
		int i_interpmethod_df,int i_interpmethod_img,bool b_padding_img,bool b_resizeimg,VISIOCYTELONG sz_resize_x,VISIOCYTELONG sz_resize_y,VISIOCYTELONG sz_resize_z,
		QString qs_filename_warp)
{
	...
	VISIOCYTELONG time_total_start,time_warp_start;
	...
	VISIOCYTELONG szBlock_x,szBlock_y,szBlock_z;
