...
template <class T> bool getHistogram(const T * pdata1d, VISIOCYTELONG datalen, double max_value, VISIOCYTELONG & histscale, QVector<int> &hist)
{
	// init hist
	...
	{
		VISIOCYTELONG ind = pdata1d[i]/max_value * histscale;
		//VISIOCYTELONG ind = pdata1d[i];
		hist[ind] ++;
	}
...
int compute(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	if(!curwin)
	{
		visiocyte_msg("No VISIOCYTE window is available for returning data ... Do nothing.", 0);
		return -1;
	}
	...
	{
		visiocyte_msg("Now we only support 8 bit image.\n");
		return -1;
	}

	//TODO add datatype judgment
	double max_value = 256;
	VISIOCYTELONG histscale = 256;
