...
typedef vector< DF<REAL, VISIOCYTELONG> > DFList;
...
public:
    ...
    void y_save(string fn) //save virtual image
    {
        ...
        VISIOCYTELONG number_tiles = tcList.size();
        //
...
// define a point type
typedef _POINT<VISIOCYTELONG, float> VISIOCYTE_POINT;
...
public:
    ...
    std::vector<VISIOCYTE_POINT> offsetsList;
...
public:
        ...
        if(pFileLUT.is_open())
        {
            ...
            if(!pFileLUT.eof()) // compatible with old .tc file
            {
                ...
                if(!pFileLUT.eof())
                {
                    ...
                    {
                        for(VISIOCYTELONG i=0; i<number_tiles; i++)
                        {
                            indexed_t t;  //
    ...
    // point navigation
    VISIOCYTE_POINT y_navigate(VISIOCYTE_POINT p)
    {
        return p;
    }
...
// usually T1 is assigned as unsigned char (uint8) and T2 VISIOCYTELONG (unsigned VISIOCYTELONG int)
template <class T1, class T2>
class Y_IMAGE
{
public:
...
// define Y_IMG types
typedef Y_IMAGE<REAL, VISIOCYTELONG> Y_IMG_REAL;
typedef Y_IMAGE<unsigned char, VISIOCYTELONG> Y_IMG_UINT8;
typedef Y_IMAGE<unsigned short, VISIOCYTELONG> Y_IMG_UINT16;
...
// YImg
// usually T1 is assigned as double and T2 VISIOCYTELONG (unsigned VISIOCYTELONG int)
...
template<class T>
bool computeWeights(Y_VIM<REAL, VISIOCYTELONG, indexed_t<VISIOCYTELONG, REAL>, LUT<VISIOCYTELONG> > vim, VISIOCYTELONG i, VISIOCYTELONG j, VISIOCYTELONG k, VISIOCYTELONG tilei, T &weights)
{
    ...
    VISIOCYTELONG sz_img = vx*vy*vz;

    QList<T> listWeights;
    listWeights.clear();

    VISIOCYTELONG numtile;
...
template <class Tdata>
bool subpixshift3D(REAL *&pShift, Tdata *pImg, VISIOCYTELONG *szImg, rPEAKS pos, VISIOCYTELONG *&effectiveEnvelope)
{
    // output subpixel translated image: pShift
    // output effective envelope of the image: effectiveEnvelope[6] x,y,z
    //
    VISIOCYTELONG vx = szImg[0];
    VISIOCYTELONG vy = szImg[1];
    VISIOCYTELONG vz = szImg[2];
    VISIOCYTELONG vc = szImg[3];
    VISIOCYTELONG pagesz = vx*vy*vz;
    VISIOCYTELONG sz_img = pagesz*vc;
    //
    memset(pShift, 0.0, sizeof(Tdata)*sz_img); // filled by zeros
    //
    REAL xd = 0.0, yd = 0.0, zd = 0.0;
    VISIOCYTELONG x_start=0, y_start=0, z_start=0; // pImg
    VISIOCYTELONG xs_start=0, ys_start=0, zs_start=0; // pShift
    VISIOCYTELONG x_end=vx, y_end=vy, z_end=vz;
    if(pos.x<0)
    {
    ...
    VISIOCYTELONG xp = 1;
    VISIOCYTELONG yp = vx;
    VISIOCYTELONG zp = vx*vy;
    REAL i1=0, i2=0, j1=0, j2=0, w1=0, w2=0;
    for(VISIOCYTELONG c=0; c<vc; c++)
    {
        VISIOCYTELONG offset_c = c*pagesz;
        for(VISIOCYTELONG k=zs_start; k<z_end; k++)
        {
            if(k-zs_start+z_start>=vz) continue;
            VISIOCYTELONG offset_ks = offset_c + k*vx*vy;
            VISIOCYTELONG offset_k = offset_c + (k-zs_start+z_start)*vx*vy;
            for(VISIOCYTELONG j=ys_start; j<y_end; j++)
            {
                if(j-ys_start+y_start>=vy) continue;
                VISIOCYTELONG offset_js = offset_ks + j*vx;
                VISIOCYTELONG offset_j = offset_k + (j-ys_start+y_start)*vx;
                for(VISIOCYTELONG i=xs_start; i<vx; i++)
                {
                    if(i-xs_start+x_start>=vx) continue;
                    VISIOCYTELONG idxs = offset_js + i;
                    VISIOCYTELONG idx = offset_j + i-xs_start+x_start;
