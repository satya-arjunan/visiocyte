...
int pairwise_stitching(char *fn_target, VISIOCYTELONG ch_tar, char *fn_subject, VISIOCYTELONG ch_sub, char *fn_stitched, char *fn_output_cs, bool flag_cmpt_ncc, PEAKS *&pos)
{
    ...
    VISIOCYTELONG sz_subject[4], sz_target[4];
...
// pairwise image blending function
template <class SDATATYPE> int groupi_fusing(VISIOCYTEPluginCallback2 &callback,SDATATYPE *pVImg, Y_VIM<REAL, VISIOCYTELONG, indexed_t<VISIOCYTELONG, REAL>, LUT<VISIOCYTELONG> > vim, VISIOCYTELONG vx, VISIOCYTELONG vy, VISIOCYTELONG vz, VISIOCYTELONG vc, bool axes_show);

// subspace stitching and linear blending tiles
template <class Tdata, class Y_IMG_DATATYPE> bool iSubspaceStitching(VISIOCYTEPluginCallback2 &callback,Tdata *pVImg, Y_VIM<REAL, VISIOCYTELONG, indexed_t<VISIOCYTELONG, REAL>, LUT<VISIOCYTELONG> > vim, Tdata intensityrange, VISIOCYTELONG channel, bool flagFusion);
...
template <class SDATATYPE>
int groupi_fusing(VISIOCYTEPluginCallback2 &callback,SDATATYPE *pVImg, Y_VIM<REAL, VISIOCYTELONG, indexed_t<VISIOCYTELONG, REAL>, LUT<VISIOCYTELONG> > vim, VISIOCYTELONG vx, VISIOCYTELONG vy, VISIOCYTELONG vz, VISIOCYTELONG vc, bool axes_show)
{
...
bool ifusing(VISIOCYTEPluginCallback2 &callback,SDATATYPE *pVImg, Y_TLUT tlut, SDATATYPE intensityrange)
{
    //
...
int group_stitching_wc(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    VISIOCYTELONG row_i, row_j, col_i, col_j, height_i, height_j;
    if(!m_FileName.isEmpty())
    {
        b_withconfig = true;

        ifstream pFile(m_FileName.toStdString().c_str());
        string str;
    ...
    QString m_InputFileName = QString(cinfo.fn_img.c_str());
    ...
    else
    {
        ...
        for(int i=0; i<NTILES; i++) // traverse all tiled images
        {
            ...
            VISIOCYTELONG current = vim.tilesList[i].n;
            if(vim.tilesList.at(current).visited) // computed
            {
                continue;
            }
            ...
            if(vim.tilesList.at(i).hasedge)
            {
                ...
                {
                    VISIOCYTELONG previous = vim.tilesList[i].preList[j];
                    ...
                    //
                    pos->x = vim.tilesList.at(current).offsetsList[j].x + (sx-1);
                    pos->y = vim.tilesList.at(current).offsetsList[j].y + (sy-1);
                    ...
                    // update offsets of current to previous
                    (&vim.tilesList.at(current))->offsetsList[j].x = pos->x - sx +1;
    ...
    {
        vim.tilesList.at(i).visited = false;
        vim.tilesList.at(i).predecessor = 0;
    }
    vim.tilesList[0].visited = true;
    vim.tilesList[0].predecessor = -1;

    while(vim.y_ks(vim.tilesList))
    {
        for(int i=1; i<NTILES; i++)
        {
            ...
            for(int j=0; j<vim.tilesList.at(i).preList.size(); j++)
            {
                VISIOCYTELONG pre = vim.tilesList[i].preList[j];
            ...
            for(int j=i+1; j<NTILES; j++)
            {
                ...
                if(vim.tilesList[j].predecessor==-1)
                {
                    ...
                    {
                        if(vim.tilesList[j].preList[k]==i)
                        {
...
int region_navigating(VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    if(m_FileName.isEmpty())
    {
        visiocyte_msg("The file name is NULL. Do nothing.");
        return -1;
    }
    ...
    if( !vim.y_load(filename) )
    {
        visiocyte_msg("Fail to load the virtual image.");
        return -1;
    }
...
//        if( (!lut_ss.any() && lut_ee.any()) || (lut_es.any() && !lut_ee.any()) || (lut_ss.any() && !lut_se.any()) )
    ...
    //display
    Image4DSimple p4DImage;
    VISIOCYTELONG dtmp[4]; dtmp[0] = vx; dtmp[1] = vy; dtmp[2] = vz; dtmp[3] = vc;

    if(datatype == VISIOCYTE_UINT8)
    {
      //  simple_saveimage_wrapper(callback, "test1.visiocyteraw", pVImg_UINT8, dtmp, datatype);
        p4DImage.setData((unsigned char*)pVImg_UINT8, vx, vy, vz, vc, datatype);
    }
    else if(datatype == VISIOCYTE_UINT16)
    {
      //  simple_saveimage_wrapper(callback, "test1.visiocyteraw", (unsigned char*)pVImg_UINT16, dtmp, datatype);
        p4DImage.setData((unsigned char*)pVImg_UINT16, vx, vy, vz, vc, datatype);
    }
    else if(datatype == VISIOCYTE_FLOAT32)
    {
      //  simple_saveimage_wrapper(callback, "test1.visiocyteraw", (unsigned char*)pVImg_FLOAT32, dtmp, datatype);
...
bool IStitchPlugin::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    ...
    else if (func_name == tr("visiocytestitch"))
    {
        ...
        {
            //print Help info
            printf("\nUsage: visiocyte -x imageStitch -f visiocytestitch -i <input_image_folder> -o <output_image_file> -p \"[#c <channalNo_reference> #x <downsample_factor_x> #y <downsample_factor_y> #z <downsample_factor_z> #l <overlap_ratio>] #sb <saving_tile_boundary 0/1> #si <saving_stitching_result 0/1> #o <orders_of_tiles>\"\n");
        ...
        //REAL* scale = NULL;
        //y_new<REAL, VISIOCYTELONG>(scale, 6);
...
//                                qDebug()<<" ... z ... "<<(&vim.tilesList.at(i))->offsets[2]<<" = "<<-vim.tilesList[j].offsetsList[k].z<<" + "<<vim.tilesList[j].offsets[2];
    ...
    else if (func_name == tr("istitch-gc"))
    {
        ...
        if(imgdatatype == VISIOCYTE_UINT8)
        {
            ...
            if(pVImg)
            {
                ...
                unsigned char intensityrange = 255;
                bool success = ifusing<unsigned char, Y_TLUT<REAL, VISIOCYTELONG, DF<REAL, VISIOCYTELONG> > >(callback,(unsigned char *)pVImg, tlut, intensityrange);
                if(!success)
                {
        ...
        else if(imgdatatype == VISIOCYTE_UINT16)
        {
            ...
            if(pVImg)
            {
                ...
                unsigned short intensityrange = 4095;
                bool success = ifusing<unsigned short, Y_TLUT<REAL, VISIOCYTELONG, DF<REAL, VISIOCYTELONG> > >(callback,(unsigned short *)pVImg, tlut, intensityrange);
                if(!success)
                {
                    printf("Fail to call function ifusing! \n");
                    return false;
    ...
    else if (func_name == tr("istitch-grouping"))
    {
        ...
        {
            //print Help info
            printf("\nUsage: visiocyte -x imageStitch.dylib -f istitch-grouping -i <input_image_folder> -o <output_file> -p \"#c <channalNo_reference>\"\n");
...
//        }
        ...
        for(VISIOCYTELONG i=0; i<NTILES; i++)
        {
            VISIOCYTELONG cur = vim.tilesList[i].n;
            //
            for(VISIOCYTELONG j=0; j<vim.tilesList[i].preList.size(); j++)
            {
                VISIOCYTELONG pre = vim.tilesList[i].preList[j];
        ...
        vector<int> sg,rg;
        for(VISIOCYTELONG i=0; i<NTILES; i++) rg.push_back(i);

        for(VISIOCYTELONG i=0; i<rg.size(); i++)
        {
            ...
            bool ccfound = true;
            VISIOCYTELONG cur, pre;

            while(ccfound)
            {
                int sgsz = sg.size();
                for(VISIOCYTELONG k=0; k<sg.size(); k++)
                {
                    pre = sg[k];
                    for(VISIOCYTELONG j=0; j<rg.size(); j++)
                    {
                        cur = rg[j];
