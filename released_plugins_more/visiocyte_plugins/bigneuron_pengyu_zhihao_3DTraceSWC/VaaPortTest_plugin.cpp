...
void VaaPortTest::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	bool* result = NULL;
	double* para = NULL;
	if (menu_name == tr("3DTrace"))
	{
		...
		if (!curwin)
		{
			visiocyte_msg("no image open");
			return;
		}
		para = VaaPortTest::getPara(parent);
		Image4DSimple *p4DImage = callback.getImage(curwin);
                string fileDir("Visiocyte_Domenu_pyzh.swc");
		...
		new_img->setData(new_image_data, sz[0], sz[1], sz[2], 1, VISIOCYTE_UINT8);//setData() will free the original memory automatically
		printf("Finish SetData\n");
		//callback.setImage(newwin, new_img);
		//callback.setImageName(newwin, QObject::tr("Trace3D"));
		//callback.updateImageWindow(newwin);
		//simple_saveimage_wrapper(callback, "trace3D_result.visiocyteraw", (unsigned char *)new_image_data, sz, VISIOCYTE_UINT8);
		delete []para;
		visiocyte_msg("Tracing Finished.");
	}
...
bool VaaPortTest::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
	double para[3]; 
	bool* result = NULL;
	...
	{
		printf("====Usage====");
                printf("./start_visiocyte.sh -x aVaaTrace3D -f func1 -p \"min_soma_size max_neurite_width neurite_width_multiplier\" -i input_image");
                printf("Sample usage: ./start_visiocyte.sh -x aVaaTrace3D -f func1 -p \"100 5 2\" -i input_image");
