...
void MovieConverter::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
    ...
    {
        visiocyte_msg(tr("Movie Format Converter (from sequence of frames to a single movie file). "
...
bool MovieConverter::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
    ...
    ffmpeg_file = getAppPath().append("/linux_ffmpeg").toStdString().c_str();
#else
    visiocyte_msg("The OS is not recognized (not Mac, Linux or Windows). Do nothing.");
    return false;
#endif
    ...
    else if (func_name == tr("help"))
	{
        ...
        printf("visiocyte -x plugin_name -f convert_frames_to_movie -i <inimg_folder> -p <converter_path> <frame_format> <output_fps> <video_type> <is_compress>\n");
        printf("inimg_folder     the video frame folder path \n");
        printf("converter_path   the movie converter(ffmpeg) path. If do not know, please set this para to NULL and the default path will be the visiocyte main path.\n");
...
void controlPanel::_slot_start()
{
          ...
          if (!QFile(selectffmpeg).exists())
          {
             visiocyte_msg("Cannot find ffmpeg, please select again or download from http://www.ffmpeg.org and put into a local folder.");
             return;
          }
          ...
          if(selectedFile == 0)
          {
             visiocyte_msg("Please select an input video frame folder");
             return;
          }
          else if (!dir.exists())
          {
          ...
          {
                    visiocyte_msg("Please choose another video type at this time.");
                    return;
          }
          else if (videoType == "mpg")
          {
          ...
          {
             visiocyte_msg(QString("movie file [%1] does not exist and it seems the movie conversion fails. Please check.\n").arg(movieFile));
             return;
          }
          else
          {
             visiocyte_msg(QString("The movie has been saved in [%1].").arg(movieFile));
             return;
          }
