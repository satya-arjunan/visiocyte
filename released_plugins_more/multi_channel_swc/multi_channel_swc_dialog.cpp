...
multi_channel_swc_dialog::multi_channel_swc_dialog(VISIOCYTEPluginCallback2 *cb, QWidget *parent) :
    QDialog(parent)
{
    callback=cb;
    create();
...
{
    QSettings settings("VISIOCYTE plugin","multiChannelSWC");
...
void multi_channel_swc_dialog::load_image()
{
    ...
    if(sz_img[3]<2){
        visiocyte_msg("Image only has 1 channel!");
        return;
    }
...
void multi_channel_swc_dialog::load_swc()
{
    ...
    if(nt->listNeuron.size()>0){
        ...
        visiocyte_msg("Error: cannot read file "+fileOpenName);
        return;
    }
...
void multi_channel_swc_dialog::run()
{
    ...
    save_result(edit_output->text(),thr1, thr2, nt, N, ratio, mean, stdev, N_P, ratio_P, mean_P, stdev_P);

    accept();
}

void save_result(const QString fname,float thr1,float thr2, NeuronTree* nt, vector<float>& N, vector<float>& ratio, vector<float>& mean, vector<float>& stdev, vector<float>& N_P, vector<float>& ratio_P, vector<float>& mean_P, vector<float>& stdev_P)
{
    ...
    if(!fp.is_open()){
        visiocyte_msg("Error: failed to open file to write: "+fname);
        return;
    }
    fp<<"# "<<fname.toStdString().c_str()<<endl;
    fp<<"# This file is generated by the multi_channel_swc plugin in Visiocyte, built by Sumit Nanda (snanda2@gmu.edu) and Hanbo Chen (cojoc.chen@gmail.com)"<<endl;
    fp<<"# The Channel information is at the bottom of the file, after the basic NeuronTree. There are two columns (ratio and mean) per extra channel. Begins with CHANNELSWC tag, followed by number of added channel, number of total features and number of rows in the next line. This is followed by the threshold value used for the primary and the secondary channel, and then the actual columns."<<endl;
//    fp<<"#CHANNELSWC"<<endl;
//    fp<<"#"<<"1"<<" 2 "<<ratio.size()<<endl;
//    fp<<"#Channel"<<endl;
//      for(VISIOCYTELONG nid=0; nid<ratio.size(); nid++){
...
//    }
 ...
 //fp<<"#"<<"1"<<" 2 "<<ratio.size()<<endl;
    ...
    fp<<"# Ratio, Mean_intensity;  "<<"Primary Channel Threshold: "<<thr1<<" ;Secondary Channel Threshold: "<<thr2<<endl;
    for(VISIOCYTELONG nid=0; nid<ratio.size(); nid++){
           // fp<<"#"<<ratio.at(nid)<<" "<<mean.at(nid)<<endl;
        fp<<"#";
    ...
    if(!fp.is_open()){
        visiocyte_msg("Error: failed to open file to write: "+fname_eswc);
        return;
    }
    fp<<"# "<<fname_eswc.toStdString().c_str()<<endl;
    fp<<"# This detailed augmented file is generated by the multi_channel_swc plugin in Visiocyte, built by Sumit Nanda (snanda2@gmu.edu) and Hanbo Chen (cojoc.chen@gmail.com)"<<endl;
