...
template<class T>
void calculate_info(T* p_img, VISIOCYTELONG sz_img[4], NeuronTree* nt,
int ch1, float thr1, int ch2, float thr2, vector<float>& N, vector<float>& ratio, vector<float>& mean, vector<float>& stdev, vector<float>& N_P, vector<float>& ratio_P, vector<float>& mean_P, vector<float>& stdev_P)
{
    ...
    for(VISIOCYTELONG nid=0; nid<nt->listNeuron.size(); nid++){
        if(nt->listNeuron.at(nid).parent>=0){//if the node has parent
            ...
            VISIOCYTELONG pid=nt->hashNeuron.value(nt->listNeuron.at(nid).pn);

            VISIOCYTELONG win_1[3];
            VISIOCYTELONG win_2[3];
            ...
            //do the calculation
            for(VISIOCYTELONG x=win_1[0]; x<win_2[0]; x++)
                for(VISIOCYTELONG y=win_1[1]; y<win_2[1]; y++)
                    for(VISIOCYTELONG z=win_1[2]; z<win_2[2]; z++){
                        VISIOCYTELONG pos1=x+y*sz_img[0]+z*sz_img[1]*sz_img[0]+ch1*sz_img[1]*sz_img[0]*sz_img[2];
                        VISIOCYTELONG pos2=x+y*sz_img[0]+z*sz_img[1]*sz_img[0]+ch2*sz_img[1]*sz_img[0]*sz_img[2];
                        if(p_img[pos1]>thr1){
                            num_vol_ch1++;
                            if(p_img[pos2]>thr2){
                                num_vol_ch2++;
