...
void MultiChannelSWC::domenu(const QString &menu_name, VISIOCYTEPluginCallback2 &callback, QWidget *parent)
{
	...
	{
        visiocyte_msg(tr("This plugin will create swc files based on multiple channel information in the neuron image.."
                   "If you use this plug-in for your research, please cite: Nanda et al. 2018, Design and implementation of multi-signal and time-varying neural reconstructions, Nature Sci Dat. www.nature.com/articles/sdata2017207"));
	}
}

bool MultiChannelSWC::dofunc(const QString & func_name, const VISIOCYTEPluginArgList & input, VISIOCYTEPluginArgList & output, VISIOCYTEPluginCallback2 & callback,  QWidget * parent)
{
...
void printHelp()
{
    ...
    qDebug()<<"visiocyte -x multi_channel_swc -f compute_swc -i file_neuron file_image -o file_output -p <primary channel (1)> <secondary channel (0)> <primary threshould (30)> <secondary threshould (30)>";
    qDebug()<<"Note: channel ID starts from 0";
    qDebug()<<"";
    qDebug()<<"Usage 2: compute eswc for render";
    qDebug()<<"visiocyte -x multi_channel_swc -f render_swc_by_eswc -i file_neuron -o file_output";
...
void do_calculate_render_eswc()
{
    ...
    if(nt.listNeuron.at(0).fea_val.size()==0){
        visiocyte_msg("Error: could not find feature informations from the file "+fileOpenName);
    }
