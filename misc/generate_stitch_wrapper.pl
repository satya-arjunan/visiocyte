...
my $USAGE = qq~
Usage:
        generate_stitch_wrapper.pl is a perl wrapper around visiocyte to blend and stitch single neuron imaging data.
...
#my $visiocyte_cmd_path = "/groups/scicomp/jacsData/sampleData/stitchedData/visiocyte_linux_fc14/visiocyte";
#my $visiocyte_cmd_path = "/groups/peng/home/brainaligner/program/visiocyte/visiocyte";
my $visiocyte_cmd_path = "~/work/visiocyte_external/visiocyte/visiocyte";
...
foreach my $region_name (sort keys %regions) {
    ...
    my $blend_raw = $line_name . "-" . $slide_group . "-" . $region_name . ".visiocyteraw";

    my $hr_images = $regions{$region_name}->{'images'};    
    my $blend_cmd = "$visiocyte_cmd_path -x libblend_multiscanstacks.so -f multiscanblend -p \"#k 1\" -i"; 
...
my $stitch_raw = $line_name . "-" . $slide_group . "-stitched.visiocyteraw";
$shell_output .= "$visiocyte_cmd_path -x imageStitch.so -f visiocytestitch -i '$stitched_dir' -o '$stitched_dir/$stitch_raw' -p \"#c 4\"\;\n";
